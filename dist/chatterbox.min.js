...
this.unchomp(this.tab.prefix[this.tab.type]+this.tab.matched[this.tab.index]+a)};Chatterbox.Control.prototype.start_tab=function(d){this.tab.hit=true;this.tab.index=-1;this.tab.matched=[];this.tab.type=0;var g=this.chomp();this.unchomp(g);if(g[0]=="/"||g[0]=="#"||g[0]=="@"){this.tab.type=g[0]=="/"?1:2;if(g[0]=="/"){g=g.slice(1)}}else{this.tab.type=0}this.tab.cache=g;g=g.toLowerCase();this.tab.matched=[];if(this.tab.type==0){var h=this.manager.client.channel(this.manager.chatbook.current.namespace);for(var a in h.info.members){if(a.toLowerCase().indexOf(g)==0){this.tab.matched.push(a)}}}else{if(this.tab.type==1){var f="";for(var b in this.manager.client.cmds){f=this.manager.client.cmds[b];if(f.indexOf(g)==0){this.tab.matched.push(f)}}}else{if(this.tab.type==2){var e=this;this.manager.client.each_channel(function(c,j){if(j.namespace.toLowerCase().indexOf(g)==0){e.tab.matched.push(j.namespace)}})}}}};Chatterbox.Control.prototype.end_tab=function(a){this.tab.hit=false;this.tab.matched=[];this.tab.cache="";
this.tab.index=-1};Chatterbox.Control.prototype.submit=function(a){var b=this.get_text();this.append_history(b);this.set_text("");this.handle(a,b);return false};Chatterbox.Control.prototype.keypress=function(c){var b=c.which||c.keyCode;var d=this.tab.hit;var a=false;switch(b){case 13:if(!this.multiline()){this.submit(c)}else{if(c.shiftKey){this.submit(c)}else{a=true}}break;case 38:if(!this.multiline()){this.scroll_history(true);break}a=true;break;case 40:if(!this.multiline()){this.scroll_history(false);break}a=true;break;case 9:if(c.shiftKey){this.manager.channel_right()}else{this.tab_item(c);d=false}break;case 219:if(c.ctrlKey){this.manager.channel_left()}else{a=true}break;case 221:if(c.ctrlKey){this.manager.channel_right()}else{a=true}break;default:a=true;break}if(d){this.end_tab(c)}return a};Chatterbox.Control.prototype.handle=function(b,e){if(e==""){return}if(!this.manager.chatbook.current){return}var h=false;if(e[0]!="/"){h=true}e=(b.shiftKey?"/npmsg ":(e[0]=="/"?"":"/say "))+e;e=e.slice(1);
var j=e.split(" ");var c=j.shift().toLowerCase();var g=this.manager.chatbook.current.namespace;var f=g;if(!h&&j[0]){var d=j[0][0];if((d=="#"||d=="@")&&j[0].length>1){f=this.manager.format_ns(j.shift())}}var k=j.join(" ");var a=this.manager.client.trigger("cmd."+c,{name:"cmd",cmd:c,args:k,target:f,ns:g});if(a==0){this.manager.pager.notice({ref:"cmd-fail",heading:"Command failed",content:'"'+c+'" is not a command.'},false,5000)}};Chatterbox.Navigation=function(a){this.manager=a;this.showclose=this.manager.settings.tabclose;this.settings={};this.settings.open=false;this.el={n:this.manager.view.find("nav.tabs"),t:this.manager.view.find("nav.tabs #chattabs"),b:this.manager.view.find("nav.tabs #tabnav"),l:this.manager.view.find("nav.tabs #tabnav .arrow_left"),r:this.manager.view.find("nav.tabs #tabnav .arrow_right"),s:this.manager.view.find("nav.tabs #tabnav #settings-button"),};if(!this.showclose){if(!this.el.t.hasClass("hc")){this.el.t.addClass("hc")}}var b=this;this.el.s.click(function(e){if(b.settings.open){return false
...
}c.item("Form",{ref:"ui",title:"UI",hint:"<b>Timestamp</b><br/>Choose between a 24 hour clock and                a 12 hour clock.\n\n<b>Theme</b><br/>Change the look of the                client.\n\n<b>Close Buttons</b><br/>Turn tab close buttons on/off.",fields:[["Dropdown",{ref:"theme",label:"Theme",items:a}],["Dropdown",{ref:"clock",label:"Timestamp Format",items:[{value:"24",title:"24 hour",selected:e.clock},{value:"12",title:"12 hour",selected:!e.clock}]}],["Checkbox",{ref:"tabclose",label:"Close Buttons",items:[{value:"yes",title:"On",selected:e.tc}]}],],event:{change:function(f){d.clock(f.data.clock=="24");d.theme(f.data.theme);d.nav.closer(f.data.tabclose.indexOf("yes")>-1)},save:function(f){e.clock=d.clock();e.theme=replaceAll(d.theme(),"wsct_","");e.tc=d.nav.closer();d.trigger("settings.save.ui",{clock:e.clock,tabclose:e.tc,theme:"wsct_"+e.theme})},close:function(f){d.clock(e.clock);d.theme(e.theme);d.nav.closer(e.tc)}}})};Chatterbox.Navigation.prototype.height=function(){var a=this.el.n.outerHeight();
