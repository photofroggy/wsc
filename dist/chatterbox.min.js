var Chatterbox={};Chatterbox.VERSION="0.6.50";Chatterbox.STATE="beta";Chatterbox.UI=function(a,b,e,c){this.events=c||new EventEmitter();this.mozilla=e;this.umuted=[];this.settings={themes:["wsct_default","wsct_dAmn"],theme:"wsct_default",monitor:["~Monitor",true],username:"",domain:"website.com",clock:true,tabclose:true};a.extend(this.settings,b);a.append('<div class="wsc '+this.settings.theme+'"></div>');this.view=a.find(".wsc");this.mns=this.format_ns(this.settings.monitor[0]);this.lun=this.settings.username.toLowerCase();this.monitoro=null;this.swidth=(function(){if($.browser.msie){var j=$('<textarea cols="10" rows="2"></textarea>').css({position:"absolute",top:-1000,left:-1000}).appendTo("body"),h=$('<textarea cols="10" rows="2" style="overflow: hidden;"></textarea>').css({position:"absolute",top:-1000,left:-1000}).appendTo("body");scrollbarWidth=j.width()-h.width();j.add(h).remove()}else{var g=$("<div />").css({width:100,height:100,overflow:"auto",position:"absolute",top:-1000,left:-1000}).prependTo("body").append("<div />").find("div").css({width:"100%",height:200});
...
Chatterbox.Popup.prototype.build.call(this);this.saveb=this.window.find("a.button.save");this.closeb=this.window.find("a.close");this.scb=this.window.find("a.button.saveclose");this.tabs=this.window.find("nav.tabs ul.tabs");this.book=this.window.find("div.book");this.config.build(this);this.window.find("ul.tabs li").first().addClass("active");this.window.find("div.book div.page").first().addClass("active");var a=this;this.window.find("form").bind("change",function(){a.changed=true});this.saveb.click(function(b){a.save();return false});this.closeb.click(function(b){if(a.changed){if(!confirm("Are you sure? You will lose any unsaved changes.")){return false}}a.close();return false});this.scb.click(function(b){a.save();a.close();return false});this.resize()};Chatterbox.Settings.prototype.resize=function(){var a=this.window.find(".inner");var c=a.find("h2");var e=a.find(".bookwrap");var b=a.find("footer");e.height(a.height()-b.outerHeight()-c.outerHeight()-15);this.book.height(e.innerHeight()-this.tabs.outerHeight()-25);
this.config.resize()};Chatterbox.Settings.prototype.switch_page=function(b){var c=this.tabs.find("li.active").first();var a=c.prop("id").split("-",1)[0];c=this.config.page(a.split("_").join(" "));c.hide();b.show()};Chatterbox.Settings.prototype.save=function(){this.config.save(this);this.changed=false};Chatterbox.Settings.prototype.close=function(){this.window.remove();this.manager.nav.settings.open=false;this.manager.nav.settings.window=null;this.config.close(this)};Chatterbox.Settings.Config=function(){this.pages=[]};Chatterbox.Settings.Config.prototype.find_page=function(a){n=a.toLowerCase();for(index in this.pages){page=this.pages[index];if(page.lname==n){return page}}return null};Chatterbox.Settings.Config.prototype.build=function(a){for(i in this.pages){this.pages[i].build(a)}};Chatterbox.Settings.Config.prototype.resize=function(){for(i in this.pages){this.pages[i].resize()}};Chatterbox.Settings.Config.prototype.page=function(b,a){var c=this.find_page(b);a=a||false;if(c==null){c=new Chatterbox.Settings.Page(b);
if(a){this.pages.push(c)}else{this.pages.unshift(c)}}return c};Chatterbox.Settings.Config.prototype.save=function(a){for(i in this.pages){this.pages[i].save(a)}};Chatterbox.Settings.Config.prototype.close=function(a){for(i in this.pages){this.pages[i].close(a)}};Chatterbox.Settings.Page=function(a){this.name=a;this.lname=a.toLowerCase();this.ref=replaceAll(this.lname," ","_");this.items=[];this.itemo={}};Chatterbox.Settings.Page.prototype.build=function(b){var a=replaceAll(Chatterbox.template.settings.tab,"{ref}",this.ref);a=replaceAll(a,"{name}",this.name);var c=replaceAll(Chatterbox.template.settings.page,"{ref}",this.ref);c=replaceAll(c,"{page-name}",this.name);b.tabs.append(a);b.book.append(c);this.view=b.book.find("div#"+this.ref+"-page");this.tab=b.tabs.find("li#"+this.ref+"-tab");var c=this;this.tab.find("a").click(function(e){if(c.tab.hasClass("active")){return false}b.switch_page(c);return false});this.content()};Chatterbox.Settings.Page.prototype.content=function(){for(i in this.items){this.items[i].build(this.view)
}};Chatterbox.Settings.Page.prototype.resize=function(){for(i in this.items){this.items[i].resize()}};Chatterbox.Settings.Page.prototype.show=function(){if(!this.tab.hasClass("active")){this.tab.addClass("active")}if(!this.view.hasClass("active")){this.view.addClass("active")}this.resize()};Chatterbox.Settings.Page.prototype.hide=function(){if(this.tab.hasClass("active")){this.tab.removeClass("active")}if(this.view.hasClass("active")){this.view.removeClass("active")}};Chatterbox.Settings.Page.prototype.item=function(c,b,a){a=a||false;var e=Chatterbox.Settings.Item.get(c,b);if(a){this.items.unshift(e)}else{this.items.push(e)}if(b.hasOwnProperty("ref")){this.itemo[b.ref]=e}return e};Chatterbox.Settings.Page.prototype.save=function(a){for(i in this.items){this.items[i].save(a,this)}};Chatterbox.Settings.Page.prototype.close=function(a){for(i in this.items){this.items[i].close(a,this)}};Chatterbox.Settings.Item=function(b,a){this.options=a||{};this.type=b||"base";this.selector=this.type.toLowerCase();
this.items=[];this.itemo={};this.view=null;this.val=null};Chatterbox.Settings.Item.prototype.build=function(h){if(!this.options.hasOwnProperty("ref")){return}var g=this.content();if(g.length==0){return}var a="";if(this.options.hasOwnProperty("wclass")){a=" "+this.options.wclass}var e=Chatterbox.render("settings.item.wrap",{type:this.type.toLowerCase().split(".").join("-"),ref:this.options.ref,"class":a});e=replaceAll(e,"{content}",g);h.append(e);this.view=h.find(".item."+this.options.ref);this.hooks(this.view);if(!this.options.hasOwnProperty("subitems")){return}var j;var c;var b;for(i in this.options.subitems){j=this.options.subitems[i];c=j[0];b=j[1];sitem=Chatterbox.Settings.Item.get(c,b);cls=["stacked"];if(sitem.options.wclass){cls.push(sitem.options.wclass)}sitem.options.wclass=cls.join(" ");sitem.build(this.view);this.items.push(sitem);if(b.hasOwnProperty("ref")){this.itemo[b.ref]=sitem}}};Chatterbox.Settings.Item.prototype.content=function(){return Chatterbox.render("settings.item."+this.selector,this.options)
...
this.value=[];this.view.find("input[checked]:checkbox").each(function(){a.value.push(a.view.find(this).val())});this.field.bind("change",function(e){a.value=[];a.view.find("input[checked]:checkbox").each(function(){a.value.push(a.view.find(this).val())})})};Chatterbox.Settings.Item.Items=function(b,a){Chatterbox.Settings.Item.call(this,b,a);this.selected=[]};Chatterbox.Settings.Item.Items.prototype=new Chatterbox.Settings.Item();Chatterbox.Settings.Item.Items.prototype.constructor=Chatterbox.Settings.Item.Items;Chatterbox.Settings.Item.Items.prototype.build=function(b){Chatterbox.Settings.Item.prototype.build.call(this,b);var a=this};Chatterbox.template={};Chatterbox.render=function(e,h){var c=Chatterbox.template;var b=e.split(".");var g={};var a=null;for(ind in b){part=b[ind];if(!c.hasOwnProperty(part)){return""}c=c[part]}if(c.hasOwnProperty("frame")){a=c;g=c.render||{};c=c.frame;if(a.hasOwnProperty("pre")){if(typeof a.pre=="function"){c=a.pre(c,h)}else{for(i in a.pre){c=a.pre[i](c,h)}}}}for(key in h){c=replaceAll(c,"{"+key+"}",(g[key]||Chatterbox.template.render_stub)(h[key]||""))
}if(a!=null){if(a.hasOwnProperty("post")){if(typeof a.post=="function"){c=a.post(c,h)}else{for(i in a.post){c=a.post[i](c,h)}}}}return c};Chatterbox.template.render_stub=function(a){return a};Chatterbox.template.clean=function(a){return function(b,c){for(i in a){b=replaceAll(b,"{"+a[i]+"}","")}return b}};Chatterbox.template.ui='<nav class="tabs"><ul id="chattabs" class="tabs"></ul>        <ul id="tabnav">            <li><a href="#left" class="button iconic arrow_left"></a></li>            <li><a href="#right" class="button iconic arrow_right"></a></li>            <li><a href="#settings" title="Change client settings" class="button iconic cog" id="settings-button"></a></li>        </ul>        </nav>        <div class="chatbook"></div>';Chatterbox.template.control='<div class="chatcontrol">            <p><a href="#multiline" title="Toggle multiline input" class="button iconic list"></a></p>            <form class="msg">                <input type="text" class="msg" />                <textarea class="msg"></textarea>                <input type="submit" value="Send" class="sendmsg" />            </form>        </div>';
