...
time=new Date(parseInt(el.prop("id")));el.text(formatTime(a,time))})};Chatterbox.Channel.prototype.server_message=function(b,a){data={ns:this.namespace,message:b,info:a};this.manager.trigger("server_message.before",data);this.log_item(Chatterbox.render("servermsg",{message:data.message,info:data.info}))};Chatterbox.Channel.prototype.clear=function(){this.logpanel.find("li.logmsg").remove();this.logpanel.find("li.loginfo").remove();this.wrap.height(0);this.resize()};Chatterbox.Channel.prototype.log_info=function(c,b){data={ns:this.namespace,ref:c,content:b};this.manager.trigger("log_info.before",data);delete data.ns;this.wrap.append(Chatterbox.render("loginfobox",data));this.scroll();var e=this;var a=this.wrap.find("li."+data.ref);a.find("a.close").click(function(g){e.wrap.find(this).parent().remove();e.resize();e.scroll();return false});return a};Chatterbox.Channel.prototype.show_whois=function(c){var g={avatar:'<a href="#"><img height="50" width="50" alt="avatar"/></a>',username:"<b>"+c.symbol+c.username+"</b>",info:[],conns:[],raw:c,};
for(i in c.connections){rcon=c.connections[i];mcon=[];if(rcon.online){stamp=(new Date-(rcon.online*1000));mcon.push(["online",DateStamp(stamp/1000)+formatTime(" [{HH}:{mm}:{ss}]",new Date(stamp))])}if(rcon.idle){mcon.push(["idle",timeLengthString(rcon.idle)])}if(rcon.agent){mcon.push(["agent",rcon.agent])}if(rcon.debug){mcon.push(["debug",rcon.debug])}mcon.push(["chatrooms",rcon.channels.join(" ")]);g.conns.push(mcon)}this.manager.trigger("log_whois.before",g);var a="";for(i in g.conns){conn=g.conns[i];text='<section class="conn"><p><em>connection '+((parseInt(i)+1).toString())+":</em></p>";text+="<ul>";for(x in conn){text+="<li><strong>"+conn[x][0]+":</strong> "+conn[x][1]+"</li>"}text+="</ul>";a+=text+"</section>"}var e="";for(i in g.info){e+="<li>"+g.info[i]+"</li>"}var b=this.log_info("whois-"+c.username,Chatterbox.render("whoiswrap",{avatar:g.avatar,info:Chatterbox.render("whoisinfo",{username:g.username,info:e,connections:a})}));b.find("div.avatar").height(b.height()-10)};Chatterbox.Channel.prototype.set_header=function(a,b){headd=this.window.find("header div."+a);
...
b=replaceAll(b,"{page-name}",this.name);a.tabs.append(tab);a.book.append(b);this.view=a.book.find("div#"+this.ref+"-page");this.tab=a.tabs.find("li#"+this.ref+"-tab");var b=this;this.tab.find("a").click(function(c){if(b.tab.hasClass("active")){return false}a.switch_page(b);return false});this.content()};Chatterbox.Settings.Page.prototype.content=function(){for(i in this.items){this.items[i].build(this.view)}};Chatterbox.Settings.Page.prototype.resize=function(){for(i in this.items){this.items[i].resize()}};Chatterbox.Settings.Page.prototype.show=function(){if(!this.tab.hasClass("active")){this.tab.addClass("active")}if(!this.view.hasClass("active")){this.view.addClass("active")}};Chatterbox.Settings.Page.prototype.hide=function(){if(this.tab.hasClass("active")){this.tab.removeClass("active")}if(this.view.hasClass("active")){this.view.removeClass("active")}};Chatterbox.Settings.Page.prototype.item=function(c,b,a){a=a||false;item=Chatterbox.Settings.Item.get(c,b);if(a){this.items.unshift(item)
...
this.label=c.lsection.find("label."+this.ref);this.lwrap=c.lsection.find("."+this.ref+".label");if(this.options.hasOwnProperty("items")){for(i in this.options.items){var b=this.options.items[i];b.name=this.ref;this.items[b.ref]=""}}c.fsection.append(Chatterbox.render("settings.item.form.field.wrap",{ref:this.ref,field:Chatterbox.render("settings.item.form.field.check",this.options)}));this.fwrap=c.fsection.find("div."+this.ref+".field");this.field=this.fwrap.find("input:checkbox");var a=this;this.value=[];this.fwrap.find("input[checked]:checkbox").each(function(){a.value.push(a.fwrap.find(this).val())});this.field.bind("change",function(e){a.value=[];a.fwrap.find("input[checked]:checkbox").each(function(){a.value.push(a.fwrap.find(this).val())})})};Chatterbox.Settings.Item.Form.Check.prototype.resize=function(){this.lwrap.height(this.fwrap.find(".checkbox").height())};Chatterbox.Settings.Item.Radio=function(b,a){Chatterbox.Settings.Item.call(this,b,a);this.value=""};Chatterbox.Settings.Item.Radio.prototype=new Chatterbox.Settings.Item();
Chatterbox.Settings.Item.Radio.prototype.constructor=Chatterbox.Settings.Item.Radio;Chatterbox.Settings.Item.Radio.prototype.build=function(c){if(this.options.hasOwnProperty("items")){for(i in this.options.items){var b=this.options.items[i];b.name=this.options.ref||"ref"}}Chatterbox.Settings.Item.prototype.build.call(this,c);this.field=this.view.find("input:radio");this.value=this.view.find("input[checked]:radio").val();var a=this;this.field.bind("change",function(e){a.value=a.view.find(this).val()})};Chatterbox.Settings.Item.Check=function(b,a){Chatterbox.Settings.Item.Radio.call(this,b,a);this.value=[]};Chatterbox.Settings.Item.Check.prototype=new Chatterbox.Settings.Item.Radio();Chatterbox.Settings.Item.Check.prototype.constructor=Chatterbox.Settings.Item.Check;Chatterbox.Settings.Item.Check.prototype.build=function(c){if(this.options.hasOwnProperty("items")){for(i in this.options.items){var b=this.options.items[i];b.name=this.options.ref||"ref"}}Chatterbox.Settings.Item.prototype.build.call(this,c);
this.field=this.view.find("input:checkbox");var a=this;this.value=[];this.view.find("input[checked]:checkbox").each(function(){a.value.push(a.view.find(this).val())});this.field.bind("change",function(e){a.value=[];a.view.find("input[checked]:checkbox").each(function(){a.value.push(a.view.find(this).val())})})};Chatterbox.Settings.Item.Form.Field.prototype.get=function(){return this.value};Chatterbox.template={};Chatterbox.render=function(e,h){var c=Chatterbox.template;var b=e.split(".");var g={};var a=null;for(ind in b){part=b[ind];if(!c.hasOwnProperty(part)){return""}c=c[part]}if(c.hasOwnProperty("frame")){a=c;g=c.render||{};c=c.frame;if(a.hasOwnProperty("pre")){if(typeof a.pre=="function"){c=a.pre(c,h)}else{for(i in a.pre){c=a.pre[i](c,h)}}}}for(key in h){c=replaceAll(c,"{"+key+"}",(g[key]||Chatterbox.template.render_stub)(h[key]||""))}if(a!=null){if(a.hasOwnProperty("post")){if(typeof a.post=="function"){c=a.post(c,h)}else{for(i in a.post){c=a.post[i](c,h)}}}}return c};Chatterbox.template.render_stub=function(a){return a
