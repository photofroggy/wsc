...
return false};Chatterbox.Control.prototype.keypress=function(c){var b=c.which||c.keyCode;var d=this.tab.hit;var a=false;switch(b){case 13:if(!this.multiline()){this.submit(c)}else{if(c.shiftKey){this.submit(c)}else{a=true}}break;case 38:if(!this.multiline()){this.scroll_history(true);break}a=true;break;case 40:if(!this.multiline()){this.scroll_history(false);break}a=true;break;case 9:if(c.shiftKey){this.manager.channel_right()}else{this.tab_item(c);d=false}break;case 219:if(c.ctrlKey){this.manager.channel_left()}else{a=true}break;case 221:if(c.ctrlKey){this.manager.channel_right()}else{a=true}break;case 84:if(c.ctrlKey&&c.altKey){this.manager.nav.toggle_sidebar()}else{a=true}break;default:a=true;break}if(d){this.end_tab(c)}return a};Chatterbox.Control.prototype.handle=function(b,e){if(e==""){return}if(!this.manager.chatbook.current){return}var h=false;if(e[0]!="/"){h=true}e=(b.shiftKey?"/npmsg ":(e[0]=="/"?"":"/say "))+e;e=e.slice(1);var j=e.split(" ");var c=j.shift().toLowerCase();var g=this.manager.chatbook.current.namespace;
