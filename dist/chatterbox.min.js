...
d.build();if(this.trail.indexOf(d.namespace)==-1){this.trail.push(d.namespace)}if(!d.visible){this.toggle_channel(b)}this.manager.resize();return d};Chatterbox.Chatbook.prototype.create_feed=function(a,d,b){var c=this.channel(a,this.feed_object(a,d,b));c.build();if(this.trail.indexOf(c.namespace)==-1){this.trail.push(c.namespace)}if(!c.visible){this.toggle_channel(a)}this.manager.resize();return c};Chatterbox.Chatbook.prototype.channel_object=function(a){return new Chatterbox.Channel(this.manager,a)};Chatterbox.Chatbook.prototype.feed_object=function(a,c,b){return new Chatterbox.Feed(this.manager,a,c,b)};Chatterbox.Chatbook.prototype.toggle_channel=function(a){var c=this.channel(a);var b=c;if(!c){return}if(c.hidden){if(this.current&&this.current==c){return}if(!this.manager.settings.developer){c.hide();return}}if(this.current){if(this.current==c){return}this.current.hide();b=this.current}else{if(this.manager.monitoro!==null){this.manager.monitoro.hide()}}c.show();this.manager.control.focus();
