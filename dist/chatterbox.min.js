...
}f.sound.bank[g].muted=h}},mute:function(){f.sound.toggle(true)},unmute:function(){f.sound.toggle(false)},};this.settings=b.extend(this.settings,c);b.append('<div class="wsc '+this.settings.theme+'"></div>');this.mw=new wsc.Middleware();this.view=b.find(".wsc");this.mns=this.format_ns(this.settings.monitor[0]);this.lun=this.settings.username.toLowerCase();this.monitoro=null;this.swidth=(function(){if($.browser.msie){var j=$('<textarea cols="10" rows="2"></textarea>').css({position:"absolute",top:-1000,left:-1000}).appendTo("body"),h=$('<textarea cols="10" rows="2" style="overflow: hidden;"></textarea>').css({position:"absolute",top:-1000,left:-1000}).appendTo("body");scrollbarWidth=j.width()-h.width();j.add(h).remove()}else{var g=$("<div />").css({width:100,height:100,overflow:"auto",position:"absolute",top:-1000,left:-1000}).prependTo("body").append("<div />").find("div").css({width:"100%",height:200});scrollbarWidth=100-g.width();g.parent().remove()}return scrollbarWidth})();if(window.mozRTCPeerConnection){Chatterbox._gum=function(h,j,g){return navigator.mozGetUserMedia(h,j,g)
