...
Chatterbox.Popup.prototype.build.call(this);this.saveb=this.window.find("a.button.save");this.closeb=this.window.find("a.close");this.scb=this.window.find("a.button.saveclose");this.tabs=this.window.find("nav.tabs ul.tabs");this.book=this.window.find("div.book");this.config.build(this.manager,this);this.window.find("ul.tabs li").first().addClass("active");this.window.find("div.book div.page").first().addClass("active");var a=this;this.window.find("form").bind("change",function(){a.changed=true});this.config.each_page(function(b,c){c.each_item(function(e,f){f._onchange=function(g){a.changed=true}})});this.saveb.click(function(b){a.save();return false});this.closeb.click(function(b){if(a.changed){if(!confirm("Are you sure? You will lose any unsaved changes.")){return false}}a.close();return false});this.scb.click(function(b){a.save();a.close();return false});this.resize()};Chatterbox.Settings.prototype.resize=function(){var a=this.window.find(".inner");var c=a.find("h2");var e=a.find(".bookwrap");
var b=a.find("footer");e.height(a.height()-b.outerHeight()-c.outerHeight()-15);this.book.height(e.innerHeight()-this.tabs.outerHeight()-25);this.config.resize()};Chatterbox.Settings.prototype.switch_page=function(b){var c=this.tabs.find("li.active").first();var a=c.prop("id").split("-",1)[0];c=this.config.page(a.split("_").join(" "));c.hide();b.show()};Chatterbox.Settings.prototype.save=function(){this.config.save(this);this.changed=false};Chatterbox.Settings.prototype.close=function(){this.window.remove();this.manager.nav.settings.open=false;this.manager.nav.settings.window=null;this.config.close(this)};Chatterbox.Settings.Config=function(){this.pages=[]};Chatterbox.Settings.Config.prototype.find_page=function(b){var e=b.toLowerCase();var c;for(var a in this.pages){c=this.pages[a];if(c.lname==e){return c}}return null};Chatterbox.Settings.Config.prototype.build=function(c,b){for(var a in this.pages){this.pages[a].build(c,b)}};Chatterbox.Settings.Config.prototype.resize=function(){for(var a in this.pages){this.pages[a].resize()
}};Chatterbox.Settings.Config.prototype.page=function(b,a){var c=this.find_page(b);a=a||false;if(c==null){c=new Chatterbox.Settings.Page(b);if(a){this.pages.push(c)}else{this.pages.unshift(c)}}return c};Chatterbox.Settings.Config.prototype.each_page=function(e){var c=null;var a=null;for(var b in this.pages){if(!this.pages.hasOwnProperty(b)){continue}c=this.pages[b];a=e(b,c);if(a===false){break}}};Chatterbox.Settings.Config.prototype.save=function(a){for(i in this.pages){this.pages[i].save(a)}};Chatterbox.Settings.Config.prototype.close=function(b){for(var a in this.pages){this.pages[a].close(b)}};Chatterbox.Settings.Page=function(a){this.name=a;this.lname=a.toLowerCase();this.ref=replaceAll(this.lname," ","_");this.items=[];this.itemo={};this.manager=null};Chatterbox.Settings.Page.prototype.build=function(e,b){this.manager=e;var a=replaceAll(Chatterbox.template.settings.tab,"{ref}",this.ref);a=replaceAll(a,"{name}",this.name);var c=replaceAll(Chatterbox.template.settings.page,"{ref}",this.ref);
c=replaceAll(c,"{page-name}",this.name);b.tabs.append(a);b.book.append(c);this.view=b.book.find("div#"+this.ref+"-page");this.tab=b.tabs.find("li#"+this.ref+"-tab");var c=this;this.tab.find("a").click(function(f){if(c.tab.hasClass("active")){return false}b.switch_page(c);return false});this.content()};Chatterbox.Settings.Page.prototype.content=function(){for(var a in this.items){this.items[a].build(this.view)}};Chatterbox.Settings.Page.prototype.resize=function(){for(var a in this.items){this.items[a].resize()}};Chatterbox.Settings.Page.prototype.show=function(){if(!this.tab.hasClass("active")){this.tab.addClass("active")}if(!this.view.hasClass("active")){this.view.addClass("active")}this.resize()};Chatterbox.Settings.Page.prototype.hide=function(){if(this.tab.hasClass("active")){this.tab.removeClass("active")}if(this.view.hasClass("active")){this.view.removeClass("active")}};Chatterbox.Settings.Page.prototype.item=function(c,b,a){a=a||false;var e=Chatterbox.Settings.Item.get(c,b,this.manager);
if(a){this.items.unshift(e)}else{this.items.push(e)}if(b.hasOwnProperty("ref")){this.itemo[b.ref]=e}return e};Chatterbox.Settings.Page.prototype.get=function(a){if(this.itemo.hasOwnProperty(a)){return this.itemo[a]}return null};Chatterbox.Settings.Page.prototype.each_item=function(e){var c=null;var a=null;for(var b in this.items){if(!this.items.hasOwnProperty(b)){continue}c=this.items[b];a=e(b,c);if(a===false){break}}};Chatterbox.Settings.Page.prototype.save=function(b){for(var a in this.items){this.items[a].save(b,this)}};Chatterbox.Settings.Page.prototype.close=function(b){for(var a in this.items){this.items[a].close(b,this)}};Chatterbox.Settings.Item=function(b,a,c){this.manager=c||null;this.options=a||{};this.type=b||"base";this.selector=this.type.toLowerCase();this.items=[];this.itemo={};this.view=null;this.val=null;this._onchange=this._event_stub};Chatterbox.Settings.Item.prototype.build=function(h){if(!this.options.hasOwnProperty("ref")){return}var g=this.content();if(g.length==0){return
...
this.value=this.view.find("input[checked]:radio").val();var a=this;this.field.bind("change",function(e){a.value=a.view.find(this).val()})};Chatterbox.Settings.Item.Checkbox=function(b,a){Chatterbox.Settings.Item.Radio.call(this,b,a);this.value=[]};Chatterbox.Settings.Item.Checkbox.prototype=new Chatterbox.Settings.Item.Radio();Chatterbox.Settings.Item.Checkbox.prototype.constructor=Chatterbox.Settings.Item.Checkbox;Chatterbox.Settings.Item.Checkbox.prototype.build=function(c){if(this.options.hasOwnProperty("items")){for(i in this.options.items){var b=this.options.items[i];b.name=this.options.ref||"ref"}}Chatterbox.Settings.Item.prototype.build.call(this,c);this.field=this.view.find("input:checkbox");var a=this;this.value=[];this.view.find("input[checked]:checkbox").each(function(){a.value.push(a.view.find(this).val())});this.field.bind("change",function(e){a.value=[];a.view.find("input[checked]:checkbox").each(function(){a.value.push(a.view.find(this).val())})})};Chatterbox.Settings.Item.Items=function(b,a){Chatterbox.Settings.Item.call(this,b,a);
this.selected=false};Chatterbox.Settings.Item.Items.prototype=new Chatterbox.Settings.Item();Chatterbox.Settings.Item.Items.prototype.constructor=Chatterbox.Settings.Item.Items;Chatterbox.Settings.Item.Items.prototype.build=function(b){Chatterbox.Settings.Item.prototype.build.call(this,b);var a=this;this.list=this.view.find("ul");this.buttons=this.view.find("section.buttons");this.list.find("li").click(function(e){var c=a.list.find(this);a.list.find("li.selected").removeClass("selected");a.selected=c.html();c.addClass("selected")});this.buttons.find("a.button").click(function(c){return false});this.buttons.find("a.button.up").click(function(e){if(a.selected===false){return false}var f=a.options.items.indexOf(a.selected);var c=f-1;if(f==-1||f>=a.options.items.length){return false}if(c<0||c>=a.options.items.length){return false}a._fevent("up",{swap:{"this":{index:f,item:a.options.items[f]},that:{index:c,item:a.options.items[c]}}});a.refresh();a._onchange({});return false});this.buttons.find("a.button.down").click(function(e){if(a.selected===false){return false
}var f=a.options.items.indexOf(a.selected);var c=f+1;if(f==-1||f>=a.options.items.length){return false}if(c<0||c>=a.options.items.length){return false}a._fevent("down",{swap:{"this":{index:f,item:a.options.items[f]},that:{index:c,item:a.options.items[c]}}});a.refresh();a._onchange({});return false});this.buttons.find("a.button.add").click(function(e){if(a.selected===false){return false}var c=new Chatterbox.Popup(a.manager,{close:false});return false});this.buttons.find("a.button.remove").click(function(e){if(a.selected===false){return false}var f=a.options.items.indexOf(a.selected);var c=f-1;if(f==-1){return false}if(c<0||c>=a.options.items.length){return false}return false})};Chatterbox.Settings.Item.Items.prototype.refresh=function(){this.view.find("section.mitems").html(Chatterbox.template.settings.krender.manageditems(this.options.items));this.list=this.view.find("ul");this.list.find("li[title="+this.selected.toLowerCase()+"]").addClass("selected");var a=this;this.list.find("li").click(function(c){var b=a.list.find(this);
a.list.find("li.selected").removeClass("selected");a.selected=b.html();b.addClass("selected")})};Chatterbox.Settings.Item.Items.prototype._fevent=function(b,e){var g=this._get_ep("inspect");var c=g?this.view.find(g[1]):null;var a=this._get_cb(b);if(typeof a=="function"){a({input:c,item:this,args:e})}else{for(var f in a){var h=c.hasOwnProperty("slice")?c.slice(f,1):c;a[f]({input:h,item:this,args:e})}}};Chatterbox.Settings.Item.Items.prototype.save=function(){this._fevent("save",{items:this.options.items||[]})};Chatterbox.template={};Chatterbox.render=function(e,g){var c=Chatterbox.template;var b=e.split(".");var f={};var a=null;for(ind in b){part=b[ind];if(!c.hasOwnProperty(part)){return""}c=c[part]}if(c.hasOwnProperty("frame")){a=c;f=c.render||{};c=c.frame;if(a.hasOwnProperty("pre")){if(typeof a.pre=="function"){c=a.pre(c,g)}else{for(i in a.pre){c=a.pre[i](c,g)}}}}for(key in g){c=replaceAll(c,"{"+key+"}",(f[key]||Chatterbox.template.render_stub)(g[key]||""))}if(a!=null){if(a.hasOwnProperty("post")){if(typeof a.post=="function"){c=a.post(c,g)
}else{for(i in a.post){c=a.post[i](c,g)}}}}return c};Chatterbox.template.render_stub=function(a){return a};Chatterbox.template.clean=function(a){return function(b,c){for(i in a){b=replaceAll(b,"{"+a[i]+"}","")}return b}};Chatterbox.template.ui='<nav class="tabs"><ul id="chattabs" class="tabs"></ul>        <ul id="tabnav">            <li><a href="#left" class="button iconic arrow_left"></a></li>            <li><a href="#right" class="button iconic arrow_right"></a></li>            <li><a href="#settings" title="Change client settings" class="button iconic cog" id="settings-button"></a></li>        </ul>        </nav>        <div class="chatbook"></div>';Chatterbox.template.control='<div class="chatcontrol">            <p><a href="#multiline" title="Toggle multiline input" class="button iconic list"></a></p>            <form class="msg">                <input type="text" class="msg" />                <textarea class="msg"></textarea>                <input type="submit" value="Send" class="sendmsg" />            </form>        </div>';
