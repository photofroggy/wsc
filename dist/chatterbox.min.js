var Chatterbox={};Chatterbox.VERSION="0.13.62";Chatterbox.STATE="beta";Chatterbox.UI=function(a,b,d,c){this.events=c||new EventEmitter();this.mozilla=d;this.umuted=[];this.settings={themes:["wsct_default","wsct_dAmn"],theme:"wsct_default",monitor:["~Monitor",true],username:"",domain:"website.com",clock:true,tabclose:true,developer:false};a.extend(this.settings,b);a.append('<div class="wsc '+this.settings.theme+'"></div>');this.view=a.find(".wsc");this.mns=this.format_ns(this.settings.monitor[0]);this.lun=this.settings.username.toLowerCase();this.monitoro=null;this.swidth=(function(){if($.browser.msie){var g=$('<textarea cols="10" rows="2"></textarea>').css({position:"absolute",top:-1000,left:-1000}).appendTo("body"),f=$('<textarea cols="10" rows="2" style="overflow: hidden;"></textarea>').css({position:"absolute",top:-1000,left:-1000}).appendTo("body");scrollbarWidth=g.width()-f.width();g.add(f).remove()}else{var e=$("<div />").css({width:100,height:100,overflow:"auto",position:"absolute",top:-1000,left:-1000}).prependTo("body").append("<div />").find("div").css({width:"100%",height:200});
...
c.find("a.close").click(function(b){f.el.l.w.find(this).parent().remove();f.resize();return false});return c};Chatterbox.Channel.prototype.log_whois=function(h){var d={avatar:'<a href="#"><img height="50" width="50" alt="avatar"/></a>',username:"<b>"+h.symbol+h.username+"</b>",info:[],conns:[],raw:h,};for(var j in h.connections){var g=h.connections[j];var l=[];if(g.online){stamp=(new Date-(g.online*1000));l.push(["online",DateStamp(stamp/1000)+formatTime(" [{HH}:{mm}:{ss}]",new Date(stamp))])}if(g.idle){l.push(["idle",timeLengthString(g.idle)])}if(g.agent){l.push(["agent",g.agent])}if(g.debug){l.push(["debug",g.debug])}l.push(["chatrooms",g.channels.join(" ")]);d.conns.push(l)}this.manager.trigger("log_whois.before",d);var a="";for(var j in d.conns){var f=d.conns[j];var n='<section class="conn"><p><em>connection '+((parseInt(j)+1).toString())+":</em></p>";n+="<ul>";for(var m in f){n+="<li><strong>"+f[m][0]+":</strong> "+f[m][1]+"</li>"}n+="</ul>";a+=n+"</section>"}var e="";for(var j in d.info){e+="<li>"+d.info[j]+"</li>"
}var k=this.log_info("whois-"+h.username,Chatterbox.render("whoiswrap",{avatar:d.avatar,info:Chatterbox.render("whoisinfo",{username:d.username,info:e,connections:a})}));var c=k.find("div.avatar");var b=k.find("div.info");b.width(k.find(".whoiswrap").width()-100);c.height(k.height()-10);this.scroll()};Chatterbox.Channel.prototype.log_pc=function(d,e){contents="";for(var c in e){if(!e.hasOwnProperty(c)){continue}var b=e[c];var a="";if(b[2].length==0){a="<em>"+(d?"default privileges":"no members")+"</em>"}else{a=b[2]}contents+=String.format("<li><em>{0}</em> <strong>{1}</strong>:<ul><li>{2}</li></ul></li>",[b[1],b[0],a])}var f={title:"Privilege class "+(d?"permissions":"members"),info:"<ul>"+contents+"</ul>"};this.log_info("pc-"+(d?"permissions":"members"),Chatterbox.render("pcinfo",f))};Chatterbox.Channel.prototype.set_header=function(a,b){a=a.toLowerCase();this.el.h[a].replaceWith(Chatterbox.render("header",{head:a,content:b||""}));this.el.h[a]=this.el.m.find("header div."+a);if(b.length>0){this.el.h[a].css({display:"block"});
