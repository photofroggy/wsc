var Chatterbox={};Chatterbox.VERSION="0.20.104";Chatterbox.STATE="beta";Chatterbox._gum=function(){};(function(a){a.fn.chatterbox=function(e,b,c){var d=a(window).data("chatterbox");if(e=="init"||d===undefined){if(d==undefined){d=new Chatterbox.UI(a(this),b,c,(a.browser.mozilla||false));a(window).resize(d.resize);setInterval(function(){d.loop()},120000);d.build()}a(window).data("chatterbox",d)}if(e!="init"&&e!=undefined){e="jq_"+e;if(e in d){d[e](a(this),c)}}return d}})(jQuery);Chatterbox.UI=function(b,a,c,e,d){this.client=a;this.events=d||new EventEmitter();this.mozilla=e;this.umuted=[];this.viewing=true;this.settings={themes:["wsct_dAmn","wsct_dark"],theme:"wsct_dark",monitor:["~Monitor",true],username:"",domain:"website.com",clock:true,tabclose:true,developer:false,media:"/static/"};this.settings=b.extend(this.settings,c);this.settings.monitor=a.settings.monitor||this.settings.monitor;this.settings.username=a.settings.username||this.settings.username;this.settings.domain=a.settings.domain||this.settings.domain;
this.settings.developer=a.settings.developer||this.settings.developer;var f=this;a.settings.agent="Chatterbox/"+Chatterbox.VERSION+" "+a.settings.agent;this.sound={bank:{m:null},add:function(g,h){if(f.sound.hasOwnProperty(g)){return false}f.sound.bank[g]=h;h.load();f.sound[g]=function(){f.sound.play(f.sound.bank[g])};return true},play:function(g){g.pause();g.currentTime=0;g.play()},toggle:function(h){for(var g in f.sound.bank){if(!f.sound.bank.hasOwnProperty(g)){continue}f.sound.bank[g].muted=h}},mute:function(){f.sound.toggle(true)},unmute:function(){f.sound.toggle(false)},};b.append('<div class="wsc '+this.settings.theme+'"></div>');this.protocol=new Chatterbox.Protocol();this.mw=new wsc.Middleware();this.view=b.find(".wsc");this.mns=this.format_ns(this.settings.monitor[0]);this.lun=this.settings.username.toLowerCase();this.monitoro=null;this.swidth=(function(){if($.browser.msie){var j=$('<textarea cols="10" rows="2"></textarea>').css({position:"absolute",top:-1000,left:-1000}).appendTo("body"),h=$('<textarea cols="10" rows="2" style="overflow: hidden;"></textarea>').css({position:"absolute",top:-1000,left:-1000}).appendTo("body");
