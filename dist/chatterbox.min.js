...
Chatterbox.Popup.prototype.build.call(this);this.saveb=this.window.find("a.button.save");this.closeb=this.window.find("a.close");this.scb=this.window.find("a.button.saveclose");this.tabs=this.window.find("nav.tabs ul.tabs");this.book=this.window.find("div.book");this.config.build(this);this.window.find("ul.tabs li").first().addClass("active");this.window.find("div.book div.page").first().addClass("active");var a=this;this.window.find("form").bind("change",function(){a.changed=true});this.config.each_page(function(b,c){c.each_item(function(e,f){f._onchange=function(g){a.changed=true}})});this.saveb.click(function(b){a.save();return false});this.closeb.click(function(b){if(a.changed){if(!confirm("Are you sure? You will lose any unsaved changes.")){return false}}a.close();return false});this.scb.click(function(b){a.save();a.close();return false});this.resize()};Chatterbox.Settings.prototype.resize=function(){var a=this.window.find(".inner");var c=a.find("h2");var e=a.find(".bookwrap");var b=a.find("footer");
e.height(a.height()-b.outerHeight()-c.outerHeight()-15);this.book.height(e.innerHeight()-this.tabs.outerHeight()-25);this.config.resize()};Chatterbox.Settings.prototype.switch_page=function(b){var c=this.tabs.find("li.active").first();var a=c.prop("id").split("-",1)[0];c=this.config.page(a.split("_").join(" "));c.hide();b.show()};Chatterbox.Settings.prototype.save=function(){this.config.save(this);this.changed=false};Chatterbox.Settings.prototype.close=function(){this.window.remove();this.manager.nav.settings.open=false;this.manager.nav.settings.window=null;this.config.close(this)};Chatterbox.Settings.Config=function(){this.pages=[]};Chatterbox.Settings.Config.prototype.find_page=function(b){var e=b.toLowerCase();var c;for(var a in this.pages){c=this.pages[a];if(c.lname==e){return c}}return null};Chatterbox.Settings.Config.prototype.build=function(b){for(var a in this.pages){this.pages[a].build(b)}};Chatterbox.Settings.Config.prototype.resize=function(){for(var a in this.pages){this.pages[a].resize()
}};Chatterbox.Settings.Config.prototype.page=function(b,a){var c=this.find_page(b);a=a||false;if(c==null){c=new Chatterbox.Settings.Page(b);if(a){this.pages.push(c)}else{this.pages.unshift(c)}}return c};Chatterbox.Settings.Config.prototype.each_page=function(e){var c=null;var a=null;for(var b in this.pages){if(!this.pages.hasOwnProperty(b)){continue}c=this.pages[b];a=e(b,c);if(a===false){break}}};Chatterbox.Settings.Config.prototype.save=function(a){for(i in this.pages){this.pages[i].save(a)}};Chatterbox.Settings.Config.prototype.close=function(b){for(var a in this.pages){this.pages[a].close(b)}};Chatterbox.Settings.Page=function(a){this.name=a;this.lname=a.toLowerCase();this.ref=replaceAll(this.lname," ","_");this.items=[];this.itemo={}};Chatterbox.Settings.Page.prototype.build=function(b){var a=replaceAll(Chatterbox.template.settings.tab,"{ref}",this.ref);a=replaceAll(a,"{name}",this.name);var c=replaceAll(Chatterbox.template.settings.page,"{ref}",this.ref);c=replaceAll(c,"{page-name}",this.name);
b.tabs.append(a);b.book.append(c);this.view=b.book.find("div#"+this.ref+"-page");this.tab=b.tabs.find("li#"+this.ref+"-tab");var c=this;this.tab.find("a").click(function(e){if(c.tab.hasClass("active")){return false}b.switch_page(c);return false});this.content()};Chatterbox.Settings.Page.prototype.content=function(){for(var a in this.items){this.items[a].build(this.view)}};Chatterbox.Settings.Page.prototype.resize=function(){for(var a in this.items){this.items[a].resize()}};Chatterbox.Settings.Page.prototype.show=function(){if(!this.tab.hasClass("active")){this.tab.addClass("active")}if(!this.view.hasClass("active")){this.view.addClass("active")}this.resize()};Chatterbox.Settings.Page.prototype.hide=function(){if(this.tab.hasClass("active")){this.tab.removeClass("active")}if(this.view.hasClass("active")){this.view.removeClass("active")}};Chatterbox.Settings.Page.prototype.item=function(c,b,a){a=a||false;var e=Chatterbox.Settings.Item.get(c,b);if(a){this.items.unshift(e)}else{this.items.push(e)
}if(b.hasOwnProperty("ref")){this.itemo[b.ref]=e}return e};Chatterbox.Settings.Page.prototype.get=function(a){if(this.itemo.hasOwnProperty(a)){return this.itemo[a]}return null};Chatterbox.Settings.Page.prototype.each_item=function(e){var c=null;var a=null;for(var b in this.items){if(!this.items.hasOwnProperty(b)){continue}c=this.items[b];a=e(b,c);if(a===false){break}}};Chatterbox.Settings.Page.prototype.save=function(b){for(var a in this.items){this.items[a].save(b,this)}};Chatterbox.Settings.Page.prototype.close=function(b){for(var a in this.items){this.items[a].close(b,this)}};Chatterbox.Settings.Item=function(b,a){this.options=a||{};this.type=b||"base";this.selector=this.type.toLowerCase();this.items=[];this.itemo={};this.view=null;this.val=null;this._onchange=this._event_stub};Chatterbox.Settings.Item.prototype.build=function(h){if(!this.options.hasOwnProperty("ref")){return}var g=this.content();if(g.length==0){return}var a="";if(this.options.hasOwnProperty("wclass")){a=" "+this.options.wclass
}var f=Chatterbox.render("settings.item.wrap",{type:this.type.toLowerCase().split(".").join("-"),ref:this.options.ref,"class":a});f=replaceAll(f,"{content}",g);h.append(f);this.view=h.find(".item."+this.options.ref);this.hooks(this.view);if(!this.options.hasOwnProperty("subitems")){return}var j;var e;var c;var b;for(i in this.options.subitems){j=this.options.subitems[i];e=j[0];c=j[1];sitem=Chatterbox.Settings.Item.get(e,c);b=["stacked"];if(sitem.options.wclass){b.push(sitem.options.wclass)}sitem.options.wclass=b.join(" ");sitem.build(this.view);this.items.push(sitem);if(c.hasOwnProperty("ref")){this.itemo[c.ref]=sitem}}};Chatterbox.Settings.Item.prototype.content=function(){return Chatterbox.render("settings.item."+this.selector,this.options)};Chatterbox.Settings.Item.prototype.resize=function(){for(var a in this.items){this.items[a].resize()}};Chatterbox.Settings.Item.prototype.hooks=function(c){if(!this.options.hasOwnProperty("event")){return}var b=this.options.event;var f=Chatterbox.template.settings.item.get(this.selector);
if(f==false){return}if(!f.hasOwnProperty("events")){return}var e=[];for(var a in f.events){e=f.events[a];if(!b.hasOwnProperty(e[0])){continue}c.find(e[1]).bind(e[0],b[e[0]])}};Chatterbox.Settings.Item.prototype._event_stub=function(){};Chatterbox.Settings.Item.prototype._get_cb=function(a){if(!this.options.hasOwnProperty("event")){return this._event_stub}return this.options.event[a]||this._event_stub};Chatterbox.Settings.Item.prototype._get_ep=function(b){var e=Chatterbox.template.settings.item.get(this.selector);if(e==null){return false}if(!e.hasOwnProperty("events")){return false}var c=[];for(var a in e.events){c=e.events[a];if(c[0]==b){return c}}};Chatterbox.Settings.Item.prototype.save=function(e,f){var g=this._get_ep("inspect");var b=g?this.view.find(g[1]):null;var a=this._get_cb("save");if(typeof a=="function"){a({input:b,item:this,page:f,window:e})}else{for(var c in a){var h=b.hasOwnProperty("slice")?b.slice(c,1):b;a[c]({input:h,item:this,page:f,window:e})}}for(var c in this.items){this.items[c].save(e,f)
}};Chatterbox.Settings.Item.prototype.close=function(e,f){var g=this._get_ep("inspect");var b=g?this.view.find(g[1]):null;var a=this._get_cb("close");if(typeof a=="function"){a({input:b,item:this,page:f,window:e});return}for(var c in a){var h=b.hasOwnProperty("slice")?b.slice(c,1):b;a[c]({input:h,item:this,page:f,window:e})}for(var c in this.items){this.items[c].close(e,f)}};Chatterbox.Settings.Item.get=function(g,c,j,b){var f=g.split(".");var h=j||Chatterbox.Settings.Item;var a;for(var e in f){a=f[e];if(!h.hasOwnProperty(a)){h=b||Chatterbox.Settings.Item;break}h=h[a]}return new h(g,c)};Chatterbox.Settings.Item.Form=function(b,a){Chatterbox.Settings.Item.call(this,b,a);this.form=null;this.fields=[];this.lsection=null;this.fsection=null;this.fieldo={}};Chatterbox.Settings.Item.Form.prototype=new Chatterbox.Settings.Item();Chatterbox.Settings.Item.Form.prototype.constructor=Chatterbox.Settings.Item.Form;Chatterbox.Settings.Item.Form.field=function(b,a){return Chatterbox.Settings.Item.get(b,a,Chatterbox.Settings.Item.Form,Chatterbox.Settings.Item.Form.Field)
