var Chatterbox={};Chatterbox.VERSION="0.6.47";Chatterbox.STATE="beta";Chatterbox.UI=function(a,b,e,c){this.events=c||new EventEmitter();this.mozilla=e;this.umuted=[];this.settings={themes:["wsct_default","wsct_dAmn"],theme:"wsct_default",monitor:["~Monitor",true],username:"",domain:"website.com",clock:true,tabclose:true};a.extend(this.settings,b);a.append('<div class="wsc '+this.settings.theme+'"></div>');this.view=a.find(".wsc");this.mns=this.format_ns(this.settings.monitor[0]);this.lun=this.settings.username.toLowerCase();this.monitoro=null;this.swidth=(function(){if($.browser.msie){var j=$('<textarea cols="10" rows="2"></textarea>').css({position:"absolute",top:-1000,left:-1000}).appendTo("body"),h=$('<textarea cols="10" rows="2" style="overflow: hidden;"></textarea>').css({position:"absolute",top:-1000,left:-1000}).appendTo("body");scrollbarWidth=j.width()-h.width();j.add(h).remove()}else{var g=$("<div />").css({width:100,height:100,overflow:"auto",position:"absolute",top:-1000,left:-1000}).prependTo("body").append("<div />").find("div").css({width:"100%",height:200});
...
this.view.find("input[checked]:checkbox").each(function(){a.value.push(a.view.find(this).val())});this.field.bind("change",function(e){a.value=[];a.view.find("input[checked]:checkbox").each(function(){a.value.push(a.view.find(this).val())})})};Chatterbox.Settings.Item.Form.Field.prototype.get=function(){return this.value};Chatterbox.template={};Chatterbox.render=function(e,h){var c=Chatterbox.template;var b=e.split(".");var g={};var a=null;for(ind in b){part=b[ind];if(!c.hasOwnProperty(part)){return""}c=c[part]}if(c.hasOwnProperty("frame")){a=c;g=c.render||{};c=c.frame;if(a.hasOwnProperty("pre")){if(typeof a.pre=="function"){c=a.pre(c,h)}else{for(i in a.pre){c=a.pre[i](c,h)}}}}for(key in h){c=replaceAll(c,"{"+key+"}",(g[key]||Chatterbox.template.render_stub)(h[key]||""))}if(a!=null){if(a.hasOwnProperty("post")){if(typeof a.post=="function"){c=a.post(c,h)}else{for(i in a.post){c=a.post[i](c,h)}}}}return c};Chatterbox.template.render_stub=function(a){return a};Chatterbox.template.clean=function(a){return function(b,c){for(i in a){b=replaceAll(b,"{"+a[i]+"}","")
