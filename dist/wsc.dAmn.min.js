...
};var E=function(M,L){var K=M.settings.page("About",true);K.item("Text",{ref:"about-wsc",title:"Wsc",text:'Currently using <a href="http://github.com/photofroggy/wsc/">wsc</a>                    version '+wsc.VERSION+" "+wsc.STATE+'.\n\nWsc                    works using HTML5, javascript, and CSS3. WebSocket is used for the connection                    where possible. The source code for this client is pretty huge.\n\nWsc was created                    by ~<a href="http://photofroggy.deviantart.com/">photofroggy</a>'})};var g=function(L,K){K.ui.theme(L.args.split(" ").shift())};var b=function(M){var L=M.args.split(" ");var K=L.shift().toLowerCase();var L=L.join(" ");if(L.length==0){l.cchannel.serverMessage("Could not set "+K,"No data supplied");return}if(!(K in l.settings)){l.cchannel.serverMessage('Unknown setting "'+K+'"');return}l.settings[K]=L;l.cchannel.serverMessage("Changed "+K+" setting","value: "+L);l.control.setLabel()};var p=function(K){l.connect()};var G=function(K){var L=K.args.split(" ");
var L=L.toString()==""?[]:L;if(K.ns!=K.target){L.unshift(K.target)}if(L.toString()==""){return}for(index in L){l.join(L[index])}};var I=function(K){var L=K.args.split(" ");if(K.ns!=K.target){L.unshift(K.target)}if(L.toString()==""){l.part(K.ns);return}for(index in L){l.part(L[index])}};var j=function(K){l.set(K.target,"title",K.args)};var v=function(L){var K=L.args.split(" ");l.promote(L.target,K[0],K[1])};var z=function(K){l.action(K.target,K.args)};var H=function(K){l.send(K.args.replace(/\\n/gm,"\n"))};var A=function(K){d=K.args.split(" ");u=d.shift();r=d.length>0?d.join(" "):null;l.kick(K.target,u,r)};var q=function(K){l.say(K.target,K.args)};var J=function(K){l.npmsg(K.target,K.args)};var F=function(L,K){K.cchannel.clear()};var D=function(L,K){for(c in K.channelo){K.channelo[c].clear()}};var C=function(L,K){K.whois(L.args.split(" ")[0])};var a=function(L,K){if(L.p!="info"){return}subs=L.pkt.sub;data=subs.shift().arg;data.username=L.sns.substr(1);data.connections=[];while(subs.length>0){conn=subs.shift().arg;
conn.channels=[];while(subs.length>0){if(subs[0].cmd!="ns"){break}conn.channels.unshift(K.deform_ns(subs.shift().param))}data.connections.push(conn)}K.cchannel.show_whois(data)};B()};wsc.Client=function(a,g,j){this.mozilla=j;this.fresh=true;this.attempts=0;this.protocol=null;this.flow=null;this.ui=null;this.events=new EventEmitter();this.conn=null;this.channelo={};this.cchannel=null;this.cmds=[];this.settings={domain:"website.com",server:"ws://website.com/wsendpoint",symbol:"",username:"",userinfo:{},pk:"",monitor:["~Monitor",true],welcome:"Welcome to the wsc web client!",autojoin:"chat:channel",control:wsc.Control,protocol:wsc.Protocol,tablumps:wsc.Tablumps,flow:wsc.Flow,ui:Chatterbox.UI,extend:[wsc.defaults.Extension],client:"chatclient",clientver:"0.3",theme:wsc.defaults.theme,themes:wsc.defaults.themes,};a.extend(this.settings,g);this.ui=new this.settings.ui(a,{themes:this.settings.themes,theme:this.settings.theme,monitor:this.settings.monitor,username:this.settings.username,domain:this.settings.domain},j);
