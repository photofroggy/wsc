...
}}arg=bits.join(" ");this.client.trigger("cmd."+cmdn,{name:"cmd",cmd:cmdn,args:arg,target:etarget,ns:ens})};var Chatterbox={};Chatterbox.VERSION="0.8.53";Chatterbox.STATE="beta";Chatterbox.UI=function(a,b,j,g){this.events=g||new EventEmitter();this.mozilla=j;this.umuted=[];this.settings={themes:["wsct_default","wsct_dAmn"],theme:"wsct_default",monitor:["~Monitor",true],username:"",domain:"website.com",clock:true,tabclose:true};a.extend(this.settings,b);a.append('<div class="wsc '+this.settings.theme+'"></div>');this.view=a.find(".wsc");this.mns=this.format_ns(this.settings.monitor[0]);this.lun=this.settings.username.toLowerCase();this.monitoro=null;this.swidth=(function(){if($.browser.msie){var m=$('<textarea cols="10" rows="2"></textarea>').css({position:"absolute",top:-1000,left:-1000}).appendTo("body"),l=$('<textarea cols="10" rows="2" style="overflow: hidden;"></textarea>').css({position:"absolute",top:-1000,left:-1000}).appendTo("body");scrollbarWidth=m.width()-l.width();m.add(l).remove()
...
Chatterbox.template.settings.item.form.field.text.pre=Chatterbox.template.settings.item.hint.prep;Chatterbox.template.settings.item.form.field.text.post=Chatterbox.template.clean(["title","text"]);Chatterbox.template.settings.item.form.field.text.render={title:Chatterbox.template.settings.krender.title,text:Chatterbox.template.settings.krender.text};Chatterbox.template.settings.item.form.field.text.frame="{title}<p>                                        {text}                                    </p>";wsc.dAmn={};wsc.dAmn.VERSION="0.3.10";wsc.dAmn.STATE="alpha";wsc.dAmn.Emotes=function(a,g,b){b.emotes.emote={};b.emotes.page=null;b.emotes.fint=null;b.emotes.notice=null;b.emotes.fetching=false;b.emotes.loaded=false;b.emotes.picker=new wsc.dAmn.Emotes.Picker(a.ui,{event:{select:function(j){b.emotes.select(j)},reload:function(){b.emotes.fetch()}}},b);b.emotes.picker.build();b.emotes.picker.hide();a.ui.control.add_button({label:"",icon:"user",href:"#emotes",title:"Emote picker.",handler:function(){if(b.emotes.picker.window.css("display")=="none"){b.emotes.picker.show()
}else{b.emotes.picker.close()}}});b.emotes.configure_page=function(k,m){var l=k.settings.page("Emotes");b.emotes.page=l;var p={};p.on=b.emotes.on;var j="";if(p.on){if(!b.emotes.loaded||b.emotes.fetching){j="<em>Fetching emotes...</em>"}else{j="<em>Emotes loaded.</em>"}}l.item("Form",{ref:"switch",title:"Enable Emotes",text:'Here you can turn custom emotes on and off.\n\n<span class="emotestatus">'+j+"</span>",fields:[["Checkbox",{ref:"enabled",items:[{value:"yes",title:"On",selected:p.on}]}]],event:{change:function(q){b.emotes.on=(q.data.enabled.indexOf("yes")!=-1);if(b.emotes.on){b.emotes.fetch();return}if(b.emotes.fint===null){return}clearTimeout(b.emotes.fint);b.emotes.fint=null},save:function(q){p.on=b.emotes.on;b.save()},close:function(q){b.emotes.on=p.on;b.load();b.emotes.page=null}}})};a.ui.on("settings.open.ran",b.emotes.configure_page);b.emotes.fetch=function(){if(b.emotes.loaded){b.emotes.picker.loading("Reloading...")}else{b.emotes.picker.loading()}b.emotes.fetching=true;jQuery.getJSON("http://www.thezikes.org/publicemotes.php?format=jsonp&jsoncallback=?&"+(new Date()).getDay(),function(j){b.emotes.fetching=false;
b.emotes.emote=j;if(!b.emotes.loaded){if(b.emotes.on){b.emotes.notice=new Chatterbox.Popup(a.ui,{title:"Emotes",content:"Custom emoticons loaded!"});b.emotes.notice.build();b.emotes.notice.window.css({bottom:60,right:50});setTimeout(function(){if(b.emotes.notice==null){return}b.emotes.notice.window.fadeOut("slow",function(){b.emotes.notice.window.remove();b.emotes.notice=null})},5000)}}b.emotes.sort();b.emotes.loaded=true;b.emotes.picker.loaded();if(b.emotes.page!==null){b.emotes.page.view.find(".emotestatus").html("<em>Emotes loaded.</em>")}b.emotes.fint=setTimeout(b.emotes.fetch,3600000)},function(){b.emotes.picker.loaded();return false});if(b.emotes.page!==null){b.emotes.page.view.find(".emotestatus").html("<em>Fetching emotes...</em>")}};b.emotes.swap=function(l){if(!b.emotes.on){return}var j=-1;for(var k in b.emotes.emote){if(!b.emotes.emote.hasOwnProperty(k)){continue}j=l.input.indexOf(k);if(j==-1){continue}l.input=replaceAll(l.input,k,":thumb"+b.emotes.emote[k]["devid"]+":")}l.input=replaceAll(l.input,":B",":bucktooth:")
};b.emotes.prepare=function(j){j=replaceAll(j,"&","&amp;");j=replaceAll(j,"<","&lt;");j=replaceAll(j,">","&gt;");return replaceAll(j,'"',"&quot;")};b.emotes.repair=function(j){j=replaceAll(j,"&quot;",'"');j=replaceAll(j,"&lt;","<");j=replaceAll(j,"&gt;",">");return replaceAll(j,"&amp;","&")};b.emotes.sort=function(){var j=[[],[],[],[],[],[],[],[],[],[],[],[],[],[],[],[],[],[],[],[],[],[],[],[],[],[],[],];var p="ABCDEFGHIJKLMNOPQRSTUVWXYZ#";var k=null;var r=null;var l=false;var v=-1;for(var q in b.emotes.emote){if(!b.emotes.emote.hasOwnProperty(q)){continue}k=b.emotes.emote[q];r={value:b.emotes.prepare(k.code),title:"created by "+k.by};v=p.indexOf(k.code.substr(1,1).toUpperCase());if(v==-1){v=p.indexOf("#")}j[v].push(r)}var u=function(A,z){return caseInsensitiveSort(A.value,z.value)};var m=b.emotes.picker.page("#");var s=null;for(var q=0;q<p.length;q++){j[q].sort(u);s=b.emotes.picker.page(p[q],m);s.options.items=j[q]}b.emotes.picker.refresh()};b.emotes.select=function(j){a.ui.control.set_text(a.ui.control.get_text()+b.emotes.repair(j))
};a.bind("send.msg.before",b.emotes.swap);a.bind("send.action.before",b.emotes.swap);a.bind("send.kick.before",b.emotes.swap);a.bind("send.set.before",b.emotes.swap);if(!b.emotes.on){return}b.emotes.fetch()};wsc.dAmn.Emotes.Picker=function(l,a,g){a=a||{};a=Object.extend({title:"Emotes",event:{select:function(p){},reload:function(){}}},a);Chatterbox.Popup.ItemPicker.call(this,l,a);this.settings=g;this.rbutton=null;var m="ABCDEFGHIJKLMNOPQRSTUVWXYZ";var k="";var j="";for(var b=0;b<m.length;b++){k=m[b];j=k.toLowerCase();this.add_page({ref:j,href:"#"+j,label:k,title:"Emotes beginning with "+k,items:[]})}this.add_page({ref:"misc",href:"#misc",label:"#",title:"Emotes not beginning with letters",items:[]})};wsc.dAmn.Emotes.Picker.prototype=new Chatterbox.Popup.ItemPicker();wsc.dAmn.Emotes.Picker.prototype.constructor=wsc.dAmn.Emotes.Picker;wsc.dAmn.Emotes.Picker.prototype.hide=function(){this.window.css({display:"none"})};wsc.dAmn.Emotes.Picker.prototype.show=function(){this.window.css({display:"block"})
};wsc.dAmn.Emotes.Picker.prototype.close=function(){this.hide()};wsc.dAmn.Emotes.Picker.prototype.build=function(b){var a=this;Chatterbox.Popup.ItemPicker.prototype.build.call(this,b);var g="Reload";this.rbutton=this.add_button({href:"#reload",title:"Reload emoticons",label:"Reload",});if(this.settings.emotes.fetching){this.loading("Loading...")}this.rbutton.click(function(){if(a.settings.emotes.fetching){return false}if(a.rbutton.hasClass("evented")){return false}a.reload();return false})};wsc.dAmn.Emotes.Picker.prototype.loaded=function(){this.rbutton.html("Reload");this.rbutton.removeClass("evented")};wsc.dAmn.Emotes.Picker.prototype.loading=function(a){a=a||"Reloading...";this.rbutton.addClass("evented");this.rbutton.html(a)};wsc.dAmn.Emotes.Picker.prototype.reload=function(){this.loading();this.options.event.reload()};wsc.dAmn.Extension=function(a){a.settings.client="dAmnClient";a.settings.clientver="0.3";a.settings.domain="deviantart.com";a.settings.agent+=" wsc/dAmn/"+wsc.dAmn.VERSION;
var g=a.storage.folder("dAmn");g.emotes=g.folder("emotes");var b={emotes:{on:false,emote:[]}};b.save=function(){g.emotes.set("on",b.emotes.on)};b.load=function(){b.emotes.on=(g.emotes.get("on","false")=="true")};b.load();b.save();a.protocol.extend_maps({dAmnServer:["version"]});a.protocol.extend_messages({dAmnServer:['<span class="servermsg">** Connected to dAmnServer {version} *</span>',false,true]});a.protocol.mparser=new wsc.dAmn.TablumpParser;a.flow.dAmnServer=a.flow.chatserver;a.exclude.push("#devart");a.ui.on("userinfo.before",function(j,k){j.user.avatar=wsc.dAmn.avatar.link(j.user.name,j.user.member.usericon);if(j.user.member.realname){j.user.info.push(j.user.member.realname)}if(j.user.member.typename){j.user.info.push(j.user.member.typename)}});a.ui.on("log_whois.before",function(j,k){j.avatar=wsc.dAmn.avatar.link(j.raw.username,j.raw.usericon);j.username=j.raw.symbol+'<b><a href="http://'+j.raw.username+'.deviantart.com/">'+j.raw.username+"</a></b>";if(j.raw.realname){j.info.push(j.raw.realname)
}if(j.raw.typename){j.info.push(j.raw.typename)}});wsc.dAmn.Emotes(a,g.emotes,b)};wsc.dAmn.avatar={};wsc.dAmn.avatar.ext=["gif","gif","jpg","png"];wsc.dAmn.avatar.link=function(b,j){j=parseInt(j);var l=(j>>2)&15;j=j&3;var g=wsc.dAmn.avatar.ext[j]||"gif";if(l){l="?"+l}else{l=""}if(j==0){k="default"}else{var a=new RegExp("\\$un(\\[([0-9]+)\\])","g");var k="$un[0]/$un[1]/{un}".replace(a,function(p,r,q){return b[q]=="-"?"_":b[q].toLowerCase()});k=replaceAll(k,"{un}",b.toLowerCase())}return'<a target="_blank" title=":icon'+b+':" href="http://'+b+'.deviantart.com/"><img class="avatar"            alt=":icon'+b+':" src="http://a.deviantart.net/avatars/'+k+"."+g+l+'" height="50" width="50" /></a>'};wsc.dAmn.TablumpString=function(a,b){this._parser=b||new wsc.dAmn.Tablumps();this.tokens=this._parser.tokenise(a);this.raw=a;this._text=null;this._html=null;this._ansi=null};wsc.dAmn.TablumpString.prototype=new wsc.MessageString;wsc.dAmn.TablumpString.prototype.constructor=wsc.dAmn.TablumpString;with(wsc.dAmn.TablumpString.prototype=new String){constructor=wsc.dAmn.TablumpString;
