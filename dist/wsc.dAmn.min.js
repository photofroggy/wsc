...
}b.trigger("peer.request",{name:"peer.request",ns:h.ns,call:g,user:f,app:l,version:a,peer:j});return true},wsc.dAmn.BDS.Peer.SignalHandler.prototype.ack=function(h,b){if(h.sns[0]!="@"){return}var g=b.bds.peer.call(h.param[0]);if(!g){return}var f=h.param[1];var j=h.param[2];var a=parseInt(h.param[3]);if(g.timeout!=null){clearTimeout(g.timeout);g.timeout=null}};wsc.dAmn.BDS.Peer.SignalHandler.prototype.reject=function(g,a){if(g.sns[0]!="@"){return}var f=a.bds.peer.call(g.param[0]);var b=g.param[1];var j=g.param[2];if(!f){return}var h=f.peer(b);if(!h){h=f.new_peer(b)}h.reject(j);a.trigger("peer.reject",{name:"peer.reject",ns:g.ns,pns:g.param[0],user:b,reason:j})};wsc.dAmn.BDS.Peer.SignalHandler.prototype.accept=function(h,b){if(h.sns[0]!="@"){return}var g=b.bds.peer.call(h.param[0]);if(!g){return}var f=h.param[1];var k=h.param[2];var a=parseInt(h.param[3]);if(f.toLowerCase()!=b.settings.username.toLowerCase()){return}var j=g.peer(h.user);if(!j){j=g.new_peer(h.user)}b.trigger("peer.accept",{name:"peer.accept",ns:h.ns,pns:h.param[0],app:k,version:a,call:g,peer:j})
};wsc.dAmn.BDS.Peer.SignalHandler.prototype.open=function(b,a){};wsc.dAmn.BDS.Peer.SignalHandler.prototype.end=function(b,a){};wsc.dAmn.BDS.Peer.SignalHandler.prototype.offer=function(h,a){if(h.sns[0]!="@"){return}var f=a.bds.peer.call(h.param[0]);if(!f){return}var l=f.pns;var b=h.param[1];var k=h.param[2];var g=new wsc.dAmn.BDS.Peer.RTC.SessionDescription(JSON.parse(h.param.slice(3).join(",")));if(k.toLowerCase()!=a.settings.username.toLowerCase()){return}var j=f.peer(b);if(!j){j=f.new_peer(b)}a.trigger("peer.offer",{name:"peer.offer",ns:h.ns,pns:f.pns,call:f,peer:j,offer:g});j.set_remote_description(g,0)};wsc.dAmn.BDS.Peer.SignalHandler.prototype.answer=function(g,a){if(g.sns[0]!="@"){return}var f=a.bds.peer.call(g.param[0]);if(!f){return}var b=g.param[1];var k=g.param[2];var j=new wsc.dAmn.BDS.Peer.RTC.SessionDescription(JSON.parse(g.param.slice(3).join(",")));if(k.toLowerCase()!=a.settings.username.toLowerCase()){return}var h=f.peer(b);if(!h){h=f.new_peer(b)}a.trigger("peer.answer",{name:"peer.answer",ns:g.ns,pns:f.pns,call:f,peer:h,answer:j});
h.set_remote_description(j,1)};wsc.dAmn.BDS.Peer.SignalHandler.prototype.candidate=function(h,a){if(h.sns[0]!="@"){return}var f=a.bds.peer.call(h.param[0]);if(!f){return}var j=f.peer(h.param[1]);if(!j){return}var k=h.param[2];if(k.toLowerCase()!=a.settings.username.toLowerCase()){return}var b=JSON.parse(h.param.slice(3).join(","));if(!b){j.ice_completed();return}var g=new wsc.dAmn.BDS.Peer.RTC.IceCandidate(b);j.candidate(g);a.trigger("peer.candidate",{name:"peer.candidate",ns:h.ns,pns:f.pns,call:f,peer:j,candidate:b,ice:g})};wsc.dAmn.BDS.Peer.SignalHandler.prototype.close=function(f,a){if(f.sns[0]!="@"){return}var b=a.bds.peer.call(f.param[0]);if(!b){return}var g=b.peer(f.param[1]);if(!g){return}if(g.user.toLowerCase()==a.settings.username.toLowerCase()){return}b.remove(g.user);a.trigger("peer.close",{name:"peer.close",ns:f.ns,evt:f,call:b,peer:g})};wsc.dAmn.BDS.Peer.RTC={PeerConnection:null,SessionDescription:null,IceCandidate:null,};if(window.mozRTCPeerConnection){wsc.dAmn.BDS.Peer.RTC.PeerConnection=mozRTCPeerConnection;
