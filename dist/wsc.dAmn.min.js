...
}n=s[o];tmod=f.mute_user(n);if(!tmod){continue}p=tmod;r=replaceAll(g.ignore,"{user}",n);if(r.indexOf("/me ")==0){r=r.substr(4);f.action(q.target,r)}else{f.say(q.target,r)}}if(p){k()}};var j=function(r){var v=r.args.split(" ");var o="";var u="";var n=-1;var q=false;var s=false;for(var p in v){if(!v.hasOwnProperty(p)){continue}o=v[p];s=f.unmute_user(o);if(!s){continue}q=s;u=replaceAll(g.unignore,"{user}",o);if(u.indexOf("/me ")==0){u=u.substr(4);f.action(r.target,u)}else{f.say(r.target,u)}}if(q){k()}};var l=function(){g.ignore=h.get("ignore","/me is ignoring {user} now");g.unignore=h.get("unignore","/me is not ignoring {user} anymore");g.count=parseInt(h.get("count",0));var o=null;if(g.count>0){o=null;for(var n=0;n<g.count;n++){}}};var k=function(){h.set("ignore",g.ignore);h.set("unignore",g.unignore);for(var n=0;n<g.count;n++){a.remove(n)}};m()};wsc.Client=function(b,f,g){this.mozilla=g;this.storage=new wsc.Storage;this.storage.aj=this.storage.folder("autojoin");this.storage.aj.channel=this.storage.aj.folder("channel");
this.fresh=true;this.attempts=0;this.connected=false;this.protocol=null;this.flow=null;this.ui=null;this.events=new EventEmitter();this.conn=null;this.channelo={};this.cchannel=null;this.cmds=[];this.settings={domain:"website.com",server:"ws://website.com/wsendpoint",symbol:"",username:"",userinfo:{},pk:"",monitor:["~Monitor",true],welcome:"Welcome to the wsc web client!",autojoin:"chat:channel",protocol:wsc.Protocol,mparser:wsc.MessageParser,flow:wsc.Flow,extend:[wsc.defaults.Extension],client:"chatclient",clientver:"0.3",developer:false};this.autojoin={on:true,count:0,channel:[]};this.away={};var a=this;this.exclude=new StringSet();this.hidden=new StringSet();this.settings=Object.extend(this.settings,f);this.config_load();this.config_save();this.mw=new wsc.Middleware();this.settings.agent="Client ("+navigator.platform+"; HTML5; JavaScript) wsc/"+wsc.VERSION+"-r"+wsc.REVISION;this.mns=this.format_ns(this.settings.monitor[0]);this.lun=this.settings.username.toLowerCase();this.protocol=new this.settings.protocol(new this.settings.mparser());
this.flow=new this.settings.flow(this.protocol);this.build();this.ext={};this.ext.defaults=wsc.defaults.Extension(this)};wsc.Client.prototype.config_load=function(){this.settings.developer=(this.storage.get("developer",this.settings.developer.toString())=="true");this.autojoin.on=(this.storage.aj.get("on","true")=="true");this.autojoin.count=parseInt(this.storage.aj.get("count","0"));this.autojoin.channel=[];var a=null;var f=0;for(var b=0;b<this.autojoin.count;b++){a=this.storage.aj.channel.get(b,null);if(a==null){continue}f++;this.autojoin.channel.push(a)}this.autojoin.count=f};wsc.Client.prototype.config_save=function(){this.storage.set("developer",this.settings.developer);this.storage.aj.set("on",this.autojoin.on.toString());this.storage.aj.set("count",this.autojoin.count);for(var a=0;a<this.autojoin.count;a++){this.storage.aj.channel.remove(a)}if(this.autojoin.channel.length==0){this.storage.aj.set("count",0)}else{var b=-1;for(var a in this.autojoin.channel){if(!this.autojoin.channel.hasOwnProperty(a)){continue
