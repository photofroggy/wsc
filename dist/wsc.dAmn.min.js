...
Chatterbox.template.settings.item.form.field.text.pre=Chatterbox.template.settings.item.hint.prep;Chatterbox.template.settings.item.form.field.text.post=Chatterbox.template.clean(["title","text"]);Chatterbox.template.settings.item.form.field.text.render={title:Chatterbox.template.settings.krender.title,text:Chatterbox.template.settings.krender.text};Chatterbox.template.settings.item.form.field.text.frame="{title}<p>                                        {text}                                    </p>";wsc.dAmn={};wsc.dAmn.VERSION="0.2.9";wsc.dAmn.STATE="alpha";wsc.dAmn.Emotes=function(a,g,b){b.emotes.page=null;b.emotes.configure_page=function(j,l){var k=j.settings.page("Emotes");b.emotes.page=k;var m={};m.on=b.emotes.on;k.item("Form",{ref:"switch",title:"Enable Emotes",text:"Here you can turn custom emotes on and off.",fields:[["Checkbox",{ref:"enabled",items:[{value:"yes",title:"On",selected:m.on}]}]],event:{change:function(p){b.emotes.on=(p.data.enabled.indexOf("yes")!=-1);if(b.emotes.on){b.emotes.fetch()
}},save:function(p){m.on=b.emotes.on;b.save()},close:function(p){b.emotes.on=m.on;b.load();b.emotes.page=null}}})};a.ui.on("settings.open.ran",b.emotes.configure_page);b.emotes.fetch=function(){jQuery.getJSON("http://www.thezikes.org/publicemotes.php?format=jsonp&jsoncallback=?&"+(new Date()).getDay(),function(j){b.emotes.emote=j})};b.emotes.swap=function(l){var j=-1;for(var k in b.emotes.emote){if(!b.emotes.emote.hasOwnProperty(k)){continue}j=l.input.indexOf(k);if(j==-1){continue}l.input=replaceAll(l.input,k,":thumb"+b.emotes.emote[k]["devid"]+":")}if(l.input.indexOf(":B")==-1){return}l.input=replaceAll(l.input,":B",":bucktooth:")};if(!b.emotes.on){return}b.emotes.fetch();a.bind("send.msg.before",b.emotes.swap);a.bind("send.action.before",b.emotes.swap);a.bind("send.kick.before",b.emotes.swap);a.bind("send.set.before",b.emotes.swap)};wsc.dAmn.Extension=function(a){a.settings.client="dAmnClient";a.settings.clientver="0.3";a.settings.domain="deviantart.com";a.settings.agent+=" wsc/dAmn/"+wsc.dAmn.VERSION;
