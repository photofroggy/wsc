...
wsc.Flow.prototype.recv_privchg=function(b,a){a.channel(b.ns).recv_privchg(b)};wsc.Flow.prototype.recv_kicked=function(b,a){a.channel(b.ns).recv_kicked(b)};wsc.defaults.Extension=function(p){var r={};r.away={on:false,reason:"",last:{},since:0,store:p.storage.folder("away"),format:{setaway:"/me is away: {reason}",setback:"/me is back",away:"{from}: I am away, reason: {reason}"}};var H=function(){p.bind("cmd.connect",g);p.bind("cmd.set",a);p.bind("cmd.join",N);p.bind("cmd.part",Q);p.bind("cmd.say",u);p.bind("cmd.npmsg",R);p.bind("cmd.me",B);p.bind("cmd.promote",E);p.bind("cmd.demote",E);p.bind("cmd.ban",m);p.bind("cmd.unban",m);p.bind("cmd.kick",j);p.bind("cmd.whois",J);p.bind("cmd.title",k);p.bind("cmd.topic",k);p.bind("cmd.admin",F);p.bind("cmd.disconnect",g);p.bind("cmd.kill",j);p.bind("cmd.raw",O);p.bind("cmd.clear",L);p.bind("cmd.clearall",K);p.bind("cmd.close",G);p.bind("pkt.property",l);p.bind("pkt.recv_admin_show",P);p.bind("pkt.recv_admin_showverbose",P);p.bind("pkt.get",C);p.bind("cmd.gettitle",A);
p.bind("cmd.gettopic",A);p.bind("cmd.setaway",v);p.bind("cmd.setback",s);p.bind("pkt.recv_msg.highlighted",z);p.bind("cmd.theme",b);p.ui.on("settings.open",q);p.ui.on("settings.open.ran",M);p.ui.on("settings.save",I)};var I=function(T,S){p.settings.ui.theme=T.theme;p.settings.ui.clock=T.clock;p.settings.ui.tabclose=T.tabclose;p.config_save()};var q=function(U,T){var S=U.settings.page("Main",true);var V={};V.username=p.settings.username;V.pk=p.settings.pk;S.item("Form",{ref:"login",title:"Login",text:"Here you can change the username and token used to                    log into the chat server.",fields:[["Textfield",{ref:"username",label:"Username","default":V.username}],["Textfield",{ref:"token",label:"Token","default":V.pk}]],event:{save:function(W){p.settings.username=W.data.username;p.settings.pk=W.data.token}}},true);S.item("Text",{ref:"intro",title:"Main",text:"Use this window to view and change your settings.\n\nCheck                    the different pages to see what settings can be changed.",},true);
S.item("Text",{ref:"debug",wclass:"faint",title:"Debug Information",text:"Chat Agent: <code>"+p.settings.agent+"</code>\n\nUser                    Agent: <code>"+navigator.userAgent+"</code>"})};var M=function(U,T){var S=U.settings.page("About",true);S.item("Text",{ref:"about-wsc",title:"Wsc",text:'Currently using <a href="http://github.com/photofroggy/wsc/">wsc</a>                    version '+wsc.VERSION+" "+wsc.STATE+'.\n\nWsc                    works using HTML5, javascript, and CSS3. WebSocket is used for the connection                    where possible. The source code for this client is pretty huge.\n\nWsc was created                    by ~<a href="http://photofroggy.deviantart.com/">photofroggy</a>'})};var b=function(T,S){S.ui.theme(T.args.split(" ").shift())};var a=function(U){var T=U.args.split(" ");var S=T.shift().toLowerCase();var T=T.join(" ");if(T.length==0){p.cchannel.serverMessage("Could not set "+S,"No data supplied");return}if(!(S in p.settings)){p.cchannel.serverMessage('Unknown setting "'+S+'"');
return}p.settings[S]=T;p.cchannel.serverMessage("Changed "+S+" setting","value: "+T);p.control.setLabel()};var g=function(S){p[S.cmd]()};var N=function(S){var T=S.args.split(" ");var T=T.toString()==""?[]:T;if(S.ns!=S.target){T.unshift(S.target)}if(T.toString()==""){return}for(index in T){p.join(T[index])}};var Q=function(S){var T=S.args.split(" ");if(S.ns!=S.target){T.unshift(S.target)}if(T.toString()==""){p.part(S.ns);return}for(index in T){p.part(T[index])}};var k=function(S){p.set(S.target,S.cmd,S.args)};var E=function(T){var S=T.args.split(" ");p[T.cmd.toLowerCase()](T.target,S[0],S[1])};var m=function(W,S){var U=W.args.split(" ");var T=U.shift();var V=W.cmd;if(V=="ban"&&U.length>0){S.kick(W.target,T,U.join(" "))}S[V](W.target,T)};var B=function(S){p.action(S.target,S.args)};var O=function(S){p.send(S.args.replace(/\\n/gm,"\n"))};var j=function(V,S){var W=V.args.split(" ");var T=W.shift();var U=W.length>0?W.join(" "):null;if(V.cmd=="kick"){S.kick(V.target,T,U)}else{S.kill(T,U)}};var u=function(S){p.say(S.target,S.args)
};var R=function(S){p.npmsg(S.target,S.args)};var L=function(U,S){if(U.args.length>0){var V=U.args.split(" ");for(var T in V){if(!V.hasOwnProperty(T)){continue}S.channel(U.target).clear(V[T])}}else{S.channel(U.target).clear()}};var K=function(T,S){var V=null;if(T.args.length>0){var U=T.args.split(" ");V=function(X,Y){for(var W in U){if(!U.hasOwnProperty(W)){continue}Y.clear(U[W])}}}else{V=function(W,X){X.clear()}}S.each_channel(V,true)};var G=function(S){p.part(S.target);p.remove_ns(S.target)};var J=function(T,S){S.whois(T.args.split(" ")[0])};var F=function(T,S){S.admin(T.target,T.args)};var D=function(T,S){S.disconnect()};var A=function(T,S){var U=T.cmd.indexOf("title")>-1?"title":"topic";S.control.ui.set_text("/"+U+" "+S.channel(T.target).info[U].content)};var v=function(U,S){r.away.on=true;r.away.last={};r.away.since=new Date();r.away.reason=U.args;var T="is away"+(r.away.reason.length>0?": "+r.away.reason:"");S.each_channel(function(V){S.action(V,T)})};var s=function(T,S){r.away.on=false;
S.each_channel(function(U){S.action(U,"is back")})};var z=function(V,S){if(!r.away.on){return}if(r.away.reason.length==0){return}if(V.user==S.settings.username){return}if(S.exclude.indexOf(V.sns.toLowerCase())!=-1){return}var T=new Date();var U=V.sns.toLowerCase();if(U in r.away.last){if((T-r.away.last[U])<=60000){return}}S.say(V.ns,V.user+": I am currently away; reason: "+r.away.reason);r.away.last[U]=T};var l=function(V,S){if(V.p!="info"){return}var T=V.pkt.sub;var W=T.shift().arg;W.username=V.sns.substr(1);W.connections=[];var U={};while(T.length>0){U=T.shift().arg;U.channels=[];while(T.length>0){if(T[0].cmd!="ns"){break}U.channels.unshift(S.deform_ns(T.shift().param))}W.connections.push(U)}S.cchannel.log_whois(W)};var C=function(T,S){if(T.ns.indexOf("login:")!=0){return}S.cchannel.server_message("Whois failed for "+(T.sns.substr(1)),"not online")};var P=function(W,T){var Z=T.channel(W.ns);var S=W.info.split("\n");var Y="";var X=[];var U="";for(var V in S){if(!S.hasOwnProperty(V)){continue
}Y=S[V].split(" ");if(W.p=="privclass"){X.push([Y.shift(),Y.shift().split("=")[1],Y.join(" ")])}else{if(W.p=="users"){U=Y.shift().split(":",1)[0];X.push([U,Z.get_privclass_order(U),Y.join(" ")])}}}Z.log_pc(W.p=="privclass",X)};H();wsc.defaults.Extension.Away(p);wsc.defaults.Extension.Ignore(p)};wsc.defaults.Extension.Away=function(a){var l=a.storage.folder("away");var b={on:false,reason:"",last:{},since:0,store:a.storage.folder("away"),format:{setaway:"/me is away: {reason}",setback:"/me is back",away:"{from}: I am away, reason: {reason}"}};var k=function(){j();g()};var j=function(){b.format.setaway=l.get("setaway","/me is away: {reason}");b.format.setback=l.get("setback","/me is back");b.format.away=l.get("away","{from}: I am away, reason: {reason}")};var g=function(){l.set("setaway",b.format.setaway);l.set("setback",b.format.setback);l.set("away",b.format.away)};k()};wsc.defaults.Extension.Ignore=function(g){var j={};var k=g.storage.folder("ignore");var a=k.folder("ignored");var q=function(){p();
