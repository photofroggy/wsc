...
if(s===null){continue}j.ignored.push(s)}}};var m=function(){k.set("ignore",j.ignore);k.set("unignore",j.unignore);for(var r=0;r<j.count;r++){a.remove(r)}if(j.ignored.length==0){k.set("count",0)}else{var s=-1;for(var r in j.ignored){if(!j.ignored.hasOwnProperty(r)){continue}s++;a.set(s.toString(),j.ignored[r])}s++;j.count=s;k.set("count",s)}};q()};wsc.defaults.Extension=function(p){var r={};r.away={on:false,reason:"",last:{},since:0};var G=function(){p.bind("cmd.connect",g);p.bind("cmd.set",a);p.bind("cmd.join",L);p.bind("cmd.part",O);p.bind("cmd.say",u);p.bind("cmd.npmsg",P);p.bind("cmd.me",B);p.bind("cmd.promote",E);p.bind("cmd.demote",E);p.bind("cmd.ban",m);p.bind("cmd.unban",m);p.bind("cmd.kick",j);p.bind("cmd.whois",H);p.bind("cmd.title",k);p.bind("cmd.topic",k);p.bind("cmd.admin",F);p.bind("cmd.disconnect",g);p.bind("cmd.kill",j);p.bind("cmd.raw",M);p.bind("cmd.clear",J);p.bind("cmd.clearall",I);p.bind("pkt.property",l);p.bind("pkt.recv_admin_show",N);p.bind("pkt.recv_admin_showverbose",N);
p.bind("pkt.get",C);p.bind("cmd.gettitle",A);p.bind("cmd.gettopic",A);p.bind("cmd.setaway",v);p.bind("cmd.setback",s);p.bind("pkt.recv_msg.highlighted",z);p.bind("cmd.theme",b);p.ui.on("settings.open",q);p.ui.on("settings.open.ran",K)};var q=function(S,R){page=S.settings.page("Main");var T={};T.theme=replaceAll(p.ui.settings.theme,"wsct_","");T.clock=p.ui.clock();T.tc=p.ui.nav.closer();T.username=p.settings.username;T.pk=p.settings.pk;var Q=[];for(i in p.ui.settings.themes){name=replaceAll(p.ui.settings.themes[i],"wsct_","");Q.push({value:name,title:name,selected:T.theme==name})}page.item("Text",{ref:"intro",title:"Main",text:"Use this window to view and change your settings.\n\nCheck                    the different pages to see what settings can be changed.",});page.item("Form",{ref:"login",title:"Login",text:"Here you can change the username and token used to                    log into the chat server.",fields:[["Textfield",{ref:"username",label:"Username","default":T.username}],["Textfield",{ref:"token",label:"Token","default":T.pk}]],event:{save:function(U){p.settings.username=U.data.username;
p.settings.pk=U.data.token}}});page.item("Form",{ref:"ui",title:"UI",hint:"<b>Timestamp</b><br/>Choose between a 24 hour clock and                    a 12 hour clock.\n\n<b>Theme</b><br/>Change the look of the                    client.\n\n<b>Close Buttons</b><br/>Turn tab close buttons on/off.",fields:[["Dropdown",{ref:"theme",label:"Theme",items:Q}],["Dropdown",{ref:"clock",label:"Timestamp Format",items:[{value:"24",title:"24 hour",selected:T.clock},{value:"12",title:"12 hour",selected:!T.clock}]}],["Check",{ref:"tabclose",label:"Close Buttons",items:[{value:"yes",title:"On",selected:T.tc}]}],],event:{change:function(U){p.ui.clock(U.data.clock=="24");p.ui.theme(U.data.theme);p.ui.nav.closer(U.data.tabclose.indexOf("yes")>-1)},save:function(U){T.clock=U.data.clock=="24";T.theme=U.data.theme;T.tc=U.data.tabclose.indexOf("yes")>-1;p.storage.set("theme","wsct_"+T.theme)},close:function(U){p.ui.clock(T.clock);p.ui.theme(T.theme);p.ui.nav.closer(T.tc)}}});page.item("Text",{ref:"debug",wclass:"faint",title:"Debug Information",text:"Chat Agent: <code>"+p.settings.agent+"</code>\n\nUser                    Agent: <code>"+navigator.userAgent+"</code>"})
...
chan.build()};wsc.Client.prototype.remove_ns=function(a){this.ui.remove_channel(a)};wsc.Client.prototype.log=function(a,b){var g=this.channel(a);if(!g){return}g.log(b)};wsc.Client.prototype.monitor=function(a){this.ui.monitor(a)};wsc.Client.prototype.send=function(a){return this.conn.send(a)};wsc.Client.prototype.handshake=function(){this.send(wsc_packetstr(this.settings.client,this.settings.clientver,{agent:this.settings.agent}))};wsc.Client.prototype.login=function(){pkt="login "+this.settings.username+"\npk="+this.settings.pk+"\n";this.send(pkt)};wsc.Client.prototype.pong=function(){this.send(wsc_packetstr("pong"))};wsc.Client.prototype.join=function(a){this.send(wsc_packetstr("join",this.format_ns(a)))};wsc.Client.prototype.part=function(a){this.send(wsc_packetstr("part",this.format_ns(a)))};wsc.Client.prototype.say=function(a,b){e={name:"send.msg.before",msg:b,ns:a};this.trigger(e);this.send(wsc_packetstr("send",this.format_ns(a),{},wsc_packetstr("msg","main",{},e.msg)))};wsc.Client.prototype.npmsg=function(a,b){e={name:"send.npmsg.before",msg:b,ns:a};
this.trigger(e);this.send(wsc_packetstr("send",this.format_ns(a),{},wsc_packetstr("npmsg","main",{},e.msg)))};wsc.Client.prototype.action=function(a,b){e={name:"send.action.before",msg:b,ns:a};this.trigger(e);this.send(wsc_packetstr("send",this.format_ns(a),{},wsc_packetstr("action","main",{},e.msg)))};wsc.Client.prototype.promote=function(g,a,b){this.send(wsc_packetstr("send",this.format_ns(g),{},wsc_packetstr("promote",a,{},(!b?"":b))))};wsc.Client.prototype.demote=function(g,a,b){this.send(wsc_packetstr("send",this.format_ns(g),{},wsc_packetstr("demote",a,{},(!b?"":b))))};wsc.Client.prototype.ban=function(b,a){this.send(wsc_packetstr("send",this.format_ns(b),{},wsc_packetstr("ban",a)))};wsc.Client.prototype.unban=function(b,a){this.send(wsc_packetstr("send",this.format_ns(namespace),{},wsc_packetstr("unban",a,{},(!pc?"":pc))))};wsc.Client.prototype.kick=function(b,a,g){this.send(wsc_packetstr("kick",this.format_ns(b),{u:a},g||null))};wsc.Client.prototype.kill=function(a,b){this.send(wsc_packetstr("kill",a,{},b||null))
...
}}arg=bits.join(" ");this.client.trigger("cmd."+cmdn,{name:"cmd",cmd:cmdn,args:arg,target:etarget,ns:ens})};var Chatterbox={};Chatterbox.VERSION="0.5.40";Chatterbox.STATE="beta";Chatterbox.UI=function(a,b,j,g){this.events=g||new EventEmitter();this.mozilla=j;this.settings={themes:["wsct_default","wsct_dAmn"],theme:"wsct_default",monitor:["~Monitor",true],username:"",domain:"website.com",clock:true};a.extend(this.settings,b);a.append('<div class="wsc '+this.settings.theme+'"></div>');this.view=a.find(".wsc");this.mns=this.format_ns(this.settings.monitor[0]);this.lun=this.settings.username.toLowerCase();this.monitoro=null;this.swidth=(function(){if($.browser.msie){var m=$('<textarea cols="10" rows="2"></textarea>').css({position:"absolute",top:-1000,left:-1000}).appendTo("body"),l=$('<textarea cols="10" rows="2" style="overflow: hidden;"></textarea>').css({position:"absolute",top:-1000,left:-1000}).appendTo("body");scrollbarWidth=m.width()-l.width();m.add(l).remove()}else{var k=$("<div />").css({width:100,height:100,overflow:"auto",position:"absolute",top:-1000,left:-1000}).prependTo("body").append("<div />").find("div").css({width:"100%",height:200});
