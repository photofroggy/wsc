...
j.unignore=z.data.unignore}}})};var b=function(v){var A=v.args.split(" ");var r="";var z="";var u=false;for(var s in A){if(!A.hasOwnProperty(s)){continue}r=A[s];if(r.length==0){continue}u=true;z=replaceAll(j.ignore,"{user}",r.toLowerCase());if(z.indexOf("/me ")==0){z=z.substr(4);g.action(v.target,z)}else{g.say(v.target,z)}r=r.toLowerCase();j.ignored.push(r)}if(u){m()}};var l=function(z){var B=z.args.split(" ");var s="";var A="";var r=-1;var v=false;for(var u in B){if(!B.hasOwnProperty(u)){continue}s=B[u];if(s.length==0){continue}r=j.ignored.indexOf(s);if(r<0){continue}v=true;A=replaceAll(j.unignore,"{user}",s.toLowerCase());if(A.indexOf("/me ")==0){A=A.substr(4);g.action(z.target,A)}else{g.say(z.target,A)}s=s.toLowerCase();j.ignored.splice(r,1)}if(v){m()}};var p=function(){j.ignore=k.get("ignore","/me is ignoring {user} now");j.unignore=k.get("unignore","/me is not ignoring {user} anymore");j.count=parseInt(k.get("count",0));j.ignored=[];if(j.count>0){var s=null;for(var r=0;r<j.count;r++){s=a.get(r,null);
if(s===null){continue}j.ignored.push(s)}}};var m=function(){k.set("ignore",j.ignore);k.set("unignore",j.unignore);for(var r=0;r<j.count;r++){a.remove(r)}if(j.ignored.length==0){k.set("count",0)}else{var s=-1;for(var r in j.ignored){if(!j.ignored.hasOwnProperty(r)){continue}s++;a.set(s.toString(),j.ignored[r])}s++;j.count=s;k.set("count",s)}};q()};wsc.Client=function(a,g,j){this.mozilla=j;this.storage=new wsc.Storage;this.fresh=true;this.attempts=0;this.connected=false;this.protocol=null;this.flow=null;this.ui=null;this.events=new EventEmitter();this.conn=null;this.channelo={};this.cchannel=null;this.exclude=[];this.cmds=[];this.settings={domain:"website.com",server:"ws://website.com/wsendpoint",symbol:"",username:"",userinfo:{},pk:"",monitor:["~Monitor",true],welcome:"Welcome to the wsc web client!",autojoin:"chat:channel",control:wsc.Control,protocol:wsc.Protocol,mparser:wsc.MessageParser,flow:wsc.Flow,ui:Chatterbox.UI,extend:[wsc.defaults.Extension],client:"chatclient",clientver:"0.3",theme:wsc.defaults.theme,themes:wsc.defaults.themes,};
