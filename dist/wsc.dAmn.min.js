...
};var Chatterbox={};Chatterbox.VERSION="0.19.94";Chatterbox.STATE="beta";Chatterbox.UI=function(b,a,f,h,g){this.client=b;this.events=g||new EventEmitter();this.mozilla=h;this.umuted=[];this.viewing=true;this.settings={themes:["wsct_dAmn","wsct_dark"],theme:"wsct_dark",monitor:["~Monitor",true],username:"",domain:"website.com",clock:true,tabclose:true,developer:false,media:"/static/"};var j=this;this.sound={bank:{m:null},add:function(k,l){if(j.sound.hasOwnProperty(k)){return false}j.sound.bank[k]=l;l.load();j.sound[k]=function(){j.sound.play(j.sound.bank[k])};return true},play:function(k){k.pause();k.currentTime=0;k.play()},toggle:function(l){for(var k in j.sound.bank){if(!j.sound.bank.hasOwnProperty(k)){continue}j.sound.bank[k].muted=l}},mute:function(){j.sound.toggle(true)},unmute:function(){j.sound.toggle(false)},};a.extend(this.settings,f);a.append('<div class="wsc '+this.settings.theme+'"></div>');this.mw=new wsc.Middleware();this.view=a.find(".wsc");this.mns=this.format_ns(this.settings.monitor[0]);
