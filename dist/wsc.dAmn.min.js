...
}g.autojoin.channel.splice(l,1);g.autojoin.save();return true};var a=function(m,l){if(m.e!="ok"){return}if(k){l.join(l.settings.autojoin);g.autojoin.join()}else{var n=false;for(key in l.channelo){if(l.channelo[key].namespace[0]!="~"){l.join(key);n=true}}if(!n){g.autojoin.join()}}k=false};var f=function(m){var q=m.args.split(" ");if(!q){return}var u=q.shift().toLowerCase();var r=null;var v="";var n="";var s=false;var l=b.channel(m.ns);switch(u){case"add":r=g.autojoin.add;v="Added {item} to your autojoin.";n="Already have {item} on your autojoin.";break;case"rem":case"remove":r=g.autojoin.remove;v="Removed {item} from your autojoin.";n=w+" is not on your autojoin list.";break;case"on":b.trigger("log",{name:"log",ns:"server:current",sns:"~current",msg:"Autojoin on"});if(!g.autojoin.on){s=true;g.autojoin.on=true}break;case"off":b.trigger("log",{name:"log",ns:"server:current",sns:"~current",msg:"Autojoin off"});if(g.autojoin.on){s=true;g.autojoin.on=false}break;default:g.autojoin.join();break
}var w="";var p=false;if(r!=null){for(var o in q){if(!q.hasOwnProperty(o)){continue}w=b.deform_ns(q[o]).toLowerCase();p=r(w);b.trigger("log",{name:"log",ns:"server:current",sns:"~current",msg:replaceAll((p?v:n),"{item}",w)})}}if(s){g.autojoin.save()}};h()};wsc.defaults.Extension.Away=function(b,g){var k=b.storage.folder("away");g.away={on:false,reason:"",last:{},since:0,interval:60000,format:{setaway:"/me is away: {reason}",setback:"/me is back",away:"{user}: I've been away for {timesince}. Reason: {reason}"}};b.away=g.away;var j=function(){g.away.load();g.away.save();b.bind("cmd.setaway",f);b.bind("cmd.setback",h);b.bind("pkt.recv_msg",a);b.bind("pkt.recv_action",a)};var f=function(n,l){g.away.on=true;g.away.last={};g.away.since=new Date();g.away.reason=n.args;var o=l.say;var m=replaceAll(g.away.format.setaway,"{reason}",g.away.reason||"[silent away]");if(m.indexOf("/me ")==0){m=m.substr(4);o=l.action}l.each_channel(function(p){o.call(l,p,m)});l.trigger("ext.away.away",{name:"ext.away.away",reason:g.away.reason||"[silent away]"})
