...
k.options.items=a.autojoin.channel},add:function(u){var v=u.args.swap;a.autojoin.channel[v["this"].index]=v.that.item;a.autojoin.channel[v.that.index]=v["this"].item;k.options.items=a.autojoin.channel},remove:function(u){var v=u.args.swap;a.autojoin.channel[v["this"].index]=v.that.item;a.autojoin.channel[v.that.index]=v["this"].item;k.options.items=a.autojoin.channel},save:function(u){s.chan=a.autojoin.channel;a.config_save()},close:function(u){a.autojoin.channel=s.chan;a.config_save()}}})};var b=function(s){var l=s.args.split(" ");if(!l){return}var q=l.shift().toLowerCase();var k=function(v){};var p=false;var u=a.channel(s.ns);switch(q){case"add":k=function(v){if(a.autojoin.channel.indexOf(v)==-1){p=true;a.autojoin.channel.push(v);u.server_message("Added "+v+" to your autojoin.")}else{u.server_message("Already have "+v+" on your autojoin.")}};break;case"rem":case"remove":k=function(z){var v=a.autojoin.channel.indexOf(z);if(v!=-1){p=true;a.autojoin.channel.splice(v,1);u.server_message("Removed "+z+" from your autojoin.")
...
g.interval=parseInt(j.get("interval",60000))};var k=function(){j.set("interval",g.interval);j.set("setaway",g.format.setaway);j.set("setback",g.format.setback);j.set("away",g.format.away)};q()};wsc.defaults.Extension.Ignore=function(g){var j={};var k=g.storage.folder("ignore");var a=k.folder("ignored");var q=function(){p();m();g.bind("cmd.ignore",b);g.bind("cmd.unignore",l);g.ui.on("settings.open",j.page)};j.page=function(u,z){var v=u.settings.page("Ignores",true);var A={};var s="<ul>";A.im=j.ignore;A.uim=j.unignore;if(g.ui.umuted.length==0){s+="<li><i>No one ignored yet</i></li></ul>"}else{for(var r in g.ui.umuted){if(!g.ui.umuted.hasOwnProperty(r)){continue}s+="<li>"+g.ui.umuted[r]+"</li>"}s+="</ul>"}v.item("Text",{ref:"intro",title:"Ignores",text:'Use <code>ignore</code> to ignore people.\n\n                    You can "ignore" other users of the chat server using the\n                    <code>/ignore</code> command. Ignoring a user hides their                    messages from you in the channel log.',});
v.item("Form",{ref:"msgs",title:"Messages",text:"Here you can set the messages displayed when you ignore or                    unignore a user.\n\nThe text <code>{user}</code> is replaced                    with the name of the user your are ignoring or unignoring.",fields:[["Textfield",{ref:"ignore",label:"Ignore","default":A.im}],["Textfield",{ref:"unignore",label:"Unignore","default":A.uim}]],event:{save:function(B){j.ignore=B.data.ignore;j.unignore=B.data.unignore;m()}}});v.item("Items",{ref:"ignoreds",title:"Users",text:"This is the list of users that you have silenced.\n\nUse the                    commands <code>/ignore</code> and <code>/unignore</code>                    to edit the list.",items:g.ui.umuted})};var b=function(v){var A=v.args.split(" ");var r="";var z="";var u=false;for(var s in A){if(!A.hasOwnProperty(s)){continue}r=A[s];tmod=g.mute_user(r);if(!tmod){continue}u=tmod;z=replaceAll(j.ignore,"{user}",r);if(z.indexOf("/me ")==0){z=z.substr(4);g.action(v.target,z)}else{g.say(v.target,z)
}}if(u){m()}};var l=function(z){var C=z.args.split(" ");var s="";var B="";var r=-1;var v=false;var A=false;for(var u in C){if(!C.hasOwnProperty(u)){continue}s=C[u];A=g.unmute_user(s);if(!A){continue}v=A;B=replaceAll(j.unignore,"{user}",s);if(B.indexOf("/me ")==0){B=B.substr(4);g.action(z.target,B)}else{g.say(z.target,B)}}if(v){m()}};var p=function(){j.ignore=k.get("ignore","/me is ignoring {user} now");j.unignore=k.get("unignore","/me is not ignoring {user} anymore");j.count=parseInt(k.get("count",0));g.ui.umuted=[];if(j.count>0){var s=null;for(var r=0;r<j.count;r++){g.mute_user.push(a.get(r,null))}}};var m=function(){k.set("ignore",j.ignore);k.set("unignore",j.unignore);for(var r=0;r<j.count;r++){a.remove(r)}if(g.ui.umuted.length==0){k.set("count",0)}else{var s=-1;for(var r in g.ui.umuted){if(!g.ui.umuted.hasOwnProperty(r)){continue}s++;a.set(s.toString(),g.ui.umuted[r])}s++;j.count=s;k.set("count",s)}};q()};wsc.Client=function(a,g,j){this.mozilla=j;this.storage=new wsc.Storage;this.storage.ui=this.storage.folder("ui");
...
Chatterbox.VERSION="0.6.50";Chatterbox.STATE="beta";Chatterbox.UI=function(a,b,j,g){this.events=g||new EventEmitter();this.mozilla=j;this.umuted=[];this.settings={themes:["wsct_default","wsct_dAmn"],theme:"wsct_default",monitor:["~Monitor",true],username:"",domain:"website.com",clock:true,tabclose:true};a.extend(this.settings,b);a.append('<div class="wsc '+this.settings.theme+'"></div>');this.view=a.find(".wsc");this.mns=this.format_ns(this.settings.monitor[0]);this.lun=this.settings.username.toLowerCase();this.monitoro=null;this.swidth=(function(){if($.browser.msie){var m=$('<textarea cols="10" rows="2"></textarea>').css({position:"absolute",top:-1000,left:-1000}).appendTo("body"),l=$('<textarea cols="10" rows="2" style="overflow: hidden;"></textarea>').css({position:"absolute",top:-1000,left:-1000}).appendTo("body");scrollbarWidth=m.width()-l.width();m.add(l).remove()}else{var k=$("<div />").css({width:100,height:100,overflow:"auto",position:"absolute",top:-1000,left:-1000}).prependTo("body").append("<div />").find("div").css({width:"100%",height:200});
