...
};wsc.Flow.prototype.recv_action=wsc.Flow.prototype.recv_msg;wsc.Flow.prototype.recv_npmsg=wsc.Flow.prototype.recv_msg;wsc.Flow.prototype.recv_privchg=function(b,a){a.channel(b.ns).recv_privchg(b)};wsc.Flow.prototype.recv_kicked=function(b,a){a.channel(b.ns).recv_kicked(b)};wsc.defaults.Extension=function(b){var f={};f.away={on:false,reason:"",last:{},since:0,store:b.storage.folder("away"),format:{setaway:"/me is away: {reason}",setback:"/me is back",away:"{from}: I am away, reason: {reason}"}};var k=function(){b.bind("cmd.connect",g.connection);b.bind("cmd.set",g.setter);b.bind("cmd.join",g.join);b.bind("cmd.chat",g.pjoin);b.bind("cmd.part",g.part);b.bind("cmd.say",g.say);b.bind("cmd.npmsg",g.npmsg);b.bind("cmd.me",g.action);b.bind("cmd.promote",g.chgpriv);b.bind("cmd.demote",g.chgpriv);b.bind("cmd.ban",g.ban);b.bind("cmd.unban",g.ban);b.bind("cmd.kick",g.killk);b.bind("cmd.whois",g.whois);b.bind("cmd.title",g.title);b.bind("cmd.topic",g.title);b.bind("cmd.admin",g.admin);b.bind("cmd.disconnect",g.connection);
b.bind("cmd.kill",g.killk);b.bind("cmd.raw",g.raw);b.bind("cmd.close",g.close);b.bind("pkt.property",h);b.bind("pkt.recv_admin_show",a);b.bind("pkt.recv_admin_showverbose",a);b.bind("pkt.get",j);b.bind("cmd.gettitle",g.gett);b.bind("cmd.gettopic",g.gett);b.bind("cmd.theme",g.theme)};f.save=function(m,l){b.settings.ui.theme=m.theme;b.settings.ui.clock=m.clock;b.settings.ui.tabclose=m.tabclose};var g={};g.theme=function(m,l){l.ui.theme(m.args.split(" ").shift())};g.setter=function(n){var m=n.args.split(" ");var l=m.shift().toLowerCase();var m=m.join(" ");if(m.length==0){b.cchannel.serverMessage("Could not set "+l,"No data supplied");return}if(!(l in b.settings)){b.cchannel.serverMessage('Unknown setting "'+l+'"');return}b.settings[l]=m;b.cchannel.serverMessage("Changed "+l+" setting","value: "+m)};g.connection=function(l){b[l.cmd]()};g.join=function(l){var m=l.args.split(" ");var m=m.toString()==""?[]:m;if(l.ns!=l.target){m.unshift(l.target)}if(m.toString()==""){return}for(index in m){b.join(m[index])
}};g.pjoin=function(l){var m=l.args.split(" ");var m=m.toString()==""?[]:m;if(l.ns!=l.target){m.unshift(l.target)}if(m.toString()==""){return}for(index in m){b.join("@"+m[index])}};g.part=function(l){var m=l.args.split(" ");if(l.ns!=l.target){m.unshift(l.target)}if(m.toString()==""){b.part(l.ns);return}for(index in m){b.part(m[index])}};g.title=function(l){b.set(l.target,l.cmd,l.args)};g.chgpriv=function(m){var l=m.args.split(" ");b[m.cmd.toLowerCase()](m.target,l[0],l[1])};g.ban=function(p,l){var n=p.args.split(" ");var m=n.shift();var o=p.cmd;if(o=="ban"&&n.length>0){l.kick(p.target,m,n.join(" "))}l[o](p.target,m)};g.action=function(l){b.action(l.target,l.args)};g.raw=function(l){b.send(l.args.replace(/\\n/gm,"\n"))};g.killk=function(o,l){var p=o.args.split(" ");var m=p.shift();var n=p.length>0?p.join(" "):null;if(o.cmd=="kick"){l.kick(o.target,m,n)}else{l.kill(m,n)}};g.say=function(l){if(b.channel(l.target).monitor){return}b.say(l.target,l.args)};g.npmsg=function(l){b.npmsg(l.target,l.args)
};g.clear=function(n,l){if(n.args.length>0){var o=n.args.split(" ");for(var m in o){if(!o.hasOwnProperty(m)){continue}l.ui.channel(n.target).clear_user(o[m])}}else{l.ui.channel(n.target).clear()}};g.clearall=function(m,l){var o=null;if(m.args.length>0){var n=m.args.split(" ");o=function(q,r){for(var p in n){if(!n.hasOwnProperty(p)){continue}r.clear_user(n[p])}}}else{o=function(p,q){q.clear()}}l.ui.chatbook.each(o,true)};g.close=function(l){b.part(l.target);b.remove_ns(l.target)};g.whois=function(m,l){l.whois(m.args.split(" ")[0])};g.admin=function(m,l){l.admin(m.target,m.args)};g.disconnect=function(m,l){l.disconnect()};g.gett=function(m,l){var n=m.cmd.indexOf("title")>-1?"title":"topic";l.ui.control.set_text("/"+n+" "+l.channel(m.target).info[n].content)};var h=function(p,l){if(p.p!="info"){return}var n=p.pkt.sub;var q=n.shift().arg;q.username=p.sns.substr(1);q.connections=[];var o=null;var r={};while(n.length>0){r=n.shift();switch(r.cmd){case"conn":if(o!=null){q.connections.push(o)}o=r.arg;
