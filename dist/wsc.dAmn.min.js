var wsc={};wsc.VERSION="0.9.67";wsc.STATE="beta";wsc.defaults={};wsc.defaults.theme="wsct_default";wsc.defaults.themes=["wsct_default","wsct_dAmn"];wsc.base={};wsc.base.UI=function(a,b,j,g){this.LIB="base";this.VERSION="1";this.STATE="rc";this.channels=[]};wsc.base.UI.prototype.build=function(){};wsc.base.UI.prototype.on=function(b,a){};wsc.base.UI.prototype.loop=function(){};wsc.base.UI.prototype.create_channel=function(a,b){};wsc.base.UI.prototype.remove_channel=function(a){};wsc.base.UI.prototype.channel=function(a){};wsc.base.UI.Channel=function(a,b){this.namespace=a;this.hidden=b};wsc.defaults.UI=wsc.base.UI;function EventEmitter(){var k={},a=this;function j(p,q){var m=k[p]||false;if(m===false){k[p]=[q];return a}k[p].unshift(q);return a}function g(m){k[m]=[];return a}function b(){var m=Array.prototype.slice.call(arguments);var r=m.shift();var q=k[r]||false;if(q===false){return a}for(var p in q){if(q.hasOwnProperty(p)){bubble=q[p].apply({},m);if(bubble===false){break}}}return a}function l(m){return k[m]||[]
...
a=j[1]}else{var b=j[0]=="*"?j.slice(1):j;m[j]=p.arg[b]||"";a=j}}}if(typeof g[1]=="string"){m[j]=p.arg.slice(0)}break;case 2:if(j instanceof Array){p.sub[0].sub=p.sub.slice(1);this.map(p.sub[0],m,j)}else{m[j]=p.body}break;case 3:m[j]=p.raw;break}if(a[0]!="*"){continue}b=a.slice(1);val=this.tablumps.parse(m[a]);m[b]=val}};wsc.Protocol.prototype.render=function(a,b){msgm=this.messages[a.name];if(!msgm){return""}msg=msgm[0];for(key in a){if(!a.hasOwnProperty(key)||key=="pkt"){continue}d=a[key];if(key=="ns"||key=="sns"){key="ns";d=a.sns}if(d.hasOwnProperty("_parser")){switch(b){case"text":d=d.text();break;case"html":d=d.html();break;case"ansi":d=d.ansi();break;default:d=d.text();break}}msg=replaceAll(msg,"{"+key+"}",d)}return msg};wsc.Protocol.prototype.log=function(a,g){msgm=this.messages[g.name];if(!msgm){return}if(g.s=="0"){return}msg=this.render(g,"html");try{if(!msgm[2]){if(!msgm[1]){a.ui.channel(g.ns).log_item(msg)}else{a.ui.channel(a.mns).log_item(msg)}}else{a.ui.log_item(msg)}}catch(b){console.log(">> Failed to log message for "+g.sns+"::");
console.log(">> "+msg)}};wsc.Flow=function(a){this.protocol=a||new wsc.Protocol()};wsc.Flow.prototype.open=function(a,g,b){a.trigger("connected",{name:"connected",pkt:new wsc.Packet("connected\n\n")});a.connected=true;a.handshake();a.attempts=0};wsc.Flow.prototype.close=function(a,b){a.trigger("closed",{name:"closed",pkt:new wsc.Packet("connection closed\n\n")});if(a.connected){a.ui.server_message("Connection closed");a.connected=false;if(a.conn instanceof wsc.SocketIO){a.ui.server_message("At the moment there is a problem with reconnecting under socket.io.");a.ui.server_message("Refresh the page to connect.");return}}else{a.ui.server_message("Connection failed")}if(a.attempts>2){a.ui.server_message("Can't connect. Try again later.");a.attempts=0;return}a.ui.server_message("Connecting in 2 seconds");setTimeout(function(){a.connect()},2000)};wsc.Flow.prototype.message=function(a,j){var b=new wsc.Packet(decodeURIComponent(j.data));if(b==null){return}var g=this.protocol.parse(b);if(g.ns==null){g.ns=a.mns
}g.sns=a.deform_ns(g.ns);this.protocol.log(a,g);this.handle(a,g);a.trigger("pkt",g);a.trigger("pkt."+g.name,g)};wsc.Flow.prototype.handle=function(a,g){try{this[g.name](g,a)}catch(b){}};wsc.Flow.prototype.ping=function(b,a){a.pong()};wsc.Flow.prototype.chatserver=function(b,a){a.login()};wsc.Flow.prototype.login=function(b,a){if(b.pkt.arg["e"]=="ok"){info=new wsc.Packet("info\n"+b.data);a.settings.username=b.pkt.param;a.settings.symbol=info.arg.symbol;a.settings.userinfo=info.arg;if(a.fresh){a.join(a.settings.autojoin)}else{for(key in a.channelo){if(a.channelo[key].namespace[0]!="~"){a.join(key)}}}}else{}if(a.fresh){a.fresh=false}};wsc.Flow.prototype.join=function(b,a){if(b.pkt.arg["e"]=="ok"){ns=a.deform_ns(b.pkt.param);a.create_ns(ns);a.ui.channel(ns).server_message("You have joined "+ns)}else{a.ui.chatbook.current.server_message("Failed to join "+a.deform_ns(b.pkt.param),b.pkt.arg["e"])}};wsc.Flow.prototype.part=function(b,a){ns=a.deform_ns(b.ns);c=a.channel(ns);if(b.e=="ok"){info="";
if(b.r.length>0){info="["+b.r+"]"}else{a.remove_ns(ns)}msg="You have left "+ns;c.server_message(msg,info);if(a.channels()==0){switch(b.r){case"bad data":case"bad msg":case"msg too big":break;default:if(b.r.indexOf("killed:")<0){return}break}this.message(a,{data:"disconnect\ne="+e.r+"\n"})}}else{a.monitor("Couldn't leave "+ns,b.e);c.server_message("Couldn't leave "+ns,b.e)}};wsc.Flow.prototype.kicked=function(b,a){if(b.r.toLowerCase().indexOf("autokicked")>-1){return}a.join(b.ns)};wsc.Flow.prototype.property=function(b,a){chan=a.channel(b.pkt.param);if(!chan){return}chan.property(b)};wsc.Flow.prototype.recv_joinpart=function(b,a){c=a.channel(b.ns);if(b.name=="recv_join"){c.recv_join(b)}else{c.recv_part(b)}};wsc.Flow.prototype.recv_join=wsc.Flow.prototype.recv_joinpart;wsc.Flow.prototype.recv_part=wsc.Flow.prototype.recv_joinpart;wsc.Flow.prototype.recv_msg=function(b,a){a.channel(b.ns).recv_msg(b)};wsc.Flow.prototype.recv_action=wsc.Flow.prototype.recv_msg;wsc.Flow.prototype.recv_npmsg=wsc.Flow.prototype.recv_msg;
wsc.Flow.prototype.recv_privchg=function(b,a){a.channel(b.ns).recv_privchg(b)};wsc.Flow.prototype.recv_kicked=function(b,a){a.channel(b.ns).recv_kicked(b)};wsc.Flow.prototype.disconnect=function(b,a){this.close(a,b)};wsc.defaults.Extension=function(p){var B=function(){p.bind("cmd.set",a);p.bind("cmd.connect",g);p.bind("cmd.join",G);p.bind("cmd.part",J);p.bind("cmd.say",r);p.bind("cmd.npmsg",K);p.bind("cmd.me",u);p.bind("cmd.promote",z);p.bind("cmd.demote",z);p.bind("cmd.ban",m);p.bind("cmd.unban",m);p.bind("cmd.kick",j);p.bind("cmd.whois",C);p.bind("cmd.title",k);p.bind("cmd.topic",k);p.bind("cmd.admin",A);p.bind("cmd.disconnect",g);p.bind("cmd.kill",j);p.bind("cmd.raw",H);p.bind("cmd.clear",E);p.bind("cmd.clearall",D);p.bind("pkt.property",l);p.bind("pkt.recv_admin_show",I);p.bind("pkt.recv_admin_showverbose",I);p.bind("pkt.get",v);p.bind("cmd.gettitle",s);p.bind("cmd.gettopic",s);p.bind("cmd.theme",b);p.ui.on("settings.open",q);p.ui.on("settings.open.ran",F)};var q=function(N,M){page=N.settings.page("Main");
var O={};O.theme=replaceAll(p.ui.settings.theme,"wsct_","");O.clock=p.ui.clock();O.tc=p.ui.nav.closer();O.username=p.settings.username;O.pk=p.settings.pk;var L=[];for(i in p.ui.settings.themes){name=replaceAll(p.ui.settings.themes[i],"wsct_","");L.push({value:name,title:name,selected:O.theme==name})}page.item("Text",{ref:"intro",title:"Main",text:"Use this window to view and change your settings.\n\nCheck                    the different pages to see what settings can be changed.",});page.item("Form",{ref:"login",title:"Login",text:"Here you can change the username and token used to                    log into the chat server.",fields:[["Textfield",{ref:"username",label:"Username","default":O.username}],["Textfield",{ref:"token",label:"Token","default":O.pk}]],event:{save:function(P){p.settings.username=P.data.username;p.settings.pk=P.data.token}}});page.item("Form",{ref:"ui",title:"UI",hint:"<b>Timestamp</b><br/>Choose between a 24 hour clock and                    a 12 hour clock.\n\n<b>Theme</b><br/>Change the look of the                    client.\n\n<b>Close Buttons</b><br/>Turn tab close buttons on/off.",fields:[["Dropdown",{ref:"theme",label:"Theme",items:L}],["Dropdown",{ref:"clock",label:"Timestamp Format",items:[{value:"24",title:"24 hour",selected:O.clock},{value:"12",title:"12 hour",selected:!O.clock}]}],["Check",{ref:"tabclose",label:"Close Buttons",items:[{value:"yes",title:"On",selected:O.tc}]}],],event:{change:function(P){p.ui.clock(P.data.clock=="24");
p.ui.theme(P.data.theme);p.ui.nav.closer(P.data.tabclose.indexOf("yes")>-1)},save:function(P){O.clock=P.data.clock=="24";O.theme=P.data.theme;O.tc=P.data.tabclose.indexOf("yes")>-1},close:function(P){p.ui.clock(O.clock);p.ui.theme(O.theme);p.ui.nav.closer(O.tc)}}});page.item("Text",{ref:"debug",wclass:"faint",title:"Debug Information",text:"Chat Agent: <code>"+p.settings.agent+"</code>\n\nUser                    Agent: <code>"+navigator.userAgent+"</code>"})};var F=function(N,M){var L=N.settings.page("About",true);L.item("Text",{ref:"about-wsc",title:"Wsc",text:'Currently using <a href="http://github.com/photofroggy/wsc/">wsc</a>                    version '+wsc.VERSION+" "+wsc.STATE+'.\n\nWsc                    works using HTML5, javascript, and CSS3. WebSocket is used for the connection                    where possible. The source code for this client is pretty huge.\n\nWsc was created                    by ~<a href="http://photofroggy.deviantart.com/">photofroggy</a>'})};var b=function(M,L){L.ui.theme(M.args.split(" ").shift())
};var a=function(N){var M=N.args.split(" ");var L=M.shift().toLowerCase();var M=M.join(" ");if(M.length==0){p.cchannel.serverMessage("Could not set "+L,"No data supplied");return}if(!(L in p.settings)){p.cchannel.serverMessage('Unknown setting "'+L+'"');return}p.settings[L]=M;p.cchannel.serverMessage("Changed "+L+" setting","value: "+M);p.control.setLabel()};var g=function(L){p[L.cmd]()};var G=function(L){var M=L.args.split(" ");var M=M.toString()==""?[]:M;if(L.ns!=L.target){M.unshift(L.target)}if(M.toString()==""){return}for(index in M){p.join(M[index])}};var J=function(L){var M=L.args.split(" ");if(L.ns!=L.target){M.unshift(L.target)}if(M.toString()==""){p.part(L.ns);return}for(index in M){p.part(M[index])}};var k=function(L){p.set(L.target,L.cmd,L.args)};var z=function(M){var L=M.args.split(" ");p[M.cmd.toLowerCase()](M.target,L[0],L[1])};var m=function(P,L){var N=P.args.split(" ");var M=N.shift();var O=P.cmd;if(O=="ban"&&N.length>0){L.kick(P.target,M,N.join(" "))}L[O](P.target,M)};var u=function(L){p.action(L.target,L.args)
};var H=function(L){p.send(L.args.replace(/\\n/gm,"\n"))};var j=function(O,L){var P=O.args.split(" ");var M=P.shift();var N=P.length>0?P.join(" "):null;if(O.cmd=="kick"){L.kick(O.target,M,N)}else{L.kill(M,N)}};var r=function(L){p.say(L.target,L.args)};var K=function(L){p.npmsg(L.target,L.args)};var E=function(M,L){L.cchannel.clear()};var D=function(M,L){for(c in L.channelo){L.channelo[c].clear()}};var C=function(M,L){L.whois(M.args.split(" ")[0])};var A=function(M,L){L.admin(M.target,M.args)};var A=function(M,L){L.disconnect()};var s=function(M,L){var N=M.cmd.indexOf("title")>-1?"title":"topic";L.control.ui.set_text("/"+N+" "+L.channel(M.target).info[N].content)};var l=function(M,L){if(M.p!="info"){return}subs=M.pkt.sub;data=subs.shift().arg;data.username=M.sns.substr(1);data.connections=[];while(subs.length>0){conn=subs.shift().arg;conn.channels=[];while(subs.length>0){if(subs[0].cmd!="ns"){break}conn.channels.unshift(L.deform_ns(subs.shift().param))}data.connections.push(conn)}L.cchannel.log_whois(data)
};var v=function(M,L){if(M.ns.indexOf("login:")!=0){return}L.cchannel.server_message("Whois failed for "+(M.sns.substr(1)),"not online")};var I=function(P,M){var S=M.channel(P.ns);var L=P.info.split("\n");var R="";var Q=[];var N="";for(var O in L){if(!L.hasOwnProperty(O)){continue}R=L[O].split(" ");if(P.p=="privclass"){Q.push([R.shift(),R.shift().split("=")[1],R.join(" ")])}else{if(P.p=="users"){N=R.shift().split(":",1)[0];Q.push([N,S.get_privclass_order(N),R.join(" ")])}}}S.log_pc(P.p=="privclass",Q)};B()};wsc.Client=function(a,g,j){this.mozilla=j;this.fresh=true;this.attempts=0;this.connected=false;this.protocol=null;this.flow=null;this.ui=null;this.events=new EventEmitter();this.conn=null;this.channelo={};this.cchannel=null;this.cmds=[];this.settings={domain:"website.com",server:"ws://website.com/wsendpoint",symbol:"",username:"",userinfo:{},pk:"",monitor:["~Monitor",true],welcome:"Welcome to the wsc web client!",autojoin:"chat:channel",control:wsc.Control,protocol:wsc.Protocol,tablumps:wsc.Tablumps,flow:wsc.Flow,ui:Chatterbox.UI,extend:[wsc.defaults.Extension],client:"chatclient",clientver:"0.3",theme:wsc.defaults.theme,themes:wsc.defaults.themes,};
a.extend(this.settings,g);this.ui=new this.settings.ui(a,{themes:this.settings.themes,theme:this.settings.theme,monitor:this.settings.monitor,username:this.settings.username,domain:this.settings.domain},j);this.settings.agent=this.ui.LIB+"/"+this.ui.VERSION+" ("+navigator.appVersion.match(/\(([^)]+)\)/)[1]+") wsc/"+wsc.VERSION;this.mns=this.format_ns(this.settings.monitor[0]);this.lun=this.settings.username.toLowerCase();this.protocol=new this.settings.protocol(new this.settings.tablumps());this.flow=new this.settings.flow(this.protocol);this.build();for(var b in this.settings.extend){this.settings.extend[b](this)}this.monitor(this.settings.welcome)};wsc.Client.prototype.build=function(){this.ui.build();this.control=new this.settings.control(this);var a=this;this.ui.on("channel.selected",function(b,g){a.cchannel=a.channel(b.ns);a.control.cache_input(b)});this.ui.on("tab.close.clicked",function(b,g){if(b.chan.monitor){return}a.part(b.ns)})};wsc.Client.prototype.loop=function(){this.ui.loop()
};wsc.Client.prototype.add_extension=function(a){this.settings.extend.push(a);a(this)};wsc.Client.prototype.bind=function(b,a){this.events.addListener(b,a);if(b.indexOf("cmd.")!=0||b.length<=4){return}cmd=b.slice(4).toLowerCase();this.cmds.push(cmd)};wsc.Client.prototype.clear_listeners=function(a){this.events.removeListeners(a)};wsc.Client.prototype.trigger=function(a,b){this.events.emit(a,b,this)};wsc.Client.prototype.connect=function(){if(this.connected){return}this.attempts++;console.log("hey");try{var a=this;this.conn=wsc.Transport.Create(this.settings.server);this.conn.open(function(g,j){a.flow.open(a,g,j)});this.conn.disconnect(function(g){a.flow.close(a,g)});this.conn.message(function(g){a.flow.message(a,g)});this.conn.connect();this.ui.server_message("Opening connection");this.trigger("start",new wsc.Packet("client connecting\ne=ok\n\n"))}catch(b){console.log(b);this.monitor("Your browser does not support WebSockets. Sorry.");this.trigger("start",new wsc.Packet("client connecting\ne=no websockets available\n\n"))
}};wsc.Client.prototype.disconnected=function(a){console.log("attempt reconnectiles");this.close()};wsc.Client.prototype.close=function(){console.log(this.conn);this.conn.close()};wsc.Client.prototype.channel=function(a,b){a=this.deform_ns(a).slice(1).toLowerCase();if(!this.channelo[a]&&b){this.channelo[a]=b}return this.channelo[a]};wsc.Client.prototype.channels=function(){chans=-1;for(ns in this.channelo){if(this.channelo[ns].hidden){continue}chans++}return chans};wsc.Client.prototype.deform_ns=function(a){if(a.indexOf("chat:")==0){return"#"+a.slice(5)}if(a.indexOf("server:")==0){return"~"+a.slice(7)}if(a.indexOf("pchat:")==0){var b=a.split(":");b.shift();for(i in b){name=b[i];if(name.toLowerCase()!=this.lun){return"@"+name}}}if(a.indexOf("login:")==0){return"@"+a.slice(6)}if(a[0]!="#"&&a[0]!="@"&&a[0]!="~"){return"#"+a}return a};wsc.Client.prototype.format_ns=function(a){if(a.indexOf("#")==0){return"chat:"+a.slice(1)}if(a.indexOf("@")==0){var b=[a.slice(1),this.lun];b.sort(caseInsensitiveSort);
...
};wsc.Client.prototype.say=function(a,b){e={name:"send.msg.before",msg:b,ns:a};this.trigger(e);this.send(wsc_packetstr("send",this.format_ns(a),{},wsc_packetstr("msg","main",{},e.msg)))};wsc.Client.prototype.npmsg=function(a,b){e={name:"send.npmsg.before",msg:b,ns:a};this.trigger(e);this.send(wsc_packetstr("send",this.format_ns(a),{},wsc_packetstr("npmsg","main",{},e.msg)))};wsc.Client.prototype.action=function(a,b){e={name:"send.action.before",msg:b,ns:a};this.trigger(e);this.send(wsc_packetstr("send",this.format_ns(a),{},wsc_packetstr("action","main",{},e.msg)))};wsc.Client.prototype.promote=function(g,a,b){this.send(wsc_packetstr("send",this.format_ns(g),{},wsc_packetstr("promote",a,{},(!b?"":b))))};wsc.Client.prototype.demote=function(g,a,b){this.send(wsc_packetstr("send",this.format_ns(g),{},wsc_packetstr("demote",a,{},(!b?"":b))))};wsc.Client.prototype.ban=function(b,a){this.send(wsc_packetstr("send",this.format_ns(b),{},wsc_packetstr("ban",a)))};wsc.Client.prototype.unban=function(b,a){this.send(wsc_packetstr("send",this.format_ns(namespace),{},wsc_packetstr("unban",a,{},(!pc?"":pc))))
...
}b.find(pair[1]).bind(pair[0],a[pair[0]])}};Chatterbox.Settings.Item.prototype._event_stub=function(){};Chatterbox.Settings.Item.prototype._get_cb=function(a){if(!this.options.hasOwnProperty("event")){return this._event_stub}return this.options.event[a]||this._event_stub};Chatterbox.Settings.Item.prototype._get_ep=function(a){var b=Chatterbox.template.settings.item.get(this.selector);if(b==null){return false}if(!b.hasOwnProperty("events")){return false}for(i in b.events){pair=b.events[i];if(pair[0]==a){return pair}}};Chatterbox.Settings.Item.prototype.save=function(g,j){var k=this._get_ep("inspect");var b=k==false?null:this.view.find(k[1]);var a=this._get_cb("save");if(typeof a=="function"){a({input:b,item:this,page:j,window:g})}else{for(i in a){var l=b.hasOwnProperty("slice")?b.slice(i,1):b;a[i]({input:l,item:this,page:j,window:g})}}for(i in this.items){this.items[i].save(g,j)}};Chatterbox.Settings.Item.prototype.close=function(a,b){pair=this._get_ep("inspect");inps=pair==false?null:this.view.find(pair[1]);
...
Chatterbox.Settings.Item.Check.prototype.build=function(g){if(this.options.hasOwnProperty("items")){for(i in this.options.items){var b=this.options.items[i];b.name=this.options.ref||"ref"}}Chatterbox.Settings.Item.prototype.build.call(this,g);this.field=this.view.find("input:checkbox");var a=this;this.value=[];this.view.find("input[checked]:checkbox").each(function(){a.value.push(a.view.find(this).val())});this.field.bind("change",function(j){a.value=[];a.view.find("input[checked]:checkbox").each(function(){a.value.push(a.view.find(this).val())})})};Chatterbox.Settings.Item.Form.Field.prototype.get=function(){return this.value};Chatterbox.template={};Chatterbox.render=function(j,l){var g=Chatterbox.template;var b=j.split(".");var k={};var a=null;for(ind in b){part=b[ind];if(!g.hasOwnProperty(part)){return""}g=g[part]}if(g.hasOwnProperty("frame")){a=g;k=g.render||{};g=g.frame;if(a.hasOwnProperty("pre")){if(typeof a.pre=="function"){g=a.pre(g,l)}else{for(i in a.pre){g=a.pre[i](g,l)}}}}for(key in l){g=replaceAll(g,"{"+key+"}",(k[key]||Chatterbox.template.render_stub)(l[key]||""))
}if(a!=null){if(a.hasOwnProperty("post")){if(typeof a.post=="function"){g=a.post(g,l)}else{for(i in a.post){g=a.post[i](g,l)}}}}return g};Chatterbox.template.render_stub=function(a){return a};Chatterbox.template.clean=function(a){return function(b,g){for(i in a){b=replaceAll(b,"{"+a[i]+"}","")}return b}};Chatterbox.template.ui='<nav class="tabs"><ul id="chattabs" class="tabs"></ul>        <ul id="tabnav">            <li><a href="#left" class="button iconic arrow_left"></a></li>            <li><a href="#right" class="button iconic arrow_right"></a></li>            <li><a href="#settings" title="Change client settings" class="button iconic cog" id="settings-button"></a></li>        </ul>        </nav>        <div class="chatbook"></div>';Chatterbox.template.control='<div class="chatcontrol">            <p><a href="#multiline" title="Toggle multiline input" class="button iconic list"></a></p>            <form class="msg">                <input type="text" class="msg" />                <textarea class="msg"></textarea>                <input type="submit" value="Send" class="sendmsg" />            </form>        </div>';
