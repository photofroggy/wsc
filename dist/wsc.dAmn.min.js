...
this.client.bind("cmd.set",this.setter.bind(b));this.client.bind("cmd.connect",this.connect.bind(b));this.client.bind("cmd.join",this.join.bind(b));this.client.bind("cmd.part",this.part.bind(b));this.client.bind("cmd.title",this.title.bind(b));this.client.bind("cmd.promote",this.promote.bind(b));this.client.bind("cmd.me",this.action.bind(b));this.client.bind("cmd.kick",this.kick.bind(b));this.client.bind("cmd.raw",this.raw.bind(b));this.client.bind("cmd.say",this.say.bind(b));this.client.bind("cmd.npmsg",this.npmsg.bind(b));this.client.bind("cmd.clear",this.clear.bind(b));this.client.bind("cmd.theme",this.theme.bind(b))},theme:function(j,g){theme=g.settings.theme;select=j.args.split(" ").shift();g.ui.theme(select)},setter:function(g){data=g.args.split(" ");setting=data.shift().toLowerCase();data=data.join(" ");if(data.length==0){this.client.cchannel.serverMessage("Could not set "+setting,"No data supplied");return}if(!(setting in this.client.settings)){this.client.cchannel.serverMessage('Unknown setting "'+setting+'"');
return}this.client.settings[setting]=data;this.client.cchannel.serverMessage("Changed "+setting+" setting","value: "+data);this.client.control.setLabel()},connect:function(g){this.client.connect()},join:function(g){chans=g.args.split(" ");chans=chans.toString()==""?[]:chans;if(g.ns!=g.target){chans.unshift(g.target)}if(chans.toString()==""){return}for(index in chans){b.client.join(chans[index])}},part:function(g){chans=g.args.split(" ");if(g.ns!=g.target){chans.unshift(g.target)}if(chans.toString()==""){b.client.part(g.ns);return}for(index in chans){b.client.part(chans[index])}},title:function(g){b.client.set(g.target,"title",g.args)},promote:function(g){bits=g.args.split(" ");b.client.promote(g.target,bits[0],bits[1])},action:function(g){b.client.action(g.target,g.args)},raw:function(g){b.client.send(g.args.replace(/\\n/gm,"\n"))},kick:function(g){d=g.args.split(" ");u=d.shift();r=d.length>0?d.join(" "):null;b.client.kick(g.target,u,r)},say:function(g){b.client.say(g.target,g.args)},npmsg:function(g){b.client.npmsg(g.target,g.args)
