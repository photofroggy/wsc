...
};var M=function(T,Q){var W=Q.channel(T.ns);var P=T.info.split("\n");var V="";var U=[];var R="";for(var S in P){if(!P.hasOwnProperty(S)){continue}V=P[S].split(" ");if(T.p=="privclass"){U.push([V.shift(),V.shift().split("=")[1],V.join(" ")])}else{if(T.p=="users"){R=V.shift().split(":",1)[0];U.push([R,W.get_privclass_order(R),V.join(" ")])}}}W.log_pc(T.p=="privclass",U)};E();wsc.defaults.Extension.Autojoin(p);wsc.defaults.Extension.Away(p);wsc.defaults.Extension.Ignore(p)};wsc.defaults.Extension.Autojoin=function(b){var j=b.storage.folder("away");j.channel=j.folder("channel");var g={on:false,channel:[],count:0};var q=function(){m();k();b.ui.on("settings.open",g.page)};g.page=function(r,u){var s=r.settings.page("Autojoin",true);s.item("Text",{ref:"intro",title:"Autojoin",text:"Add any channels you want to join automatically when you                    connect to the chat server.",})};var a=function(u,r){g.on=true;g.last={};g.since=new Date();g.reason=u.args;var v=r.say;var s=replaceAll(g.format.setaway,"{reason}",g.reason||"[silent away]");
if(s.indexOf("/me ")==0){s=s.substr(4);v=r.action}r.each_channel(function(z){v.call(r,z,s)})};var p=function(u,r){g.on=false;var v=r.say;var s=g.format.setback;if(s.indexOf("/me ")==0){s=s.substr(4);v=r.action}r.each_channel(function(z){v.call(r,z,s)})};var l=function(z,r){if(!g.on){return}if(g.reason.length==0){return}if(z.user==r.settings.username){return}if(r.exclude.indexOf(z.sns.toLowerCase())!=-1){return}var u=new Date();var v=z.sns.toLowerCase();if(v in g.last){if((u-g.last[v])<=g.interval){return}}var s=timeLengthString((u-g.since)/1000);var A=replaceAll(g.format.away,"{user}",z.user);A=replaceAll(A,"{timesince}",s);r.say(z.ns,replaceAll(A,"{reason}",g.reason));g.last[v]=u};var m=function(){g.on=(j.get("on","true")=="true");g.count=(parseInt(j.get("count","0")));var r=null;var u=0;for(var s=0;s<g.count;s++){r=j.channel.get(s,null);if(r==null){continue}u++;g.channel.push(r)}g.count=u};var k=function(){j.set("on",g.on.toString());j.set("count",g.count);for(var r=0;r<g.count;r++){j.channel.remove(r)
