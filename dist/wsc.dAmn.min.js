...
wsc.Flow.prototype.recv_msg=function(b,a){a.channel(b.ns).recv_msg(b)};wsc.Flow.prototype.recv_action=wsc.Flow.prototype.recv_msg;wsc.Flow.prototype.recv_npmsg=wsc.Flow.prototype.recv_msg;wsc.Flow.prototype.recv_privchg=function(b,a){a.channel(b.ns).recv_privchg(b)};wsc.Flow.prototype.recv_kicked=function(b,a){a.channel(b.ns).recv_kicked(b)};wsc.defaults.Extension=function(f){var a={};a.away={on:false,reason:"",last:{},since:0,store:f.storage.folder("away"),format:{setaway:"/me is away: {reason}",setback:"/me is back",away:"{from}: I am away, reason: {reason}"}};var n=function(){f.bind("cmd.connect",b.connection);f.bind("cmd.set",b.setter);f.bind("cmd.join",b.join);f.bind("cmd.chat",b.pjoin);f.bind("cmd.part",b.part);f.bind("cmd.say",b.say);f.bind("cmd.npmsg",b.npmsg);f.bind("cmd.me",b.action);f.bind("cmd.promote",b.chgpriv);f.bind("cmd.demote",b.chgpriv);f.bind("cmd.ban",b.ban);f.bind("cmd.unban",b.ban);f.bind("cmd.kick",b.killk);f.bind("cmd.whois",b.whois);f.bind("cmd.title",b.title);
f.bind("cmd.topic",b.title);f.bind("cmd.admin",b.admin);f.bind("cmd.disconnect",b.connection);f.bind("cmd.kill",b.killk);f.bind("cmd.raw",b.raw);f.bind("cmd.clear",b.clear);f.bind("cmd.clearall",b.clearall);f.bind("cmd.close",b.close);f.bind("pkt.property",m);f.bind("pkt.recv_admin_show",g);f.bind("pkt.recv_admin_showverbose",g);f.bind("pkt.get",l);f.bind("cmd.gettitle",b.gett);f.bind("cmd.gettopic",b.gett);f.bind("cmd.theme",b.theme);f.ui.on("settings.open",j);f.ui.on("settings.open.ran",h);f.ui.on("settings.save.ui",k);f.ui.on("settings.save",function(){f.config_save()})};var k=function(p,o){f.settings.ui.theme=p.theme;f.settings.ui.clock=p.clock;f.settings.ui.tabclose=p.tabclose};var j=function(q,p){var o=q.settings.page("Main");var r={};r.username=f.settings.username;r.pk=f.settings.pk;r.devel=f.settings.developer;o.item("Form",{ref:"login",title:"Login",text:"Here you can change the username and token used to                    log into the chat server.",fields:[["Textfield",{ref:"username",label:"Username","default":r.username}],["Textfield",{ref:"token",label:"Token","default":r.pk}]],event:{save:function(s){f.settings.username=s.data.username;
f.settings.pk=s.data.token}}},true);o.item("Form",{ref:"developer",title:"Developer Mode",text:"Turn developer mode on or off.\n\nDeveloper mode will expose any hidden                channel tabs, amongst other things. Keep this turned off unless you're working                on implementing something.",fields:[["Checkbox",{ref:"enabled",items:[{value:"on",title:"On",selected:r.devel}]}]],event:{change:function(s){f.settings.developer=(s.data.enabled.indexOf("on")!=-1);f.ui.developer(f.settings.developer)},save:function(s){r.devel=f.settings.developer},close:function(s){f.settings.developer=r.devel;f.ui.developer(f.settings.developer)}}});o.item("Text",{ref:"intro",title:"Main",text:"Use this window to view and change your settings.\n\nCheck                    the different pages to see what settings can be changed.",},true);o.item("Text",{ref:"debug",wclass:"faint",title:"Debug Information",text:"Chat Agent: <code>"+f.settings.agent+"</code>\n\nUser                    Agent: <code>"+navigator.userAgent+"</code>"})
};var h=function(q,p){var o=q.settings.page("About",true);o.item("Text",{ref:"about-wsc",title:"Wsc",text:'Currently using <a href="http://github.com/photofroggy/wsc/">wsc</a>                    version '+wsc.VERSION+" "+wsc.STATE+'.\n\nWsc                    works using HTML5, javascript, and CSS3. WebSocket is used for the connection                    where possible. The source code for this client is pretty huge.\n\nWsc was created                    by ~<a href="http://photofroggy.deviantart.com/">photofroggy</a>'})};var b={};b.theme=function(p,o){o.ui.theme(p.args.split(" ").shift())};b.setter=function(q){var p=q.args.split(" ");var o=p.shift().toLowerCase();var p=p.join(" ");if(p.length==0){f.cchannel.serverMessage("Could not set "+o,"No data supplied");return}if(!(o in f.settings)){f.cchannel.serverMessage('Unknown setting "'+o+'"');return}f.settings[o]=p;f.cchannel.serverMessage("Changed "+o+" setting","value: "+p)};b.connection=function(o){f[o.cmd]()};b.join=function(o){var p=o.args.split(" ");
var p=p.toString()==""?[]:p;if(o.ns!=o.target){p.unshift(o.target)}if(p.toString()==""){return}for(index in p){f.join(p[index])}};b.pjoin=function(o){var p=o.args.split(" ");var p=p.toString()==""?[]:p;if(o.ns!=o.target){p.unshift(o.target)}if(p.toString()==""){return}for(index in p){f.join("@"+p[index])}};b.part=function(o){var p=o.args.split(" ");if(o.ns!=o.target){p.unshift(o.target)}if(p.toString()==""){f.part(o.ns);return}for(index in p){f.part(p[index])}};b.title=function(o){f.set(o.target,o.cmd,o.args)};b.chgpriv=function(p){var o=p.args.split(" ");f[p.cmd.toLowerCase()](p.target,o[0],o[1])};b.ban=function(s,o){var q=s.args.split(" ");var p=q.shift();var r=s.cmd;if(r=="ban"&&q.length>0){o.kick(s.target,p,q.join(" "))}o[r](s.target,p)};b.action=function(o){f.action(o.target,o.args)};b.raw=function(o){f.send(o.args.replace(/\\n/gm,"\n"))};b.killk=function(s,o){var v=s.args.split(" ");var p=v.shift();var q=v.length>0?v.join(" "):null;if(s.cmd=="kick"){o.kick(s.target,p,q)}else{o.kill(p,q)
}};b.say=function(o){if(f.channel(o.target).monitor){return}f.say(o.target,o.args)};b.npmsg=function(o){f.npmsg(o.target,o.args)};b.clear=function(q,o){if(q.args.length>0){var r=q.args.split(" ");for(var p in r){if(!r.hasOwnProperty(p)){continue}o.channel(q.target).clear(r[p])}}else{o.channel(q.target).clear()}};b.clearall=function(p,o){var r=null;if(p.args.length>0){var q=p.args.split(" ");r=function(u,v){for(var s in q){if(!q.hasOwnProperty(s)){continue}v.clear(q[s])}}}else{r=function(s,u){u.clear()}}o.each_channel(r,true)};b.close=function(o){f.part(o.target);f.remove_ns(o.target)};b.whois=function(p,o){o.whois(p.args.split(" ")[0])};b.admin=function(p,o){o.admin(p.target,p.args)};b.disconnect=function(p,o){o.disconnect()};b.gett=function(p,o){var q=p.cmd.indexOf("title")>-1?"title":"topic";o.ui.control.set_text("/"+q+" "+o.channel(p.target).info[q].content)};var m=function(r,o){if(r.p!="info"){return}var p=r.pkt.sub;var s=p.shift().arg;s.username=r.sns.substr(1);s.connections=[];var q={};
while(p.length>0){q=p.shift().arg;q.channels=[];while(p.length>0){if(p[0].cmd!="ns"){break}q.channels.unshift(o.deform_ns(p.shift().param))}s.connections.push(q)}o.cchannel.log_whois(s)};var l=function(p,o){if(p.ns.indexOf("login:")!=0){return}var q=p.sns.substr(1);o.ui.pager.notice({ref:"whois-"+q,heading:"Whois Failed",content:"Whois failed for "+q+".\nNo such user online."},false,5000)};var g=function(s,p){var x=p.channel(s.ns);var o=s.info.split("\n");var v="";var u=[];var q="";for(var r in o){if(!o.hasOwnProperty(r)){continue}v=o[r].split(" ");if(s.p=="privclass"){u.push([v.shift(),v.shift().split("=")[1],v.join(" ")])}else{if(s.p=="users"){q=v.shift().split(":",1)[0];u.push([q,x.get_privclass_order(q),v.join(" ")])}}}x.log_pc(s.p=="privclass",u)};n();wsc.defaults.Extension.Ignore(f);wsc.defaults.Extension.Away(f);wsc.defaults.Extension.Autojoin(f)};wsc.defaults.Extension.Autojoin=function(a){var f=a.autojoin;a.ui.nav.add_button({icon:"chat",label:"",title:"Join your autojoin channels",href:"#autojoin-do",handler:function(){for(var h in a.autojoin.channel){if(!a.autojoin.channel.hasOwnProperty(h)){continue
