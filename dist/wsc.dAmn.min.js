...
var f=h.param[1];var l=h.param[2];var a=parseInt(h.param[3]);var g=b.bds.peer.call(k);if(!g){g=b.bds.peer.open(h.ns,k,f,l,a)}g.signal.ack(f,l,a);var j=g.peer(f);if(!j){j=g.new_peer(f)}b.trigger("peer.request",{name:"peer.request",ns:h.ns,call:g,user:f,app:l,version:a,peer:j});return true},wsc.dAmn.BDS.Peer.SignalHandler.prototype.ack=function(b,a){};wsc.dAmn.BDS.Peer.SignalHandler.prototype.reject=function(b,a){if(b.sns[0]!="@"){return}a.trigger("peer.reject",{name:"peer.reject",ns:b.ns,pns:b.param[0],user:b.param[1],reason:b.param[2]})};wsc.dAmn.BDS.Peer.SignalHandler.prototype.accept=function(h,b){if(h.sns[0]!="@"){return}var g=b.bds.peer.call(h.param[0]);if(!g){return}var f=h.param[1];var k=h.param[2];var a=parseInt(h.param[3]);if(f.toLowerCase()!=b.settings.username.toLowerCase()){return}var j=g.peer(h.user);if(!j){j=g.new_peer(h.user)}b.trigger("peer.accept",{name:"peer.accept",ns:h.ns,pns:h.param[0],app:k,version:a,call:g,peer:j})};wsc.dAmn.BDS.Peer.SignalHandler.prototype.open=function(b,a){};
...
};wsc.dAmn.BDS.Peer.SignalHandler.prototype.candidate=function(h,a){if(h.sns[0]!="@"){return}var f=a.bds.peer.call(h.param[0]);if(!f){return}var j=f.peer(h.param[1]);if(!j){return}var k=h.param[2];if(k.toLowerCase()!=a.settings.username.toLowerCase()){return}var b=JSON.parse(h.param.slice(3).join(","));if(!b){j.ice_completed();return}var g=new wsc.dAmn.BDS.Peer.RTC.IceCandidate(b);j.candidate(g);a.trigger("peer.candidate",{name:"peer.candidate",ns:h.ns,pns:f.pns,call:f,peer:j,candidate:b,ice:g})};wsc.dAmn.BDS.Peer.SignalHandler.prototype.close=function(f,a){if(f.sns[0]!="@"){return}var b=a.bds.peer.call(f.param[0]);if(!b){return}var g=b.peer(f.param[1]);console.log(">close",g);if(!g){return}if(g.user.toLowerCase()==a.settings.username.toLowerCase()){return}console.log(">remove");b.remove(g.user);a.trigger("peer.close",{name:"peer.close",ns:f.ns,evt:f,call:b,peer:g})};wsc.dAmn.BDS.Peer.RTC={PeerConnection:null,SessionDescription:null,IceCandidate:null,};if(window.mozRTCPeerConnection){wsc.dAmn.BDS.Peer.RTC.PeerConnection=mozRTCPeerConnection;
