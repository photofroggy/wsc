...
wsc.Flow.prototype.recv_privchg=function(b,a){a.channel(b.ns).recv_privchg(b)};wsc.Flow.prototype.recv_kicked=function(b,a){a.channel(b.ns).recv_kicked(b)};wsc.defaults.Extension=function(p){var r={};r.away={on:false,reason:"",last:{},since:0};var G=function(){p.bind("cmd.connect",g);p.bind("cmd.set",a);p.bind("cmd.join",M);p.bind("cmd.part",P);p.bind("cmd.say",u);p.bind("cmd.npmsg",Q);p.bind("cmd.me",B);p.bind("cmd.promote",E);p.bind("cmd.demote",E);p.bind("cmd.ban",m);p.bind("cmd.unban",m);p.bind("cmd.kick",j);p.bind("cmd.whois",I);p.bind("cmd.title",k);p.bind("cmd.topic",k);p.bind("cmd.admin",F);p.bind("cmd.disconnect",g);p.bind("cmd.kill",j);p.bind("cmd.raw",N);p.bind("cmd.clear",K);p.bind("cmd.clearall",J);p.bind("pkt.property",l);p.bind("pkt.recv_admin_show",O);p.bind("pkt.recv_admin_showverbose",O);p.bind("pkt.get",C);p.bind("cmd.gettitle",A);p.bind("cmd.gettopic",A);p.bind("cmd.setaway",v);p.bind("cmd.setback",s);p.bind("pkt.recv_msg.highlighted",z);p.bind("cmd.theme",b);p.ui.on("settings.open",q);
p.ui.on("settings.open.ran",L);p.ui.on("settings.save",H)};var H=function(S,R){p.settings.ui.theme=S.theme;p.settings.ui.clock=S.clock;p.settings.ui.tabclose=S.tabclose;p.config_save()};var q=function(T,S){var R=T.settings.page("Main",true);var U={};U.username=p.settings.username;U.pk=p.settings.pk;R.item("Form",{ref:"login",title:"Login",text:"Here you can change the username and token used to                    log into the chat server.",fields:[["Textfield",{ref:"username",label:"Username","default":U.username}],["Textfield",{ref:"token",label:"Token","default":U.pk}]],event:{save:function(V){p.settings.username=V.data.username;p.settings.pk=V.data.token}}},true);R.item("Text",{ref:"intro",title:"Main",text:"Use this window to view and change your settings.\n\nCheck                    the different pages to see what settings can be changed.",},true);R.item("Text",{ref:"debug",wclass:"faint",title:"Debug Information",text:"Chat Agent: <code>"+p.settings.agent+"</code>\n\nUser                    Agent: <code>"+navigator.userAgent+"</code>"})
};var L=function(T,S){var R=T.settings.page("About",true);R.item("Text",{ref:"about-wsc",title:"Wsc",text:'Currently using <a href="http://github.com/photofroggy/wsc/">wsc</a>                    version '+wsc.VERSION+" "+wsc.STATE+'.\n\nWsc                    works using HTML5, javascript, and CSS3. WebSocket is used for the connection                    where possible. The source code for this client is pretty huge.\n\nWsc was created                    by ~<a href="http://photofroggy.deviantart.com/">photofroggy</a>'})};var b=function(S,R){R.ui.theme(S.args.split(" ").shift())};var a=function(T){var S=T.args.split(" ");var R=S.shift().toLowerCase();var S=S.join(" ");if(S.length==0){p.cchannel.serverMessage("Could not set "+R,"No data supplied");return}if(!(R in p.settings)){p.cchannel.serverMessage('Unknown setting "'+R+'"');return}p.settings[R]=S;p.cchannel.serverMessage("Changed "+R+" setting","value: "+S);p.control.setLabel()};var g=function(R){p[R.cmd]()};var M=function(R){var S=R.args.split(" ");
var S=S.toString()==""?[]:S;if(R.ns!=R.target){S.unshift(R.target)}if(S.toString()==""){return}for(index in S){p.join(S[index])}};var P=function(R){var S=R.args.split(" ");if(R.ns!=R.target){S.unshift(R.target)}if(S.toString()==""){p.part(R.ns);return}for(index in S){p.part(S[index])}};var k=function(R){p.set(R.target,R.cmd,R.args)};var E=function(S){var R=S.args.split(" ");p[S.cmd.toLowerCase()](S.target,R[0],R[1])};var m=function(V,R){var T=V.args.split(" ");var S=T.shift();var U=V.cmd;if(U=="ban"&&T.length>0){R.kick(V.target,S,T.join(" "))}R[U](V.target,S)};var B=function(R){p.action(R.target,R.args)};var N=function(R){p.send(R.args.replace(/\\n/gm,"\n"))};var j=function(U,R){var V=U.args.split(" ");var S=V.shift();var T=V.length>0?V.join(" "):null;if(U.cmd=="kick"){R.kick(U.target,S,T)}else{R.kill(S,T)}};var u=function(R){p.say(R.target,R.args)};var Q=function(R){p.npmsg(R.target,R.args)};var K=function(T,R){if(T.args.length>0){var U=T.args.split(" ");for(var S in U){if(!U.hasOwnProperty(S)){continue
}R.channel(T.target).clear(U[S])}}else{R.channel(T.target).clear()}};var J=function(S,R){var U=null;if(S.args.length>0){var T=S.args.split(" ");U=function(W,X){for(var V in T){if(!T.hasOwnProperty(V)){continue}X.clear(T[V])}}}else{U=function(V,W){W.clear()}}R.each_channel(U,true)};var I=function(S,R){R.whois(S.args.split(" ")[0])};var F=function(S,R){R.admin(S.target,S.args)};var D=function(S,R){R.disconnect()};var A=function(S,R){var T=S.cmd.indexOf("title")>-1?"title":"topic";R.control.ui.set_text("/"+T+" "+R.channel(S.target).info[T].content)};var v=function(T,R){r.away.on=true;r.away.last={};r.away.since=new Date();r.away.reason=T.args;var S="is away"+(r.away.reason.length>0?": "+r.away.reason:"");R.each_channel(function(U){R.action(U,S)})};var s=function(S,R){r.away.on=false;R.each_channel(function(T){R.action(T,"is back")})};var z=function(U,R){if(!r.away.on){return}if(r.away.reason.length==0){return}if(U.user==R.settings.username){return}if(R.exclude.indexOf(U.sns.toLowerCase())!=-1){return
}var S=new Date();var T=U.sns.toLowerCase();if(T in r.away.last){if((S-r.away.last[T])<=60000){return}}R.say(U.ns,U.user+": I am currently away; reason: "+r.away.reason);r.away.last[T]=S};var l=function(U,R){if(U.p!="info"){return}var S=U.pkt.sub;var V=S.shift().arg;V.username=U.sns.substr(1);V.connections=[];var T={};while(S.length>0){T=S.shift().arg;T.channels=[];while(S.length>0){if(S[0].cmd!="ns"){break}T.channels.unshift(R.deform_ns(S.shift().param))}V.connections.push(T)}R.cchannel.log_whois(V)};var C=function(S,R){if(S.ns.indexOf("login:")!=0){return}R.cchannel.server_message("Whois failed for "+(S.sns.substr(1)),"not online")};var O=function(V,S){var Y=S.channel(V.ns);var R=V.info.split("\n");var X="";var W=[];var T="";for(var U in R){if(!R.hasOwnProperty(U)){continue}X=R[U].split(" ");if(V.p=="privclass"){W.push([X.shift(),X.shift().split("=")[1],X.join(" ")])}else{if(V.p=="users"){T=X.shift().split(":",1)[0];W.push([T,Y.get_privclass_order(T),X.join(" ")])}}}Y.log_pc(V.p=="privclass",W)
};G();wsc.defaults.Extension.Ignore(p)};wsc.defaults.Extension.Ignore=function(g){var j={};var k=g.storage.folder("ignore");var a=k.folder("ignored");var q=function(){p();m();g.bind("cmd.ignore",b);g.bind("cmd.unignore",l);g.ui.on("settings.open",j.page)};j.page=function(u,z){var v=u.settings.page("Ignores",true);var B={};var s="<ul>";B.im=j.ignore;B.uim=j.unignore;if(g.ui.umuted.length==0){s+="<li><i>No one ignored yet</i></li></ul>"}else{for(var r in g.ui.umuted){if(!g.ui.umuted.hasOwnProperty(r)){continue}s+="<li>"+g.ui.umuted[r]+"</li>"}s+="</ul>"}v.item("Text",{ref:"intro",title:"Ignores",text:'Use <code>ignore</code> to ignore people.\n\n                    You can "ignore" other users of the chat server using the\n                    <code>/ignore</code> command. Ignoring a user hides their                    messages from you in the channel log.',});v.item("Form",{ref:"msgs",title:"Messages",text:"Here you can set the messages displayed when you ignore or                    unignore a user.\n\nThe text <code>{user}</code> is replaced                    with the name of the user your are ignoring or unignoring.",fields:[["Textfield",{ref:"ignore",label:"Ignore","default":B.im}],["Textfield",{ref:"unignore",label:"Unignore","default":B.uim}]],event:{save:function(C){j.ignore=C.data.ignore;
j.unignore=C.data.unignore}}});var A=v.item("Form",{ref:"ignored",wclass:"boxed-ff-indv",title:"Users",text:"This is the list of users that you have silenced.\n\nUse the                    commands <code>/ignore</code> and <code>/unignore</code>                    to edit the list.",fields:[["Text",{ref:"users",text:s}]]})};var b=function(v){var A=v.args.split(" ");var r="";var z="";var u=false;for(var s in A){if(!A.hasOwnProperty(s)){continue}r=A[s];tmod=g.mute_user(r);if(!tmod){continue}u=tmod;z=replaceAll(j.ignore,"{user}",r);if(z.indexOf("/me ")==0){z=z.substr(4);g.action(v.target,z)}else{g.say(v.target,z)}}if(u){m()}};var l=function(z){var C=z.args.split(" ");var s="";var B="";var r=-1;var v=false;var A=false;for(var u in C){if(!C.hasOwnProperty(u)){continue}s=C[u];A=g.unmute_user(s);if(!A){continue}v=A;B=replaceAll(j.unignore,"{user}",s);if(B.indexOf("/me ")==0){B=B.substr(4);g.action(z.target,B)}else{g.say(z.target,B)}}if(v){m()}};var p=function(){j.ignore=k.get("ignore","/me is ignoring {user} now");
j.unignore=k.get("unignore","/me is not ignoring {user} anymore");j.count=parseInt(k.get("count",0));g.ui.umuted=[];if(j.count>0){var s=null;for(var r=0;r<j.count;r++){g.mute_user.push(a.get(r,null))}}};var m=function(){k.set("ignore",j.ignore);k.set("unignore",j.unignore);for(var r=0;r<j.count;r++){a.remove(r)}if(g.ui.umuted.length==0){k.set("count",0)}else{var s=-1;for(var r in g.ui.umuted){if(!g.ui.umuted.hasOwnProperty(r)){continue}s++;a.set(s.toString(),g.ui.umuted[r])}s++;j.count=s;k.set("count",s)}};q()};wsc.Client=function(a,g,j){this.mozilla=j;this.storage=new wsc.Storage;this.uistore=this.storage.folder("ui");this.fresh=true;this.attempts=0;this.connected=false;this.protocol=null;this.flow=null;this.ui=null;this.events=new EventEmitter();this.conn=null;this.channelo={};this.cchannel=null;this.exclude=[];this.cmds=[];this.settings={domain:"website.com",server:"ws://website.com/wsendpoint",symbol:"",username:"",userinfo:{},pk:"",monitor:["~Monitor",true],welcome:"Welcome to the wsc web client!",autojoin:"chat:channel",control:wsc.Control,protocol:wsc.Protocol,mparser:wsc.MessageParser,flow:wsc.Flow,ui_object:Chatterbox.UI,extend:[wsc.defaults.Extension],client:"chatclient",clientver:"0.3",ui:{theme:wsc.defaults.theme,themes:wsc.defaults.themes,tabclose:true,clock:true}};
this.settings=Object.extend(this.settings,g);this.config_load();this.config_save();this.ui=new this.settings.ui_object(a,{themes:this.settings.ui.themes,theme:this.settings.ui.theme,monitor:this.settings.monitor,username:this.settings.username,domain:this.settings.domain,clock:this.settings.ui.clock,tabclose:this.settings.tabclose},j);this.settings.agent=this.ui.LIB+"/"+this.ui.VERSION+" ("+navigator.appVersion.match(/\(([^)]+)\)/)[1]+") wsc/"+wsc.VERSION;this.mns=this.format_ns(this.settings.monitor[0]);this.lun=this.settings.username.toLowerCase();this.protocol=new this.settings.protocol(new this.settings.mparser());this.flow=new this.settings.flow(this.protocol);this.build();for(var b in this.settings.extend){this.settings.extend[b](this)}this.monitor(this.settings.welcome)};wsc.Client.prototype.config_load=function(){this.settings.ui.theme=this.uistore.get("theme",wsc.defaults.theme);this.settings.ui.clock=this.uistore.get("clock",this.settings.ui.clock.toString())=="true";this.settings.ui.tabclose=this.uistore.get("tabclose",this.settings.ui.tabclose.toString())=="true"
};wsc.Client.prototype.config_save=function(){this.uistore.set("theme",this.settings.ui.theme);this.uistore.set("clock",this.settings.ui.clock.toString());this.uistore.set("tabclose",this.settings.ui.tabclose.toString())};wsc.Client.prototype.build=function(){this.ui.build();this.control=new this.settings.control(this);var a=this;this.ui.on("channel.selected",function(b,g){a.cchannel=a.channel(b.ns);a.control.cache_input(b)});this.ui.on("tab.close.clicked",function(b,g){if(b.chan.monitor){return false}a.part(b.ns);return false})};wsc.Client.prototype.loop=function(){this.ui.loop()};wsc.Client.prototype.add_extension=function(a){this.settings.extend.push(a);a(this)};wsc.Client.prototype.bind=function(b,a){this.events.addListener(b,a);if(b.indexOf("cmd.")!=0||b.length<=4){return}cmd=b.slice(4).toLowerCase();this.cmds.push(cmd)};wsc.Client.prototype.clear_listeners=function(a){this.events.removeListeners(a)};wsc.Client.prototype.trigger=function(a,b){this.events.emit(a,b,this)};wsc.Client.prototype.connect=function(){if(this.connected){return
...
}};Chatterbox.Settings.Config.prototype.page=function(b,a){var g=this.find_page(b);a=a||false;if(g==null){g=new Chatterbox.Settings.Page(b);if(a){this.pages.push(g)}else{this.pages.unshift(g)}}return g};Chatterbox.Settings.Config.prototype.save=function(a){for(i in this.pages){this.pages[i].save(a)}};Chatterbox.Settings.Config.prototype.close=function(a){for(i in this.pages){this.pages[i].close(a)}};Chatterbox.Settings.Page=function(a){this.name=a;this.lname=a.toLowerCase();this.ref=replaceAll(this.lname," ","_");this.items=[]};Chatterbox.Settings.Page.prototype.build=function(b){var a=replaceAll(Chatterbox.template.settings.tab,"{ref}",this.ref);a=replaceAll(a,"{name}",this.name);var g=replaceAll(Chatterbox.template.settings.page,"{ref}",this.ref);g=replaceAll(g,"{page-name}",this.name);b.tabs.append(a);b.book.append(g);this.view=b.book.find("div#"+this.ref+"-page");this.tab=b.tabs.find("li#"+this.ref+"-tab");var g=this;this.tab.find("a").click(function(j){if(g.tab.hasClass("active")){return false
}b.switch_page(g);return false});this.content()};Chatterbox.Settings.Page.prototype.content=function(){for(i in this.items){this.items[i].build(this.view)}};Chatterbox.Settings.Page.prototype.resize=function(){for(i in this.items){this.items[i].resize()}};Chatterbox.Settings.Page.prototype.show=function(){if(!this.tab.hasClass("active")){this.tab.addClass("active")}if(!this.view.hasClass("active")){this.view.addClass("active")}this.resize()};Chatterbox.Settings.Page.prototype.hide=function(){if(this.tab.hasClass("active")){this.tab.removeClass("active")}if(this.view.hasClass("active")){this.view.removeClass("active")}};Chatterbox.Settings.Page.prototype.item=function(g,b,a){a=a||false;var j=Chatterbox.Settings.Item.get(g,b);if(a){this.items.unshift(j)}else{this.items.push(j)}return j};Chatterbox.Settings.Page.prototype.save=function(a){for(i in this.items){this.items[i].save(a,this)}};Chatterbox.Settings.Page.prototype.close=function(a){for(i in this.items){this.items[i].close(a,this)}};
