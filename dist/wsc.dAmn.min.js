...
}T.connections.push(R)}P.cchannel.log_whois(T)};var z=function(Q,P){if(Q.ns.indexOf("login:")!=0){return}P.cchannel.server_message("Whois failed for "+(Q.sns.substr(1)),"not online")};var M=function(T,Q){var W=Q.channel(T.ns);var P=T.info.split("\n");var V="";var U=[];var R="";for(var S in P){if(!P.hasOwnProperty(S)){continue}V=P[S].split(" ");if(T.p=="privclass"){U.push([V.shift(),V.shift().split("=")[1],V.join(" ")])}else{if(T.p=="users"){R=V.shift().split(":",1)[0];U.push([R,W.get_privclass_order(R),V.join(" ")])}}}W.log_pc(T.p=="privclass",U)};E();wsc.defaults.Extension.Ignore(p);wsc.defaults.Extension.Away(p);wsc.defaults.Extension.Autojoin(p)};wsc.defaults.Extension.Autojoin=function(a){var g=a.autojoin;a.ui.control.add_button({label:"Autojoin",title:"Join your autojoin channels",href:"#autojoin-do",handler:function(){for(var k in a.autojoin.channel){if(!a.autojoin.channel.hasOwnProperty(k)){continue}a.join(a.autojoin.channel[k])}}});var j=function(){a.bind("cmd.autojoin",b);a.ui.on("settings.open",g.page)
};g.page=function(p,r){var q=p.settings.page("Autojoin");var m="<ul>";var s={};s.ajon=a.autojoin.on;s.chan=a.autojoin.channel;if(a.autojoin.channel.length==0){m+="<li><i>No autojoin channels set</i></li></ul>"}else{for(var l in a.autojoin.channel){if(!a.autojoin.channel.hasOwnProperty(l)){continue}m+="<li>"+a.autojoin.channel[l]+"</li>"}m+="</ul>"}q.item("Checkbox",{ref:"eaj",title:"Autojoin",text:"Turn on autojoin to automatically join selected channels                    when you connect to the chat server.",items:[{value:"yes",title:"On",selected:s.ajon}],event:{change:function(u){if(u.target.value=="yes"){a.autojoin.on=u.target.checked}},save:function(u){s.ajon=a.autojoin.on;a.config_save()},close:function(u){a.autojoin.on=s.ajon}}});var k=q.item("Items",{ref:"channelss",title:"Channels",text:"Add any channels you want to join automatically when you                    connect to the chat server.",items:a.autojoin.channel,prompt:{title:"Add Channel",label:"Channel:",},event:{up:function(u){var v=u.args.swap;
a.autojoin.channel[v["this"].index]=v.that.item;a.autojoin.channel[v.that.index]=v["this"].item;k.options.items=a.autojoin.channel},down:function(u){var v=u.args.swap;a.autojoin.channel[v["this"].index]=v.that.item;a.autojoin.channel[v.that.index]=v["this"].item;k.options.items=a.autojoin.channel},add:function(z){var v=a.deform_ns(z.args.item).toLowerCase();var u=a.autojoin.channel.indexOf(v);if(u!=-1){return}a.autojoin.channel.push(v);k.options.items=a.autojoin.channel},remove:function(u){a.autojoin.channel.splice(u.args.index,1);k.options.items=a.autojoin.channel},save:function(u){s.chan=a.autojoin.channel;a.config_save()},close:function(u){a.config_load()}}})};var b=function(s){var l=s.args.split(" ");if(!l){return}var q=l.shift().toLowerCase();var k=function(v){};var p=false;var u=a.channel(s.ns);switch(q){case"add":k=function(v){if(a.autojoin.channel.indexOf(v)==-1){p=true;a.autojoin.channel.push(v);u.server_message("Added "+v+" to your autojoin.")}else{u.server_message("Already have "+v+" on your autojoin.")
}};break;case"rem":case"remove":k=function(z){var v=a.autojoin.channel.indexOf(z);if(v!=-1){p=true;a.autojoin.channel.splice(v,1);u.server_message("Removed "+z+" from your autojoin.")}else{u.server_message(z+" is not on your autojoin list.")}};break;case"on":u.server_message("Autojoin on.");if(!a.autojoin.on){p=true;a.autojoin.on=true}break;case"off":u.server_message("Autojoin off.");if(a.autojoin.on){p=true;a.autojoin.on=false}break}var r="";for(var m in l){if(!l.hasOwnProperty(m)){continue}r=a.deform_ns(l[m]).toLowerCase();k(r)}if(p){a.config_save()}};j()};wsc.defaults.Extension.Away=function(b){var j=b.storage.folder("away");var g={on:false,reason:"",last:{},since:0,interval:60000,format:{setaway:"/me is away: {reason}",setback:"/me is back",away:"{user}: I've been away for {timesince}. Reason: {reason}"}};var q=function(){m();k();b.bind("cmd.setaway",a);b.bind("cmd.setback",p);b.bind("pkt.recv_msg.highlighted",l);b.ui.on("settings.open",g.page)};g.page=function(s,v){var r=function(B){B=replaceAll(B,"<","&lt;");
B=replaceAll(B,">","&gt;");B=replaceAll(B,'"',"&quot;");return B};var z=function(B){B=replaceAll(B,"&lt;","<");B=replaceAll(B,"&gt;",">");B=replaceAll(B,"&quot;",'"');return B};var u=s.settings.page("Away");var A={};A.away=g.format.away;A.sa=g.format.setaway;A.sb=g.format.setback;A.intr=g.interval;u.item("Text",{ref:"intro",title:"Away Messages",text:"Use away messages when you are away from the chats.\n\n                    You can set yourself away using the                     <code>/setaway [reason]</code> command. When you get back,                    use <code>/setback</code>. While you are away, the client                    will automatically respond when people try to talk to you,                    telling them you're away.",});u.item("Form",{ref:"msgs",title:"Messages",text:"Here you can set the messages displayed when you set                    yourself away or back. You can also change the away message                    format.",hint:"<b>{user}</b><br/>This is replaced with the username of the person trying to talk to you\n\n<b>{reason}</b>                    <br/>This is replaced by your reason for being away.\n\n<b>{timesince}</b>                    <br/>Use this to show how long you have been away for.",fields:[["Textfield",{ref:"away",label:"Away","default":r(A.away)}],["Textfield",{ref:"setaway",label:"Setaway","default":r(A.sa)}],["Textfield",{ref:"setback",label:"Setback","default":r(A.sb)}]],event:{save:function(B){g.format.away=z(B.data.away);
g.format.setaway=z(B.data.setaway);g.format.setback=z(B.data.setback);k()}}});u.item("Form",{ref:"interval",title:"Message Interval",text:"Here you can set the amount of time to wait before                     displaying another away message. The interval is in seconds.",fields:[["Textfield",{ref:"interval",label:"Interval","default":(A.intr/1000).toString()}]],event:{save:function(B){g.interval=parseInt(B.data.interval)*1000;k()}}})};var a=function(u,r){g.on=true;g.last={};g.since=new Date();g.reason=u.args;var v=r.say;var s=replaceAll(g.format.setaway,"{reason}",g.reason||"[silent away]");if(s.indexOf("/me ")==0){s=s.substr(4);v=r.action}r.each_channel(function(z){v.call(r,z,s)})};var p=function(u,r){g.on=false;var v=r.say;var s=g.format.setback;if(s.indexOf("/me ")==0){s=s.substr(4);v=r.action}r.each_channel(function(z){v.call(r,z,s)})};var l=function(z,r){if(!g.on){return}if(g.reason.length==0){return}if(z.user==r.settings.username){return}if(r.exclude.indexOf(z.sns.toLowerCase())!=-1){return
}var u=new Date();var v=z.sns.toLowerCase();if(v in g.last){if((u-g.last[v])<=g.interval){return}}var s=timeLengthString((u-g.since)/1000);var A=replaceAll(g.format.away,"{user}",z.user);A=replaceAll(A,"{timesince}",s);r.say(z.ns,replaceAll(A,"{reason}",g.reason));g.last[v]=u};var m=function(){g.format.setaway=j.get("setaway","/me is away: {reason}");g.format.setback=j.get("setback","/me is back");g.format.away=j.get("away","{user}: I've been away for {timesince}. Reason: {reason}");g.interval=parseInt(j.get("interval",60000))};var k=function(){j.set("interval",g.interval);j.set("setaway",g.format.setaway);j.set("setback",g.format.setback);j.set("away",g.format.away)};q()};wsc.defaults.Extension.Ignore=function(g){var j={};var k=g.storage.folder("ignore");var a=k.folder("ignored");var q=function(){p();m();g.bind("cmd.ignore",b);g.bind("cmd.unignore",l);g.ui.on("settings.open",j.page)};j.page=function(s,v){var u=s.settings.page("Ignores");var z={};z.im=j.ignore;z.uim=j.unignore;z.usr=g.ui.umuted;
u.item("Text",{ref:"intro",title:"Ignores",text:'Use <code>ignore</code> to ignore people.\n\n                    You can "ignore" other users of the chat server using the\n                    <code>/ignore</code> command. Ignoring a user hides their                    messages from you in the channel log.',});u.item("Form",{ref:"msgs",title:"Messages",text:"Here you can set the messages displayed when you ignore or                    unignore a user.\n\nThe text <code>{user}</code> is replaced                    with the name of the user your are ignoring or unignoring.",fields:[["Textfield",{ref:"ignore",label:"Ignore","default":z.im}],["Textfield",{ref:"unignore",label:"Unignore","default":z.uim}]],event:{save:function(A){j.ignore=A.data.ignore;j.unignore=A.data.unignore;m()}}});var r=u.item("Items",{ref:"ignoreds",title:"Users",text:"This is the list of users that you have silenced.\n\nUse the                    commands <code>/ignore</code> and <code>/unignore</code>                    to edit the list.",items:z.usr,prompt:{title:"Add User",label:"User:",},event:{up:function(A){var B=A.args.swap;
g.ui.umuted[B["this"].index]=B.that.item;g.ui.umuted[B.that.index]=B["this"].item;r.options.items=g.ui.umuted},down:function(A){var B=A.args.swap;g.ui.umuted[B["this"].index]=B.that.item;g.ui.umuted[B.that.index]=B["this"].item;r.options.items=g.ui.umuted},add:function(A){g.mute_user(A.args.item);r.options.items=g.ui.umuted},remove:function(A){g.unmute_user(A.args.item);r.options.items=g.ui.umuted},save:function(A){z.usr=g.ui.umuted;m()},close:function(A){p()}}})};var b=function(v){var A=v.args.split(" ");var r="";var z="";var u=false;for(var s in A){if(!A.hasOwnProperty(s)){continue}r=A[s];tmod=g.mute_user(r);if(!tmod){continue}u=tmod;z=replaceAll(j.ignore,"{user}",r);if(z.indexOf("/me ")==0){z=z.substr(4);g.action(v.target,z)}else{g.say(v.target,z)}}if(u){m()}};var l=function(z){var C=z.args.split(" ");var s="";var B="";var r=-1;var v=false;var A=false;for(var u in C){if(!C.hasOwnProperty(u)){continue}s=C[u];A=g.unmute_user(s);if(!A){continue}v=A;B=replaceAll(j.unignore,"{user}",s);if(B.indexOf("/me ")==0){B=B.substr(4);
g.action(z.target,B)}else{g.say(z.target,B)}}if(v){m()}};var p=function(){j.ignore=k.get("ignore","/me is ignoring {user} now");j.unignore=k.get("unignore","/me is not ignoring {user} anymore");j.count=parseInt(k.get("count",0));var s=null;for(var r in g.ui.umuted){if(!g.ui.umuted.hasOwnProperty(r)){continue}g.unmute_user(g.ui.umuted[r])}g.ui.umuted=[];if(j.count>0){s=null;for(var r=0;r<j.count;r++){g.mute_user(a.get(r,null))}}};var m=function(){k.set("ignore",j.ignore);k.set("unignore",j.unignore);for(var r=0;r<j.count;r++){a.remove(r)}if(g.ui.umuted.length==0){k.set("count",0)}else{var s=-1;for(var r in g.ui.umuted){if(!g.ui.umuted.hasOwnProperty(r)){continue}s++;a.set(s.toString(),g.ui.umuted[r])}s++;j.count=s;k.set("count",s)}};q()};wsc.Client=function(a,g,j){this.mozilla=j;this.storage=new wsc.Storage;this.storage.ui=this.storage.folder("ui");this.storage.aj=this.storage.folder("autojoin");this.storage.aj.channel=this.storage.aj.folder("channel");this.fresh=true;this.attempts=0;this.connected=false;
...
this.settings={};this.settings.open=false;if(!this.showclose){if(!this.tabs.hasClass("hc")){this.tabs.addClass("hc")}}var b=this;this.settingsb.click(function(k){if(b.settings.open){return false}var g={e:k,settings:new Chatterbox.Settings.Config(b.manager)};b.configure_page(g);b.manager.trigger("settings.open",g);b.manager.trigger("settings.open.ran",g);var j=g.settings.page("About");j.item("text",{ref:"about-chatterbox",wclass:"centered faint",text:'Using <a href="http://github.com/photofroggy/wsc/">Chatterbox</a> version '+Chatterbox.VERSION+" "+Chatterbox.STATE+' by ~<a href="http://photofroggy.deviantart.com/">photofroggy</a>.'});b.settings.window=new Chatterbox.Settings(b.manager,g.settings);b.settings.window.build();b.settings.open=true;return false});this.tableft.click(function(){b.manager.channel_left();return false});this.tabright.click(function(){b.manager.channel_right();return false})};Chatterbox.Navigation.prototype.configure_page=function(b){var j=this.manager;var g=b.settings.page("Main");
var k={};k.theme=replaceAll(j.settings.theme,"wsct_","");k.clock=j.clock();k.tc=j.nav.closer();var a=[];for(i in j.settings.themes){name=replaceAll(j.settings.themes[i],"wsct_","");a.push({value:name,title:name,selected:k.theme==name})}g.item("Form",{ref:"ui",title:"UI",hint:"<b>Timestamp</b><br/>Choose between a 24 hour clock and                a 12 hour clock.\n\n<b>Theme</b><br/>Change the look of the                client.\n\n<b>Close Buttons</b><br/>Turn tab close buttons on/off.",fields:[["Dropdown",{ref:"theme",label:"Theme",items:a}],["Dropdown",{ref:"clock",label:"Timestamp Format",items:[{value:"24",title:"24 hour",selected:k.clock},{value:"12",title:"12 hour",selected:!k.clock}]}],["Checkbox",{ref:"tabclose",label:"Close Buttons",items:[{value:"yes",title:"On",selected:k.tc}]}],],event:{change:function(l){j.clock(l.data.clock=="24");j.theme(l.data.theme);j.nav.closer(l.data.tabclose.indexOf("yes")>-1)},save:function(l){k.clock=j.clock();k.theme=replaceAll(j.theme(),"wsct_","");
k.tc=j.nav.closer();j.trigger("settings.save",{clock:k.clock,tabclose:k.tc,theme:"wsct_"+k.theme})},close:function(l){j.clock(k.clock);j.theme(k.theme);j.nav.closer(k.tc)}}})};Chatterbox.Navigation.prototype.height=function(){return this.nav.height()};Chatterbox.Navigation.prototype.add_tab=function(a,b){this.tabs.append(Chatterbox.render("tab",{selector:a,ns:b}));return this.tabs.find("#"+a+"-tab")};Chatterbox.Navigation.prototype.resize=function(){this.tabs.width(this.nav.width()-this.buttons.outerWidth()-20);if(this.settings.open){this.settings.window.resize()}};Chatterbox.Navigation.prototype.closer=function(a){if(a==undefined||a==this.showclose){return this.showclose}this.showclose=a;if(this.showclose){if(!this.tabs.hasClass("hc")){return}this.tabs.removeClass("hc");return}if(this.tabs.hasClass("hc")){return}this.tabs.addClass("hc")};Chatterbox.Popup=function(b,a){this.manager=b;this.window=null;this.closeb=null;this.options=Object.extend({ref:"popup",title:"Popup",close:true,content:""},(a||{}))
...
this.tab.find("a").click(function(){a.picker.select_page(a);return false})};Chatterbox.Popup.ItemPicker.Page.prototype.refresh=function(){var a=this.build_list();if(a.length==0){this.options.content="<em>No items on this page.</em>"}else{this.options.content="<ul>"+a+"</ul>"}this.view.html(this.options.content);this.items=this.view.find("ul");this.hook_events()};Chatterbox.Popup.ItemPicker.Page.prototype.hook_events=function(){var a=this;this.items.find("li").each(function(b,j){var k=a.view.find(j);var g=k.find(".value").html();k.click(function(){a.picker.select(g)})})};Chatterbox.Popup.ItemPicker.Page.prototype.build_list=function(){var b=[];var g=null;var k,j;for(var a in this.options.items){if(!this.options.items.hasOwnProperty(a)){continue}g=this.options.items[a];j=g.value||g;k=g.title||j;b.push('<li class="item" title="'+k+'">            <span class="hicon"><i class="iconic check"></i></span>            <span class="value">'+j+"</span>            </li>")}return b.join("")};Chatterbox.Popup.ItemPicker.Page.prototype.show=function(){this.tab.addClass("selected");
...
var a=this;this.window.find("form").bind("change",function(){a.changed=true});this.config.each_page(function(b,g){g.each_item(function(j,k){k._onchange=function(l){a.changed=true}})});this.saveb.click(function(b){a.save();return false});this.closeb.click(function(b){if(a.changed){if(!confirm("Are you sure? You will lose any unsaved changes.")){return false}}a.close();return false});this.scb.click(function(b){a.save();a.close();return false});this.resize()};Chatterbox.Settings.prototype.resize=function(){var a=this.window.find(".inner");var g=a.find("h2");var j=a.find(".bookwrap");var b=a.find("footer");j.height(a.height()-b.outerHeight()-g.outerHeight()-15);this.book.height(j.innerHeight()-this.tabs.outerHeight()-25);this.config.resize()};Chatterbox.Settings.prototype.switch_page=function(b){var g=this.tabs.find("li.active").first();var a=g.prop("id").split("-",1)[0];g=this.config.page(a.split("_").join(" "));g.hide();b.show()};Chatterbox.Settings.prototype.save=function(){this.config.save(this);
this.changed=false};Chatterbox.Settings.prototype.close=function(){this.window.remove();this.manager.nav.settings.open=false;this.manager.nav.settings.window=null;this.config.close(this)};Chatterbox.Settings.Config=function(a){this.manager=a||null;this.pages=[]};Chatterbox.Settings.Config.prototype.find_page=function(b){var j=b.toLowerCase();var g;for(var a in this.pages){g=this.pages[a];if(g.lname==j){return g}}return null};Chatterbox.Settings.Config.prototype.build=function(g,b){g=g||this.manager;for(var a in this.pages){this.pages[a].build(g,b)}};Chatterbox.Settings.Config.prototype.resize=function(){for(var a in this.pages){this.pages[a].resize()}};Chatterbox.Settings.Config.prototype.page=function(b,a){var g=this.find_page(b);a=a||true;if(g==null){g=new Chatterbox.Settings.Page(b,this.manager);if(a){this.pages.push(g)}else{this.pages.unshift(g)}}return g};Chatterbox.Settings.Config.prototype.each_page=function(j){var g=null;var a=null;for(var b in this.pages){if(!this.pages.hasOwnProperty(b)){continue
}g=this.pages[b];a=j(b,g);if(a===false){break}}};Chatterbox.Settings.Config.prototype.save=function(b){for(var a in this.pages){this.pages[a].save(b)}};Chatterbox.Settings.Config.prototype.close=function(b){for(var a in this.pages){this.pages[a].close(b)}};Chatterbox.Settings.Page=function(a,b){this.name=a;this.lname=a.toLowerCase();this.ref=replaceAll(this.lname," ","_");this.items=[];this.itemo={};this.manager=b};Chatterbox.Settings.Page.prototype.build=function(j,b){var a=replaceAll(Chatterbox.template.settings.tab,"{ref}",this.ref);a=replaceAll(a,"{name}",this.name);var g=replaceAll(Chatterbox.template.settings.page,"{ref}",this.ref);g=replaceAll(g,"{page-name}",this.name);b.tabs.append(a);b.book.append(g);this.view=b.book.find("div#"+this.ref+"-page");this.tab=b.tabs.find("li#"+this.ref+"-tab");var g=this;this.tab.find("a").click(function(k){if(g.tab.hasClass("active")){return false}b.switch_page(g);return false});this.content()};Chatterbox.Settings.Page.prototype.content=function(){for(var a in this.items){this.items[a].build(this.view)
}};Chatterbox.Settings.Page.prototype.resize=function(){for(var a in this.items){this.items[a].resize()}};Chatterbox.Settings.Page.prototype.show=function(){if(!this.tab.hasClass("active")){this.tab.addClass("active")}if(!this.view.hasClass("active")){this.view.addClass("active")}this.resize()};Chatterbox.Settings.Page.prototype.hide=function(){if(this.tab.hasClass("active")){this.tab.removeClass("active")}if(this.view.hasClass("active")){this.view.removeClass("active")}};Chatterbox.Settings.Page.prototype.item=function(g,b,a){a=a||false;var j=Chatterbox.Settings.Item.get(g,b,this.manager);if(a){this.items.unshift(j)}else{this.items.push(j)}if(b.hasOwnProperty("ref")){this.itemo[b.ref]=j}return j};Chatterbox.Settings.Page.prototype.get=function(a){if(this.itemo.hasOwnProperty(a)){return this.itemo[a]}return null};Chatterbox.Settings.Page.prototype.each_item=function(j){var g=null;var a=null;for(var b in this.items){if(!this.items.hasOwnProperty(b)){continue}g=this.items[b];a=j(b,g);if(a===false){break
}}};Chatterbox.Settings.Page.prototype.save=function(b){for(var a in this.items){this.items[a].save(b,this)}};Chatterbox.Settings.Page.prototype.close=function(b){for(var a in this.items){this.items[a].close(b,this)}};Chatterbox.Settings.Item=function(b,a,g){this.manager=g||null;this.options=a||{};this.type=b||"base";this.selector=this.type.toLowerCase();this.items=[];this.itemo={};this.view=null;this.val=null;this._onchange=this._event_stub};Chatterbox.Settings.Item.prototype.build=function(m){if(!this.options.hasOwnProperty("ref")){return}var l=this.content();if(l.length==0){return}var a="";if(this.options.hasOwnProperty("wclass")){a=" "+this.options.wclass}var k=Chatterbox.render("settings.item.wrap",{type:this.type.toLowerCase().split(".").join("-"),ref:this.options.ref,"class":a});k=replaceAll(k,"{content}",l);m.append(k);this.view=m.find(".item."+this.options.ref);this.hooks(this.view);if(!this.options.hasOwnProperty("subitems")){return}var p;var j;var g;var b;for(i in this.options.subitems){p=this.options.subitems[i];
j=p[0];g=p[1];sitem=Chatterbox.Settings.Item.get(j,g);b=["stacked"];if(sitem.options.wclass){b.push(sitem.options.wclass)}sitem.options.wclass=b.join(" ");sitem.build(this.view);this.items.push(sitem);if(g.hasOwnProperty("ref")){this.itemo[g.ref]=sitem}}};Chatterbox.Settings.Item.prototype.content=function(){return Chatterbox.render("settings.item."+this.selector,this.options)};Chatterbox.Settings.Item.prototype.resize=function(){for(var a in this.items){this.items[a].resize()}};Chatterbox.Settings.Item.prototype.hooks=function(g){if(!this.options.hasOwnProperty("event")){return}var b=this.options.event;var k=Chatterbox.template.settings.item.get(this.selector);if(k==false){return}if(!k.hasOwnProperty("events")){return}var j=[];for(var a in k.events){j=k.events[a];if(!b.hasOwnProperty(j[0])){continue}g.find(j[1]).bind(j[0],b[j[0]])}};Chatterbox.Settings.Item.prototype._event_stub=function(){};Chatterbox.Settings.Item.prototype._get_cb=function(a){if(!this.options.hasOwnProperty("event")){return this._event_stub
}return this.options.event[a]||this._event_stub};Chatterbox.Settings.Item.prototype._get_ep=function(b){var j=Chatterbox.template.settings.item.get(this.selector);if(j==null){return false}if(!j.hasOwnProperty("events")){return false}var g=[];for(var a in j.events){g=j.events[a];if(g[0]==b){return g}}};Chatterbox.Settings.Item.prototype.save=function(j,k){var l=this._get_ep("inspect");var b=l?this.view.find(l[1]):null;var a=this._get_cb("save");if(typeof a=="function"){a({input:b,item:this,page:k,window:j})}else{for(var g in a){var m=b.hasOwnProperty("slice")?b.slice(g,1):b;a[g]({input:m,item:this,page:k,window:j})}}for(var g in this.items){this.items[g].save(j,k)}};Chatterbox.Settings.Item.prototype.close=function(j,k){var l=this._get_ep("inspect");var b=l?this.view.find(l[1]):null;var a=this._get_cb("close");if(typeof a=="function"){a({input:b,item:this,page:k,window:j});return}for(var g in a){var m=b.hasOwnProperty("slice")?b.slice(g,1):b;a[g]({input:m,item:this,page:k,window:j})}for(var g in this.items){this.items[g].close(j,k)
}};Chatterbox.Settings.Item.get=function(k,q,l,b,a){var j=k.split(".");var m=b||Chatterbox.Settings.Item;var p;for(var g in j){p=j[g];if(!m.hasOwnProperty(p)){m=a||Chatterbox.Settings.Item;break}m=m[p]}return new m(k,q,l)};Chatterbox.Settings.Item.Form=function(b,a){Chatterbox.Settings.Item.call(this,b,a);this.form=null;this.fields=[];this.lsection=null;this.fsection=null;this.fieldo={}};Chatterbox.Settings.Item.Form.prototype=new Chatterbox.Settings.Item();Chatterbox.Settings.Item.Form.prototype.constructor=Chatterbox.Settings.Item.Form;Chatterbox.Settings.Item.Form.field=function(b,a){return Chatterbox.Settings.Item.get(b,a,this.manager,Chatterbox.Settings.Item.Form,Chatterbox.Settings.Item.Form.Field)};Chatterbox.Settings.Item.Form.prototype.build=function(j){Chatterbox.Settings.Item.prototype.build.call(this,j);if(this.view==null){return}this.lsection=this.view.find("section.labels");this.fsection=this.view.find("section.fields");if(!this.options.hasOwnProperty("fields")){return}var g;
var k;for(var a in this.options.fields){g=this.options.fields[a];k=Chatterbox.Settings.Item.Form.field(g[0],g[1]);this.fields.push(k);k.build(this);if(g[1].hasOwnProperty("ref")){this.fieldo[g[1].ref]=k}}this.form=this.view.find("form");var b=this;this.form.bind("change",function(l){b.change()})};Chatterbox.Settings.Item.Form.prototype.resize=function(){for(var a in this.fields){this.fields[a].resize()}};Chatterbox.Settings.Item.Form.prototype.change=function(){var g={};var j;for(var b in this.fields){j=this.fields[b];g[j.ref]=j.get()}var a=this._get_cb("change");if(typeof a=="function"){a({data:g,form:this})}else{for(var b in a){a[b]({data:g,form:this})}}};Chatterbox.Settings.Item.Form.prototype.save=function(j,l){var k={};var b;for(var g in this.fields){field=this.fields[g];k[field.ref]=field.get()}var a=this._get_cb("save");if(typeof a=="function"){a({data:k,form:this,page:l,window:j})}else{for(var g in a){a[g]({data:k,form:this,page:l,window:j})}}};Chatterbox.Settings.Item.Form.prototype.close=function(g,k){var j={};
var l;for(var b in this.fields){l=this.fields[b];j[l.ref]=l.get()}var a=this._get_cb("close");if(typeof a=="function"){a({data:j,form:this,page:k,window:g})}else{for(var b in a){a[b]({data:j,form:this,page:k,window:g})}}};Chatterbox.Settings.Item.Form.Field=function(b,a){Chatterbox.Settings.Item.call(this,b,a);this.ref=this.options.ref||"ref";this.label=null;this.field=null;this.value=""};Chatterbox.Settings.Item.Form.Field.prototype=new Chatterbox.Settings.Item();Chatterbox.Settings.Item.Form.Field.prototype.constructor=Chatterbox.Settings.Item.Form.Field;Chatterbox.Settings.Item.Form.Field.prototype.build=function(a){a.lsection.append(Chatterbox.render("settings.item.form.label",{ref:this.ref,label:this.options.label||"","class":(this.options["class"]?" "+this.options["class"]:"")}));this.label=a.lsection.find("label."+this.ref);this.lwrap=a.lsection.find("."+this.ref+".label");a.fsection.append(Chatterbox.render("settings.item.form.field.wrap",{ref:this.ref,field:Chatterbox.render("settings.item.form.field."+this.selector,this.options)}));
...
if(this.options.hasOwnProperty("items")){for(i in this.options.items){var b=this.options.items[i];b.name=this.ref;this.items[b.ref]=""}}g.fsection.append(Chatterbox.render("settings.item.form.field.wrap",{ref:this.ref,field:Chatterbox.render("settings.item.form.field.radio",this.options)}));this.fwrap=g.fsection.find("div."+this.ref+".field");this.field=this.fwrap.find("input:radio");this.value=this.fwrap.find("input[checked]:radio").val();var a=this;this.field.bind("change",function(j){a.value=a.fwrap.find(this).val()})};Chatterbox.Settings.Item.Form.Radio.prototype.resize=function(){this.lwrap.height(this.fwrap.find(".radiobox").height())};Chatterbox.Settings.Item.Form.Radio.prototype.get=function(){return this.value};Chatterbox.Settings.Item.Form.Checkbox=function(b,a){Chatterbox.Settings.Item.Form.Radio.call(this,b,a);this.value=[]};Chatterbox.Settings.Item.Form.Checkbox.prototype=new Chatterbox.Settings.Item.Form.Radio();Chatterbox.Settings.Item.Form.Checkbox.prototype.constructor=Chatterbox.Settings.Item.Form.Checkbox;
Chatterbox.Settings.Item.Form.Checkbox.prototype.build=function(g){g.lsection.append(Chatterbox.render("settings.item.form.label",{ref:this.ref,label:this.options.label||""}));this.label=g.lsection.find("label."+this.ref);this.lwrap=g.lsection.find("."+this.ref+".label");if(this.options.hasOwnProperty("items")){for(i in this.options.items){var b=this.options.items[i];b.name=this.ref;this.items[b.ref]=""}}g.fsection.append(Chatterbox.render("settings.item.form.field.wrap",{ref:this.ref,field:Chatterbox.render("settings.item.form.field.checkbox",this.options)}));this.fwrap=g.fsection.find("div."+this.ref+".field");this.field=this.fwrap.find("input:checkbox");var a=this;this.value=[];this.fwrap.find("input[checked]:checkbox").each(function(){a.value.push(a.fwrap.find(this).val())});this.field.bind("change",function(j){a.value=[];a.fwrap.find("input[checked]:checkbox").each(function(){a.value.push(a.fwrap.find(this).val())})})};Chatterbox.Settings.Item.Form.Checkbox.prototype.resize=function(){this.lwrap.height(this.fwrap.find(".checkbox").height())
};Chatterbox.Settings.Item.Radio=function(b,a){Chatterbox.Settings.Item.call(this,b,a);this.value=""};Chatterbox.Settings.Item.Radio.prototype=new Chatterbox.Settings.Item();Chatterbox.Settings.Item.Radio.prototype.constructor=Chatterbox.Settings.Item.Radio;Chatterbox.Settings.Item.Radio.prototype.build=function(g){if(this.options.hasOwnProperty("items")){for(i in this.options.items){var b=this.options.items[i];b.name=this.options.ref||"ref"}}Chatterbox.Settings.Item.prototype.build.call(this,g);this.field=this.view.find("input:radio");this.value=this.view.find("input[checked]:radio").val();var a=this;this.field.bind("change",function(j){a.value=a.view.find(this).val()})};Chatterbox.Settings.Item.Checkbox=function(b,a){Chatterbox.Settings.Item.Radio.call(this,b,a);this.value=[]};Chatterbox.Settings.Item.Checkbox.prototype=new Chatterbox.Settings.Item.Radio();Chatterbox.Settings.Item.Checkbox.prototype.constructor=Chatterbox.Settings.Item.Checkbox;Chatterbox.Settings.Item.Checkbox.prototype.build=function(g){if(this.options.hasOwnProperty("items")){for(i in this.options.items){var b=this.options.items[i];
b.name=this.options.ref||"ref"}}Chatterbox.Settings.Item.prototype.build.call(this,g);this.field=this.view.find("input:checkbox");var a=this;this.value=[];this.view.find("input[checked]:checkbox").each(function(){a.value.push(a.view.find(this).val())});this.field.bind("change",function(j){a.value=[];a.view.find("input[checked]:checkbox").each(function(){a.value.push(a.view.find(this).val())})})};Chatterbox.Settings.Item.Items=function(b,a,g){a=Object.extend({prompt:{title:"Add Item",label:"Item:","submit-button":"Add"}},(a||{}));Chatterbox.Settings.Item.call(this,b,a,g);this.selected=false};Chatterbox.Settings.Item.Items.prototype=new Chatterbox.Settings.Item();Chatterbox.Settings.Item.Items.prototype.constructor=Chatterbox.Settings.Item.Items;Chatterbox.Settings.Item.Items.prototype.build=function(b){Chatterbox.Settings.Item.prototype.build.call(this,b);var a=this;this.list=this.view.find("ul");this.buttons=this.view.find("section.buttons p");var g=this.list.find("li");g.click(function(k){var j=a.list.find(this);
a.list.find("li.selected").removeClass("selected");a.selected=j.find(".item").html();j.addClass("selected")});g.each(function(j,l){var k=a.list.find(l);k.find("a.close").click(function(m){a.list.find("li.selected").removeClass("selected");a.selected=k.find(".item").html();k.addClass("selected");a.remove_item();return false})});this.buttons.find("a.button.up").click(function(j){a.shift_item(true);return false});this.buttons.find("a.button.down").click(function(j){a.shift_item();return false});this.buttons.find("a.button.add").click(function(j){var k=new Chatterbox.Popup.Prompt(a.manager,{position:[j.clientX-100,j.clientY-50],title:a.options.prompt.title,label:a.options.prompt.label,"submit-button":a.options.prompt["submit-button"],event:{submit:function(l){var p=l.data;if(!p){l.options.event.cancel(l);return}p=p.toLowerCase();var m=a.options.items.indexOf(p);if(m!=-1){l.options.event.cancel(l);return}a._fevent("add",{item:p});a.refresh();a._onchange({})},cancel:function(l){a._fevent("cancel",{});
a.refresh();a._onchange({})}}});k.build();return false});this.buttons.find("a.button.close").click(function(j){a.remove_item();return false})};Chatterbox.Settings.Item.Items.prototype.shift_item=function(b){if(this.selected===false){return}var g=this.options.items.indexOf(this.selected);var a=g+1;if(b){a=g-1}if(g==-1||g>=this.options.items.length){return}if(a<0||a>=this.options.items.length){return}this._fevent((b?"up":"down"),{swap:{"this":{index:g,item:this.options.items[g]},that:{index:a,item:this.options.items[a]}}});this.refresh();this._onchange({});return};Chatterbox.Settings.Item.Items.prototype.remove_item=function(){if(this.selected===false){return false}var a=this.options.items.indexOf(this.selected);if(a==-1||a>=this.options.items.length){return}this._fevent("remove",{index:a,item:this.selected});this.selected=false;this.refresh();this._onchange({})};Chatterbox.Settings.Item.Items.prototype.refresh=function(){this.view.find("section.mitems").html(Chatterbox.template.settings.krender.manageditems(this.options.items));
this.list=this.view.find("ul");this.list.find("li[title="+(this.selected||"").toLowerCase()+"]").addClass("selected");var a=this;var b=this.list.find("li");b.click(function(j){var g=a.list.find(this);a.list.find("li.selected").removeClass("selected");a.selected=g.find(".item").html();g.addClass("selected")});b.each(function(g,k){var j=a.list.find(k);j.find("a.close").click(function(l){a.list.find("li.selected").removeClass("selected");a.selected=j.find(".item").html();j.addClass("selected");a.remove_item();return false})})};Chatterbox.Settings.Item.Items.prototype._fevent=function(b,j){var l=this._get_ep("inspect");var g=l?this.view.find(l[1]):null;var a=this._get_cb(b);if(typeof a=="function"){a({input:g,item:this,args:j})}else{for(var k in a){var m=g.hasOwnProperty("slice")?g.slice(k,1):g;a[k]({input:m,item:this,args:j})}}};Chatterbox.Settings.Item.Items.prototype.save=function(){this._fevent("save",{items:this.options.items||[]})};Chatterbox.template={};Chatterbox.render=function(p,r,a,b){var k=b||Chatterbox.template;
var l={};var m=null;var g=null;if(a!==undefined&&a===true){k=p}else{var q=p.split(".");for(var j in q){g=q[j];if(!k.hasOwnProperty(g)){return""}k=k[g]}}if(k.hasOwnProperty("frame")){m=k;l=k.render||{};k=k.frame;if(m.hasOwnProperty("pre")){if(typeof m.pre=="function"){k=m.pre(k,r)}else{for(i in m.pre){k=m.pre[i](k,r)}}}}for(key in r){k=replaceAll(k,"{"+key+"}",(l[key]||Chatterbox.template.render_stub)(r[key]||""))}if(m!=null){if(m.hasOwnProperty("post")){if(typeof m.post=="function"){k=m.post(k,r)}else{for(i in m.post){k=m.post[i](k,r)}}}}return k};Chatterbox.template.render_stub=function(a){return a};Chatterbox.template.clean=function(a){return function(b,g){for(i in a){b=replaceAll(b,"{"+a[i]+"}","")}return b}};Chatterbox.template.ui='<nav class="tabs"><ul id="chattabs" class="tabs"></ul>        <ul id="tabnav">            <li><a href="#left" class="button iconic arrow_left"></a></li>            <li><a href="#right" class="button iconic arrow_right"></a></li>            <li><a href="#settings" title="Change client settings" class="button iconic cog" id="settings-button"></a></li>        </ul>        </nav>        <div class="chatbook"></div>';
...
}ritem='<div class="'+item.value+' field check"><input class="'+item.value+'" type="checkbox" name="'+item.name+'" value="'+item.value+'"';if(item.selected){ritem+=' checked="yes"'}fields.push(ritem+" /></div>")}if(labels.length>0){render+='<section class="labels">'+labels.join("")+"</section>"}return render+'<section class="fields">'+fields.join("")+"</section>"};Chatterbox.template.settings.krender.manageditems=function(b){if(b.length==0){return"<i>No items in this list</i>"}var j="<ul>";var l=[];var a=[];var k;for(var g in b){if(!b.hasOwnProperty(g)){continue}k=b[g];j+='<li title="'+k.toLowerCase()+'">                  <span class="remove"><a href="#remove" title="Remove item" class="close iconic x"></a></span>                  <span class="item">'+k+"</span>                  </li>"}return j+"</ul>"};Chatterbox.template.settings.item={};Chatterbox.template.settings.item.get=function(a){var g=a.split(".");var b=Chatterbox.template.settings.item;for(i in g){tc=g[i];if(b.hasOwnProperty(tc)){b=b[tc];
...
Chatterbox.template.settings.item.form.field.text.pre=Chatterbox.template.settings.item.hint.prep;Chatterbox.template.settings.item.form.field.text.post=Chatterbox.template.clean(["title","text"]);Chatterbox.template.settings.item.form.field.text.render={title:Chatterbox.template.settings.krender.title,text:Chatterbox.template.settings.krender.text};Chatterbox.template.settings.item.form.field.text.frame="{title}<p>                                        {text}                                    </p>";wsc.dAmn={};wsc.dAmn.VERSION="0.4.11";wsc.dAmn.STATE="alpha";wsc.dAmn.Emotes=function(a,g,b){b.emotes.emote={};b.emotes.page=null;b.emotes.fint=null;b.emotes.notice=null;b.emotes.fetching=false;b.emotes.loaded=false;b.emotes.picker=new wsc.dAmn.Emotes.Picker(a.ui,{event:{select:function(j){b.emotes.select(j)},reload:function(){b.emotes.fetch()}}},b);b.emotes.picker.build();b.emotes.picker.hide();a.ui.control.add_button({label:"",icon:"user",href:"#emotes",title:"Emote picker.",handler:function(){if(b.emotes.picker.window.css("display")=="none"){b.emotes.picker.show()
}else{b.emotes.picker.close()}}});b.emotes.configure_page=function(k,m){var l=k.settings.page("Emotes");b.emotes.page=l;var p={};p.on=b.emotes.on;var j="";if(p.on){if(!b.emotes.loaded||b.emotes.fetching){j="<em>Fetching emotes...</em>"}else{j="<em>Emotes loaded.</em>"}}l.item("Form",{ref:"switch",title:"Enable Emotes",text:"Here you can turn custom emotes on and off.",fields:[["Checkbox",{ref:"enabled",items:[{value:"yes",title:"On",selected:p.on}]}]],event:{change:function(q){b.emotes.on=(q.data.enabled.indexOf("yes")!=-1);if(b.emotes.on){b.emotes.fetch();return}if(b.emotes.fint===null){return}clearTimeout(b.emotes.fint);b.emotes.fint=null},save:function(q){p.on=b.emotes.on;if(!b.emotes.on){b.emotes.picker.loading("Disabled")}else{if(!b.emotes.fetching){b.emotes.picker.loaded()}}b.save()},close:function(q){b.emotes.on=p.on;b.load();b.emotes.page=null;if(b.emotes.fint===null){return}clearTimeout(b.emotes.fint);b.emotes.fint=null}}})};a.ui.on("settings.open.ran",b.emotes.configure_page);
b.emotes.fetch=function(){if(b.emotes.loaded){b.emotes.picker.loading("Reloading...")}else{b.emotes.picker.loading()}b.emotes.fetching=true;jQuery.getJSON("http://www.thezikes.org/publicemotes.php?format=jsonp&jsoncallback=?&"+(new Date()).getDay(),function(j){b.emotes.fetching=false;b.emotes.emote=j;if(!b.emotes.loaded){if(b.emotes.on){b.emotes.notice=new Chatterbox.Popup(a.ui,{title:"Emotes",content:"Custom emoticons loaded!"});b.emotes.notice.build();b.emotes.notice.window.css({bottom:60,right:50});setTimeout(function(){if(b.emotes.notice==null){return}b.emotes.notice.window.fadeOut("slow",function(){b.emotes.notice.window.remove();b.emotes.notice=null})},5000)}}b.emotes.sort();b.emotes.loaded=true;b.emotes.picker.loaded();b.emotes.fint=setTimeout(b.emotes.fetch,3600000)},function(){b.emotes.picker.loaded();return false})};b.emotes.swap=function(l){if(!b.emotes.on){return}var j=-1;for(var k in b.emotes.emote){if(!b.emotes.emote.hasOwnProperty(k)){continue}j=l.input.indexOf(k);if(j==-1){continue
}l.input=replaceAll(l.input,k,":thumb"+b.emotes.emote[k]["devid"]+":")}l.input=replaceAll(l.input,":B",":bucktooth:")};b.emotes.prepare=function(j){j=replaceAll(j,"&","&amp;");j=replaceAll(j,"<","&lt;");j=replaceAll(j,">","&gt;");return replaceAll(j,'"',"&quot;")};b.emotes.repair=function(j){j=replaceAll(j,"&quot;",'"');j=replaceAll(j,"&lt;","<");j=replaceAll(j,"&gt;",">");return replaceAll(j,"&amp;","&")};b.emotes.sort=function(){var j=[[],[],[],[],[],[],[],[],[],[],[],[],[],[],[],[],[],[],[],[],[],[],[],[],[],[],[],];var p="ABCDEFGHIJKLMNOPQRSTUVWXYZ#";var k=null;var r=null;var l=false;var v=-1;for(var q in b.emotes.emote){if(!b.emotes.emote.hasOwnProperty(q)){continue}k=b.emotes.emote[q];r={value:b.emotes.prepare(k.code),title:"created by "+k.by};v=p.indexOf(k.code.substr(1,1).toUpperCase());if(v==-1){v=p.indexOf("#")}j[v].push(r)}var u=function(A,z){return caseInsensitiveSort(A.value,z.value)};var m=b.emotes.picker.page("#");var s=null;for(var q=0;q<p.length;q++){j[q].sort(u);s=b.emotes.picker.page(p[q],m);
s.options.items=j[q]}b.emotes.picker.refresh()};b.emotes.select=function(j){a.ui.control.set_text(a.ui.control.get_text()+b.emotes.repair(j))};a.bind("send.msg.before",b.emotes.swap);a.bind("send.action.before",b.emotes.swap);a.bind("send.kick.before",b.emotes.swap);a.bind("send.set.before",b.emotes.swap);if(!b.emotes.on){return}b.emotes.fetch()};wsc.dAmn.Emotes.Picker=function(l,a,g){a=a||{};a=Object.extend({title:"Emotes",event:{select:function(p){},reload:function(){}}},a);Chatterbox.Popup.ItemPicker.call(this,l,a);this.settings=g;this.rbutton=null;var m="ABCDEFGHIJKLMNOPQRSTUVWXYZ";var k="";var j="";for(var b=0;b<m.length;b++){k=m[b];j=k.toLowerCase();this.add_page({ref:j,href:"#"+j,label:k,title:"Emotes beginning with "+k,items:[]})}this.add_page({ref:"misc",href:"#misc",label:"#",title:"Emotes not beginning with letters",items:[]})};wsc.dAmn.Emotes.Picker.prototype=new Chatterbox.Popup.ItemPicker();wsc.dAmn.Emotes.Picker.prototype.constructor=wsc.dAmn.Emotes.Picker;wsc.dAmn.Emotes.Picker.prototype.hide=function(){this.window.css({display:"none"})
};wsc.dAmn.Emotes.Picker.prototype.show=function(){this.window.css({display:"block"})};wsc.dAmn.Emotes.Picker.prototype.close=function(){this.hide()};wsc.dAmn.Emotes.Picker.prototype.build=function(b){var a=this;Chatterbox.Popup.ItemPicker.prototype.build.call(this,b);var g="Reload";this.rbutton=this.add_button({href:"#reload",title:"Reload emoticons",label:"Reload"});if(this.settings.emotes.fetching){this.loading("Loading...")}if(!this.settings.emotes.on){this.loading("Disabled")}this.rbutton.click(function(){if(!a.settings.emotes.on){return false}if(a.settings.emotes.fetching){return false}if(a.rbutton.hasClass("evented")){return false}a.reload();return false})};wsc.dAmn.Emotes.Picker.prototype.loaded=function(){this.rbutton.html("Reload");this.rbutton.removeClass("evented")};wsc.dAmn.Emotes.Picker.prototype.loading=function(a){a=a||"Reloading...";this.rbutton.addClass("evented");this.rbutton.html(a)};wsc.dAmn.Emotes.Picker.prototype.reload=function(){this.loading();this.options.event.reload()
};wsc.dAmn.Extension=function(a){a.settings.client="dAmnClient";a.settings.clientver="0.3";a.settings.domain="deviantart.com";a.settings.agent+=" wsc/dAmn/"+wsc.dAmn.VERSION;var g=a.storage.folder("dAmn");g.emotes=g.folder("emotes");var b={emotes:{on:false,emote:[]}};b.save=function(){g.emotes.set("on",b.emotes.on)};b.load=function(){b.emotes.on=(g.emotes.get("on","false")=="true")};b.load();b.save();a.protocol.extend_maps({dAmnServer:["version"]});a.protocol.extend_messages({dAmnServer:['<span class="servermsg">** Connected to dAmnServer {version} *</span>',false,true]});a.protocol.mparser=new wsc.dAmn.TablumpParser;a.flow.dAmnServer=a.flow.chatserver;a.exclude.push("#devart");a.ui.on("userinfo.before",function(j,k){j.user.avatar=wsc.dAmn.avatar.link(j.user.name,j.user.member.usericon);if(j.user.member.realname){j.user.info.push(j.user.member.realname)}if(j.user.member.typename){j.user.info.push(j.user.member.typename)}});a.ui.on("log_whois.before",function(j,k){j.avatar=wsc.dAmn.avatar.link(j.raw.username,j.raw.usericon);
j.username=j.raw.symbol+'<b><a href="http://'+j.raw.username+'.deviantart.com/">'+j.raw.username+"</a></b>";if(j.raw.realname){j.info.push(j.raw.realname)}if(j.raw.typename){j.info.push(j.raw.typename)}});wsc.dAmn.Emotes(a,g.emotes,b)};wsc.dAmn.avatar={};wsc.dAmn.avatar.ext=["gif","gif","jpg","png"];wsc.dAmn.avatar.link=function(b,j){j=parseInt(j);var l=(j>>2)&15;j=j&3;var g=wsc.dAmn.avatar.ext[j]||"gif";if(l){l="?"+l}else{l=""}if(j==0){k="default"}else{var a=new RegExp("\\$un(\\[([0-9]+)\\])","g");var k="$un[0]/$un[1]/{un}".replace(a,function(p,r,q){return b[q]=="-"?"_":b[q].toLowerCase()});k=replaceAll(k,"{un}",b.toLowerCase())}return'<a target="_blank" title=":icon'+b+':" href="http://'+b+'.deviantart.com/"><img class="avatar"            alt=":icon'+b+':" src="http://a.deviantart.net/avatars/'+k+"."+g+l+'" height="50" width="50" /></a>'};wsc.dAmn.TablumpString=function(a,b){this._parser=b||new wsc.dAmn.Tablumps();this.tokens=this._parser.tokenise(a);this.raw=a;this._text=null;this._html=null;
