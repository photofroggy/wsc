...
if(g.ns==null){g.ns=a.mns}g.sns=a.deform_ns(g.ns);this.protocol.log(a,g);this.handle(a,g);a.trigger("pkt",g);a.trigger("pkt."+g.name,g)};wsc.Flow.prototype.handle=function(a,g){try{this[g.name](g,a)}catch(b){}};wsc.Flow.prototype.ping=function(b,a){a.pong()};wsc.Flow.prototype.chatserver=function(b,a){a.login()};wsc.Flow.prototype.login=function(b,a){if(b.pkt.arg["e"]=="ok"){info=new wsc.Packet("info\n"+b.data);a.settings.username=b.pkt.param;a.settings.symbol=info.arg.symbol;a.settings.userinfo=info.arg;if(a.fresh){a.join(a.settings.autojoin)}else{for(key in a.channelo){if(a.channelo[key].namespace[0]!="~"){a.join(key)}}}}else{}if(a.fresh){a.fresh=false}};wsc.Flow.prototype.join=function(b,a){if(b.pkt.arg["e"]=="ok"){ns=a.deform_ns(b.pkt.param);a.create_ns(ns);a.ui.channel(ns).server_message("You have joined "+ns)}else{a.ui.chatbook.current.server_message("Failed to join "+a.deform_ns(b.pkt.param),b.pkt.arg["e"])}};wsc.Flow.prototype.part=function(g,a){var b=a.deform_ns(g.ns);var j=a.channel(b);
if(g.e=="ok"){info="";if(g.r.length>0){info="["+g.r+"]"}else{a.remove_ns(b)}msg="You have left "+b;j.server_message(msg,info);if(a.channels()==0){switch(g.r){case"bad data":case"bad msg":case"msg too big":break;default:if(g.r.indexOf("killed:")<0){return}break}this.message(a,{data:"disconnect\ne="+e.r+"\n"})}}else{a.monitor("Couldn't leave "+b,g.e);j.server_message("Couldn't leave "+b,g.e)}};wsc.Flow.prototype.kicked=function(b,a){if(b.r.toLowerCase().indexOf("autokicked")>-1){return}a.join(b.ns)};wsc.Flow.prototype.property=function(b,a){chan=a.channel(b.pkt.param);if(!chan){return}chan.property(b)};wsc.Flow.prototype.recv_joinpart=function(b,a){c=a.channel(b.ns);if(b.name=="recv_join"){c.recv_join(b)}else{c.recv_part(b)}};wsc.Flow.prototype.recv_join=wsc.Flow.prototype.recv_joinpart;wsc.Flow.prototype.recv_part=wsc.Flow.prototype.recv_joinpart;wsc.Flow.prototype.recv_msg=function(b,a){a.channel(b.ns).recv_msg(b)};wsc.Flow.prototype.recv_action=wsc.Flow.prototype.recv_msg;wsc.Flow.prototype.recv_npmsg=wsc.Flow.prototype.recv_msg;
wsc.Flow.prototype.recv_privchg=function(b,a){a.channel(b.ns).recv_privchg(b)};wsc.Flow.prototype.recv_kicked=function(b,a){a.channel(b.ns).recv_kicked(b)};wsc.defaults.Extension=function(p){var r={};r.away={on:false,reason:"",last:{},since:0};var G=function(){p.bind("cmd.connect",g);p.bind("cmd.set",a);p.bind("cmd.join",L);p.bind("cmd.part",O);p.bind("cmd.say",u);p.bind("cmd.npmsg",P);p.bind("cmd.me",B);p.bind("cmd.promote",E);p.bind("cmd.demote",E);p.bind("cmd.ban",m);p.bind("cmd.unban",m);p.bind("cmd.kick",j);p.bind("cmd.whois",H);p.bind("cmd.title",k);p.bind("cmd.topic",k);p.bind("cmd.admin",F);p.bind("cmd.disconnect",g);p.bind("cmd.kill",j);p.bind("cmd.raw",M);p.bind("cmd.clear",J);p.bind("cmd.clearall",I);p.bind("pkt.property",l);p.bind("pkt.recv_admin_show",N);p.bind("pkt.recv_admin_showverbose",N);p.bind("pkt.get",C);p.bind("cmd.gettitle",A);p.bind("cmd.gettopic",A);p.bind("cmd.setaway",v);p.bind("cmd.setback",s);p.bind("pkt.recv_msg.highlighted",z);p.bind("cmd.theme",b);p.ui.on("settings.open",q);
p.ui.on("settings.open.ran",K)};var q=function(S,R){page=S.settings.page("Main");var T={};T.theme=replaceAll(p.ui.settings.theme,"wsct_","");T.clock=p.ui.clock();T.tc=p.ui.nav.closer();T.username=p.settings.username;T.pk=p.settings.pk;var Q=[];for(i in p.ui.settings.themes){name=replaceAll(p.ui.settings.themes[i],"wsct_","");Q.push({value:name,title:name,selected:T.theme==name})}page.item("Text",{ref:"intro",title:"Main",text:"Use this window to view and change your settings.\n\nCheck                    the different pages to see what settings can be changed.",});page.item("Form",{ref:"login",title:"Login",text:"Here you can change the username and token used to                    log into the chat server.",fields:[["Textfield",{ref:"username",label:"Username","default":T.username}],["Textfield",{ref:"token",label:"Token","default":T.pk}]],event:{save:function(U){p.settings.username=U.data.username;p.settings.pk=U.data.token}}});page.item("Form",{ref:"ui",title:"UI",hint:"<b>Timestamp</b><br/>Choose between a 24 hour clock and                    a 12 hour clock.\n\n<b>Theme</b><br/>Change the look of the                    client.\n\n<b>Close Buttons</b><br/>Turn tab close buttons on/off.",fields:[["Dropdown",{ref:"theme",label:"Theme",items:Q}],["Dropdown",{ref:"clock",label:"Timestamp Format",items:[{value:"24",title:"24 hour",selected:T.clock},{value:"12",title:"12 hour",selected:!T.clock}]}],["Check",{ref:"tabclose",label:"Close Buttons",items:[{value:"yes",title:"On",selected:T.tc}]}],],event:{change:function(U){p.ui.clock(U.data.clock=="24");
p.ui.theme(U.data.theme);p.ui.nav.closer(U.data.tabclose.indexOf("yes")>-1)},save:function(U){T.clock=U.data.clock=="24";T.theme=U.data.theme;T.tc=U.data.tabclose.indexOf("yes")>-1;p.storage.set("theme","wsct_"+T.theme)},close:function(U){p.ui.clock(T.clock);p.ui.theme(T.theme);p.ui.nav.closer(T.tc)}}});page.item("Text",{ref:"debug",wclass:"faint",title:"Debug Information",text:"Chat Agent: <code>"+p.settings.agent+"</code>\n\nUser                    Agent: <code>"+navigator.userAgent+"</code>"})};var K=function(S,R){var Q=S.settings.page("About",true);Q.item("Text",{ref:"about-wsc",title:"Wsc",text:'Currently using <a href="http://github.com/photofroggy/wsc/">wsc</a>                    version '+wsc.VERSION+" "+wsc.STATE+'.\n\nWsc                    works using HTML5, javascript, and CSS3. WebSocket is used for the connection                    where possible. The source code for this client is pretty huge.\n\nWsc was created                    by ~<a href="http://photofroggy.deviantart.com/">photofroggy</a>'})
};var b=function(R,Q){Q.ui.theme(R.args.split(" ").shift())};var a=function(S){var R=S.args.split(" ");var Q=R.shift().toLowerCase();var R=R.join(" ");if(R.length==0){p.cchannel.serverMessage("Could not set "+Q,"No data supplied");return}if(!(Q in p.settings)){p.cchannel.serverMessage('Unknown setting "'+Q+'"');return}p.settings[Q]=R;p.cchannel.serverMessage("Changed "+Q+" setting","value: "+R);p.control.setLabel()};var g=function(Q){p[Q.cmd]()};var L=function(Q){var R=Q.args.split(" ");var R=R.toString()==""?[]:R;if(Q.ns!=Q.target){R.unshift(Q.target)}if(R.toString()==""){return}for(index in R){p.join(R[index])}};var O=function(Q){var R=Q.args.split(" ");if(Q.ns!=Q.target){R.unshift(Q.target)}if(R.toString()==""){p.part(Q.ns);return}for(index in R){p.part(R[index])}};var k=function(Q){p.set(Q.target,Q.cmd,Q.args)};var E=function(R){var Q=R.args.split(" ");p[R.cmd.toLowerCase()](R.target,Q[0],Q[1])};var m=function(U,Q){var S=U.args.split(" ");var R=S.shift();var T=U.cmd;if(T=="ban"&&S.length>0){Q.kick(U.target,R,S.join(" "))
}Q[T](U.target,R)};var B=function(Q){p.action(Q.target,Q.args)};var M=function(Q){p.send(Q.args.replace(/\\n/gm,"\n"))};var j=function(T,Q){var U=T.args.split(" ");var R=U.shift();var S=U.length>0?U.join(" "):null;if(T.cmd=="kick"){Q.kick(T.target,R,S)}else{Q.kill(R,S)}};var u=function(Q){p.say(Q.target,Q.args)};var P=function(Q){p.npmsg(Q.target,Q.args)};var J=function(R,Q){Q.cchannel.clear()};var I=function(R,Q){for(c in Q.channelo){Q.channelo[c].clear()}};var H=function(R,Q){Q.whois(R.args.split(" ")[0])};var F=function(R,Q){Q.admin(R.target,R.args)};var D=function(R,Q){Q.disconnect()};var A=function(R,Q){var S=R.cmd.indexOf("title")>-1?"title":"topic";Q.control.ui.set_text("/"+S+" "+Q.channel(R.target).info[S].content)};var v=function(S,Q){r.away.on=true;r.away.last={};r.away.since=new Date();r.away.reason=S.args;var R="is away"+(r.away.reason.length>0?": "+r.away.reason:"");Q.each_channel(function(T){Q.action(T,R)})};var s=function(R,Q){r.away.on=false;Q.each_channel(function(S){Q.action(S,"is back")
})};var z=function(T,Q){if(!r.away.on){return}if(r.away.reason.length==0){return}if(T.user==Q.settings.username){return}if(Q.exclude.indexOf(T.sns.toLowerCase())!=-1){return}var R=new Date();var S=T.sns.toLowerCase();if(S in r.away.last){if((R-r.away.last[S])<=60000){return}}Q.say(T.ns,T.user+": I am currently away; reason: "+r.away.reason);r.away.last[S]=R};var l=function(T,Q){if(T.p!="info"){return}var R=T.pkt.sub;var U=R.shift().arg;U.username=T.sns.substr(1);U.connections=[];var S={};while(R.length>0){S=R.shift().arg;S.channels=[];while(R.length>0){if(R[0].cmd!="ns"){break}S.channels.unshift(Q.deform_ns(R.shift().param))}U.connections.push(S)}Q.cchannel.log_whois(U)};var C=function(R,Q){if(R.ns.indexOf("login:")!=0){return}Q.cchannel.server_message("Whois failed for "+(R.sns.substr(1)),"not online")};var N=function(U,R){var X=R.channel(U.ns);var Q=U.info.split("\n");var W="";var V=[];var S="";for(var T in Q){if(!Q.hasOwnProperty(T)){continue}W=Q[T].split(" ");if(U.p=="privclass"){V.push([W.shift(),W.shift().split("=")[1],W.join(" ")])
}else{if(U.p=="users"){S=W.shift().split(":",1)[0];V.push([S,X.get_privclass_order(S),W.join(" ")])}}}X.log_pc(U.p=="privclass",V)};G();wsc.defaults.Extension.Ignore(p)};wsc.defaults.Extension.Ignore=function(g){var j={};var k=g.storage.folder("ignore");var a=k.folder("ignored");var q=function(){p();m();g.bind("cmd.ignore",b);g.bind("cmd.unignore",l);g.bind("settings.open",j.page)};j.page=function(r,s){};var b=function(r){};var l=function(r){};var p=function(){j.ignore=k.get("ignore","/me is ignoring {user} now");j.unignore=k.get("unignore","/me is not ignoring {user} anymore");j.count=parseInt(k.get("count",0));j.ignored=[];if(j.count>0){var s=null;for(var r=0;r<j.count;r++){s=a.get(r,null);if(s===null){continue}j.ignored.push(s)}}};var m=function(){k.set("ignore",j.ignore);k.set("unignore",j.unignore);if(j.count==0){k.set("count",0)}else{var s=-1;for(var r in j.ignored.length){if(!j.ignored.hasOwnProperty(r)){continue}s++;a.set(s,j.ignored[r])}k.set("count",s)}};q()};wsc.Client=function(a,g,j){this.mozilla=j;
this.storage=new wsc.Storage;this.fresh=true;this.attempts=0;this.connected=false;this.protocol=null;this.flow=null;this.ui=null;this.events=new EventEmitter();this.conn=null;this.channelo={};this.cchannel=null;this.exclude=[];this.cmds=[];this.settings={domain:"website.com",server:"ws://website.com/wsendpoint",symbol:"",username:"",userinfo:{},pk:"",monitor:["~Monitor",true],welcome:"Welcome to the wsc web client!",autojoin:"chat:channel",control:wsc.Control,protocol:wsc.Protocol,mparser:wsc.MessageParser,flow:wsc.Flow,ui:Chatterbox.UI,extend:[wsc.defaults.Extension],client:"chatclient",clientver:"0.3",theme:wsc.defaults.theme,themes:wsc.defaults.themes,};a.extend(this.settings,g);this.settings.theme=this.storage.get("theme",wsc.defaults.theme);this.storage.set("theme",this.settings.theme);this.ui=new this.settings.ui(a,{themes:this.settings.themes,theme:this.settings.theme,monitor:this.settings.monitor,username:this.settings.username,domain:this.settings.domain},j);this.settings.agent=this.ui.LIB+"/"+this.ui.VERSION+" ("+navigator.appVersion.match(/\(([^)]+)\)/)[1]+") wsc/"+wsc.VERSION;
this.mns=this.format_ns(this.settings.monitor[0]);this.lun=this.settings.username.toLowerCase();this.protocol=new this.settings.protocol(new this.settings.mparser());this.flow=new this.settings.flow(this.protocol);this.build();for(var b in this.settings.extend){this.settings.extend[b](this)}this.monitor(this.settings.welcome)};wsc.Client.prototype.build=function(){this.ui.build();this.control=new this.settings.control(this);var a=this;this.ui.on("channel.selected",function(b,g){a.cchannel=a.channel(b.ns);a.control.cache_input(b)});this.ui.on("tab.close.clicked",function(b,g){if(b.chan.monitor){return false}a.part(b.ns);return false})};wsc.Client.prototype.loop=function(){this.ui.loop()};wsc.Client.prototype.add_extension=function(a){this.settings.extend.push(a);a(this)};wsc.Client.prototype.bind=function(b,a){this.events.addListener(b,a);if(b.indexOf("cmd.")!=0||b.length<=4){return}cmd=b.slice(4).toLowerCase();this.cmds.push(cmd)};wsc.Client.prototype.clear_listeners=function(a){this.events.removeListeners(a)
};wsc.Client.prototype.trigger=function(a,b){this.events.emit(a,b,this)};wsc.Client.prototype.connect=function(){if(this.connected){return}this.attempts++;try{var a=this;this.conn=wsc.Transport.Create(this.settings.server);this.conn.open(function(g,j){a.flow.open(a,g,j)});this.conn.disconnect(function(g){a.flow.close(a,g)});this.conn.message(function(g){a.flow.message(a,g)});this.conn.connect();this.ui.server_message("Opening connection");this.trigger("start",new wsc.Packet("client connecting\ne=ok\n\n"))}catch(b){console.log(b);this.monitor("Your browser does not support WebSockets. Sorry.");this.trigger("start",new wsc.Packet("client connecting\ne=no websockets available\n\n"))}};wsc.Client.prototype.close=function(){console.log(this.conn);this.conn.close()};wsc.Client.prototype.channel=function(a,b){a=this.deform_ns(a).slice(1).toLowerCase();if(!this.channelo[a]&&b){this.channelo[a]=b}return this.channelo[a]};wsc.Client.prototype.channels=function(b){var a=[];for(ns in this.channelo){if(!this.channelo.hasOwnProperty(ns)){continue
...
}return a};wsc.Client.prototype.create_ns=function(a,b){chan=this.channel(a,new wsc.Channel(this,a,b));chan.build()};wsc.Client.prototype.remove_ns=function(a){this.ui.remove_channel(a)};wsc.Client.prototype.log=function(a,b){var g=this.channel(a);if(!g){return}g.log(b)};wsc.Client.prototype.monitor=function(a){this.ui.monitor(a)};wsc.Client.prototype.send=function(a){return this.conn.send(a)};wsc.Client.prototype.handshake=function(){this.send(wsc_packetstr(this.settings.client,this.settings.clientver,{agent:this.settings.agent}))};wsc.Client.prototype.login=function(){pkt="login "+this.settings.username+"\npk="+this.settings.pk+"\n";this.send(pkt)};wsc.Client.prototype.pong=function(){this.send(wsc_packetstr("pong"))};wsc.Client.prototype.join=function(a){this.send(wsc_packetstr("join",this.format_ns(a)))};wsc.Client.prototype.part=function(a){this.send(wsc_packetstr("part",this.format_ns(a)))};wsc.Client.prototype.say=function(a,b){e={name:"send.msg.before",msg:b,ns:a};this.trigger(e);
this.send(wsc_packetstr("send",this.format_ns(a),{},wsc_packetstr("msg","main",{},e.msg)))};wsc.Client.prototype.npmsg=function(a,b){e={name:"send.npmsg.before",msg:b,ns:a};this.trigger(e);this.send(wsc_packetstr("send",this.format_ns(a),{},wsc_packetstr("npmsg","main",{},e.msg)))};wsc.Client.prototype.action=function(a,b){e={name:"send.action.before",msg:b,ns:a};this.trigger(e);this.send(wsc_packetstr("send",this.format_ns(a),{},wsc_packetstr("action","main",{},e.msg)))};wsc.Client.prototype.promote=function(g,a,b){this.send(wsc_packetstr("send",this.format_ns(g),{},wsc_packetstr("promote",a,{},(!b?"":b))))};wsc.Client.prototype.demote=function(g,a,b){this.send(wsc_packetstr("send",this.format_ns(g),{},wsc_packetstr("demote",a,{},(!b?"":b))))};wsc.Client.prototype.ban=function(b,a){this.send(wsc_packetstr("send",this.format_ns(b),{},wsc_packetstr("ban",a)))};wsc.Client.prototype.unban=function(b,a){this.send(wsc_packetstr("send",this.format_ns(namespace),{},wsc_packetstr("unban",a,{},(!pc?"":pc))))
...
}};Chatterbox.Settings.Item.prototype._event_stub=function(){};Chatterbox.Settings.Item.prototype._get_cb=function(a){if(!this.options.hasOwnProperty("event")){return this._event_stub}return this.options.event[a]||this._event_stub};Chatterbox.Settings.Item.prototype._get_ep=function(a){var b=Chatterbox.template.settings.item.get(this.selector);if(b==null){return false}if(!b.hasOwnProperty("events")){return false}for(i in b.events){pair=b.events[i];if(pair[0]==a){return pair}}};Chatterbox.Settings.Item.prototype.save=function(g,j){var k=this._get_ep("inspect");var b=k==false?null:this.view.find(k[1]);var a=this._get_cb("save");if(typeof a=="function"){a({input:b,item:this,page:j,window:g})}else{for(i in a){var l=b.hasOwnProperty("slice")?b.slice(i,1):b;a[i]({input:l,item:this,page:j,window:g})}}for(i in this.items){this.items[i].save(g,j)}};Chatterbox.Settings.Item.prototype.close=function(a,b){pair=this._get_ep("inspect");inps=pair==false?null:this.view.find(pair[1]);cb=this._get_cb("close");
