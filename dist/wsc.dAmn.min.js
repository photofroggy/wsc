...
Chatterbox.Popup.prototype.build.call(this);this.saveb=this.window.find("a.button.save");this.closeb=this.window.find("a.close");this.scb=this.window.find("a.button.saveclose");this.tabs=this.window.find("nav.tabs ul.tabs");this.book=this.window.find("div.book");this.config.build(this);this.window.find("ul.tabs li").first().addClass("active");this.window.find("div.book div.page").first().addClass("active");var a=this;this.window.find("form").bind("change",function(){a.changed=true});this.config.each_page(function(b,g){g.each_item(function(j,k){k._onchange=function(l){a.changed=true}})});this.saveb.click(function(b){a.save();return false});this.closeb.click(function(b){if(a.changed){if(!confirm("Are you sure? You will lose any unsaved changes.")){return false}}a.close();return false});this.scb.click(function(b){a.save();a.close();return false});this.resize()};Chatterbox.Settings.prototype.resize=function(){var a=this.window.find(".inner");var g=a.find("h2");var j=a.find(".bookwrap");var b=a.find("footer");
j.height(a.height()-b.outerHeight()-g.outerHeight()-15);this.book.height(j.innerHeight()-this.tabs.outerHeight()-25);this.config.resize()};Chatterbox.Settings.prototype.switch_page=function(b){var g=this.tabs.find("li.active").first();var a=g.prop("id").split("-",1)[0];g=this.config.page(a.split("_").join(" "));g.hide();b.show()};Chatterbox.Settings.prototype.save=function(){this.config.save(this);this.changed=false};Chatterbox.Settings.prototype.close=function(){this.window.remove();this.manager.nav.settings.open=false;this.manager.nav.settings.window=null;this.config.close(this)};Chatterbox.Settings.Config=function(){this.pages=[]};Chatterbox.Settings.Config.prototype.find_page=function(b){var j=b.toLowerCase();var g;for(var a in this.pages){g=this.pages[a];if(g.lname==j){return g}}return null};Chatterbox.Settings.Config.prototype.build=function(b){for(var a in this.pages){this.pages[a].build(b)}};Chatterbox.Settings.Config.prototype.resize=function(){for(var a in this.pages){this.pages[a].resize()
}};Chatterbox.Settings.Config.prototype.page=function(b,a){var g=this.find_page(b);a=a||false;if(g==null){g=new Chatterbox.Settings.Page(b);if(a){this.pages.push(g)}else{this.pages.unshift(g)}}return g};Chatterbox.Settings.Config.prototype.each_page=function(j){var g=null;var a=null;for(var b in this.pages){if(!this.pages.hasOwnProperty(b)){continue}g=this.pages[b];a=j(b,g);if(a===false){break}}};Chatterbox.Settings.Config.prototype.save=function(a){for(i in this.pages){this.pages[i].save(a)}};Chatterbox.Settings.Config.prototype.close=function(b){for(var a in this.pages){this.pages[a].close(b)}};Chatterbox.Settings.Page=function(a){this.name=a;this.lname=a.toLowerCase();this.ref=replaceAll(this.lname," ","_");this.items=[];this.itemo={}};Chatterbox.Settings.Page.prototype.build=function(b){var a=replaceAll(Chatterbox.template.settings.tab,"{ref}",this.ref);a=replaceAll(a,"{name}",this.name);var g=replaceAll(Chatterbox.template.settings.page,"{ref}",this.ref);g=replaceAll(g,"{page-name}",this.name);
b.tabs.append(a);b.book.append(g);this.view=b.book.find("div#"+this.ref+"-page");this.tab=b.tabs.find("li#"+this.ref+"-tab");var g=this;this.tab.find("a").click(function(j){if(g.tab.hasClass("active")){return false}b.switch_page(g);return false});this.content()};Chatterbox.Settings.Page.prototype.content=function(){for(var a in this.items){this.items[a].build(this.view)}};Chatterbox.Settings.Page.prototype.resize=function(){for(var a in this.items){this.items[a].resize()}};Chatterbox.Settings.Page.prototype.show=function(){if(!this.tab.hasClass("active")){this.tab.addClass("active")}if(!this.view.hasClass("active")){this.view.addClass("active")}this.resize()};Chatterbox.Settings.Page.prototype.hide=function(){if(this.tab.hasClass("active")){this.tab.removeClass("active")}if(this.view.hasClass("active")){this.view.removeClass("active")}};Chatterbox.Settings.Page.prototype.item=function(g,b,a){a=a||false;var j=Chatterbox.Settings.Item.get(g,b);if(a){this.items.unshift(j)}else{this.items.push(j)
}if(b.hasOwnProperty("ref")){this.itemo[b.ref]=j}return j};Chatterbox.Settings.Page.prototype.get=function(a){if(this.itemo.hasOwnProperty(a)){return this.itemo[a]}return null};Chatterbox.Settings.Page.prototype.each_item=function(j){var g=null;var a=null;for(var b in this.items){if(!this.items.hasOwnProperty(b)){continue}g=this.items[b];a=j(b,g);if(a===false){break}}};Chatterbox.Settings.Page.prototype.save=function(b){for(var a in this.items){this.items[a].save(b,this)}};Chatterbox.Settings.Page.prototype.close=function(b){for(var a in this.items){this.items[a].close(b,this)}};Chatterbox.Settings.Item=function(b,a){this.options=a||{};this.type=b||"base";this.selector=this.type.toLowerCase();this.items=[];this.itemo={};this.view=null;this.val=null;this._onchange=this._event_stub};Chatterbox.Settings.Item.prototype.build=function(m){if(!this.options.hasOwnProperty("ref")){return}var l=this.content();if(l.length==0){return}var a="";if(this.options.hasOwnProperty("wclass")){a=" "+this.options.wclass
}var k=Chatterbox.render("settings.item.wrap",{type:this.type.toLowerCase().split(".").join("-"),ref:this.options.ref,"class":a});k=replaceAll(k,"{content}",l);m.append(k);this.view=m.find(".item."+this.options.ref);this.hooks(this.view);if(!this.options.hasOwnProperty("subitems")){return}var p;var j;var g;var b;for(i in this.options.subitems){p=this.options.subitems[i];j=p[0];g=p[1];sitem=Chatterbox.Settings.Item.get(j,g);b=["stacked"];if(sitem.options.wclass){b.push(sitem.options.wclass)}sitem.options.wclass=b.join(" ");sitem.build(this.view);this.items.push(sitem);if(g.hasOwnProperty("ref")){this.itemo[g.ref]=sitem}}};Chatterbox.Settings.Item.prototype.content=function(){return Chatterbox.render("settings.item."+this.selector,this.options)};Chatterbox.Settings.Item.prototype.resize=function(){for(var a in this.items){this.items[a].resize()}};Chatterbox.Settings.Item.prototype.hooks=function(g){if(!this.options.hasOwnProperty("event")){return}var b=this.options.event;var k=Chatterbox.template.settings.item.get(this.selector);
if(k==false){return}if(!k.hasOwnProperty("events")){return}var j=[];for(var a in k.events){j=k.events[a];if(!b.hasOwnProperty(j[0])){continue}g.find(j[1]).bind(j[0],b[j[0]])}};Chatterbox.Settings.Item.prototype._event_stub=function(){};Chatterbox.Settings.Item.prototype._get_cb=function(a){if(!this.options.hasOwnProperty("event")){return this._event_stub}return this.options.event[a]||this._event_stub};Chatterbox.Settings.Item.prototype._get_ep=function(b){var j=Chatterbox.template.settings.item.get(this.selector);if(j==null){return false}if(!j.hasOwnProperty("events")){return false}var g=[];for(var a in j.events){g=j.events[a];if(g[0]==b){return g}}};Chatterbox.Settings.Item.prototype.save=function(j,k){var l=this._get_ep("inspect");var b=l?this.view.find(l[1]):null;var a=this._get_cb("save");if(typeof a=="function"){a({input:b,item:this,page:k,window:j})}else{for(var g in a){var m=b.hasOwnProperty("slice")?b.slice(g,1):b;a[g]({input:m,item:this,page:k,window:j})}}for(var g in this.items){this.items[g].save(j,k)
}};Chatterbox.Settings.Item.prototype.close=function(j,k){var l=this._get_ep("inspect");var b=l?this.view.find(l[1]):null;var a=this._get_cb("close");if(typeof a=="function"){a({input:b,item:this,page:k,window:j});return}for(var g in a){var m=b.hasOwnProperty("slice")?b.slice(g,1):b;a[g]({input:m,item:this,page:k,window:j})}for(var g in this.items){this.items[g].close(j,k)}};Chatterbox.Settings.Item.get=function(l,g,p,b){var k=l.split(".");var m=p||Chatterbox.Settings.Item;var a;for(var j in k){a=k[j];if(!m.hasOwnProperty(a)){m=b||Chatterbox.Settings.Item;break}m=m[a]}return new m(l,g)};Chatterbox.Settings.Item.Form=function(b,a){Chatterbox.Settings.Item.call(this,b,a);this.form=null;this.fields=[];this.lsection=null;this.fsection=null;this.fieldo={}};Chatterbox.Settings.Item.Form.prototype=new Chatterbox.Settings.Item();Chatterbox.Settings.Item.Form.prototype.constructor=Chatterbox.Settings.Item.Form;Chatterbox.Settings.Item.Form.field=function(b,a){return Chatterbox.Settings.Item.get(b,a,Chatterbox.Settings.Item.Form,Chatterbox.Settings.Item.Form.Field)
