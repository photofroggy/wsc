...
}var u=new Date();var v=z.sns.toLowerCase();if(v in g.last){if((u-g.last[v])<=g.interval){return}}var s=timeLengthString((u-g.since)/1000);var A=replaceAll(g.format.away,"{user}",z.user);A=replaceAll(A,"{timesince}",s);r.say(z.ns,replaceAll(A,"{reason}",g.reason));g.last[v]=u};var m=function(){g.format.setaway=j.get("setaway","/me is away: {reason}");g.format.setback=j.get("setback","/me is back");g.format.away=j.get("away","{user}: I've been away for {timesince}. Reason: {reason}");g.interval=parseInt(j.get("interval",60000))};var k=function(){j.set("interval",g.interval);j.set("setaway",g.format.setaway);j.set("setback",g.format.setback);j.set("away",g.format.away)};q()};wsc.defaults.Extension.Ignore=function(g){var j={};var k=g.storage.folder("ignore");var a=k.folder("ignored");var q=function(){p();m();g.bind("cmd.ignore",b);g.bind("cmd.unignore",l);g.ui.on("settings.open",j.page)};j.page=function(u,z){var v=u.settings.page("Ignores",true);var B={};var s="<ul>";B.im=j.ignore;B.uim=j.unignore;
if(g.ui.umuted.length==0){s+="<li><i>No one ignored yet</i></li></ul>"}else{for(var r in g.ui.umuted){if(!g.ui.umuted.hasOwnProperty(r)){continue}s+="<li>"+g.ui.umuted[r]+"</li>"}s+="</ul>"}v.item("Text",{ref:"intro",title:"Ignores",text:'Use <code>ignore</code> to ignore people.\n\n                    You can "ignore" other users of the chat server using the\n                    <code>/ignore</code> command. Ignoring a user hides their                    messages from you in the channel log.',});v.item("Form",{ref:"msgs",title:"Messages",text:"Here you can set the messages displayed when you ignore or                    unignore a user.\n\nThe text <code>{user}</code> is replaced                    with the name of the user your are ignoring or unignoring.",fields:[["Textfield",{ref:"ignore",label:"Ignore","default":B.im}],["Textfield",{ref:"unignore",label:"Unignore","default":B.uim}]],event:{save:function(C){j.ignore=C.data.ignore;j.unignore=C.data.unignore;m()}}});v.item("Items",{ref:"ignoreds",title:"Users",text:"This is the list of users that you have silenced.\n\nUse the                    commands <code>/ignore</code> and <code>/unignore</code>                    to edit the list.",items:g.ui.umuted});
var A=v.item("Form",{ref:"ignored",wclass:"boxed-ff-indv",title:"Users",text:"This is the list of users that you have silenced.\n\nUse the                    commands <code>/ignore</code> and <code>/unignore</code>                    to edit the list.",fields:[["Text",{ref:"users",text:s}]]})};var b=function(v){var A=v.args.split(" ");var r="";var z="";var u=false;for(var s in A){if(!A.hasOwnProperty(s)){continue}r=A[s];tmod=g.mute_user(r);if(!tmod){continue}u=tmod;z=replaceAll(j.ignore,"{user}",r);if(z.indexOf("/me ")==0){z=z.substr(4);g.action(v.target,z)}else{g.say(v.target,z)}}if(u){m()}};var l=function(z){var C=z.args.split(" ");var s="";var B="";var r=-1;var v=false;var A=false;for(var u in C){if(!C.hasOwnProperty(u)){continue}s=C[u];A=g.unmute_user(s);if(!A){continue}v=A;B=replaceAll(j.unignore,"{user}",s);if(B.indexOf("/me ")==0){B=B.substr(4);g.action(z.target,B)}else{g.say(z.target,B)}}if(v){m()}};var p=function(){j.ignore=k.get("ignore","/me is ignoring {user} now");j.unignore=k.get("unignore","/me is not ignoring {user} anymore");
j.count=parseInt(k.get("count",0));g.ui.umuted=[];if(j.count>0){var s=null;for(var r=0;r<j.count;r++){g.mute_user.push(a.get(r,null))}}};var m=function(){k.set("ignore",j.ignore);k.set("unignore",j.unignore);for(var r=0;r<j.count;r++){a.remove(r)}if(g.ui.umuted.length==0){k.set("count",0)}else{var s=-1;for(var r in g.ui.umuted){if(!g.ui.umuted.hasOwnProperty(r)){continue}s++;a.set(s.toString(),g.ui.umuted[r])}s++;j.count=s;k.set("count",s)}};q()};wsc.Client=function(a,g,j){this.mozilla=j;this.storage=new wsc.Storage;this.storage.ui=this.storage.folder("ui");this.storage.aj=this.storage.folder("autojoin");this.storage.aj.channel=this.storage.aj.folder("channel");this.fresh=true;this.attempts=0;this.connected=false;this.protocol=null;this.flow=null;this.ui=null;this.events=new EventEmitter();this.conn=null;this.channelo={};this.cchannel=null;this.exclude=[];this.cmds=[];this.settings={domain:"website.com",server:"ws://website.com/wsendpoint",symbol:"",username:"",userinfo:{},pk:"",monitor:["~Monitor",true],welcome:"Welcome to the wsc web client!",autojoin:"chat:channel",control:wsc.Control,protocol:wsc.Protocol,mparser:wsc.MessageParser,flow:wsc.Flow,ui_object:Chatterbox.UI,extend:[wsc.defaults.Extension],client:"chatclient",clientver:"0.3",ui:{theme:wsc.defaults.theme,themes:wsc.defaults.themes,tabclose:true,clock:true}};
...
}}b=(a.shiftKey?"/npmsg ":(b[0]=="/"?"":"/say "))+b;b=b.slice(1);bits=b.split(" ");cmdn=bits.shift().toLowerCase();ens=this.client.cchannel.namespace;etarget=ens;if(bits[0]){hash=bits[0][0];if((hash=="#"||hash=="@")&&bits[0].length>1){etarget=this.client.format_ns(bits.shift())}}arg=bits.join(" ");this.client.trigger("cmd."+cmdn,{name:"cmd",cmd:cmdn,args:arg,target:etarget,ns:ens})};var Chatterbox={};Chatterbox.VERSION="0.6.50";Chatterbox.STATE="beta";Chatterbox.UI=function(a,b,j,g){this.events=g||new EventEmitter();this.mozilla=j;this.umuted=[];this.settings={themes:["wsct_default","wsct_dAmn"],theme:"wsct_default",monitor:["~Monitor",true],username:"",domain:"website.com",clock:true,tabclose:true};a.extend(this.settings,b);a.append('<div class="wsc '+this.settings.theme+'"></div>');this.view=a.find(".wsc");this.mns=this.format_ns(this.settings.monitor[0]);this.lun=this.settings.username.toLowerCase();this.monitoro=null;this.swidth=(function(){if($.browser.msie){var m=$('<textarea cols="10" rows="2"></textarea>').css({position:"absolute",top:-1000,left:-1000}).appendTo("body"),l=$('<textarea cols="10" rows="2" style="overflow: hidden;"></textarea>').css({position:"absolute",top:-1000,left:-1000}).appendTo("body");
...
Chatterbox.Settings.Item.Form.field=function(b,a){return Chatterbox.Settings.Item.get(b,a,Chatterbox.Settings.Item.Form,Chatterbox.Settings.Item.Form.Field)};Chatterbox.Settings.Item.Form.prototype.build=function(k){Chatterbox.Settings.Item.prototype.build.call(this,k);if(this.view==null){return}this.lsection=this.view.find("section.labels");this.fsection=this.view.find("section.fields");if(!this.options.hasOwnProperty("fields")){return}var j;var l;for(var a in this.options.fields){j=this.options.fields[a];try{l=Chatterbox.Settings.Item.Form.field(j[0],j[1]);this.fields.push(l);l.build(this);if(j[1].hasOwnProperty("ref")){this.fieldo[j[1].ref]=l}}catch(g){console.log(g,j)}}this.form=this.view.find("form");var b=this;this.form.bind("change",function(m){b.change()})};Chatterbox.Settings.Item.Form.prototype.resize=function(){for(var a in this.fields){this.fields[a].resize()}};Chatterbox.Settings.Item.Form.prototype.change=function(){var g={};var j;for(var b in this.fields){j=this.fields[b];
g[j.ref]=j.get()}var a=this._get_cb("change");if(typeof a=="function"){a({data:g,form:this})}else{for(var b in a){a[b]({data:g,form:this})}}};Chatterbox.Settings.Item.Form.prototype.save=function(j,l){var k={};var b;for(var g in this.fields){field=this.fields[g];k[field.ref]=field.get()}var a=this._get_cb("save");if(typeof a=="function"){a({data:k,form:this,page:l,window:j})}else{for(var g in a){a[g]({data:k,form:this,page:l,window:j})}}};Chatterbox.Settings.Item.Form.prototype.close=function(g,k){var j={};var l;for(var b in this.fields){l=this.fields[b];j[l.ref]=l.get()}var a=this._get_cb("close");if(typeof a=="function"){a({data:j,form:this,page:k,window:g})}else{for(var b in a){a[b]({data:j,form:this,page:k,window:g})}}};Chatterbox.Settings.Item.Form.Field=function(b,a){Chatterbox.Settings.Item.call(this,b,a);this.ref=this.options.ref||"ref";this.label=null;this.field=null;this.value=""};Chatterbox.Settings.Item.Form.Field.prototype=new Chatterbox.Settings.Item();Chatterbox.Settings.Item.Form.Field.prototype.constructor=Chatterbox.Settings.Item.Form.Field;
...
this.value=this.view.find("input[checked]:radio").val();var a=this;this.field.bind("change",function(j){a.value=a.view.find(this).val()})};Chatterbox.Settings.Item.Checkbox=function(b,a){Chatterbox.Settings.Item.Radio.call(this,b,a);this.value=[]};Chatterbox.Settings.Item.Checkbox.prototype=new Chatterbox.Settings.Item.Radio();Chatterbox.Settings.Item.Checkbox.prototype.constructor=Chatterbox.Settings.Item.Checkbox;Chatterbox.Settings.Item.Checkbox.prototype.build=function(g){if(this.options.hasOwnProperty("items")){for(i in this.options.items){var b=this.options.items[i];b.name=this.options.ref||"ref"}}Chatterbox.Settings.Item.prototype.build.call(this,g);this.field=this.view.find("input:checkbox");var a=this;this.value=[];this.view.find("input[checked]:checkbox").each(function(){a.value.push(a.view.find(this).val())});this.field.bind("change",function(j){a.value=[];a.view.find("input[checked]:checkbox").each(function(){a.value.push(a.view.find(this).val())})})};Chatterbox.Settings.Item.Items=function(b,a){Chatterbox.Settings.Item.call(this,b,a);
this.selected=false};Chatterbox.Settings.Item.Items.prototype=new Chatterbox.Settings.Item();Chatterbox.Settings.Item.Items.prototype.constructor=Chatterbox.Settings.Item.Items;Chatterbox.Settings.Item.Items.prototype.build=function(b){Chatterbox.Settings.Item.prototype.build.call(this,b);var a=this;this.list=this.view.find("ul");this.buttons=this.view.find("section.buttons");this.list.find("li").click(function(j){var g=a.list.find(this);a.list.find("li.selected").removeClass("selected");a.selected=g.html();g.addClass("selected")});this.buttons.find("a.button").click(function(g){return false});this.buttons.find("a.button.up").click(function(j){if(a.selected===false){return false}var k=a.options.items.indexOf(a.selected);var g=k-1;if(k==-1||k>=a.options.items.length){return false}if(g<0||g>=a.options.items.length){return false}a._fevent("up",{swap:{"this":{index:k,item:a.options.items[k]},that:{index:g,item:a.options.items[g]}}});a.refresh();a._onchange({});return false});this.buttons.find("a.button.down").click(function(j){if(a.selected===false){return false
}var k=a.options.items.indexOf(a.selected);var g=k+1;if(k==-1||k>=a.options.items.length){return false}if(g<0||g>=a.options.items.length){return false}a._fevent("down",{swap:{"this":{index:k,item:a.options.items[k]},that:{index:g,item:a.options.items[g]}}});a.refresh();a._onchange({});return false});this.buttons.find("a.button.add").click(function(j){if(a.selected===false){return false}var g=new Chatterbox.Popup(a.manager,{close:false});return false});this.buttons.find("a.button.remove").click(function(j){if(a.selected===false){return false}var k=a.options.items.indexOf(a.selected);var g=k-1;if(k==-1){return false}if(g<0||g>=a.options.items.length){return false}return false})};Chatterbox.Settings.Item.Items.prototype.refresh=function(){this.view.find("section.mitems").html(Chatterbox.template.settings.krender.manageditems(this.options.items));this.list=this.view.find("ul");this.list.find("li[title="+this.selected.toLowerCase()+"]").addClass("selected");var a=this;this.list.find("li").click(function(g){var b=a.list.find(this);
a.list.find("li.selected").removeClass("selected");a.selected=b.html();b.addClass("selected")})};Chatterbox.Settings.Item.Items.prototype._fevent=function(b,j){var l=this._get_ep("inspect");var g=l?this.view.find(l[1]):null;var a=this._get_cb(b);if(typeof a=="function"){a({input:g,item:this,args:j})}else{for(var k in a){var m=g.hasOwnProperty("slice")?g.slice(k,1):g;a[k]({input:m,item:this,args:j})}}};Chatterbox.template={};Chatterbox.render=function(j,l){var g=Chatterbox.template;var b=j.split(".");var k={};var a=null;for(ind in b){part=b[ind];if(!g.hasOwnProperty(part)){return""}g=g[part]}if(g.hasOwnProperty("frame")){a=g;k=g.render||{};g=g.frame;if(a.hasOwnProperty("pre")){if(typeof a.pre=="function"){g=a.pre(g,l)}else{for(i in a.pre){g=a.pre[i](g,l)}}}}for(key in l){g=replaceAll(g,"{"+key+"}",(k[key]||Chatterbox.template.render_stub)(l[key]||""))}if(a!=null){if(a.hasOwnProperty("post")){if(typeof a.post=="function"){g=a.post(g,l)}else{for(i in a.post){g=a.post[i](g,l)}}}}return g};Chatterbox.template.render_stub=function(a){return a
