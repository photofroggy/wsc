...
var n=function(){f.bind("cmd.connect",b.connection);f.bind("cmd.set",b.setter);f.bind("cmd.join",b.join);f.bind("cmd.chat",b.pjoin);f.bind("cmd.part",b.part);f.bind("cmd.say",b.say);f.bind("cmd.npmsg",b.npmsg);f.bind("cmd.me",b.action);f.bind("cmd.promote",b.chgpriv);f.bind("cmd.demote",b.chgpriv);f.bind("cmd.ban",b.ban);f.bind("cmd.unban",b.ban);f.bind("cmd.kick",b.killk);f.bind("cmd.whois",b.whois);f.bind("cmd.title",b.title);f.bind("cmd.topic",b.title);f.bind("cmd.admin",b.admin);f.bind("cmd.disconnect",b.connection);f.bind("cmd.kill",b.killk);f.bind("cmd.raw",b.raw);f.bind("cmd.clear",b.clear);f.bind("cmd.clearall",b.clearall);f.bind("cmd.close",b.close);f.bind("pkt.property",m);f.bind("pkt.recv_admin_show",g);f.bind("pkt.recv_admin_showverbose",g);f.bind("pkt.get",l);f.bind("cmd.gettitle",b.gett);f.bind("cmd.gettopic",b.gett);f.bind("cmd.theme",b.theme)};var k=function(p,o){f.settings.ui.theme=p.theme;f.settings.ui.clock=p.clock;f.settings.ui.tabclose=p.tabclose};var j=function(q,p){var o=q.settings.page("Main");
var r={};r.username=f.settings.username;r.pk=f.settings.pk;r.devel=f.settings.developer;o.item("Form",{ref:"login",title:"Login",text:"Here you can change the username and token used to                    log into the chat server.",fields:[["Textfield",{ref:"username",label:"Username","default":r.username}],["Textfield",{ref:"token",label:"Token","default":r.pk}]],event:{save:function(s){f.settings.username=s.data.username;f.settings.pk=s.data.token}}},true);o.item("Form",{ref:"developer",title:"Developer Mode",text:"Turn developer mode on or off.\n\nDeveloper mode will expose any hidden                channel tabs, amongst other things. Keep this turned off unless you're working                on implementing something.",fields:[["Checkbox",{ref:"enabled",items:[{value:"on",title:"On",selected:r.devel}]}]],event:{change:function(s){f.settings.developer=(s.data.enabled.indexOf("on")!=-1);f.ui.developer(f.settings.developer)},save:function(s){r.devel=f.settings.developer},close:function(s){f.settings.developer=r.devel;
f.ui.developer(f.settings.developer)}}});o.item("Text",{ref:"intro",title:"Main",text:"Use this window to view and change your settings.\n\nCheck                    the different pages to see what settings can be changed.",},true);o.item("Text",{ref:"debug",wclass:"faint",title:"Debug Information",text:"Chat Agent: <code>"+f.settings.agent+"</code>\n\nUser                    Agent: <code>"+navigator.userAgent+"</code>"})};var h=function(q,p){var o=q.settings.page("About",true);o.item("Text",{ref:"about-wsc",title:"Wsc",text:'Currently using <a href="http://github.com/photofroggy/wsc/">wsc</a>                    version '+wsc.VERSION+" "+wsc.STATE+'.\n\nWsc                    works using HTML5, javascript, and CSS3. WebSocket is used for the connection                    where possible. The source code for this client is pretty huge.\n\nWsc was created                    by ~<a href="http://photofroggy.deviantart.com/">photofroggy</a>'})};var b={};b.theme=function(p,o){o.ui.theme(p.args.split(" ").shift())
};b.setter=function(q){var p=q.args.split(" ");var o=p.shift().toLowerCase();var p=p.join(" ");if(p.length==0){f.cchannel.serverMessage("Could not set "+o,"No data supplied");return}if(!(o in f.settings)){f.cchannel.serverMessage('Unknown setting "'+o+'"');return}f.settings[o]=p;f.cchannel.serverMessage("Changed "+o+" setting","value: "+p)};b.connection=function(o){f[o.cmd]()};b.join=function(o){var p=o.args.split(" ");var p=p.toString()==""?[]:p;if(o.ns!=o.target){p.unshift(o.target)}if(p.toString()==""){return}for(index in p){f.join(p[index])}};b.pjoin=function(o){var p=o.args.split(" ");var p=p.toString()==""?[]:p;if(o.ns!=o.target){p.unshift(o.target)}if(p.toString()==""){return}for(index in p){f.join("@"+p[index])}};b.part=function(o){var p=o.args.split(" ");if(o.ns!=o.target){p.unshift(o.target)}if(p.toString()==""){f.part(o.ns);return}for(index in p){f.part(p[index])}};b.title=function(o){f.set(o.target,o.cmd,o.args)};b.chgpriv=function(p){var o=p.args.split(" ");f[p.cmd.toLowerCase()](p.target,o[0],o[1])
};b.ban=function(s,o){var q=s.args.split(" ");var p=q.shift();var r=s.cmd;if(r=="ban"&&q.length>0){o.kick(s.target,p,q.join(" "))}o[r](s.target,p)};b.action=function(o){f.action(o.target,o.args)};b.raw=function(o){f.send(o.args.replace(/\\n/gm,"\n"))};b.killk=function(s,o){var v=s.args.split(" ");var p=v.shift();var q=v.length>0?v.join(" "):null;if(s.cmd=="kick"){o.kick(s.target,p,q)}else{o.kill(p,q)}};b.say=function(o){if(f.channel(o.target).monitor){return}f.say(o.target,o.args)};b.npmsg=function(o){f.npmsg(o.target,o.args)};b.clear=function(q,o){if(q.args.length>0){var r=q.args.split(" ");for(var p in r){if(!r.hasOwnProperty(p)){continue}o.ui.channel(q.target).clear_user(r[p])}}else{o.ui.channel(q.target).clear()}};b.clearall=function(p,o){var r=null;if(p.args.length>0){var q=p.args.split(" ");r=function(u,v){for(var s in q){if(!q.hasOwnProperty(s)){continue}v.clear_user(q[s])}}}else{r=function(s,u){u.clear()}}o.ui.chatbook.each(r,true)};b.close=function(o){f.part(o.target);f.remove_ns(o.target)
};b.whois=function(p,o){o.whois(p.args.split(" ")[0])};b.admin=function(p,o){o.admin(p.target,p.args)};b.disconnect=function(p,o){o.disconnect()};b.gett=function(p,o){var q=p.cmd.indexOf("title")>-1?"title":"topic";o.ui.control.set_text("/"+q+" "+o.channel(p.target).info[q].content)};var m=function(r,o){if(r.p!="info"){return}var p=r.pkt.sub;var s=p.shift().arg;s.username=r.sns.substr(1);s.connections=[];var q={};while(p.length>0){q=p.shift().arg;q.channels=[];while(p.length>0){if(p[0].cmd!="ns"){break}q.channels.unshift(o.deform_ns(p.shift().param))}s.connections.push(q)}o.ui.chatbook.current.log_whois(s)};var l=function(p,o){if(p.ns.indexOf("login:")!=0){return}var q=p.sns.substr(1);o.ui.pager.notice({ref:"whois-"+q,heading:"Whois Failed",content:"Whois failed for "+q+".\nNo such user online."},false,5000)};var g=function(s,p){var x=p.channel(s.ns);var o=s.info.split("\n");var v="";var u=[];var q="";for(var r in o){if(!o.hasOwnProperty(r)){continue}v=o[r].split(" ");if(s.p=="privclass"){u.push([v.shift(),v.shift().split("=")[1],v.join(" ")])
}else{if(s.p=="users"){q=v.shift().split(":",1)[0];u.push([q,x.get_privclass_order(q),v.join(" ")])}}}p.ui.chatbook.current.log_pc(s.p=="privclass",u)};n()};wsc.defaults.Extension.Autojoin=function(a){var f=a.autojoin;a.ui.nav.add_button({icon:"chat",label:"",title:"Join your autojoin channels",href:"#autojoin-do",handler:function(){for(var h in a.autojoin.channel){if(!a.autojoin.channel.hasOwnProperty(h)){continue}a.join(a.autojoin.channel[h])}}});var g=function(){a.bind("cmd.autojoin",b);a.ui.on("settings.open",f.page)};f.page=function(l,n){var m=l.settings.page("Autojoin");var k="<ul>";var o={};o.ajon=a.autojoin.on;o.chan=a.autojoin.channel;if(a.autojoin.channel.length==0){k+="<li><i>No autojoin channels set</i></li></ul>"}else{for(var j in a.autojoin.channel){if(!a.autojoin.channel.hasOwnProperty(j)){continue}k+="<li>"+a.autojoin.channel[j]+"</li>"}k+="</ul>"}m.item("Checkbox",{ref:"eaj",title:"Autojoin",text:"Turn on autojoin to automatically join selected channels                    when you connect to the chat server.",items:[{value:"yes",title:"On",selected:o.ajon}],event:{change:function(p){if(p.target.value=="yes"){a.autojoin.on=p.target.checked
}},save:function(p){o.ajon=a.autojoin.on;a.config_save()},close:function(p){a.autojoin.on=o.ajon}}});var h=m.item("Items",{ref:"channelss",title:"Channels",text:"Add any channels you want to join automatically when you                    connect to the chat server.",items:a.autojoin.channel,prompt:{title:"Add Channel",label:"Channel:",},event:{up:function(p){var q=p.args.swap;a.autojoin.channel[q["this"].index]=q.that.item;a.autojoin.channel[q.that.index]=q["this"].item;h.options.items=a.autojoin.channel},down:function(p){var q=p.args.swap;a.autojoin.channel[q["this"].index]=q.that.item;a.autojoin.channel[q.that.index]=q["this"].item;h.options.items=a.autojoin.channel},add:function(r){var q=a.deform_ns(r.args.item).toLowerCase();var p=a.autojoin.channel.indexOf(q);if(p!=-1){return}a.autojoin.channel.push(q);h.options.items=a.autojoin.channel},remove:function(p){a.autojoin.channel.splice(p.args.index,1);h.options.items=a.autojoin.channel},save:function(p){o.chan=a.autojoin.channel;a.config_save()
},close:function(p){a.config_load()}}})};var b=function(o){var j=o.args.split(" ");if(!j){return}var m=j.shift().toLowerCase();var h=function(q){};var l=false;var p=a.channel(o.ns);switch(m){case"add":h=function(q){if(a.autojoin.channel.indexOf(q)==-1){l=true;a.autojoin.channel.push(q);p.server_message("Added "+q+" to your autojoin.")}else{p.server_message("Already have "+q+" on your autojoin.")}};break;case"rem":case"remove":h=function(r){var q=a.autojoin.channel.indexOf(r);if(q!=-1){l=true;a.autojoin.channel.splice(q,1);p.server_message("Removed "+r+" from your autojoin.")}else{p.server_message(r+" is not on your autojoin list.")}};break;case"on":p.server_message("Autojoin on.");if(!a.autojoin.on){l=true;a.autojoin.on=true}break;case"off":p.server_message("Autojoin off.");if(a.autojoin.on){l=true;a.autojoin.on=false}break}var n="";for(var k in j){if(!j.hasOwnProperty(k)){continue}n=a.deform_ns(j[k]).toLowerCase();h(n)}if(l){a.config_save()}};g()};wsc.defaults.Extension.Away=function(b){var g=b.storage.folder("away");
var f={on:false,reason:"",last:{},since:0,interval:60000,format:{setaway:"/me is away: {reason}",setback:"/me is back",away:"{user}: I've been away for {timesince}. Reason: {reason}"}};b.away=f;var m=function(){k();h();b.bind("cmd.setaway",a);b.bind("cmd.setback",l);b.ui.on("tabbed",j);b.ui.on("settings.open",f.page)};f.page=function(o,q){var n=function(u){u=replaceAll(u,"<","&lt;");u=replaceAll(u,">","&gt;");u=replaceAll(u,'"',"&quot;");return u};var r=function(u){u=replaceAll(u,"&lt;","<");u=replaceAll(u,"&gt;",">");u=replaceAll(u,"&quot;",'"');return u};var p=o.settings.page("Away");var s={};s.away=f.format.away;s.sa=f.format.setaway;s.sb=f.format.setback;s.intr=f.interval;p.item("Text",{ref:"intro",title:"Away Messages",text:"Use away messages when you are away from the chats.\n\n                    You can set yourself away using the                     <code>/setaway [reason]</code> command. When you get back,                    use <code>/setback</code>. While you are away, the client                    will automatically respond when people try to talk to you,                    telling them you're away.",});
p.item("Form",{ref:"msgs",title:"Messages",text:"Here you can set the messages displayed when you set                    yourself away or back. You can also change the away message                    format.",hint:"<b>{user}</b><br/>This is replaced with the username of the person trying to talk to you\n\n<b>{reason}</b>                    <br/>This is replaced by your reason for being away.\n\n<b>{timesince}</b>                    <br/>Use this to show how long you have been away for.",fields:[["Textfield",{ref:"away",label:"Away","default":n(s.away)}],["Textfield",{ref:"setaway",label:"Setaway","default":n(s.sa)}],["Textfield",{ref:"setback",label:"Setback","default":n(s.sb)}]],event:{save:function(u){f.format.away=r(u.data.away);f.format.setaway=r(u.data.setaway);f.format.setback=r(u.data.setback);h()}}});p.item("Form",{ref:"interval",title:"Message Interval",text:"Here you can set the amount of time to wait before                     displaying another away message. The interval is in seconds.",fields:[["Textfield",{ref:"interval",label:"Interval","default":(s.intr/1000).toString()}]],event:{save:function(u){f.interval=parseInt(u.data.interval)*1000;
h()}}})};var a=function(p,n){f.on=true;f.last={};f.since=new Date();f.reason=p.args;var q=n.say;var o=replaceAll(f.format.setaway,"{reason}",f.reason||"[silent away]");if(o.indexOf("/me ")==0){o=o.substr(4);q=n.action}n.each_channel(function(r){q.call(n,r,o)});n.ui.control.add_state({ref:"away",label:"Away, reason: <i>"+(f.reason||"[silent away]")+"</i>"})};var l=function(p,n){if(!f.on){return}f.on=false;var q=n.say;var o=f.format.setback;if(o.indexOf("/me ")==0){o=o.substr(4);q=n.action}n.each_channel(function(r){q.call(n,r,o)});n.ui.control.rem_state("away")};var j=function(q){if(!f.on){return}if(f.reason.length==0){return}if(q.user==b.settings.username){return}if(b.exclude.contains(q.ns)){return}var o=new Date();var p=q.sns.toLowerCase();if(p in f.last){if((o-f.last[p])<=f.interval){return}}var n=timeLengthString((o-f.since)/1000);var r=replaceAll(f.format.away,"{user}",q.user);r=replaceAll(r,"{timesince}",n);b.say(q.ns,replaceAll(r,"{reason}",f.reason));f.last[p]=o};var k=function(){f.format.setaway=g.get("setaway","/me is away: {reason}");
f.format.setback=g.get("setback","/me is back");f.format.away=g.get("away","{user}: I've been away for {timesince}. Reason: {reason}");f.interval=parseInt(g.get("interval",60000))};var h=function(){g.set("interval",f.interval);g.set("setaway",f.format.setaway);g.set("setback",f.format.setback);g.set("away",f.format.away)};m()};wsc.defaults.Extension.Ignore=function(f){var g={};var h=f.storage.folder("ignore");var a=h.folder("ignored");var m=function(){l();k();f.bind("cmd.ignore",b);f.bind("cmd.unignore",j);f.ui.on("settings.open",g.page)};g.page=function(o,q){var p=o.settings.page("Ignores");var r={};r.im=g.ignore;r.uim=g.unignore;r.usr=f.ui.umuted;p.item("Text",{ref:"intro",title:"Ignores",text:'Use <code>ignore</code> to ignore people.\n\n                    You can "ignore" other users of the chat server using the\n                    <code>/ignore</code> command. Ignoring a user hides their                    messages from you in the channel log.',});p.item("Form",{ref:"msgs",title:"Messages",text:"Here you can set the messages displayed when you ignore or                    unignore a user.\n\nThe text <code>{user}</code> is replaced                    with the name of the user your are ignoring or unignoring.",fields:[["Textfield",{ref:"ignore",label:"Ignore","default":r.im}],["Textfield",{ref:"unignore",label:"Unignore","default":r.uim}]],event:{save:function(s){g.ignore=s.data.ignore;
g.unignore=s.data.unignore;k()}}});var n=p.item("Items",{ref:"ignoreds",title:"Users",text:"This is the list of users that you have silenced.\n\nUse the                    commands <code>/ignore</code> and <code>/unignore</code>                    to edit the list.",items:r.usr,prompt:{title:"Add User",label:"User:",},event:{up:function(s){var u=s.args.swap;f.ui.umuted[u["this"].index]=u.that.item;f.ui.umuted[u.that.index]=u["this"].item;n.options.items=f.ui.umuted},down:function(s){var u=s.args.swap;f.ui.umuted[u["this"].index]=u.that.item;f.ui.umuted[u.that.index]=u["this"].item;n.options.items=f.ui.umuted},add:function(s){f.mute_user(s.args.item);n.options.items=f.ui.umuted},remove:function(s){f.unmute_user(s.args.item);n.options.items=f.ui.umuted},save:function(s){r.usr=f.ui.umuted;k()},close:function(s){l()}}})};var b=function(q){var s=q.args.split(" ");var n="";var r="";var p=false;for(var o in s){if(!s.hasOwnProperty(o)){continue}n=s[o];tmod=f.mute_user(n);if(!tmod){continue}p=tmod;
r=replaceAll(g.ignore,"{user}",n);if(r.indexOf("/me ")==0){r=r.substr(4);f.action(q.target,r)}else{f.say(q.target,r)}}if(p){k()}};var j=function(r){var v=r.args.split(" ");var o="";var u="";var n=-1;var q=false;var s=false;for(var p in v){if(!v.hasOwnProperty(p)){continue}o=v[p];s=f.unmute_user(o);if(!s){continue}q=s;u=replaceAll(g.unignore,"{user}",o);if(u.indexOf("/me ")==0){u=u.substr(4);f.action(r.target,u)}else{f.say(r.target,u)}}if(q){k()}};var l=function(){g.ignore=h.get("ignore","/me is ignoring {user} now");g.unignore=h.get("unignore","/me is not ignoring {user} anymore");g.count=parseInt(h.get("count",0));var o=null;for(var n in f.ui.umuted){if(!f.ui.umuted.hasOwnProperty(n)){continue}f.unmute_user(f.ui.umuted[n])}f.ui.umuted=[];if(g.count>0){o=null;for(var n=0;n<g.count;n++){f.mute_user(a.get(n,null))}}};var k=function(){h.set("ignore",g.ignore);h.set("unignore",g.unignore);for(var n=0;n<g.count;n++){a.remove(n)}if(f.ui.umuted.length==0){h.set("count",0)}else{var o=-1;for(var n in f.ui.umuted){if(!f.ui.umuted.hasOwnProperty(n)){continue
}o++;a.set(o.toString(),f.ui.umuted[n])}o++;g.count=o;h.set("count",o)}};m()};wsc.Client=function(b,f,g){this.mozilla=g;this.storage=new wsc.Storage;this.storage.ui=this.storage.folder("ui");this.storage.aj=this.storage.folder("autojoin");this.storage.aj.channel=this.storage.aj.folder("channel");this.fresh=true;this.attempts=0;this.connected=false;this.protocol=null;this.flow=null;this.ui=null;this.events=new EventEmitter();this.conn=null;this.channelo={};this.cchannel=null;this.cmds=[];this.settings={domain:"website.com",server:"ws://website.com/wsendpoint",symbol:"",username:"",userinfo:{},pk:"",monitor:["~Monitor",true],welcome:"Welcome to the wsc web client!",autojoin:"chat:channel",protocol:wsc.Protocol,mparser:wsc.MessageParser,flow:wsc.Flow,ui_object:Chatterbox.UI,extend:[wsc.defaults.Extension],client:"chatclient",clientver:"0.3",developer:false};this.autojoin={on:true,count:0,channel:[]};this.away={};var a=this;this.exclude=new StringSet();this.hidden=new StringSet();this.settings=Object.extend(this.settings,f);
