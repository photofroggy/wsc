...
a[k]({input:m,item:this,args:j})}}};Chatterbox.Settings.Item.Items.prototype.save=function(){this._fevent("save",{items:this.options.items||[]})};Chatterbox.template={};Chatterbox.render=function(j,l){var g=Chatterbox.template;var b=j.split(".");var k={};var a=null;for(ind in b){part=b[ind];if(!g.hasOwnProperty(part)){return""}g=g[part]}if(g.hasOwnProperty("frame")){a=g;k=g.render||{};g=g.frame;if(a.hasOwnProperty("pre")){if(typeof a.pre=="function"){g=a.pre(g,l)}else{for(i in a.pre){g=a.pre[i](g,l)}}}}for(key in l){g=replaceAll(g,"{"+key+"}",(k[key]||Chatterbox.template.render_stub)(l[key]||""))}if(a!=null){if(a.hasOwnProperty("post")){if(typeof a.post=="function"){g=a.post(g,l)}else{for(i in a.post){g=a.post[i](g,l)}}}}return g};Chatterbox.template.render_stub=function(a){return a};Chatterbox.template.clean=function(a){return function(b,g){for(i in a){b=replaceAll(b,"{"+a[i]+"}","")}return b}};Chatterbox.template.ui='<nav class="tabs"><ul id="chattabs" class="tabs"></ul>        <ul id="tabnav">            <li><a href="#left" class="button iconic arrow_left"></a></li>            <li><a href="#right" class="button iconic arrow_right"></a></li>            <li><a href="#settings" title="Change client settings" class="button iconic cog" id="settings-button"></a></li>        </ul>        </nav>        <div class="chatbook"></div>';
...
Chatterbox.template.settings.item.form.field.text.pre=Chatterbox.template.settings.item.hint.prep;Chatterbox.template.settings.item.form.field.text.post=Chatterbox.template.clean(["title","text"]);Chatterbox.template.settings.item.form.field.text.render={title:Chatterbox.template.settings.krender.title,text:Chatterbox.template.settings.krender.text};Chatterbox.template.settings.item.form.field.text.frame="{title}<p>                                        {text}                                    </p>";wsc.dAmn={};wsc.dAmn.VERSION="0.3.10";wsc.dAmn.STATE="alpha";wsc.dAmn.Emotes=function(a,g,b){b.emotes.page=null;b.emotes.fint=null;b.emotes.configure_page=function(j,l){var k=j.settings.page("Emotes");b.emotes.page=k;var m={};m.on=b.emotes.on;k.item("Form",{ref:"switch",title:"Enable Emotes",text:"Here you can turn custom emotes on and off.",fields:[["Checkbox",{ref:"enabled",items:[{value:"yes",title:"On",selected:m.on}]}]],event:{change:function(p){b.emotes.on=(p.data.enabled.indexOf("yes")!=-1);
if(b.emotes.on){b.emotes.fetch();return}if(b.emotes.fint===null){return}clearTimeout(b.emotes.fint);b.emotes.fint=null},save:function(p){m.on=b.emotes.on;b.save()},close:function(p){b.emotes.on=m.on;b.load();b.emotes.page=null}}})};a.ui.on("settings.open.ran",b.emotes.configure_page);b.emotes.fetch=function(){jQuery.getJSON("http://www.thezikes.org/publicemotes.php?format=jsonp&jsoncallback=?&"+(new Date()).getDay(),function(j){b.emotes.emote=j});b.emotes.fint=setTimeout(b.emotes.fetch,3600000)};b.emotes.swap=function(l){if(!b.emotes.on){return}var j=-1;for(var k in b.emotes.emote){if(!b.emotes.emote.hasOwnProperty(k)){continue}j=l.input.indexOf(k);if(j==-1){continue}l.input=replaceAll(l.input,k,":thumb"+b.emotes.emote[k]["devid"]+":")}if(l.input.indexOf(":B")==-1){return}l.input=replaceAll(l.input,":B",":bucktooth:")};a.bind("send.msg.before",b.emotes.swap);a.bind("send.action.before",b.emotes.swap);a.bind("send.kick.before",b.emotes.swap);a.bind("send.set.before",b.emotes.swap);if(!b.emotes.on){return
