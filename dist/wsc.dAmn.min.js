...
}T.connections.push(R)}P.cchannel.log_whois(T)};var z=function(Q,P){if(Q.ns.indexOf("login:")!=0){return}P.cchannel.server_message("Whois failed for "+(Q.sns.substr(1)),"not online")};var M=function(T,Q){var W=Q.channel(T.ns);var P=T.info.split("\n");var V="";var U=[];var R="";for(var S in P){if(!P.hasOwnProperty(S)){continue}V=P[S].split(" ");if(T.p=="privclass"){U.push([V.shift(),V.shift().split("=")[1],V.join(" ")])}else{if(T.p=="users"){R=V.shift().split(":",1)[0];U.push([R,W.get_privclass_order(R),V.join(" ")])}}}W.log_pc(T.p=="privclass",U)};E();wsc.defaults.Extension.Autojoin(p);wsc.defaults.Extension.Away(p);wsc.defaults.Extension.Ignore(p)};wsc.defaults.Extension.Autojoin=function(a){var g=a.autojoin;var j=function(){a.bind("cmd.autojoin",b);a.ui.on("settings.open",g.page)};g.page=function(p,r){var q=p.settings.page("Autojoin");var m="<ul>";var s={};s.ajon=a.autojoin.on;s.chan=a.autojoin.channel;if(a.autojoin.channel.length==0){m+="<li><i>No autojoin channels set</i></li></ul>"
...
Chatterbox.template.settings.item.form.field.text.pre=Chatterbox.template.settings.item.hint.prep;Chatterbox.template.settings.item.form.field.text.post=Chatterbox.template.clean(["title","text"]);Chatterbox.template.settings.item.form.field.text.render={title:Chatterbox.template.settings.krender.title,text:Chatterbox.template.settings.krender.text};Chatterbox.template.settings.item.form.field.text.frame="{title}<p>                                        {text}                                    </p>";wsc.dAmn={};wsc.dAmn.VERSION="0.2.9";wsc.dAmn.STATE="alpha";wsc.dAmn.Emotes=function(a,g,b){b.emotes.page=function(k,m){var l=k.settings.page("Emotes");var p={};p.on=b.emotes.on;p.es=b.emotes.emote;l.item("Form",{ref:"switch",title:"Enable Emotes",text:"Here you can turn custom emotes on and off.",fields:[["Checkbox",{ref:"enabled",label:"On",items:[{value:"yes",title:"On",selected:p.on}]}]],event:{change:function(q){console.log(q)},save:function(q){b.save()}}});var j=l.item("Items",{ref:"emotes",title:"Emotes",text:"This is the list of emoticons you can use.",items:p.es})
};a.ui.on("settings.open.ran",b.emotes.page);if(!b.on){return}};wsc.dAmn.Extension=function(a){a.settings.client="dAmnClient";a.settings.clientver="0.3";a.settings.domain="deviantart.com";a.settings.agent+=" wsc/dAmn/"+wsc.dAmn.VERSION;var g=a.storage.folder("dAmn");g.emotes=g.folder("emotes");var b={emotes:{on:false,emote:[]}};b.save=function(){g.emotes.set("on",b.emotes.on)};b.load=function(){b.emotes.on=(g.emotes.get("on","false")=="true")};b.save();b.load();a.protocol.extend_maps({dAmnServer:["version"]});a.protocol.extend_messages({dAmnServer:['<span class="servermsg">** Connected to dAmnServer {version} *</span>',false,true]});a.protocol.mparser=new wsc.dAmn.TablumpParser;a.flow.dAmnServer=a.flow.chatserver;a.exclude.push("#devart");a.ui.on("userinfo.before",function(j,k){j.user.avatar=wsc.dAmn.avatar.link(j.user.name,j.user.member.usericon);if(j.user.member.realname){j.user.info.push(j.user.member.realname)}if(j.user.member.typename){j.user.info.push(j.user.member.typename)}});a.ui.on("log_whois.before",function(j,k){j.avatar=wsc.dAmn.avatar.link(j.raw.username,j.raw.usericon);
