...
};wsc.Client.prototype.kick=function(b,a,f){var g=this;this.cascade("send.kick",function(h){g.send(wsc_packetstr("kick",g.format_ns(h.ns),{u:h.user},h.input||null))},{input:f||"",ns:b,user:a})};wsc.Client.prototype.kill=function(a,b){this.send(wsc_packetstr("kill","login:"+a,{},b||null))};wsc.Client.prototype.admin=function(a,b){this.send(wsc_packetstr("send",this.format_ns(a),{},wsc_packetstr("admin","",{},b)))};wsc.Client.prototype.property=function(a,b){this.send(wsc_packetstr("get",this.format_ns(a),{p:b}))};wsc.Client.prototype.set=function(a,f,b){var g=this;this.cascade("send.set",function(h){g.send(wsc_packetstr("set",g.format_ns(h.ns),{p:h.property},h.input))},{input:b,ns:a,property:f})};wsc.Client.prototype.whois=function(a){this.send(wsc_packetstr("get","login:"+a,{p:"info"}))};wsc.Client.prototype.disconnect=function(){this.send(wsc_packetstr("disconnect"))};wsc.dAmn={};wsc.dAmn.VERSION="0.10.30";wsc.dAmn.STATE="alpha";wsc.dAmn.BDS=function(f,k,g){var h={};var a={};g.bds={version:"0.4",mns:"chat:datashare",gate:"chat:dsgateway",channel:(new StringSet()),provides:["BOTCHECK","CLINK"]};
f.bds=g.bds;g.bds.channel.add(g.bds.mns);g.bds.channel.add(g.bds.gate);var l=function(){f.hidden.add(g.bds.mns);f.exclude.add(g.bds.mns);f.hidden.add(g.bds.gate);f.exclude.add(g.bds.gate);f.bind("pkt.login",m);f.bind("pkt.recv_msg",b);f.bind("pkt.join",j.join);f.bind("BDS.BOTCHECK.DIRECT",j.botcheck);f.bind("BDS.BOTCHECK.ALL",j.botcheck);f.bind("BDS.BOTCHECK.OK",j.checkresp);f.bind("BDS.BOTCHECK.DENIED",j.checkresp);f.bind("CDS.LINK.REQUEST",j.clreq);f.bind("CDS.LINK.REJECT",j.clrj);f.bind("CDS.LINK.ACK",j.clra);f.bind("pkt.recv_join",j.pcrj);f.bind("pkt.recv_part",j.pcrp);f.bind("pkt.property",j.pcp);f.bind("closed",j.closed);f.middle("chan.recv_msg",function(o,n){j.hfilter(o,n)})};var m=function(n){if(n.pkt.arg.e!="ok"){return}f.join(g.bds.gate)};var b=function(q){if(q.user.toLowerCase()==f.settings.username.toLowerCase()){return}if(!g.bds.channel.contains(q.ns)){return}var p={ns:q.ns,sns:q.sns,message:q.message,user:q.user,name:"",payload:"",head:[null,null,null],params:[]};var s=q.message.split(":");
var o=[null,null,null];var r=null;for(var n in o){o[n]=s.shift()||null;if(o[n]==null){return}}r=s.join(":");p.name=o.join(".");p.payload=r;p.head=o;p.param=r.split(",");f.trigger(o[0],p);f.trigger(o[0]+"."+o[1],p);f.trigger(p.name,p)};var j={filter:function(o,n){if(f.settings.developer){n(o);return}if(o.ns[0]!="@"){n(o);return}var p=o.message.match(/<span class="cmsg u-([^"]+)">(.*)<\/span>/);if(!p){n(o);return}if(p[2].match(/^([A-Z0-9-_]+):([A-Z0-9-_]+):([A-Z0-9-_]+)(:.*|)$/)){return}n(o)},hfilter:function(o,n){if(f.settings.developer){n(o);return}if(o.sns[0]!="@"){n(o);return}if(o.message.match(/^([A-Z0-9-_]+):([A-Z0-9-_]+):([A-Z0-9-_]+)(:.*|)$/)){return}n(o)},closed:function(n){f.remove_ns(g.bds.mns);f.remove_ns(g.bds.gate)},join:function(n){if(n.ns.toLowerCase()!=g.bds.mns){return}if(n.pkt.arg.e!="ok"){return}},botcheck:function(o){if(o.head[2]!="ALL"&&o.payload!=f.settings.username){return}if(o.ns.toLowerCase()==g.bds.gate&&f.channel(g.bds.mns)!=null){return}var n=wsc.VERSION+"/0.0.0/"+wsc.dAmn.VERSION+"/"+g.bds.version;
var p=CryptoJS.MD5(("wsc.dAmn"+n+f.settings.username+o.user).toLowerCase());f.npmsg(o.ns,"BDS:BOTCHECK:CLIENT:"+o.user+",wsc.dAmn,"+n+","+p)},checkresp:function(n){if(n.ns.toLowerCase()!=g.bds.gate){return}if(n.param[0].toLowerCase()!=f.settings.username.toLowerCase()){return}if(f.channel(n.ns).info.members[f.settings.username].pc=="Visitors"){f.part(n.ns)}if(n.head[2]=="OK"){if(f.channel(g.bds.mns)==null){f.join(g.bds.mns)}return}f.ui.pager.notice({ref:"botcheckdenied",heading:"BDS Error",content:"Denied entry to backend channel.\nReason provided: <code>"+n.param.slice(1).join(",")+"</code>"})},clreq:function(r){if(r.payload.toLowerCase()!=f.settings.username.toLowerCase()){return}if(f.ui.umuted.indexOf(r.user.toLowerCase())!=-1){f.npmsg(r.ns,"CDS:LINK:REJECT:"+r.user+",You have been blocked");return}if(f.away.on){var p=new Date();var q=r.sns.toLowerCase();var n=timeLengthString((p-f.away.since)/1000);var s=replaceAll(f.away.format.away,"{user}",r.user);s=replaceAll(s,"{timesince}",n);
f.npmsg(r.ns,"CDS:LINK:REJECT:"+r.user+",Away; "+f.away.reason);return}f.npmsg(r.ns,"CDS:LINK:ACK:"+r.user);var o=f.ui.pager.notice({ref:"clink-"+r.user,icon:'<img src="'+wsc.dAmn.avatar.src(r.user,f.channel(r.ns).info.members[r.user].usericon)+'" />',heading:"Chat "+r.user,content:r.user+" wants to talk in private.\nType <code>/chat "+r.user+"</code> to talk to them, or use the buttons below.",buttons:{accept:{ref:"accept",target:"accept",label:"Accept",title:"Join the private chat",click:function(){f.ui.pager.remove_notice(o);f.join("@"+r.user);return false}},reject:{ref:"reject",target:"reject",label:"Reject",title:"Reject the private chat",click:function(){f.npmsg(r.ns,"CDS:LINK:REJECT:"+r.user);f.ui.pager.remove_notice(o);return false}}}},true);a[r.user]=o;o.onclose=function(){f.npmsg(r.ns,"CDS:LINK:REJECT:"+r.user)}},clrj:function(q){var n=q.user.toLowerCase();var r=q.payload.split(",");var o=r.shift();r=r.join(",");if(o.toLowerCase()!=f.settings.username.toLowerCase()){return}if(!(n in h)){return
}clearTimeout(h[n]);f.channel("@"+n).server_message("Chat request rejected",r)},clra:function(q){var n=q.user.toLowerCase();var r=q.payload.split(",");var o=r.shift();if(o.toLowerCase()!=f.settings.username.toLowerCase()){return}clearTimeout(h[n])},pcp:function(p){if(p.ns.indexOf("pchat")!=0){return}if(f.bds.channel.contains(p.ns)){return}if(p.pkt.arg.p!="members"){return}if(f.channel(p.ns).get_usernames().length==2){f.bds.channel.add(p.ns);return}var n=p.sns.substr(1);var o=p.ns;f.ui.channel(o).server_message(n+" has not yet joined","Sending them a notice...");f.npmsg("chat:datashare","CDS:LINK:REQUEST:"+n);h[n.toLowerCase()]=setTimeout(function(){try{f.ui.channel(o).server_message("Notification failed",n+" does not seem to be using a client that supports pchat notices.")}catch(q){}},10000)},pcrj:function(o){if(o.sns[0]!="@"){return}try{clearTimeout(h[o.user.toLowerCase()])}catch(n){}f.bds.channel.add(o.ns);if(!(o.user in a)){return}f.ui.pager.remove_notice(a[o.user])},pcrp:function(n){if(n.sns[0]!="@"){return
}f.bds.channel.remove(n.ns)}};l()};wsc.dAmn.Colours=function(a,f,b){b.colours.page=null;b.colours.configure_page=function(g,j){var h=g.settings.page("Colours");b.colours.page=h;var k={};k.on=b.colours.on;k.send=b.colours.send;k.msg=b.colours.msg;k.user=b.colours.user;h.item("Form",{ref:"switch",title:"Enable Chromacity Colours",text:"Here you can turn chromacity colours on and off.\n\nYou                    can choose to parse other people's colours, and send your                    own colours.",fields:[["Checkbox",{ref:"enabled",items:[{value:"parse",title:"Parse colours",selected:k.on},{value:"send",title:"Send colours",selected:k.send}]}]],event:{change:function(l){b.colours.on=(l.data.enabled.indexOf("parse")!=-1);b.colours.send=(l.data.enabled.indexOf("send")!=-1)},save:function(l){k.on=b.colours.on;k.send=b.colours.send},close:function(l){b.colours.on=k.on;b.colours.send=k.send}}});h.item("Form",{ref:"colours",title:"Custom Colours",text:"Here you can set your own username colour and message colour.",fields:[["Colour",{ref:"usr",label:"Username Colour","default":"#"+k.user}],["Colour",{ref:"msg",label:"Message Colour","default":"#"+k.msg}]],event:{change:function(l){b.colours.user=l.data.usr.substr(1).toUpperCase();
...
wsc.dAmn.Emotes.Page.prototype.refresh=function(){if(!this.picker.settings.emotes.on){return}Chatterbox.Popup.ItemPicker.Page.prototype.refresh.call(this);var a=this;a.view.find("span.thumb img").error(function(){var f=a.view.find(this);var b=f.parent().parent();var g=a.picker.settings.emotes.repair(b.find(".value").html());delete a.picker.settings.emotes.emote[g];b.remove();return false});a.view.find("span.thumb img").load(function(){var j=a.view.find(this);var b=j.width();var g=j.height();var k,f;if(b/g>1){k=parseInt((g*100)/b);f=100}else{f=parseInt((b*100)/g);k=100}if(f>b||k>g){f=b;k=g}j.css({height:k,width:f});j.parent().css({"float":"right",display:"block"})})};wsc.dAmn.Extension=function(a,f){a.settings.client="dAmnClient";a.settings.clientver="0.3";a.settings.domain="deviantart.com";a.settings.agent+=" wsc/dAmn/"+wsc.dAmn.VERSION;var g=a.storage.folder("dAmn");g.bds=g.folder("bds");g.emotes=g.folder("emotes");g.colours=g.folder("colours");var b={emotes:{on:false,emote:[]},colours:{on:false,send:false,msg:"000000",user:"000000"}};
b.save=function(){g.emotes.set("on",b.emotes.on);g.colours.set("on",b.colours.on);g.colours.set("send",b.colours.send);g.colours.set("msg",b.colours.msg);g.colours.set("user",b.colours.user)};b.load=function(){b.emotes.on=(g.emotes.get("on","false")=="true");b.colours.on=(g.colours.get("on","false")=="true");b.colours.send=(g.colours.get("send","false")=="true");b.colours.msg=g.colours.get("msg","");b.colours.user=g.colours.get("user","")};b.load();b.save();a.protocol.extend_maps({dAmnServer:["version"]});a.protocol.mparser=new wsc.dAmn.TablumpParser;a.flow.dAmnServer=a.flow.chatserver;a.exclude.add("chat:devart");a.exclude.add("chat:damnidlers");wsc.dAmn.BDS(a,g.bds,b);wsc.dAmn.BDS.Link(a,g.bds,b);wsc.dAmn.BDS.Peer(a,g.bds,b)};wsc.dAmn.avatar={};wsc.dAmn.avatar.ext=["gif","gif","jpg","png"];wsc.dAmn.avatar.link=function(a,b){var f=wsc.dAmn.avatar.src(a,b);return'<a target="_blank" title=":icon'+a+':" href="http://'+a+'.deviantart.com/"><img class="avatar"            alt=":icon'+a+':" src="'+f+'" height="50" /></a>'
