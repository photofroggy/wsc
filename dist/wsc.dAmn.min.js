...
}j.sound.bank[k]=l;l.load();j.sound[k]=function(){j.sound.play(j.sound.bank[k])};return true},play:function(k){k.pause();k.currentTime=0;k.play()},toggle:function(l){for(var k in j.sound.bank){if(!j.sound.bank.hasOwnProperty(k)){continue}j.sound.bank[k].muted=l}},mute:function(){j.sound.toggle(true)},unmute:function(){j.sound.toggle(false)},};this.settings=b.extend(this.settings,f);b.append('<div class="wsc '+this.settings.theme+'"></div>');this.mw=new wsc.Middleware();this.view=b.find(".wsc");this.mns=this.format_ns(this.settings.monitor[0]);this.lun=this.settings.username.toLowerCase();this.monitoro=null;this.swidth=(function(){if($.browser.msie){var m=$('<textarea cols="10" rows="2"></textarea>').css({position:"absolute",top:-1000,left:-1000}).appendTo("body"),l=$('<textarea cols="10" rows="2" style="overflow: hidden;"></textarea>').css({position:"absolute",top:-1000,left:-1000}).appendTo("body");scrollbarWidth=m.width()-l.width();m.add(l).remove()}else{var k=$("<div />").css({width:100,height:100,overflow:"auto",position:"absolute",top:-1000,left:-1000}).prependTo("body").append("<div />").find("div").css({width:"100%",height:200});
