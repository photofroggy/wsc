...
return false};Chatterbox.Control.prototype.keypress=function(f){var b=f.which||f.keyCode;var g=this.tab.hit;var a=false;switch(b){case 13:if(!this.multiline()){this.submit(f)}else{if(f.shiftKey){this.submit(f)}else{a=true}}break;case 38:if(!this.multiline()){this.scroll_history(true);break}a=true;break;case 40:if(!this.multiline()){this.scroll_history(false);break}a=true;break;case 9:if(f.shiftKey){this.manager.channel_right()}else{this.tab_item(f);g=false}break;case 219:if(f.ctrlKey){this.manager.channel_left()}else{a=true}break;case 221:if(f.ctrlKey){this.manager.channel_right()}else{a=true}break;case 84:if(f.ctrlKey&&f.altKey){this.manager.nav.toggle_sidebar()}else{a=true}break;default:a=true;break}if(g){this.end_tab(f)}return a};Chatterbox.Control.prototype.handle=function(b,h){if(h==""){return}if(!this.manager.chatbook.current){return}var l=false;if(h[0]!="/"){l=true}h=(b.shiftKey?"/npmsg ":(h[0]=="/"?"":"/say "))+h;h=h.slice(1);var m=h.split(" ");var f=m.shift().toLowerCase();var k=this.manager.chatbook.current.namespace;
