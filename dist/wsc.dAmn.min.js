...
wsc.Flow.prototype.recv_privchg=function(b,a){a.channel(b.ns).recv_privchg(b)};wsc.Flow.prototype.recv_kicked=function(b,a){a.channel(b.ns).recv_kicked(b)};wsc.defaults.Extension=function(p){var C=function(){p.bind("cmd.set",a);p.bind("cmd.connect",g);p.bind("cmd.join",H);p.bind("cmd.part",K);p.bind("cmd.say",r);p.bind("cmd.npmsg",L);p.bind("cmd.me",u);p.bind("cmd.promote",A);p.bind("cmd.demote",A);p.bind("cmd.ban",m);p.bind("cmd.unban",m);p.bind("cmd.kick",j);p.bind("cmd.whois",D);p.bind("cmd.title",k);p.bind("cmd.topic",k);p.bind("cmd.admin",B);p.bind("cmd.disconnect",g);p.bind("cmd.kill",j);p.bind("cmd.raw",I);p.bind("cmd.clear",F);p.bind("cmd.clearall",E);p.bind("pkt.property",l);p.bind("pkt.recv_admin_show",J);p.bind("pkt.recv_admin_showverbose",J);p.bind("pkt.get",v);p.bind("cmd.gettitle",s);p.bind("cmd.gettopic",s);p.bind("cmd.theme",b);p.ui.on("settings.open",q);p.ui.on("settings.open.ran",G)};var q=function(O,N){page=O.settings.page("Main");var P={};P.theme=replaceAll(p.ui.settings.theme,"wsct_","");
P.clock=p.ui.clock();P.tc=p.ui.nav.closer();P.username=p.settings.username;P.pk=p.settings.pk;var M=[];for(i in p.ui.settings.themes){name=replaceAll(p.ui.settings.themes[i],"wsct_","");M.push({value:name,title:name,selected:P.theme==name})}page.item("Text",{ref:"intro",title:"Main",text:"Use this window to view and change your settings.\n\nCheck                    the different pages to see what settings can be changed.",});page.item("Form",{ref:"login",title:"Login",text:"Here you can change the username and token used to                    log into the chat server.",fields:[["Textfield",{ref:"username",label:"Username","default":P.username}],["Textfield",{ref:"token",label:"Token","default":P.pk}]],event:{save:function(Q){p.settings.username=Q.data.username;p.settings.pk=Q.data.token}}});page.item("Form",{ref:"ui",title:"UI",hint:"<b>Timestamp</b><br/>Choose between a 24 hour clock and                    a 12 hour clock.\n\n<b>Theme</b><br/>Change the look of the                    client.\n\n<b>Close Buttons</b><br/>Turn tab close buttons on/off.",fields:[["Dropdown",{ref:"theme",label:"Theme",items:M}],["Dropdown",{ref:"clock",label:"Timestamp Format",items:[{value:"24",title:"24 hour",selected:P.clock},{value:"12",title:"12 hour",selected:!P.clock}]}],["Check",{ref:"tabclose",label:"Close Buttons",items:[{value:"yes",title:"On",selected:P.tc}]}],],event:{change:function(Q){p.ui.clock(Q.data.clock=="24");
p.ui.theme(Q.data.theme);p.ui.nav.closer(Q.data.tabclose.indexOf("yes")>-1)},save:function(Q){P.clock=Q.data.clock=="24";P.theme=Q.data.theme;P.tc=Q.data.tabclose.indexOf("yes")>-1},close:function(Q){p.ui.clock(P.clock);p.ui.theme(P.theme);p.ui.nav.closer(P.tc)}}});page.item("Text",{ref:"debug",wclass:"faint",title:"Debug Information",text:"Chat Agent: <code>"+p.settings.agent+"</code>\n\nUser                    Agent: <code>"+navigator.userAgent+"</code>"})};var G=function(O,N){var M=O.settings.page("About",true);M.item("Text",{ref:"about-wsc",title:"Wsc",text:'Currently using <a href="http://github.com/photofroggy/wsc/">wsc</a>                    version '+wsc.VERSION+" "+wsc.STATE+'.\n\nWsc                    works using HTML5, javascript, and CSS3. WebSocket is used for the connection                    where possible. The source code for this client is pretty huge.\n\nWsc was created                    by ~<a href="http://photofroggy.deviantart.com/">photofroggy</a>'})};var b=function(N,M){M.ui.theme(N.args.split(" ").shift())
};var a=function(O){var N=O.args.split(" ");var M=N.shift().toLowerCase();var N=N.join(" ");if(N.length==0){p.cchannel.serverMessage("Could not set "+M,"No data supplied");return}if(!(M in p.settings)){p.cchannel.serverMessage('Unknown setting "'+M+'"');return}p.settings[M]=N;p.cchannel.serverMessage("Changed "+M+" setting","value: "+N);p.control.setLabel()};var g=function(M){p[M.cmd]()};var H=function(M){var N=M.args.split(" ");var N=N.toString()==""?[]:N;if(M.ns!=M.target){N.unshift(M.target)}if(N.toString()==""){return}for(index in N){p.join(N[index])}};var K=function(M){var N=M.args.split(" ");if(M.ns!=M.target){N.unshift(M.target)}if(N.toString()==""){p.part(M.ns);return}for(index in N){p.part(N[index])}};var k=function(M){p.set(M.target,M.cmd,M.args)};var A=function(N){var M=N.args.split(" ");p[N.cmd.toLowerCase()](N.target,M[0],M[1])};var m=function(Q,M){var O=Q.args.split(" ");var N=O.shift();var P=Q.cmd;if(P=="ban"&&O.length>0){M.kick(Q.target,N,O.join(" "))}M[P](Q.target,N)};var u=function(M){p.action(M.target,M.args)
};var I=function(M){p.send(M.args.replace(/\\n/gm,"\n"))};var j=function(P,M){var Q=P.args.split(" ");var N=Q.shift();var O=Q.length>0?Q.join(" "):null;if(P.cmd=="kick"){M.kick(P.target,N,O)}else{M.kill(N,O)}};var r=function(M){p.say(M.target,M.args)};var L=function(M){p.npmsg(M.target,M.args)};var F=function(N,M){M.cchannel.clear()};var E=function(N,M){for(c in M.channelo){M.channelo[c].clear()}};var D=function(N,M){M.whois(N.args.split(" ")[0])};var B=function(N,M){M.admin(N.target,N.args)};var z=function(N,M){M.disconnect()};var s=function(N,M){var O=N.cmd.indexOf("title")>-1?"title":"topic";M.control.ui.set_text("/"+O+" "+M.channel(N.target).info[O].content)};var l=function(N,M){if(N.p!="info"){return}subs=N.pkt.sub;data=subs.shift().arg;data.username=N.sns.substr(1);data.connections=[];while(subs.length>0){conn=subs.shift().arg;conn.channels=[];while(subs.length>0){if(subs[0].cmd!="ns"){break}conn.channels.unshift(M.deform_ns(subs.shift().param))}data.connections.push(conn)}M.cchannel.log_whois(data)
};var v=function(N,M){if(N.ns.indexOf("login:")!=0){return}M.cchannel.server_message("Whois failed for "+(N.sns.substr(1)),"not online")};var J=function(Q,N){var T=N.channel(Q.ns);var M=Q.info.split("\n");var S="";var R=[];var O="";for(var P in M){if(!M.hasOwnProperty(P)){continue}S=M[P].split(" ");if(Q.p=="privclass"){R.push([S.shift(),S.shift().split("=")[1],S.join(" ")])}else{if(Q.p=="users"){O=S.shift().split(":",1)[0];R.push([O,T.get_privclass_order(O),S.join(" ")])}}}T.log_pc(Q.p=="privclass",R)};C()};wsc.Client=function(a,g,j){this.mozilla=j;this.fresh=true;this.attempts=0;this.connected=false;this.protocol=null;this.flow=null;this.ui=null;this.events=new EventEmitter();this.conn=null;this.channelo={};this.cchannel=null;this.cmds=[];this.settings={domain:"website.com",server:"ws://website.com/wsendpoint",symbol:"",username:"",userinfo:{},pk:"",monitor:["~Monitor",true],welcome:"Welcome to the wsc web client!",autojoin:"chat:channel",control:wsc.Control,protocol:wsc.Protocol,tablumps:wsc.Tablumps,flow:wsc.Flow,ui:Chatterbox.UI,extend:[wsc.defaults.Extension],client:"chatclient",clientver:"0.3",theme:wsc.defaults.theme,themes:wsc.defaults.themes,};
