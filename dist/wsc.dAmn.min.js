...
a.attempts=0};wsc.Flow.prototype.close=function(b,f){var a={name:"closed",pkt:new wsc.Packet("connection closed\n\n"),reason:"",evt:f};b.trigger("closed",a);if(b.connected){b.ui.server_message("Connection closed");b.connected=false;if(b.conn instanceof wsc.SocketIO){b.ui.server_message("At the moment there is a problem with reconnecting under socket.io.");b.ui.server_message("Refresh the page to connect.");return}}else{b.ui.server_message("Connection failed")}a.name="quit";if(b.attempts>2){b.ui.server_message("Can't connect. Try again later.");b.attempts=0;b.trigger("quit",a);return}if(f.cause){if(f.cause.hasOwnProperty("name")&&f.cause.name=="login"){b.trigger("quit",a);return}}b.ui.server_message("Connecting in 2 seconds");setTimeout(function(){b.connect()},2000)};wsc.Flow.prototype.message=function(a,g){var b=new wsc.Packet(unescape(replaceAll(g.data,"+"," ")));if(b==null){return}var f=this.protocol.parse(b);if(f.ns==null){f.ns=a.mns}f.sns=a.deform_ns(f.ns);this.handle(a,f);a.trigger("pkt",f);
a.trigger("pkt."+f.name,f)};wsc.Flow.prototype.handle=function(a,f){try{this[f.name](f,a)}catch(b){}};wsc.Flow.prototype.ping=function(b,a){a.pong()};wsc.Flow.prototype.chatserver=function(b,a){a.login()};wsc.Flow.prototype.login=function(f,a){if(f.pkt.arg.e=="ok"){var h=f.pkt.sub[0];a.settings.username=f.pkt.param;a.settings.symbol=h.arg.symbol;a.settings.userinfo=h.arg;var b=function(){a.join(a.settings.autojoin);if(a.autojoin.on){for(var j in a.autojoin.channel){if(!a.autojoin.channel.hasOwnProperty(j)){continue}a.join(a.autojoin.channel[j])}}};if(a.fresh){b()}else{var g=false;for(key in a.channelo){if(a.channelo[key].namespace[0]!="~"){a.join(key);g=true}}if(!g){b()}}}else{a.close(f)}if(a.fresh){a.fresh=false}};wsc.Flow.prototype.join=function(f,a){if(f.pkt.arg["e"]=="ok"){var b=a.deform_ns(f.pkt.param);a.create_ns(b,a.hidden.contains(f.pkt.param));a.ui.channel(b).server_message("You have joined "+b)}else{a.ui.chatbook.current.server_message("Failed to join "+a.deform_ns(f.pkt.param),f.pkt.arg["e"])
...
}};b.say=function(o){if(f.channel(o.target).monitor){return}f.say(o.target,o.args)};b.npmsg=function(o){f.npmsg(o.target,o.args)};b.clear=function(q,o){if(q.args.length>0){var r=q.args.split(" ");for(var p in r){if(!r.hasOwnProperty(p)){continue}o.ui.channel(q.target).clear_user(r[p])}}else{o.ui.channel(q.target).clear()}};b.clearall=function(p,o){var r=null;if(p.args.length>0){var q=p.args.split(" ");r=function(u,v){for(var s in q){if(!q.hasOwnProperty(s)){continue}v.clear_user(q[s])}}}else{r=function(s,u){u.clear()}}o.ui.chatbook.each(r,true)};b.close=function(o){f.part(o.target);f.remove_ns(o.target)};b.whois=function(p,o){o.whois(p.args.split(" ")[0])};b.admin=function(p,o){o.admin(p.target,p.args)};b.disconnect=function(p,o){o.disconnect()};b.gett=function(p,o){var q=p.cmd.indexOf("title")>-1?"title":"topic";o.ui.control.set_text("/"+q+" "+o.channel(p.target).info[q].content)};var m=function(r,o){if(r.p!="info"){return}var p=r.pkt.sub;var s=p.shift().arg;s.username=r.sns.substr(1);
s.connections=[];var q={};while(p.length>0){q=p.shift().arg;q.channels=[];while(p.length>0){if(p[0].cmd!="ns"){break}q.channels.unshift(o.deform_ns(p.shift().param))}s.connections.push(q)}o.ui.chatbook.current.log_whois(s)};var l=function(p,o){if(p.ns.indexOf("login:")!=0){return}var q=p.sns.substr(1);o.ui.pager.notice({ref:"whois-"+q,heading:"Whois Failed",content:"Whois failed for "+q+".\nNo such user online."},false,5000)};var g=function(s,p){var x=p.channel(s.ns);var o=s.info.split("\n");var v="";var u=[];var q="";for(var r in o){if(!o.hasOwnProperty(r)){continue}v=o[r].split(" ");if(s.p=="privclass"){u.push([v.shift(),v.shift().split("=")[1],v.join(" ")])}else{if(s.p=="users"){q=v.shift().split(":",1)[0];u.push([q,x.get_privclass_order(q),v.join(" ")])}}}p.ui.chatbook.current.log_pc(s.p=="privclass",u)};n();wsc.defaults.Extension.Ignore(f);wsc.defaults.Extension.Away(f);wsc.defaults.Extension.Autojoin(f)};wsc.defaults.Extension.Autojoin=function(a){var f=a.autojoin;a.ui.nav.add_button({icon:"chat",label:"",title:"Join your autojoin channels",href:"#autojoin-do",handler:function(){for(var h in a.autojoin.channel){if(!a.autojoin.channel.hasOwnProperty(h)){continue
