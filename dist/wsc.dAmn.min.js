...
}T.connections.push(R)}P.cchannel.log_whois(T)};var z=function(Q,P){if(Q.ns.indexOf("login:")!=0){return}P.cchannel.server_message("Whois failed for "+(Q.sns.substr(1)),"not online")};var M=function(T,Q){var W=Q.channel(T.ns);var P=T.info.split("\n");var V="";var U=[];var R="";for(var S in P){if(!P.hasOwnProperty(S)){continue}V=P[S].split(" ");if(T.p=="privclass"){U.push([V.shift(),V.shift().split("=")[1],V.join(" ")])}else{if(T.p=="users"){R=V.shift().split(":",1)[0];U.push([R,W.get_privclass_order(R),V.join(" ")])}}}W.log_pc(T.p=="privclass",U)};E();wsc.defaults.Extension.Autojoin(p);wsc.defaults.Extension.Away(p);wsc.defaults.Extension.Ignore(p)};wsc.defaults.Extension.Autojoin=function(a){var j=a.autojoin;var m=function(){a.bind("cmd.autojoin",g);a.ui.on("settings.open",j.page)};j.page=function(r,u){var s=r.settings.page("Autojoin",true);var q="<ul>";var z={};z.ajon=a.autojoin.on;if(a.autojoin.count==0){q+="<li><i>No autojoin channels set</i></li></ul>"}else{for(var p in a.autojoin.channel){if(!a.autojoin.channel.hasOwnProperty(p)){continue
}q+="<li>"+a.autojoin.channel[p]+"</li>"}q+="</ul>"}s.item("Text",{ref:"intro",title:"Autojoin",text:"Add any channels you want to join automatically when you                    connect to the chat server."});var v=s.item("Form",{ref:"autojoin",wclass:"boxed-ff-indv",title:"Channels",text:"This is the list of channels on your autojoin.\n\nUse the                    command <code>/autojoin</code> to edit the list.",fields:[["Checkbox",{ref:"enabled",label:"Autojoin",items:[{value:"yes",title:"On",selected:z.ajon}]}],["Text",{ref:"channels",text:q}]],event:{change:function(A){},save:function(A){}}})};var g=function(p){};var b=function(r,p){j.on=true;j.last={};j.since=new Date();j.reason=r.args;var s=p.say;var q=replaceAll(j.format.setaway,"{reason}",j.reason||"[silent away]");if(q.indexOf("/me ")==0){q=q.substr(4);s=p.action}p.each_channel(function(u){s.call(p,u,q)})};var k=function(r,p){j.on=false;var s=p.say;var q=j.format.setback;if(q.indexOf("/me ")==0){q=q.substr(4);s=p.action}p.each_channel(function(u){s.call(p,u,q)
