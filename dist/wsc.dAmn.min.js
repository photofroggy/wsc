...
};wsc.Protocol.prototype.extend_maps=function(a){for(key in a){this.maps[key]=a[key]}};wsc.Protocol.prototype.extend_messages=function(a){for(key in a){this.messages[key]=a[key]}};wsc.Protocol.prototype.parse=function(b){name=this.event(b);if(!(name in this.maps)){console.log("unknown: ",name);console.log(this.maps);mapping=this.maps.unknown;name="unknown"}else{mapping=this.maps[name]}var a={name:name,pkt:b,ns:null};cmd=b.cmd;if(this.mapper[cmd]){this.mapper[cmd](b,a,mapping)}else{this.map(b,a,mapping)}return a};wsc.Protocol.prototype.event=function(b){var g=b.cmd;var a=null;for(var f in this.chains){a=this.chains[f];if(a[0]!=g){continue}var h=b.sub[0];g=g+"_"+h.cmd;if(a.length>1&&h.param!=undefined){if(a[1]==h.cmd){return g+"_"+h.param}}}return g};wsc.Protocol.prototype.map=function(l,j,f){for(var h in f){if(f[h]==null){continue}var g=f[h];var a=g;switch(parseInt(h)){case 0:j[g]=l.param;break;case 1:if(f[1] instanceof Array){for(var m in f[1]){g=f[1][m];if(g instanceof Array){j[g[1]]=l.arg[g[0]];
a=g[1]}else{var b=g[0]=="*"?g.slice(1):g;j[g]=l.arg[b]||"";a=g}}}if(typeof f[1]=="string"){j[g]=l.arg.slice(0)}break;case 2:if(g instanceof Array){l.sub[0].sub=l.sub.slice(1);this.map(l.sub[0],j,g)}else{j[g]=l.body}break;case 3:j[g]=l.raw;break}if(a[0]!="*"){continue}b=a.slice(1);val=this.mparser.parse(j[a]);j[b]=val}};wsc.Protocol.prototype.render=function(b,f){var a=this.messages[b.name];if(!a){return""}var h=a[0];var g="";for(key in b){if(!b.hasOwnProperty(key)||key=="pkt"){continue}g=b[key];if(key=="ns"||key=="sns"){key="ns";g=b.sns}if(g.hasOwnProperty("_parser")){switch(f){case"text":g=g.text();break;case"html":g=g.html();break;case"ansi":g=g.ansi();break;default:g=g.text();break}}h=replaceAll(h,"{"+key+"}",g)}return h};wsc.Protocol.prototype.log=function(a,f){msgm=this.messages[f.name];if(!msgm){return}if(f.s=="0"){return}f.html=this.render(f,"html");try{if(!msgm[2]){if(!msgm[1]){a.ui.channel(f.ns).log_item(f)}else{a.ui.channel(a.mns).log_item(f)}}else{a.ui.log_item(f)}}catch(b){try{a.ui.channel(a.mns).server_message("Failed to log for "+f.sns,f.html)
...
wsc.Flow.prototype.recv_msg=function(b,a){a.channel(b.ns).recv_msg(b)};wsc.Flow.prototype.recv_action=wsc.Flow.prototype.recv_msg;wsc.Flow.prototype.recv_npmsg=wsc.Flow.prototype.recv_msg;wsc.Flow.prototype.recv_privchg=function(b,a){a.channel(b.ns).recv_privchg(b)};wsc.Flow.prototype.recv_kicked=function(b,a){a.channel(b.ns).recv_kicked(b)};wsc.defaults.Extension=function(l){var n={};n.away={on:false,reason:"",last:{},since:0,store:l.storage.folder("away"),format:{setaway:"/me is away: {reason}",setback:"/me is back",away:"{from}: I am away, reason: {reason}"}};var y=function(){l.bind("cmd.connect",g);l.bind("cmd.set",a);l.bind("cmd.join",E);l.bind("cmd.chat",G);l.bind("cmd.part",I);l.bind("cmd.say",o);l.bind("cmd.npmsg",J);l.bind("cmd.me",q);l.bind("cmd.promote",u);l.bind("cmd.demote",u);l.bind("cmd.ban",k);l.bind("cmd.unban",k);l.bind("cmd.kick",b);l.bind("cmd.whois",A);l.bind("cmd.title",h);l.bind("cmd.topic",h);l.bind("cmd.admin",v);l.bind("cmd.disconnect",g);l.bind("cmd.kill",b);l.bind("cmd.raw",F);
l.bind("cmd.clear",C);l.bind("cmd.clearall",B);l.bind("cmd.close",x);l.bind("pkt.property",j);l.bind("pkt.recv_admin_show",H);l.bind("pkt.recv_admin_showverbose",H);l.bind("pkt.get",r);l.bind("cmd.gettitle",p);l.bind("cmd.gettopic",p);l.bind("cmd.theme",f);l.ui.on("settings.open",m);l.ui.on("settings.open.ran",D);l.ui.on("settings.save.ui",z);l.ui.on("settings.save",function(){l.config_save()})};var z=function(L,K){l.settings.ui.theme=L.theme;l.settings.ui.clock=L.clock;l.settings.ui.tabclose=L.tabclose};var m=function(M,L){var K=M.settings.page("Main");var N={};N.username=l.settings.username;N.pk=l.settings.pk;N.devel=l.settings.developer;K.item("Form",{ref:"login",title:"Login",text:"Here you can change the username and token used to                    log into the chat server.",fields:[["Textfield",{ref:"username",label:"Username","default":N.username}],["Textfield",{ref:"token",label:"Token","default":N.pk}]],event:{save:function(O){l.settings.username=O.data.username;l.settings.pk=O.data.token
}}},true);K.item("Form",{ref:"developer",title:"Developer Mode",text:"Turn developer mode on or off.\n\nDeveloper mode will expose any hidden                channel tabs, amongst other things. Keep this turned off unless you're working                on implementing something.",fields:[["Checkbox",{ref:"enabled",items:[{value:"on",title:"On",selected:N.devel}]}]],event:{change:function(O){l.settings.developer=(O.data.enabled.indexOf("on")!=-1);l.ui.developer(l.settings.developer)},save:function(O){N.devel=l.settings.developer},close:function(O){l.settings.developer=N.devel;l.ui.developer(l.settings.developer)}}});K.item("Text",{ref:"intro",title:"Main",text:"Use this window to view and change your settings.\n\nCheck                    the different pages to see what settings can be changed.",},true);K.item("Text",{ref:"debug",wclass:"faint",title:"Debug Information",text:"Chat Agent: <code>"+l.settings.agent+"</code>\n\nUser                    Agent: <code>"+navigator.userAgent+"</code>"})
};var D=function(M,L){var K=M.settings.page("About",true);K.item("Text",{ref:"about-wsc",title:"Wsc",text:'Currently using <a href="http://github.com/photofroggy/wsc/">wsc</a>                    version '+wsc.VERSION+" "+wsc.STATE+'.\n\nWsc                    works using HTML5, javascript, and CSS3. WebSocket is used for the connection                    where possible. The source code for this client is pretty huge.\n\nWsc was created                    by ~<a href="http://photofroggy.deviantart.com/">photofroggy</a>'})};var f=function(L,K){K.ui.theme(L.args.split(" ").shift())};var a=function(M){var L=M.args.split(" ");var K=L.shift().toLowerCase();var L=L.join(" ");if(L.length==0){l.cchannel.serverMessage("Could not set "+K,"No data supplied");return}if(!(K in l.settings)){l.cchannel.serverMessage('Unknown setting "'+K+'"');return}l.settings[K]=L;l.cchannel.serverMessage("Changed "+K+" setting","value: "+L);l.control.setLabel()};var g=function(K){l[K.cmd]()};var E=function(K){var L=K.args.split(" ");
var L=L.toString()==""?[]:L;if(K.ns!=K.target){L.unshift(K.target)}if(L.toString()==""){return}for(index in L){l.join(L[index])}};var G=function(K){var L=K.args.split(" ");var L=L.toString()==""?[]:L;if(K.ns!=K.target){L.unshift(K.target)}if(L.toString()==""){return}for(index in L){l.join("@"+L[index])}};var I=function(K){var L=K.args.split(" ");if(K.ns!=K.target){L.unshift(K.target)}if(L.toString()==""){l.part(K.ns);return}for(index in L){l.part(L[index])}};var h=function(K){l.set(K.target,K.cmd,K.args)};var u=function(L){var K=L.args.split(" ");l[L.cmd.toLowerCase()](L.target,K[0],K[1])};var k=function(O,K){var M=O.args.split(" ");var L=M.shift();var N=O.cmd;if(N=="ban"&&M.length>0){K.kick(O.target,L,M.join(" "))}K[N](O.target,L)};var q=function(K){l.action(K.target,K.args)};var F=function(K){l.send(K.args.replace(/\\n/gm,"\n"))};var b=function(N,K){var O=N.args.split(" ");var L=O.shift();var M=O.length>0?O.join(" "):null;if(N.cmd=="kick"){K.kick(N.target,L,M)}else{K.kill(L,M)}};var o=function(K){if(l.channel(K.target).monitor){return
}l.say(K.target,K.args)};var J=function(K){l.npmsg(K.target,K.args)};var C=function(M,K){if(M.args.length>0){var N=M.args.split(" ");for(var L in N){if(!N.hasOwnProperty(L)){continue}K.channel(M.target).clear(N[L])}}else{K.channel(M.target).clear()}};var B=function(L,K){var N=null;if(L.args.length>0){var M=L.args.split(" ");N=function(P,Q){for(var O in M){if(!M.hasOwnProperty(O)){continue}Q.clear(M[O])}}}else{N=function(O,P){P.clear()}}K.each_channel(N,true)};var x=function(K){l.part(K.target);l.remove_ns(K.target)};var A=function(L,K){K.whois(L.args.split(" ")[0])};var v=function(L,K){K.admin(L.target,L.args)};var s=function(L,K){K.disconnect()};var p=function(L,K){var M=L.cmd.indexOf("title")>-1?"title":"topic";K.control.ui.set_text("/"+M+" "+K.channel(L.target).info[M].content)};var j=function(N,K){if(N.p!="info"){return}var L=N.pkt.sub;var O=L.shift().arg;O.username=N.sns.substr(1);O.connections=[];var M={};while(L.length>0){M=L.shift().arg;M.channels=[];while(L.length>0){if(L[0].cmd!="ns"){break
}M.channels.unshift(K.deform_ns(L.shift().param))}O.connections.push(M)}K.cchannel.log_whois(O)};var r=function(L,K){if(L.ns.indexOf("login:")!=0){return}var M=L.sns.substr(1);K.ui.pager.notice({ref:"whois-"+M,heading:"Whois Failed",content:"Whois failed for "+M+".\nNo such user online."},false,5000)};var H=function(O,L){var R=L.channel(O.ns);var K=O.info.split("\n");var Q="";var P=[];var M="";for(var N in K){if(!K.hasOwnProperty(N)){continue}Q=K[N].split(" ");if(O.p=="privclass"){P.push([Q.shift(),Q.shift().split("=")[1],Q.join(" ")])}else{if(O.p=="users"){M=Q.shift().split(":",1)[0];P.push([M,R.get_privclass_order(M),Q.join(" ")])}}}R.log_pc(O.p=="privclass",P)};y();wsc.defaults.Extension.Ignore(l);wsc.defaults.Extension.Away(l);wsc.defaults.Extension.Autojoin(l)};wsc.defaults.Extension.Autojoin=function(a){var f=a.autojoin;a.ui.nav.add_button({icon:"chat",label:"",title:"Join your autojoin channels",href:"#autojoin-do",handler:function(){for(var h in a.autojoin.channel){if(!a.autojoin.channel.hasOwnProperty(h)){continue
}a.join(a.autojoin.channel[h])}}});var g=function(){a.bind("cmd.autojoin",b);a.ui.on("settings.open",f.page)};f.page=function(l,n){var m=l.settings.page("Autojoin");var k="<ul>";var o={};o.ajon=a.autojoin.on;o.chan=a.autojoin.channel;if(a.autojoin.channel.length==0){k+="<li><i>No autojoin channels set</i></li></ul>"}else{for(var j in a.autojoin.channel){if(!a.autojoin.channel.hasOwnProperty(j)){continue}k+="<li>"+a.autojoin.channel[j]+"</li>"}k+="</ul>"}m.item("Checkbox",{ref:"eaj",title:"Autojoin",text:"Turn on autojoin to automatically join selected channels                    when you connect to the chat server.",items:[{value:"yes",title:"On",selected:o.ajon}],event:{change:function(p){if(p.target.value=="yes"){a.autojoin.on=p.target.checked}},save:function(p){o.ajon=a.autojoin.on;a.config_save()},close:function(p){a.autojoin.on=o.ajon}}});var h=m.item("Items",{ref:"channelss",title:"Channels",text:"Add any channels you want to join automatically when you                    connect to the chat server.",items:a.autojoin.channel,prompt:{title:"Add Channel",label:"Channel:",},event:{up:function(p){var q=p.args.swap;
a.autojoin.channel[q["this"].index]=q.that.item;a.autojoin.channel[q.that.index]=q["this"].item;h.options.items=a.autojoin.channel},down:function(p){var q=p.args.swap;a.autojoin.channel[q["this"].index]=q.that.item;a.autojoin.channel[q.that.index]=q["this"].item;h.options.items=a.autojoin.channel},add:function(r){var q=a.deform_ns(r.args.item).toLowerCase();var p=a.autojoin.channel.indexOf(q);if(p!=-1){return}a.autojoin.channel.push(q);h.options.items=a.autojoin.channel},remove:function(p){a.autojoin.channel.splice(p.args.index,1);h.options.items=a.autojoin.channel},save:function(p){o.chan=a.autojoin.channel;a.config_save()},close:function(p){a.config_load()}}})};var b=function(o){var j=o.args.split(" ");if(!j){return}var m=j.shift().toLowerCase();var h=function(q){};var l=false;var p=a.channel(o.ns);switch(m){case"add":h=function(q){if(a.autojoin.channel.indexOf(q)==-1){l=true;a.autojoin.channel.push(q);p.server_message("Added "+q+" to your autojoin.")}else{p.server_message("Already have "+q+" on your autojoin.")
}};break;case"rem":case"remove":h=function(r){var q=a.autojoin.channel.indexOf(r);if(q!=-1){l=true;a.autojoin.channel.splice(q,1);p.server_message("Removed "+r+" from your autojoin.")}else{p.server_message(r+" is not on your autojoin list.")}};break;case"on":p.server_message("Autojoin on.");if(!a.autojoin.on){l=true;a.autojoin.on=true}break;case"off":p.server_message("Autojoin off.");if(a.autojoin.on){l=true;a.autojoin.on=false}break}var n="";for(var k in j){if(!j.hasOwnProperty(k)){continue}n=a.deform_ns(j[k]).toLowerCase();h(n)}if(l){a.config_save()}};g()};wsc.defaults.Extension.Away=function(b){var g=b.storage.folder("away");var f={on:false,reason:"",last:{},since:0,interval:60000,format:{setaway:"/me is away: {reason}",setback:"/me is back",away:"{user}: I've been away for {timesince}. Reason: {reason}"}};b.away=f;var m=function(){k();h();b.bind("cmd.setaway",a);b.bind("cmd.setback",l);b.bind("pkt.recv_msg.highlighted",j);b.ui.on("settings.open",f.page)};f.page=function(o,q){var n=function(u){u=replaceAll(u,"<","&lt;");
u=replaceAll(u,">","&gt;");u=replaceAll(u,'"',"&quot;");return u};var r=function(u){u=replaceAll(u,"&lt;","<");u=replaceAll(u,"&gt;",">");u=replaceAll(u,"&quot;",'"');return u};var p=o.settings.page("Away");var s={};s.away=f.format.away;s.sa=f.format.setaway;s.sb=f.format.setback;s.intr=f.interval;p.item("Text",{ref:"intro",title:"Away Messages",text:"Use away messages when you are away from the chats.\n\n                    You can set yourself away using the                     <code>/setaway [reason]</code> command. When you get back,                    use <code>/setback</code>. While you are away, the client                    will automatically respond when people try to talk to you,                    telling them you're away.",});p.item("Form",{ref:"msgs",title:"Messages",text:"Here you can set the messages displayed when you set                    yourself away or back. You can also change the away message                    format.",hint:"<b>{user}</b><br/>This is replaced with the username of the person trying to talk to you\n\n<b>{reason}</b>                    <br/>This is replaced by your reason for being away.\n\n<b>{timesince}</b>                    <br/>Use this to show how long you have been away for.",fields:[["Textfield",{ref:"away",label:"Away","default":n(s.away)}],["Textfield",{ref:"setaway",label:"Setaway","default":n(s.sa)}],["Textfield",{ref:"setback",label:"Setback","default":n(s.sb)}]],event:{save:function(u){f.format.away=r(u.data.away);
...
};wsc.Client.prototype.kick=function(b,a,f){var g=this;this.cascade("send.kick",function(h){g.send(wsc_packetstr("kick",g.format_ns(h.ns),{u:h.user},h.input||null))},{input:f||"",ns:b,user:a})};wsc.Client.prototype.kill=function(a,b){this.send(wsc_packetstr("kill","login:"+a,{},b||null))};wsc.Client.prototype.admin=function(a,b){this.send(wsc_packetstr("send",this.format_ns(a),{},wsc_packetstr("admin","",{},b)))};wsc.Client.prototype.property=function(a,b){this.send(wsc_packetstr("get",this.format_ns(a),{p:b}))};wsc.Client.prototype.set=function(a,f,b){this.trigger("send.set.before",e);var g=this;this.cascade("send.set",function(h){g.send(wsc_packetstr("set",g.format_ns(h.ns),{p:h.property},h.input))},{input:b,ns:a,property:f})};wsc.Client.prototype.whois=function(a){this.send(wsc_packetstr("get","login:"+a,{p:"info"}))};wsc.Client.prototype.disconnect=function(){this.send(wsc_packetstr("disconnect"))};var Chatterbox={};Chatterbox.VERSION="0.16.77";Chatterbox.STATE="beta";Chatterbox.UI=function(b,a,f,h,g){this.client=b;
this.events=g||new EventEmitter();this.mozilla=h;this.umuted=[];this.viewing=true;this.settings={themes:["wsct_default","wsct_dAmn"],theme:"wsct_default",monitor:["~Monitor",true],username:"",domain:"website.com",clock:true,tabclose:true,developer:false,media:"/static/"};var j=this;this.sound={play:function(k){k.pause();k.currentTime=0;k.play()},toggle:function(l){for(var k in j.sound.bank){if(!j.sound.bank.hasOwnProperty(k)){continue}j.sound.bank[k].muted=l}},mute:function(){j.sound.toggle(true)},unmute:function(){j.sound.toggle(false)},bank:{m:null,c:null},click:null,};a.extend(this.settings,f);a.append('<div class="wsc '+this.settings.theme+'"></div>');this.mw=new wsc.Middleware();this.view=a.find(".wsc");this.mns=this.format_ns(this.settings.monitor[0]);this.lun=this.settings.username.toLowerCase();this.monitoro=null;this.swidth=(function(){if($.browser.msie){var m=$('<textarea cols="10" rows="2"></textarea>').css({position:"absolute",top:-1000,left:-1000}).appendTo("body"),l=$('<textarea cols="10" rows="2" style="overflow: hidden;"></textarea>').css({position:"absolute",top:-1000,left:-1000}).appendTo("body");
...
}var j=this.content();if(j.length==0){return}var a="";if(this.options.hasOwnProperty("wclass")){a=" "+this.options.wclass}var h=Chatterbox.render("settings.item.wrap",{type:this.type.toLowerCase().split(".").join("-"),ref:this.options.ref,"class":a});h=replaceAll(h,"{content}",j);k.append(h);this.view=k.find(".item."+this.options.ref);this.hooks(this.view);if(!this.options.hasOwnProperty("subitems")){return}var l;var g;var f;var b;for(i in this.options.subitems){l=this.options.subitems[i];g=l[0];f=l[1];sitem=Chatterbox.Settings.Item.get(g,f);b=["stacked"];if(sitem.options.wclass){b.push(sitem.options.wclass)}sitem.options.wclass=b.join(" ");sitem.build(this.view);this.items.push(sitem);if(f.hasOwnProperty("ref")){this.itemo[f.ref]=sitem}}};Chatterbox.Settings.Item.prototype.content=function(){return Chatterbox.render("settings.item."+this.selector,this.options)};Chatterbox.Settings.Item.prototype.resize=function(){for(var a in this.items){this.items[a].resize()}};Chatterbox.Settings.Item.prototype.hooks=function(f){if(!this.options.hasOwnProperty("event")){return
}var b=this.options.event;var h=Chatterbox.template.settings.item.get(this.selector);if(h==false){return}if(!h.hasOwnProperty("events")){return}var g=[];for(var a in h.events){g=h.events[a];if(!b.hasOwnProperty(g[0])){continue}f.find(g[1]).bind(g[0],b[g[0]])}};Chatterbox.Settings.Item.prototype._event_stub=function(){};Chatterbox.Settings.Item.prototype._get_cb=function(a){if(!this.options.hasOwnProperty("event")){return this._event_stub}return this.options.event[a]||this._event_stub};Chatterbox.Settings.Item.prototype._get_ep=function(b){var g=Chatterbox.template.settings.item.get(this.selector);if(g==null){return false}if(!g.hasOwnProperty("events")){return false}var f=[];for(var a in g.events){f=g.events[a];if(f[0]==b){return f}}};Chatterbox.Settings.Item.prototype.save=function(g,h){var j=this._get_ep("inspect");var b=j?this.view.find(j[1]):null;var a=this._get_cb("save");if(typeof a=="function"){a({input:b,item:this,page:h,window:g})}else{for(var f in a){var k=b.hasOwnProperty("slice")?b.slice(f,1):b;
a[f]({input:k,item:this,page:h,window:g})}}for(var f in this.items){this.items[f].save(g,h)}};Chatterbox.Settings.Item.prototype.close=function(g,h){var j=this._get_ep("inspect");var b=j?this.view.find(j[1]):null;var a=this._get_cb("close");if(typeof a=="function"){a({input:b,item:this,page:h,window:g});return}for(var f in a){var k=b.hasOwnProperty("slice")?b.slice(f,1):b;a[f]({input:k,item:this,page:h,window:g})}for(var f in this.items){this.items[f].close(g,h)}};Chatterbox.Settings.Item.get=function(h,m,j,b,a){var g=h.split(".");var k=b||Chatterbox.Settings.Item;var l;for(var f in g){l=g[f];if(!k.hasOwnProperty(l)){k=a||Chatterbox.Settings.Item;break}k=k[l]}return new k(h,m,j)};Chatterbox.Settings.Item.Form=function(b,a){Chatterbox.Settings.Item.call(this,b,a);this.form=null;this.fields=[];this.lsection=null;this.fsection=null;this.fieldo={}};Chatterbox.Settings.Item.Form.prototype=new Chatterbox.Settings.Item();Chatterbox.Settings.Item.Form.prototype.constructor=Chatterbox.Settings.Item.Form;
Chatterbox.Settings.Item.Form.field=function(b,a){return Chatterbox.Settings.Item.get(b,a,this.manager,Chatterbox.Settings.Item.Form,Chatterbox.Settings.Item.Form.Field)};Chatterbox.Settings.Item.Form.prototype.build=function(h){Chatterbox.Settings.Item.prototype.build.call(this,h);if(this.view==null){return}this.lsection=this.view.find("section.labels");this.fsection=this.view.find("section.fields");if(!this.options.hasOwnProperty("fields")){return}var g;var j;for(var a in this.options.fields){g=this.options.fields[a];j=Chatterbox.Settings.Item.Form.field(g[0],g[1]);this.fields.push(j);j.build(this);if(g[1].hasOwnProperty("ref")){this.fieldo[g[1].ref]=j}}this.form=this.view.find("form");var b=this;this.form.bind("change",function(f){b.change()})};Chatterbox.Settings.Item.Form.prototype.resize=function(){for(var a in this.fields){this.fields[a].resize()}};Chatterbox.Settings.Item.Form.prototype.change=function(){var f={};var g;for(var b in this.fields){g=this.fields[b];f[g.ref]=g.get()}var a=this._get_cb("change");
if(typeof a=="function"){a({data:f,form:this})}else{for(var b in a){a[b]({data:f,form:this})}}};Chatterbox.Settings.Item.Form.prototype.save=function(g,j){var h={};var b;for(var f in this.fields){field=this.fields[f];h[field.ref]=field.get()}var a=this._get_cb("save");if(typeof a=="function"){a({data:h,form:this,page:j,window:g})}else{for(var f in a){a[f]({data:h,form:this,page:j,window:g})}}};Chatterbox.Settings.Item.Form.prototype.close=function(f,h){var g={};var j;for(var b in this.fields){j=this.fields[b];g[j.ref]=j.get()}var a=this._get_cb("close");if(typeof a=="function"){a({data:g,form:this,page:h,window:f})}else{for(var b in a){a[b]({data:g,form:this,page:h,window:f})}}};Chatterbox.Settings.Item.Form.Field=function(b,a){Chatterbox.Settings.Item.call(this,b,a);this.ref=this.options.ref||"ref";this.label=null;this.field=null;this.value=""};Chatterbox.Settings.Item.Form.Field.prototype=new Chatterbox.Settings.Item();Chatterbox.Settings.Item.Form.Field.prototype.constructor=Chatterbox.Settings.Item.Form.Field;
...
Chatterbox.template.settings.item.form.field.colour={};Chatterbox.template.settings.item.form.field.colour.post=Chatterbox.template.clean(["ref","default"]);Chatterbox.template.settings.item.form.field.colour.frame='<input class="{ref}" type="color" value="{default}" />';wsc.dAmn={};wsc.dAmn.VERSION="0.9.25";wsc.dAmn.STATE="alpha";wsc.dAmn.BDS=function(f,k,g){var h={};var a={};g.bds={version:"0.4",mns:"chat:datashare",gate:"chat:dsgateway",channel:(new StringSet()),provides:["BOTCHECK","CLINK"]};f.bds=g.bds;g.bds.channel.add(g.bds.mns);g.bds.channel.add(g.bds.gate);var l=function(){f.hidden.add(g.bds.mns);f.exclude.add(g.bds.mns);f.hidden.add(g.bds.gate);f.exclude.add(g.bds.gate);f.bind("pkt.login",m);f.bind("pkt.recv_msg",b);f.bind("pkt.join",j.join);f.bind("BDS.BOTCHECK.DIRECT",j.botcheck);f.bind("BDS.BOTCHECK.ALL",j.botcheck);f.bind("BDS.BOTCHECK.OK",j.checkresp);f.bind("BDS.BOTCHECK.DENIED",j.checkresp);f.bind("CDS.LINK.REQUEST",j.clreq);f.bind("CDS.LINK.REJECT",j.clrj);f.bind("CDS.LINK.ACK",j.clra);
f.bind("pkt.recv_join",j.pcrj);f.bind("pkt.property",j.pcp);f.bind("closed",j.closed)};var m=function(n){if(n.pkt.arg.e!="ok"){return}f.join(g.bds.gate)};var b=function(q){if(q.user.toLowerCase()==f.settings.username.toLowerCase()){return}if(!g.bds.channel.contains(q.ns)){return}var p={ns:q.ns,sns:q.sns,message:q.message,user:q.user,name:"",payload:"",head:[null,null,null],params:[]};var s=q.message.split(":");var o=[null,null,null];var r=null;for(var n in o){o[n]=s.shift()||null;if(o[n]==null){return}}r=s.join(":");p.name=o.join(".");p.payload=r;p.head=o;p.param=r.split(",");f.trigger(o[0],p);f.trigger(o[0]+"."+o[1],p);f.trigger(p.name,p)};var j={closed:function(n){f.remove_ns(g.bds.mns);f.remove_ns(g.bds.gate)},join:function(n){if(n.ns.toLowerCase()!=g.bds.mns){return}if(n.pkt.arg.e!="ok"){return}f.npmsg(n.ns,"BDS:PROVIDER:CAPS:"+g.bds.provides.join(","))},botcheck:function(o){if(o.head[2]!="ALL"&&o.payload!=f.settings.username){return}var n=wsc.VERSION+"/"+f.ui.VERSION+"/"+wsc.dAmn.VERSION+"/"+g.bds.version;
var p=CryptoJS.MD5(("wsc.dAmn"+n+f.settings.username+o.user).toLowerCase());f.npmsg(o.ns,"BDS:BOTCHECK:CLIENT:"+o.user+",wsc.dAmn,"+n+","+p)},checkresp:function(n){if(n.ns.toLowerCase()!=g.bds.gate){return}if(n.param[0].toLowerCase()!=f.settings.username.toLowerCase()){return}if(f.channel(n.ns).info.members[f.settings.username].pc=="Visitors"){f.part(n.ns)}if(n.head[2]=="OK"){f.join(g.bds.mns);return}f.ui.pager.notice({ref:"botcheckdenied",heading:"BDS Error",content:"Denied entry to backend channel.\nReason provided: <code>"+n.param.slice(1).join(",")+"</code>"})},clreq:function(r){if(r.payload.toLowerCase()!=f.settings.username.toLowerCase()){return}if(f.ui.umuted.indexOf(r.user.toLowerCase())!=-1){f.npmsg(r.ns,"CDS:LINK:REJECT:"+r.user+",You have been blocked");return}if(f.away.on){var p=new Date();var q=r.sns.toLowerCase();var n=timeLengthString((p-f.away.since)/1000);var s=replaceAll(f.away.format.away,"{user}",r.user);s=replaceAll(s,"{timesince}",n);f.npmsg(r.ns,"CDS:LINK:REJECT:"+r.user+",Away; "+f.away.reason);
return}f.npmsg(r.ns,"CDS:LINK:ACK:"+r.user);var o=f.ui.pager.notice({ref:"clink-"+r.user,icon:'<img src="'+wsc.dAmn.avatar.src(r.user,f.channel(r.ns).info.members[r.user].usericon)+'" />',heading:"Chat "+r.user,content:r.user+" wants to talk in private.\nType <code>/chat "+r.user+"</code> to talk to them, or use the buttons below.",buttons:{accept:{ref:"accept",target:"accept",label:"Accept",title:"Join the private chat",click:function(){f.ui.pager.remove_notice(o);f.join("@"+r.user);return false}},reject:{ref:"reject",target:"reject",label:"Reject",title:"Reject the private chat",click:function(){f.npmsg(r.ns,"CDS:LINK:REJECT:"+r.user);f.ui.pager.remove_notice(o);return false}}}},true);a[r.user]=o;o.onclose=function(){f.npmsg(r.ns,"CDS:LINK:REJECT:"+r.user)}},clrj:function(q){var n=q.user.toLowerCase();var r=q.payload.split(",");var o=r.shift();r=r.join(",");if(o.toLowerCase()!=f.settings.username.toLowerCase()){return}if(!(n in h)){return}clearTimeout(h[n]);f.channel("@"+n).server_message("Chat request rejected",r)
},clra:function(q){var n=q.user.toLowerCase();var r=q.payload.split(",");var o=r.shift();if(o.toLowerCase()!=f.settings.username.toLowerCase()){return}clearTimeout(h[n])},pcp:function(p){if(p.ns.indexOf("pchat")!=0){return}if(f.bds.channel.contains(p.ns)){return}if(p.pkt.arg.p!="members"){return}if(f.channel(p.ns).get_usernames().length==2){return}var n=p.sns.substr(1);var o=p.ns;f.channel(o).server_message(n+" has not yet joined","Sending them a notice...");f.npmsg("chat:datashare","CDS:LINK:REQUEST:"+n);h[n.toLowerCase()]=setTimeout(function(){try{f.channel(o).server_message("Notification failed",n+" does not seem to be using a client that supports pchat notices.")}catch(q){}},10000)},pcrj:function(o){if(o.sns[0]!="@"){return}try{clearTimeout(h[o.user.toLowerCase()])}catch(n){}if(!(o.user in a)){return}f.ui.pager.remove_notice(a[o.user])}};l()};wsc.dAmn.Colours=function(a,f,b){b.colours.page=null;b.colours.configure_page=function(g,j){var h=g.settings.page("Colours");b.colours.page=h;
var k={};k.on=b.colours.on;k.send=b.colours.send;k.msg=b.colours.msg;k.user=b.colours.user;h.item("Form",{ref:"switch",title:"Enable Chromacity Colours",text:"Here you can turn chromacity colours on and off.\n\nYou                    can choose to parse other people's colours, and send your                    own colours.",fields:[["Checkbox",{ref:"enabled",items:[{value:"parse",title:"Parse colours",selected:k.on},{value:"send",title:"Send colours",selected:k.send}]}]],event:{change:function(l){b.colours.on=(l.data.enabled.indexOf("parse")!=-1);b.colours.send=(l.data.enabled.indexOf("send")!=-1)},save:function(l){k.on=b.colours.on;k.send=b.colours.send},close:function(l){b.colours.on=k.on;b.colours.send=k.send}}});h.item("Form",{ref:"colours",title:"Custom Colours",text:"Here you can set your own username colour and message colour.",fields:[["Colour",{ref:"usr",label:"Username Colour","default":"#"+k.user}],["Colour",{ref:"msg",label:"Message Colour","default":"#"+k.msg}]],event:{change:function(l){b.colours.user=l.data.usr.substr(1).toUpperCase();
b.colours.msg=l.data.msg.substr(1).toUpperCase()},save:function(l){k.user=b.colours.user;k.msg=b.colours.msg},close:function(l){b.colours.user=k.user;b.colours.msg=k.msg},}})};b.colours.send_colour=function(h,g){if(!b.colours.send){g(h);return}h.input+='<abbr title="colors:'+b.colours.user+":"+b.colours.msg+'"></abbr>';g(h)};b.colours.parse_colour=function(j){if(!b.colours.on){return}var h=j.item.find("abbr");if(h.length==0){return}h=h.last();var g=h.prop("title").match(/colors:([A-F0-9]{6}):([A-F0-9]{6})/);if(g==null){return}h.remove();j.item.find(".cmsg, .caction").css("color","#"+g[2]);j.item.find(".cmsg.user, .caction.user").css("color","#"+g[1])};a.ui.on("settings.open.ran",b.colours.configure_page);a.ui.on("log_item.after",b.colours.parse_colour);a.middle("send.msg",b.colours.send_colour);a.middle("send.action",b.colours.send_colour)};wsc.dAmn.Emotes=function(a,f,b){b.emotes.emote={};b.emotes.page=null;b.emotes.fint=null;b.emotes.notice=null;b.emotes.fetching=false;b.emotes.loaded=false;
b.emotes.picker=new wsc.dAmn.Emotes.Picker(a.ui,{event:{select:function(g){b.emotes.select(g)},reload:function(){b.emotes.fetch()}}},b);b.emotes.picker.build();b.emotes.picker.hide();a.ui.control.add_button({label:"",icon:"user",href:"#emotes",title:"Emote picker.",handler:function(){if(b.emotes.picker.window.css("display")=="none"){b.emotes.picker.show()}else{b.emotes.picker.close()}}});b.emotes.configure_page=function(h,k){var j=h.settings.page("Emotes");b.emotes.page=j;var l={};l.on=b.emotes.on;var g="";if(l.on){if(!b.emotes.loaded||b.emotes.fetching){g="<em>Fetching emotes...</em>"}else{g="<em>Emotes loaded.</em>"}}j.item("Form",{ref:"switch",title:"Enable Emotes",text:"Here you can turn custom emotes on and off.",fields:[["Checkbox",{ref:"enabled",items:[{value:"yes",title:"On",selected:l.on}]}]],event:{change:function(m){b.emotes.on=(m.data.enabled.indexOf("yes")!=-1);b.emotes.picker.state(b.emotes.on);if(b.emotes.on){b.emotes.fetch();return}if(b.emotes.fint===null){return}clearTimeout(b.emotes.fint);
b.emotes.fint=null},save:function(m){l.on=b.emotes.on;if(!b.emotes.on){b.emotes.picker.loading("Disabled")}else{if(!b.emotes.fetching){b.emotes.picker.loaded()}}},close:function(m){b.emotes.on=l.on;b.load();b.emotes.page=null;if(b.emotes.fint===null){return}clearTimeout(b.emotes.fint);b.emotes.fint=null}}})};a.ui.on("settings.open.ran",b.emotes.configure_page);b.emotes.fetch=function(){if(b.emotes.loaded){b.emotes.picker.loading("Reloading...")}else{b.emotes.picker.loading()}b.emotes.fetching=true;jQuery.getJSON("http://www.thezikes.org/publicemotes.php?format=jsonp&jsoncallback=?&"+(new Date()).getDay(),function(g){b.emotes.fetching=false;b.emotes.emote=g;if(!b.emotes.loaded){if(b.emotes.on){a.ui.pager.notice({ref:"emotes-loaded",heading:"Emote CLOUD",content:"Emoticons from zike's Emote CLOUD have been loaded."},false,5000,true)}}b.emotes.sort();b.emotes.loaded=true;b.emotes.picker.loaded();b.emotes.fint=setTimeout(b.emotes.fetch,3600000)},function(){b.emotes.picker.loaded();return false
})};b.emotes.swap=function(m,g){if(!b.emotes.on){g(m);return}var h=m.input.match(/:([\S]+):/g);var l="";var k=-1;var j=function(n){return n!=h[k]};for(k in h){l=h[k];h=h.filter(j);if(!b.emotes.emote.hasOwnProperty(l)){continue}m.input=replaceAll(m.input,l,":thumb"+b.emotes.emote[l]["devid"]+":")}m.input=replaceAll(m.input,":B",":bucktooth:");g(m)};b.emotes.prepare=function(g){g=replaceAll(g,"&","&amp;");g=replaceAll(g,"<","&lt;");g=replaceAll(g,">","&gt;");return replaceAll(g,'"',"&quot;")};b.emotes.repair=function(g){g=replaceAll(g,"&quot;",'"');g=replaceAll(g,"&lt;","<");g=replaceAll(g,"&gt;",">");return replaceAll(g,"&amp;","&")};b.emotes.sort=function(){var h=[[],[],[],[],[],[],[],[],[],[],[],[],[],[],[],[],[],[],[],[],[],[],[],[],[],[],[],];var n="ABCDEFGHIJKLMNOPQRSTUVWXYZ#";var k=null;var p=null;var l=false;var u=-1;var j=true;for(var o in b.emotes.emote){if(!b.emotes.emote.hasOwnProperty(o)){continue}k=b.emotes.emote[o];var r=replaceAll(k.img.split("/").pop().split(".").shift(),"__"," ");
r=replaceAll(r,"_"," ");var g=wsc.dAmn.Emotes.Thumb(k.devid,{user:k.by,title:r,otitle:"created by "+k.by,aclose:"",anchor:"",thumb:k.img});p={value:'<span class="thumb">'+g+'</span><span class="value">'+b.emotes.prepare(k.code)+"</span>",title:"created by "+k.by,html:true};u=n.indexOf(k.code.substr(1,1).toUpperCase());if(u==-1){u=n.indexOf("#")}h[u].push(p)}var s=function(x,v){return caseInsensitiveSort(x.value,v.value)};var m=b.emotes.picker.page("#");var q=null;for(var o=0;o<n.length;o++){h[o].sort(s);q=b.emotes.picker.page(n[o],m);q.options.items=h[o]}b.emotes.picker.refresh()};b.emotes.select=function(g){a.ui.control.set_text(a.ui.control.get_text()+b.emotes.repair(g))};a.middle("send.msg",b.emotes.swap);a.middle("send.action",b.emotes.swap);a.middle("send.kick",b.emotes.swap);a.middle("send.set",b.emotes.swap);if(!b.emotes.on){return}b.emotes.fetch()};wsc.dAmn.Emotes.Tablumps=function(j){var n={id:j[0],user:j[2],thumb:j[5],server:parseInt(j[4]),flags:j[6].split(":"),dimensions:"",title:"",anchor:"",otitle:j[1]};
