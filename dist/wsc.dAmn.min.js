...
default:if(b.r.indexOf("killed:")<0){return}break}this.process_data({data:"disconnect\ne="+e.r+"\n"})}}else{a.monitor("Couldn't leave "+ns,b.e);c.server_message("Couldn't leave "+ns,b.e)}};wsc.Flow.prototype.kicked=function(b,a){if(b.r.toLowerCase().indexOf("autokicked")>-1){return}a.join(b.ns)};wsc.Flow.prototype.property=function(b,a){chan=a.channel(b.pkt.param);if(!chan){return}chan.property(b)};wsc.Flow.prototype.recv_joinpart=function(b,a){c=a.channel(b.ns);if(b.name=="recv_join"){c.recv_join(b)}else{c.recv_part(b)}};wsc.Flow.prototype.recv_join=wsc.Flow.prototype.recv_joinpart;wsc.Flow.prototype.recv_part=wsc.Flow.prototype.recv_joinpart;wsc.Flow.prototype.recv_msg=function(b,a){a.channel(b.ns).recv_msg(b)};wsc.Flow.prototype.recv_action=wsc.Flow.prototype.recv_msg;wsc.Flow.prototype.recv_npmsg=wsc.Flow.prototype.recv_msg;wsc.Flow.prototype.recv_privchg=function(b,a){a.channel(b.ns).recv_privchg(b)};wsc.Flow.prototype.recv_kicked=function(b,a){a.channel(b.ns).recv_kicked(b)};wsc.defaults.Extension=function(l){var C=function(){l.bind("cmd.set",b);
l.bind("cmd.connect",q);l.bind("cmd.join",H);l.bind("cmd.part",J);l.bind("cmd.title",j);l.bind("cmd.promote",z);l.bind("cmd.demote",p);l.bind("cmd.me",A);l.bind("cmd.kick",B);l.bind("cmd.raw",I);l.bind("cmd.say",v);l.bind("cmd.npmsg",K);l.bind("cmd.clear",F);l.bind("cmd.clearall",E);l.bind("cmd.whois",D);l.bind("pkt.property",a);l.bind("cmd.theme",g);l.ui.on("settings.open",m);l.ui.on("settings.open.ran",G)};var m=function(N,M){page=N.settings.page("Main");var O={};O.theme=replaceAll(l.ui.settings.theme,"wsct_","");O.clock=l.ui.clock();O.tc=l.ui.nav.closer();O.username=l.settings.username;O.pk=l.settings.pk;var L=[];for(i in l.ui.settings.themes){name=replaceAll(l.ui.settings.themes[i],"wsct_","");L.push({value:name,title:name,selected:O.theme==name})}page.item("Text",{ref:"intro",title:"Main",text:"Use this window to view and change your settings.\n\nCheck                    the different pages to see what settings can be changed.",});page.item("Form",{ref:"login",title:"Login",text:"Here you can change the username and token used to                    log into the chat server.",fields:[["Textfield",{ref:"username",label:"Username","default":O.username}],["Textfield",{ref:"token",label:"Token","default":O.pk}]],event:{save:function(P){l.settings.username=P.data.username;
l.settings.pk=P.data.token}}});page.item("Form",{ref:"ui",title:"UI",hint:"<b>Timestamp</b><br/>Choose between a 24 hour clock and                    a 12 hour clock.\n\n<b>Theme</b><br/>Change the look of the                    client.\n\n<b>Close Buttons</b><br/>Turn tab close buttons on/off.",fields:[["Dropdown",{ref:"theme",label:"Theme",items:L}],["Dropdown",{ref:"clock",label:"Timestamp Format",items:[{value:"24",title:"24 hour",selected:O.clock},{value:"12",title:"12 hour",selected:!O.clock}]}],["Check",{ref:"tabclose",label:"Close Buttons",items:[{value:"yes",title:"On",selected:O.tc}]}],],event:{change:function(P){l.ui.clock(P.data.clock=="24");l.ui.theme(P.data.theme);l.ui.nav.closer(P.data.tabclose.indexOf("yes")>-1)},save:function(P){O.clock=P.data.clock=="24";O.theme=P.data.theme;O.tc=P.data.tabclose.indexOf("yes")>-1},close:function(P){l.ui.clock(O.clock);l.ui.theme(O.theme);l.ui.nav.closer(O.tc)}}});page.item("Text",{ref:"debug",wclass:"faint",title:"Debug Information",text:"Chat Agent: <code>"+l.settings.agent+"</code>\n\nUser                    Agent: <code>"+navigator.userAgent+"</code>"})
};var G=function(N,M){var L=N.settings.page("About",true);L.item("Text",{ref:"about-wsc",title:"Wsc",text:'Currently using <a href="http://github.com/photofroggy/wsc/">wsc</a>                    version '+wsc.VERSION+" "+wsc.STATE+'.\n\nWsc                    works using HTML5, javascript, and CSS3. WebSocket is used for the connection                    where possible. The source code for this client is pretty huge.\n\nWsc was created                    by ~<a href="http://photofroggy.deviantart.com/">photofroggy</a>'})};var g=function(M,L){L.ui.theme(M.args.split(" ").shift())};var b=function(N){var M=N.args.split(" ");var L=M.shift().toLowerCase();var M=M.join(" ");if(M.length==0){l.cchannel.serverMessage("Could not set "+L,"No data supplied");return}if(!(L in l.settings)){l.cchannel.serverMessage('Unknown setting "'+L+'"');return}l.settings[L]=M;l.cchannel.serverMessage("Changed "+L+" setting","value: "+M);l.control.setLabel()};var q=function(L){l.connect()};var H=function(L){var M=L.args.split(" ");
var M=M.toString()==""?[]:M;if(L.ns!=L.target){M.unshift(L.target)}if(M.toString()==""){return}for(index in M){l.join(M[index])}};var J=function(L){var M=L.args.split(" ");if(L.ns!=L.target){M.unshift(L.target)}if(M.toString()==""){l.part(L.ns);return}for(index in M){l.part(M[index])}};var j=function(L){l.set(L.target,"title",L.args)};var z=function(M){var L=M.args.split(" ");l.promote(M.target,L[0],L[1])};var p=function(M){var L=M.args.split(" ");l.demote(M.target,L[0],L[1])};var A=function(L){l.action(L.target,L.args)};var I=function(L){l.send(L.args.replace(/\\n/gm,"\n"))};var B=function(L){d=L.args.split(" ");u=d.shift();r=d.length>0?d.join(" "):null;l.kick(L.target,u,r)};var v=function(L){l.say(L.target,L.args)};var K=function(L){l.npmsg(L.target,L.args)};var F=function(M,L){L.cchannel.clear()};var E=function(M,L){for(c in L.channelo){L.channelo[c].clear()}};var D=function(M,L){L.whois(M.args.split(" ")[0])};var a=function(M,L){if(M.p!="info"){return}subs=M.pkt.sub;data=subs.shift().arg;
data.username=M.sns.substr(1);data.connections=[];while(subs.length>0){conn=subs.shift().arg;conn.channels=[];while(subs.length>0){if(subs[0].cmd!="ns"){break}conn.channels.unshift(L.deform_ns(subs.shift().param))}data.connections.push(conn)}L.cchannel.show_whois(data)};C()};wsc.Client=function(a,g,j){this.mozilla=j;this.fresh=true;this.attempts=0;this.protocol=null;this.flow=null;this.ui=null;this.events=new EventEmitter();this.conn=null;this.channelo={};this.cchannel=null;this.cmds=[];this.settings={domain:"website.com",server:"ws://website.com/wsendpoint",symbol:"",username:"",userinfo:{},pk:"",monitor:["~Monitor",true],welcome:"Welcome to the wsc web client!",autojoin:"chat:channel",control:wsc.Control,protocol:wsc.Protocol,tablumps:wsc.Tablumps,flow:wsc.Flow,ui:Chatterbox.UI,extend:[wsc.defaults.Extension],client:"chatclient",clientver:"0.3",theme:wsc.defaults.theme,themes:wsc.defaults.themes,};a.extend(this.settings,g);this.ui=new this.settings.ui(a,{themes:this.settings.themes,theme:this.settings.theme,monitor:this.settings.monitor,username:this.settings.username,domain:this.settings.domain},j);
...
this.logpanel.find("li.loginfo").remove();this.wrap.height(0);this.resize()};Chatterbox.Channel.prototype.log_info=function(g,b){data={ns:this.namespace,ref:g,content:b};this.manager.trigger("log_info.before",data);delete data.ns;this.wrap.append(Chatterbox.render("loginfobox",data));this.scroll();var j=this;var a=this.wrap.find("li."+data.ref);a.find("a.close").click(function(l){j.wrap.find(this).parent().remove();j.resize();return false});return a};Chatterbox.Channel.prototype.show_whois=function(q){var j={avatar:'<a href="#"><img height="50" width="50" alt="avatar"/></a>',username:"<b>"+q.symbol+q.username+"</b>",info:[],conns:[],raw:q,};for(var v in q.connections){var p=q.connections[v];var A=[];if(p.online){stamp=(new Date-(p.online*1000));A.push(["online",DateStamp(stamp/1000)+formatTime(" [{HH}:{mm}:{ss}]",new Date(stamp))])}if(p.idle){A.push(["idle",timeLengthString(p.idle)])}if(p.agent){A.push(["agent",p.agent])}if(p.debug){A.push(["debug",p.debug])}A.push(["chatrooms",p.channels.join(" ")]);
j.conns.push(A)}this.manager.trigger("log_whois.before",j);var a="";for(var v in j.conns){var m=j.conns[v];var B='<section class="conn"><p><em>connection '+((parseInt(v)+1).toString())+":</em></p>";B+="<ul>";for(x in m){B+="<li><strong>"+m[x][0]+":</strong> "+m[x][1]+"</li>"}B+="</ul>";a+=B+"</section>"}var l="";for(var v in j.info){l+="<li>"+j.info[v]+"</li>"}var z=this.log_info("whois-"+q.username,Chatterbox.render("whoiswrap",{avatar:j.avatar,info:Chatterbox.render("whoisinfo",{username:j.username,info:l,connections:a})}));var g=z.find("div.avatar");var b=z.find("div.info");b.width(z.find(".whoiswrap").width()-100);g.height(z.height()-10);this.scroll()};Chatterbox.Channel.prototype.set_header=function(a,b){headd=this.window.find("header div."+a);headd.replaceWith(Chatterbox.render("header",{head:a,content:b||""}));headd=this.window.find("header div."+a);if(b){headd.css({display:"block"})}else{this.window.find("header div."+a).css({display:"none"})}this.resize()};Chatterbox.Channel.prototype.get_header=function(a){return this.window.find("header div."+a)
