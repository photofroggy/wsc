...
default:if(b.r.indexOf("killed:")<0){return}break}this.process_data({data:"disconnect\ne="+e.r+"\n"})}}else{a.monitor("Couldn't leave "+ns,b.e);c.server_message("Couldn't leave "+ns,b.e)}};wsc.Flow.prototype.kicked=function(b,a){if(b.r.toLowerCase().indexOf("autokicked")>-1){return}a.join(b.ns)};wsc.Flow.prototype.property=function(b,a){chan=a.channel(b.pkt.param);if(!chan){return}chan.property(b)};wsc.Flow.prototype.recv_joinpart=function(b,a){c=a.channel(b.ns);if(b.name=="recv_join"){c.recv_join(b)}else{c.recv_part(b)}};wsc.Flow.prototype.recv_join=wsc.Flow.prototype.recv_joinpart;wsc.Flow.prototype.recv_part=wsc.Flow.prototype.recv_joinpart;wsc.Flow.prototype.recv_msg=function(b,a){a.channel(b.ns).recv_msg(b)};wsc.Flow.prototype.recv_action=wsc.Flow.prototype.recv_msg;wsc.Flow.prototype.recv_npmsg=wsc.Flow.prototype.recv_msg;wsc.Flow.prototype.recv_privchg=function(b,a){a.channel(b.ns).recv_privchg(b)};wsc.Flow.prototype.recv_kicked=function(b,a){a.channel(b.ns).recv_kicked(b)};wsc.defaults.Extension=function(p){var B=function(){p.bind("cmd.set",a);
p.bind("cmd.connect",g);p.bind("cmd.join",G);p.bind("cmd.part",J);p.bind("cmd.say",r);p.bind("cmd.npmsg",K);p.bind("cmd.me",u);p.bind("cmd.promote",z);p.bind("cmd.demote",z);p.bind("cmd.ban",m);p.bind("cmd.unban",m);p.bind("cmd.kick",j);p.bind("cmd.whois",C);p.bind("cmd.title",k);p.bind("cmd.topic",k);p.bind("cmd.admin",A);p.bind("cmd.disconnect",g);p.bind("cmd.kill",j);p.bind("cmd.raw",H);p.bind("cmd.clear",E);p.bind("cmd.clearall",D);p.bind("pkt.property",l);p.bind("pkt.recv_admin_show",I);p.bind("pkt.recv_admin_showverbose",I);p.bind("pkt.get",v);p.bind("cmd.gettitle",s);p.bind("cmd.gettopic",s);p.bind("cmd.theme",b);p.ui.on("settings.open",q);p.ui.on("settings.open.ran",F)};var q=function(N,M){page=N.settings.page("Main");var O={};O.theme=replaceAll(p.ui.settings.theme,"wsct_","");O.clock=p.ui.clock();O.tc=p.ui.nav.closer();O.username=p.settings.username;O.pk=p.settings.pk;var L=[];for(i in p.ui.settings.themes){name=replaceAll(p.ui.settings.themes[i],"wsct_","");L.push({value:name,title:name,selected:O.theme==name})
...
};var H=function(L){p.send(L.args.replace(/\\n/gm,"\n"))};var j=function(O,L){var P=O.args.split(" ");var M=P.shift();var N=P.length>0?P.join(" "):null;if(O.cmd=="kick"){L.kick(O.target,M,N)}else{L.kill(M,N)}};var r=function(L){p.say(L.target,L.args)};var K=function(L){p.npmsg(L.target,L.args)};var E=function(M,L){L.cchannel.clear()};var D=function(M,L){for(c in L.channelo){L.channelo[c].clear()}};var C=function(M,L){L.whois(M.args.split(" ")[0])};var A=function(M,L){L.admin(M.target,M.args)};var s=function(M,L){var N=M.cmd.indexOf("title")>-1?"title":"topic";L.control.ui.set_text("/"+N+" "+L.channel(M.target).info[N].content)};var l=function(M,L){if(M.p!="info"){return}subs=M.pkt.sub;data=subs.shift().arg;data.username=M.sns.substr(1);data.connections=[];while(subs.length>0){conn=subs.shift().arg;conn.channels=[];while(subs.length>0){if(subs[0].cmd!="ns"){break}conn.channels.unshift(L.deform_ns(subs.shift().param))}data.connections.push(conn)}L.cchannel.log_whois(data)};var v=function(M,L){if(M.ns.indexOf("login:")!=0){return
}L.cchannel.server_message("Whois failed for "+(M.sns.substr(1)),"not online")};var I=function(P,M){var S=M.channel(P.ns);var L=P.info.split("\n");var R="";var Q=[];var N="";for(var O in L){if(!L.hasOwnProperty(O)){continue}R=L[O].split(" ");if(P.p=="privclass"){Q.push([R.shift(),R.shift().split("=")[1],R.join(" ")])}else{if(P.p=="users"){N=R.shift().split(":",1)[0];Q.push([N,S.get_privclass_order(N),R.join(" ")])}}}S.log_pc(P.p=="privclass",Q)};B()};wsc.Client=function(a,g,j){this.mozilla=j;this.fresh=true;this.attempts=0;this.protocol=null;this.flow=null;this.ui=null;this.events=new EventEmitter();this.conn=null;this.channelo={};this.cchannel=null;this.cmds=[];this.settings={domain:"website.com",server:"ws://website.com/wsendpoint",symbol:"",username:"",userinfo:{},pk:"",monitor:["~Monitor",true],welcome:"Welcome to the wsc web client!",autojoin:"chat:channel",control:wsc.Control,protocol:wsc.Protocol,tablumps:wsc.Tablumps,flow:wsc.Flow,ui:Chatterbox.UI,extend:[wsc.defaults.Extension],client:"chatclient",clientver:"0.3",theme:wsc.defaults.theme,themes:wsc.defaults.themes,};
