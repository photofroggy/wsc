...
p.bind("cmd.connect",g);p.bind("cmd.join",G);p.bind("cmd.part",I);p.bind("cmd.say",r);p.bind("cmd.npmsg",J);p.bind("cmd.me",u);p.bind("cmd.promote",z);p.bind("cmd.demote",z);p.bind("cmd.ban",m);p.bind("cmd.unban",m);p.bind("cmd.kick",j);p.bind("cmd.whois",C);p.bind("cmd.title",k);p.bind("cmd.topic",k);p.bind("cmd.admin",A);p.bind("cmd.disconnect",g);p.bind("cmd.kill",j);p.bind("cmd.raw",H);p.bind("cmd.clear",E);p.bind("cmd.clearall",D);p.bind("pkt.property",l);p.bind("pkt.get",v);p.bind("cmd.gettopic",s);p.bind("cmd.theme",b);p.ui.on("settings.open",q);p.ui.on("settings.open.ran",F)};var q=function(M,L){page=M.settings.page("Main");var N={};N.theme=replaceAll(p.ui.settings.theme,"wsct_","");N.clock=p.ui.clock();N.tc=p.ui.nav.closer();N.username=p.settings.username;N.pk=p.settings.pk;var K=[];for(i in p.ui.settings.themes){name=replaceAll(p.ui.settings.themes[i],"wsct_","");K.push({value:name,title:name,selected:N.theme==name})}page.item("Text",{ref:"intro",title:"Main",text:"Use this window to view and change your settings.\n\nCheck                    the different pages to see what settings can be changed.",});
page.item("Form",{ref:"login",title:"Login",text:"Here you can change the username and token used to                    log into the chat server.",fields:[["Textfield",{ref:"username",label:"Username","default":N.username}],["Textfield",{ref:"token",label:"Token","default":N.pk}]],event:{save:function(O){p.settings.username=O.data.username;p.settings.pk=O.data.token}}});page.item("Form",{ref:"ui",title:"UI",hint:"<b>Timestamp</b><br/>Choose between a 24 hour clock and                    a 12 hour clock.\n\n<b>Theme</b><br/>Change the look of the                    client.\n\n<b>Close Buttons</b><br/>Turn tab close buttons on/off.",fields:[["Dropdown",{ref:"theme",label:"Theme",items:K}],["Dropdown",{ref:"clock",label:"Timestamp Format",items:[{value:"24",title:"24 hour",selected:N.clock},{value:"12",title:"12 hour",selected:!N.clock}]}],["Check",{ref:"tabclose",label:"Close Buttons",items:[{value:"yes",title:"On",selected:N.tc}]}],],event:{change:function(O){p.ui.clock(O.data.clock=="24");
p.ui.theme(O.data.theme);p.ui.nav.closer(O.data.tabclose.indexOf("yes")>-1)},save:function(O){N.clock=O.data.clock=="24";N.theme=O.data.theme;N.tc=O.data.tabclose.indexOf("yes")>-1},close:function(O){p.ui.clock(N.clock);p.ui.theme(N.theme);p.ui.nav.closer(N.tc)}}});page.item("Text",{ref:"debug",wclass:"faint",title:"Debug Information",text:"Chat Agent: <code>"+p.settings.agent+"</code>\n\nUser                    Agent: <code>"+navigator.userAgent+"</code>"})};var F=function(M,L){var K=M.settings.page("About",true);K.item("Text",{ref:"about-wsc",title:"Wsc",text:'Currently using <a href="http://github.com/photofroggy/wsc/">wsc</a>                    version '+wsc.VERSION+" "+wsc.STATE+'.\n\nWsc                    works using HTML5, javascript, and CSS3. WebSocket is used for the connection                    where possible. The source code for this client is pretty huge.\n\nWsc was created                    by ~<a href="http://photofroggy.deviantart.com/">photofroggy</a>'})};var b=function(L,K){K.ui.theme(L.args.split(" ").shift())
};var a=function(M){var L=M.args.split(" ");var K=L.shift().toLowerCase();var L=L.join(" ");if(L.length==0){p.cchannel.serverMessage("Could not set "+K,"No data supplied");return}if(!(K in p.settings)){p.cchannel.serverMessage('Unknown setting "'+K+'"');return}p.settings[K]=L;p.cchannel.serverMessage("Changed "+K+" setting","value: "+L);p.control.setLabel()};var g=function(K){p[K.cmd]()};var G=function(K){var L=K.args.split(" ");var L=L.toString()==""?[]:L;if(K.ns!=K.target){L.unshift(K.target)}if(L.toString()==""){return}for(index in L){p.join(L[index])}};var I=function(K){var L=K.args.split(" ");if(K.ns!=K.target){L.unshift(K.target)}if(L.toString()==""){p.part(K.ns);return}for(index in L){p.part(L[index])}};var k=function(K){p.set(K.target,K.cmd,K.args)};var z=function(L){var K=L.args.split(" ");p[L.cmd.toLowerCase()](L.target,K[0],K[1])};var m=function(O,K){var M=O.args.split(" ");var L=M.shift();var N=O.cmd;if(N=="ban"&&M.length>0){K.kick(O.target,L,M.join(" "))}K[N](O.target,L)};var u=function(K){p.action(K.target,K.args)
};var H=function(K){p.send(K.args.replace(/\\n/gm,"\n"))};var j=function(N,K){var O=N.args.split(" ");var L=O.shift();var M=O.length>0?O.join(" "):null;if(N.cmd=="kick"){K.kick(N.target,L,M)}else{K.kill(L,M)}};var r=function(K){p.say(K.target,K.args)};var J=function(K){p.npmsg(K.target,K.args)};var E=function(L,K){K.cchannel.clear()};var D=function(L,K){for(c in K.channelo){K.channelo[c].clear()}};var C=function(L,K){K.whois(L.args.split(" ")[0])};var A=function(L,K){K.admin(L.target,L.args)};var s=function(L,K){var M=L.cmd.indexOf("title")>-1?"title":"topic";K.control.ui.set_text("/"+M+" "+K.channel(L.target).info[M].content)};var l=function(L,K){if(L.p!="info"){return}subs=L.pkt.sub;data=subs.shift().arg;data.username=L.sns.substr(1);data.connections=[];while(subs.length>0){conn=subs.shift().arg;conn.channels=[];while(subs.length>0){if(subs[0].cmd!="ns"){break}conn.channels.unshift(K.deform_ns(subs.shift().param))}data.connections.push(conn)}K.cchannel.show_whois(data)};var v=function(L,K){if(L.ns.indexOf("login:")!=0){return
}K.cchannel.server_message("Whois failed for "+(L.sns.substr(1)),"not online")};B()};wsc.Client=function(a,g,j){this.mozilla=j;this.fresh=true;this.attempts=0;this.protocol=null;this.flow=null;this.ui=null;this.events=new EventEmitter();this.conn=null;this.channelo={};this.cchannel=null;this.cmds=[];this.settings={domain:"website.com",server:"ws://website.com/wsendpoint",symbol:"",username:"",userinfo:{},pk:"",monitor:["~Monitor",true],welcome:"Welcome to the wsc web client!",autojoin:"chat:channel",control:wsc.Control,protocol:wsc.Protocol,tablumps:wsc.Tablumps,flow:wsc.Flow,ui:Chatterbox.UI,extend:[wsc.defaults.Extension],client:"chatclient",clientver:"0.3",theme:wsc.defaults.theme,themes:wsc.defaults.themes,};a.extend(this.settings,g);this.ui=new this.settings.ui(a,{themes:this.settings.themes,theme:this.settings.theme,monitor:this.settings.monitor,username:this.settings.username,domain:this.settings.domain},j);this.settings.agent=this.ui.LIB+"/"+this.ui.VERSION+" ("+navigator.appVersion.match(/\(([^)]+)\)/)[1]+") wsc/"+wsc.VERSION;
