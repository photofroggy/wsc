...
}T.connections.push(R)}P.cchannel.log_whois(T)};var z=function(Q,P){if(Q.ns.indexOf("login:")!=0){return}P.cchannel.server_message("Whois failed for "+(Q.sns.substr(1)),"not online")};var M=function(T,Q){var W=Q.channel(T.ns);var P=T.info.split("\n");var V="";var U=[];var R="";for(var S in P){if(!P.hasOwnProperty(S)){continue}V=P[S].split(" ");if(T.p=="privclass"){U.push([V.shift(),V.shift().split("=")[1],V.join(" ")])}else{if(T.p=="users"){R=V.shift().split(":",1)[0];U.push([R,W.get_privclass_order(R),V.join(" ")])}}}W.log_pc(T.p=="privclass",U)};E();wsc.defaults.Extension.Ignore(p);wsc.defaults.Extension.Away(p);wsc.defaults.Extension.Autojoin(p)};wsc.defaults.Extension.Autojoin=function(a){var g=a.autojoin;a.ui.control.add_button(function(){for(var k in a.autojoin.channel){if(!a.autojoin.channel.hasOwnProperty(k)){continue}a.join(a.autojoin.channel[k])}},{label:"Autojoin",title:"Join your autojoin channels",href:"#autojoin-do"});var j=function(){a.bind("cmd.autojoin",b);a.ui.on("settings.open",g.page)
...
}}arg=bits.join(" ");this.client.trigger("cmd."+cmdn,{name:"cmd",cmd:cmdn,args:arg,target:etarget,ns:ens})};var Chatterbox={};Chatterbox.VERSION="0.7.52";Chatterbox.STATE="beta";Chatterbox.UI=function(a,b,j,g){this.events=g||new EventEmitter();this.mozilla=j;this.umuted=[];this.settings={themes:["wsct_default","wsct_dAmn"],theme:"wsct_default",monitor:["~Monitor",true],username:"",domain:"website.com",clock:true,tabclose:true};a.extend(this.settings,b);a.append('<div class="wsc '+this.settings.theme+'"></div>');this.view=a.find(".wsc");this.mns=this.format_ns(this.settings.monitor[0]);this.lun=this.settings.username.toLowerCase();this.monitoro=null;this.swidth=(function(){if($.browser.msie){var m=$('<textarea cols="10" rows="2"></textarea>').css({position:"absolute",top:-1000,left:-1000}).appendTo("body"),l=$('<textarea cols="10" rows="2" style="overflow: hidden;"></textarea>').css({position:"absolute",top:-1000,left:-1000}).appendTo("body");scrollbarWidth=m.width()-l.width();m.add(l).remove()
...
};Chatterbox.Control.prototype.set_handlers=function(b,a){if(this.manager.mozilla){this.input.keypress(b||this._onkeypress);this.mli.keypress(b||this._onkeypress)}else{this.input.keydown(b||this._onkeypress);this.mli.keydown(b||this._onkeypress)}this.form.submit(a||this._onsubmit)};Chatterbox.Control.prototype.multiline=function(a){if(a==undefined||this.ml==a){return this.ml}this.ml=a;if(this.ml){this.input.css("display","none");this.mli.css("display","inline-block");this.ci=this.mli;this.manager.resize();return this.ml}this.mli.css("display","none");this.input.css("display","inline-block");this.ci=this.input;this.manager.resize();return this.mli};Chatterbox.Control.prototype.add_button=function(g,a){a=Object.extend({label:"New",icon:false,href:"#button",title:"Button."},(a||{}));if(a.icon!==false){a.icon=" iconic "+a.icon}else{a.icon=" text"}this.brow.append(Chatterbox.render("control_button",a));var b=this.brow.find("a[href="+a.href+"].button");b.click(function(j){g();return false})};
