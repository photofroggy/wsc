...
}chans++}return chans};Chatterbox.Chatbook.prototype.create_channel=function(b,f,a){var g=this.channel(b,this.channel_object(b,f,a));g.build();if(this.trail.indexOf(g.namespace)==-1){this.trail.push(g.namespace)}if(!g.visible){this.toggle_channel(b)}this.manager.resize();return g};Chatterbox.Chatbook.prototype.create_feed=function(a,g,b){var f=this.channel(a,this.feed_object(a,g,b));f.build();if(this.trail.indexOf(f.namespace)==-1){this.trail.push(f.namespace)}if(!f.visible){this.toggle_channel(a)}this.manager.resize();return f};Chatterbox.Chatbook.prototype.channel_object=function(a){return new Chatterbox.Channel(this.manager,a)};Chatterbox.Chatbook.prototype.feed_object=function(a,f,b){return new Chatterbox.Feed(this.manager,a,f,b)};Chatterbox.Chatbook.prototype.toggle_channel=function(a){var f=this.channel(a);var b=f;if(!f){return}if(f.hidden){if(this.current&&this.current==f){return}if(!this.manager.settings.developer){f.hide();return}}if(this.current){if(this.current==f){return}this.current.hide();
