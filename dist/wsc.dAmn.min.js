...
f.ui.pager.remove_notice(o);return false}}}},true);a[r.user]=o;o.onclose=function(){f.npmsg(r.ns,"CDS:LINK:REJECT:"+r.user)}},clrj:function(q){var n=q.user.toLowerCase();var r=q.payload.split(",");var o=r.shift();r=r.join(",");if(o.toLowerCase()!=f.settings.username.toLowerCase()){return}if(!(n in h)){return}clearTimeout(h[n]);f.channel("@"+n).server_message("Chat request rejected",r)},clra:function(q){var n=q.user.toLowerCase();var r=q.payload.split(",");var o=r.shift();if(o.toLowerCase()!=f.settings.username.toLowerCase()){return}clearTimeout(h[n])},pcp:function(p){if(p.ns.indexOf("pchat")!=0){return}if(f.bds.channel.contains(p.ns)){return}if(p.pkt.arg.p!="members"){return}if(f.channel(p.ns).get_usernames().length==2){f.bds.channel.add(p.ns);return}var n=p.sns.substr(1);var o=p.ns;f.channel(o).server_message(n+" has not yet joined","Sending them a notice...");f.npmsg("chat:datashare","CDS:LINK:REQUEST:"+n);h[n.toLowerCase()]=setTimeout(function(){try{f.channel(o).server_message("Notification failed",n+" does not seem to be using a client that supports pchat notices.")
...
}var j=f.bds.link.conn(k.sns.substr(1));j.state=f.bds.link.state.closed;var l={name:"BDS.LINK.CLOSED",link:j,ns:j.ns};j.onclose(l,a);a.trigger("BDS.LINK.CLOSED",l);delete f.bds.link.connections[j.un]},recv_join:function(k){if(!f.bds.channel.contains(k.ns)){return}var j=f.bds.link.conn(k.sns.substr(1));if(j.state!=f.bds.link.state.opening){return}j.state=f.bds.link.state.open;var l={name:"BDS.LINK.OPEN",link:j,ns:j.ns};j.onopen(l,a);a.trigger("BDS.LINK.OPEN",l)},recv_part:function(k){if(!f.bds.channel.contains(k.ns)){return}var j=f.bds.link.conn(k.sns.substr(1));j.close(true)},};g()};wsc.dAmn.BDS.Peer=function(a,h,b){var g=function(){a.bind("BDS.PEER.REQUEST",function(k,j){f.signal.request(k,j)});a.bind("BDS.PEER.ACK",function(k,j){f.signal.ack(k,j)});a.bind("BDS.PEER.REJECT",function(k,j){f.signal.reject(k,j)});a.bind("BDS.PEER.ACCEPT",function(k,j){f.signal.accept(k,j)});a.bind("BDS.PEER.OPEN",function(k,j){f.signal.open(k,j)});a.bind("BDS.PEER.END",function(k,j){f.signal.end(k,j)});a.bind("BDS.PEER.OFFER",function(k,j){f.signal.offer(k,j)
});a.bind("BDS.PEER.ANSWER",function(k,j){f.signal.answer(k,j)});a.bind("BDS.PEER.CLOSE",function(k,j){f.signal.close(k,j)})};var f={signal:{request:function(l,j){if(l.sns[0]!="@"){return}var k=l.param[0];var m=l.param[1];if(j.ui.umuted.indexOf(k.toLowerCase())!=-1){j.npmsg(l.ns,"BDS:PEER:REJECT:"+m+","+k+",You have been blocked");return}if(j.away.on){j.npmsg(l.ns,"BDS:PEER:REJECT:"+m+","+k+",Away; "+j.away.reason);return}if(phone.call!=null){if(!phone.call.group){j.npmsg(l.ns,"BDS:PEER:REJECT:"+m+","+k+",Already in a call");return}}j.npmsg(l.ns,"BDS:PEER:ACK:"+m+","+k);phone.incoming(m,k,l)},ack:function(k,j){},reject:function(k,j){if(k.sns[0]!="@"){return}},accept:function(m,j){if(m.sns[0]!="@"){return}if(!phone.call){return}var l=dVideo.phone.call;var p=m.param[0];var k=m.param[1];var o=m.param[2]||m.ns;if(k.toLowerCase()!=j.settings.username.toLowerCase()){return}var n=phone.call.new_peer(p,m.user);if(!n){return}n.conn.ready(function(){dVideo.signal.offer(n)})},open:function(k,j){},end:function(k,j){},offer:function(n,j){if(n.sns[0]!="@"){return
}if(!phone.call){return}var l=phone.call;var q=n.param[0];var k=n.param[1];var p=n.param[2];var m=new dVideo.RTC.SessionDescription(JSON.parse(n.param.slice(3).join(",")));if(p.toLowerCase()!=j.settings.username.toLowerCase()){return}if(j.ui.umuted.indexOf(k.toLowerCase())!=-1){dVideo.signal.reject(k,"You have been blocked");return}if(j.away.on){dVideo.signal.reject(k,"Away, reason: "+j.away.reason);return}var o=l.peer(k);if(!o){if(!l.group){return}o=l.new_peer(q,k)}o.conn.ready(function(){dVideo.signal.answer(o);console.log("new peer",o.user)},m)},answer:function(n,j){if(n.sns[0]!="@"){return}if(!phone.call){return}var l=phone.call;var q=n.param[0];var k=n.param[1];var p=n.param[2];var m=new dVideo.RTC.SessionDescription(JSON.parse(n.param.slice(3).join(",")));if(p.toLowerCase()!=j.settings.username.toLowerCase()){return}var o=l.peer(k);if(!o){return}o.conn.open(function(){console.log("> connected to new peer "+o.user)},m)},candidate:function(n,j){if(n.sns[0]!="@"){return}if(!phone.call){return
}var m=phone.call;var q=n.param[0];var k=n.param[1];var p=n.param[2];var l=new dVideo.RTC.SessionDescription(JSON.parse(n.param.slice(3).join(",")));if(p.toLowerCase()!=j.settings.username.toLowerCase()){return}var o=m.peer(k);if(!o){return}o.conn.candidate(l)},close:function(k,j){},}};g()};wsc.dAmn.BDS.Peer.bots=["botdom","damnphone"];wsc.dAmn.BDS.Peer.peer_options={iceServers:[{url:"stun:stun.l.google.com:19302"}]};wsc.dAmn.BDS.Peer.RTC={PeerConnection:null,SessionDescription:null,IceCandidate:null,};wsc.dAmn.BDS.Peer._gum=function(){};wsc.dAmn.BDS.Peer.getUserMedia=function(b,f,a){return wsc.dAmn.BDS.Peer._gum(b,f,a)};if(window.mozRTCPeerConnection){wsc.dAmn.BDS.Peer.RTC.PeerConnection=mozRTCPeerConnection;wsc.dAmn.BDS.Peer.RTC.SessionDescription=mozRTCSessionDescription;wsc.dAmn.BDS.Peer.RTC.IceCandidate=mozRTCIceCandidate;wsc.dAmn.BDS.Peer._gum=function(b,f,a){return navigator.mozGetUserMedia(b,f,a)}}if(window.webkitRTCPeerConnection){wsc.dAmn.BDS.Peer.RTC.PeerConnection=webkitRTCPeerConnection;
wsc.dAmn.BDS.Peer._gum=function(b,f,a){return navigator.webkitGetUserMedia(b,f,a)}}if(window.RTCPeerConnection){wsc.dAmn.BDS.Peer.RTC.PeerConnection=RTCPeerConnection;wsc.dAmn.BDS.Peer._gum=function(b,f,a){return navigator.getUserMedia(b,f,a)}}if(window.RTCSessionDescription){wsc.dAmn.BDS.Peer.RTC.SessionDescription=RTCSessionDescription;wsc.dAmn.BDS.Peer.RTC.IceCandidate=RTCIceCandidate}wsc.dAmn.BDS.Peer.connection=function(a,b){if(!wsc.dAmn.BDS.Peer.RTC.PeerConnection){return null}return new wsc.dAmn.BDS.Peer.Connection(a,b)};wsc.dAmn.BDS.Peer.Connection=function(a,b){this.user=a;this.pc=new wsc.dAmn.BDS.Peer.RTC.PeerConnection(wsc.dAmn.BDS.Peer.peer_options);this.offer="";this.remote_offer=b||null;this.responding=this.remote_offer!=null;this.streamed=false;this.bindings();if(this.remote_offer){this.set_remote_description(this.remote_offer)}};wsc.dAmn.BDS.Peer.Connection.prototype.bindings=function(){var b=this;var a=this.user;this.pc.onicecandidate=function(g){wsc.dAmn.BDS.Peer.signal.candidate(wsc.dAmn.BDS.Peer.phone.call.peer(a),g)
};var f=function(){};this.onready=f;this.onopen=f};wsc.dAmn.BDS.Peer.Connection.prototype.ready=function(a,g){this.onready=a||this.onready;this.remote_offer=g||this.remote_offer;this.responding=this.remote_offer!=null;if(this.responding){var f=this.onopen;var b=this;this.onopen=function(){b.answer();b.onopen=f};this.set_remote_description(this.remote_offer);return}this.create_offer()};wsc.dAmn.BDS.Peer.Connection.prototype.open=function(a,b){if(!this.offer){return}this.remote_offer=b||this.remote_offer;this.onopen=a;if(!this.remote_offer){return}this.set_remote_description(this.remote_offer)};wsc.dAmn.BDS.Peer.Connection.prototype.close=function(){this.pc.close()};wsc.dAmn.BDS.Peer.Connection.prototype.onerror=function(a){console.log(">> Got an error:",'"',a.message,'"',a)};wsc.dAmn.BDS.Peer.Connection.prototype.candidate=function(a){this.pc.addIceCandidate(a)};wsc.dAmn.BDS.Peer.Connection.prototype.create_offer=function(){var a=this;this.pc.createOffer(function(b){a.offer_created(b)},function(b){a.onerror(b)
})};wsc.dAmn.BDS.Peer.Connection.prototype.offer_created=function(b){this.offer=b;var a=this;this.pc.setLocalDescription(this.offer,function(){a.local_description_set()},this.onerror)};wsc.dAmn.BDS.Peer.Connection.prototype.set_remote_description=function(b){this.remote_offer=b;var a=this;this.pc.setRemoteDescription(this.remote_offer,function(){a.remote_description_set()},this.onerror)};wsc.dAmn.BDS.Peer.Connection.prototype.local_description_set=function(){this.onready()};wsc.dAmn.BDS.Peer.Connection.prototype.remote_description_set=function(){this.onopen()};wsc.dAmn.BDS.Peer.Connection.prototype.answer=function(){var a=this;this.responding=true;this.pc.createAnswer(function(b){a.answer_created(b)},function(b){a.onerror(b)})};wsc.dAmn.BDS.Peer.Connection.prototype.answer_created=function(b){this.offer=b;var a=this;this.pc.setLocalDescription(this.offer,function(){a.local_description_set()},function(f){a.onerror(f)})};wsc.dAmn.Stash=function(a,h,b){var g=function(){a.ui.on("log_item.after",f.log_item)
};var f={log_item:function(k){var j=k.item.find('a[href^="http://sta.sh"]');j.each(function(m,l){var n=k.item.find(l);wsc.dAmn.Stash.fetch(k,n)})}};g()};wsc.dAmn.Stash.fetch=function(b,a){$.getJSON("http://backend.deviantart.com/oembed?url="+a.prop("href")+"&format=jsonp&callback=?",function(f){wsc.dAmn.Stash.render(b,a,f)})};wsc.dAmn.Stash.render=function(r,j,x){if("error" in x){return}if(x.type!="photo"){return}var f=j.prop("href");var l=x.thumbnail_width;var s=x.thumbnail_height;var v,b;var y=x.title+" by "+x.author_name;var k='<a target="_blank" href="'+f+'" title="'+y+'">';if(l/s>1){b=parseInt((s*100)/l);v=100}else{v=parseInt((l*100)/s);b=100}if(v>l||b>s){v=l;b=s}var q='width="'+v+'" height="'+b+'"';var g=!x.thumbnail_url.match(/.png$/i)?" shadow":"";var o=f.split("/").pop();var u='<span class="stashthumb" id="'+o+'">'+k+'<img class="smaller thumb'+g+'"'+q+' " alt="'+f+'" src="'+x.thumbnail_url_150+'" />        </a><a href="#toggle" class="button iconic plus" title="Larger"></a></span>';
