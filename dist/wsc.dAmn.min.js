...
};Chatterbox.Settings.Item.prototype.resize=function(){for(var a in this.items){this.items[a].resize()}};Chatterbox.Settings.Item.prototype.hooks=function(g){if(!this.options.hasOwnProperty("event")){return}var b=this.options.event;var k=Chatterbox.template.settings.item.get(this.selector);if(k==false){return}if(!k.hasOwnProperty("events")){return}var j=[];for(var a in k.events){j=k.events[a];if(!b.hasOwnProperty(j[0])){continue}g.find(j[1]).bind(j[0],b[j[0]])}};Chatterbox.Settings.Item.prototype._event_stub=function(){};Chatterbox.Settings.Item.prototype._get_cb=function(a){if(!this.options.hasOwnProperty("event")){return this._event_stub}return this.options.event[a]||this._event_stub};Chatterbox.Settings.Item.prototype._get_ep=function(b){var j=Chatterbox.template.settings.item.get(this.selector);if(j==null){return false}if(!j.hasOwnProperty("events")){return false}var g=[];for(var a in j.events){g=j.events[a];if(g[0]==b){return g}}};Chatterbox.Settings.Item.prototype.save=function(j,k){var l=this._get_ep("inspect");
var b=l?this.view.find(l[1]):null;var a=this._get_cb("save");if(typeof a=="function"){a({input:b,item:this,page:k,window:j})}else{for(var g in a){var m=b.hasOwnProperty("slice")?b.slice(g,1):b;a[g]({input:m,item:this,page:k,window:j})}}for(var g in this.items){this.items[g].save(j,k)}};Chatterbox.Settings.Item.prototype.close=function(j,k){var l=this._get_ep("inspect");var b=l?this.view.find(l[1]):null;var a=this._get_cb("close");if(typeof a=="function"){a({input:b,item:this,page:k,window:j});return}for(var g in a){var m=b.hasOwnProperty("slice")?b.slice(g,1):b;a[g]({input:m,item:this,page:k,window:j})}for(var g in this.items){this.items[g].close(j,k)}};Chatterbox.Settings.Item.get=function(k,q,l,b,a){var j=k.split(".");var m=b||Chatterbox.Settings.Item;var p;for(var g in j){p=j[g];if(!m.hasOwnProperty(p)){m=a||Chatterbox.Settings.Item;break}m=m[p]}return new m(k,q,l)};Chatterbox.Settings.Item.Form=function(b,a){Chatterbox.Settings.Item.call(this,b,a);this.form=null;this.fields=[];this.lsection=null;
...
}};Chatterbox.Settings.Item.Form.prototype.change=function(){var g={};var j;for(var b in this.fields){j=this.fields[b];g[j.ref]=j.get()}var a=this._get_cb("change");if(typeof a=="function"){a({data:g,form:this})}else{for(var b in a){a[b]({data:g,form:this})}}};Chatterbox.Settings.Item.Form.prototype.save=function(j,l){var k={};var b;for(var g in this.fields){field=this.fields[g];k[field.ref]=field.get()}var a=this._get_cb("save");if(typeof a=="function"){a({data:k,form:this,page:l,window:j})}else{for(var g in a){a[g]({data:k,form:this,page:l,window:j})}}};Chatterbox.Settings.Item.Form.prototype.close=function(g,k){var j={};var l;for(var b in this.fields){l=this.fields[b];j[l.ref]=l.get()}var a=this._get_cb("close");if(typeof a=="function"){a({data:j,form:this,page:k,window:g})}else{for(var b in a){a[b]({data:j,form:this,page:k,window:g})}}};Chatterbox.Settings.Item.Form.Field=function(b,a){Chatterbox.Settings.Item.call(this,b,a);this.ref=this.options.ref||"ref";this.label=null;this.field=null;
...
this.value=[];this.view.find("input[checked]:checkbox").each(function(){a.value.push(a.view.find(this).val())});this.field.bind("change",function(j){a.value=[];a.view.find("input[checked]:checkbox").each(function(){a.value.push(a.view.find(this).val())})})};Chatterbox.Settings.Item.Items=function(b,a,g){a=Object.extend({prompt:{title:"Add Item",label:"Item:","submit-button":"Add"}},(a||{}));Chatterbox.Settings.Item.call(this,b,a,g);this.selected=false};Chatterbox.Settings.Item.Items.prototype=new Chatterbox.Settings.Item();Chatterbox.Settings.Item.Items.prototype.constructor=Chatterbox.Settings.Item.Items;Chatterbox.Settings.Item.Items.prototype.build=function(b){Chatterbox.Settings.Item.prototype.build.call(this,b);var a=this;this.list=this.view.find("ul");this.buttons=this.view.find("section.buttons p");var g=this.list.find("li");g.click(function(k){var j=a.list.find(this);a.list.find("li.selected").removeClass("selected");a.selected=j.find(".item").html();j.addClass("selected")});g.each(function(j,l){var k=a.list.find(l);
k.find("a.close").click(function(m){a.list.find("li.selected").removeClass("selected");a.selected=k.find(".item").html();k.addClass("selected");a.remove_item();return false})});this.buttons.find("a.button.up").click(function(j){a.shift_item(true);return false});this.buttons.find("a.button.down").click(function(j){a.shift_item();return false});this.buttons.find("a.button.add").click(function(j){var k=new Chatterbox.Popup.Prompt(a.manager,{position:[j.clientX-100,j.clientY-50],title:a.options.prompt.title,label:a.options.prompt.label,"submit-button":a.options.prompt["submit-button"],event:{submit:function(l){var p=l.data;if(!p){l.options.event.cancel(l);return}p=p.toLowerCase();var m=a.options.items.indexOf(p);if(m!=-1){l.options.event.cancel(l);return}a._fevent("add",{item:p});a.refresh();a._onchange({})},cancel:function(l){a._fevent("cancel",{});a.refresh();a._onchange({})}}});k.build();return false});this.buttons.find("a.button.close").click(function(j){a.remove_item();return false})};
Chatterbox.Settings.Item.Items.prototype.shift_item=function(b){if(this.selected===false){return}var g=this.options.items.indexOf(this.selected);var a=g+1;if(b){a=g-1}if(g==-1||g>=this.options.items.length){return}if(a<0||a>=this.options.items.length){return}this._fevent((b?"up":"down"),{swap:{"this":{index:g,item:this.options.items[g]},that:{index:a,item:this.options.items[a]}}});this.refresh();this._onchange({});return};Chatterbox.Settings.Item.Items.prototype.remove_item=function(){if(this.selected===false){return false}var a=this.options.items.indexOf(this.selected);if(a==-1||a>=this.options.items.length){return}this._fevent("remove",{index:a,item:this.selected});this.selected=false;this.refresh();this._onchange({})};Chatterbox.Settings.Item.Items.prototype.refresh=function(){this.view.find("section.mitems").html(Chatterbox.template.settings.krender.manageditems(this.options.items));this.list=this.view.find("ul");this.list.find("li[title="+(this.selected||"").toLowerCase()+"]").addClass("selected");
...
if(!b.emotes.on){return}b.emotes.fetch()};wsc.dAmn.Emotes.Picker=function(k,a){a=a||{};a=Object.extend({title:"Emotes",event:{submit:function(){},cancel:function(){}}},a);Chatterbox.Popup.ItemPicker.call(this,k,a);this.data=this.options["default"];var l="ABCDEFGHIJKLMNOP";var j="";var g="";for(var b=0;b<l.length;b++){j=l[b];g=j.toLowerCase();this.add_page({ref:g,href:"#"+g,label:j,title:"Emotes beginning with "+j,items:[]})}this.add_page({ref:"misc",href:"#misc",label:"#",title:"Emotes not beginning with letters",items:[]})};wsc.dAmn.Emotes.Picker.prototype=new Chatterbox.Popup.ItemPicker();wsc.dAmn.Emotes.Picker.prototype.constructor=wsc.dAmn.Emotes.Picker;wsc.dAmn.Emotes.Picker.prototype.hide=function(){this.window.css({display:"none"})};wsc.dAmn.Emotes.Picker.prototype.show=function(){this.window.css({display:"block"})};wsc.dAmn.Emotes.Picker.prototype.close=function(){this.hide()};wsc.dAmn.Extension=function(a){a.settings.client="dAmnClient";a.settings.clientver="0.3";a.settings.domain="deviantart.com";
a.settings.agent+=" wsc/dAmn/"+wsc.dAmn.VERSION;var g=a.storage.folder("dAmn");g.emotes=g.folder("emotes");var b={emotes:{on:false,emote:[]}};b.save=function(){g.emotes.set("on",b.emotes.on)};b.load=function(){b.emotes.on=(g.emotes.get("on","false")=="true")};b.load();b.save();a.protocol.extend_maps({dAmnServer:["version"]});a.protocol.extend_messages({dAmnServer:['<span class="servermsg">** Connected to dAmnServer {version} *</span>',false,true]});a.protocol.mparser=new wsc.dAmn.TablumpParser;a.flow.dAmnServer=a.flow.chatserver;a.exclude.push("#devart");a.ui.on("userinfo.before",function(j,k){j.user.avatar=wsc.dAmn.avatar.link(j.user.name,j.user.member.usericon);if(j.user.member.realname){j.user.info.push(j.user.member.realname)}if(j.user.member.typename){j.user.info.push(j.user.member.typename)}});a.ui.on("log_whois.before",function(j,k){j.avatar=wsc.dAmn.avatar.link(j.raw.username,j.raw.usericon);j.username=j.raw.symbol+'<b><a href="http://'+j.raw.username+'.deviantart.com/">'+j.raw.username+"</a></b>";
