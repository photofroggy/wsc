...
}}}m.ui.chatbook.current.log_pc(p.p=="privclass",q)};k();wsc.defaults.Extension.Ignore(b);wsc.defaults.Extension.Away(b,f);wsc.defaults.Extension.Autojoin(b,f);return f};wsc.defaults.Extension.Autojoin=function(b,g){g.autojoin={on:false,channel:[]};var k=true;var j=b.storage.folder("autojoin");var h=function(){g.autojoin.load();g.autojoin.save();b.bind("cmd.autojoin",f);b.bind("pkt.login",a)};g.autojoin.load=function(){g.autojoin.on=(j.get("on","true")=="true");g.autojoin.channel=JSON.parse(j.get("channel","[]"))};g.autojoin.save=function(){j.set("on",g.autojoin.on.toString());j.set("channel",JSON.stringify(g.autojoin.channel))};g.autojoin.join=function(){for(var l in g.autojoin.channel){if(!g.autojoin.channel.hasOwnProperty(l)){continue}b.join(g.autojoin.channel[l])}};g.autojoin.add=function(l){if(g.autojoin.channel.indexOf(l)!=-1){return false}g.autojoin.channel.push(l);g.autojoin.save();return true};g.autojoin.remove=function(m){var l=g.autojoin.channel.indexOf(m);if(l==-1){return false
...
},save:function(m){l.user=b.colours.user;l.msg=b.colours.msg},close:function(m){b.colours.user=l.user;b.colours.msg=l.msg;f.colours.page=null},}})};f.colours.parse_colour=function(k){if(!b.colours.on){return}var j=k.item.find("abbr");if(j.length==0){return}j=j.last();var h=j.prop("title").match(/colors:([A-F0-9]{6}):([A-F0-9]{6})/);if(h==null){return}j.remove();k.item.find(".cmsg, .caction").css("color","#"+h[2]);k.item.find(".cmsg.user, .caction.user").css("color","#"+h[1])};g.on("settings.open",f.colours.configure_page);g.on("log_item.after",f.colours.parse_colour)};wsc.dAmn.Emotes={};wsc.dAmn.wsc.Emotes=function(a,f,b){b.emotes.emote={};b.emotes.fint=null;b.emotes.fetching=false;b.emotes.loaded=false;b.emotes.fetch=function(){if(b.emotes.loaded){}else{}b.emotes.fetching=true;jQuery.getJSON("http://www.thezikes.org/publicemotes.php?format=jsonp&jsoncallback=?&"+(new Date()).getDay(),function(g){b.emotes.fetching=false;b.emotes.emote=g;if(!b.emotes.loaded){if(b.emotes.on){a.trigger("dAmn.emotes.loaded",{name:"dAmn.emotes.loaded"})
