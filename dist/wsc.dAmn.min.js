...
default:if(b.r.indexOf("killed:")<0){return}break}this.process_data({data:"disconnect\ne="+e.r+"\n"})}}else{a.monitor("Couldn't leave "+ns,b.e);c.server_message("Couldn't leave "+ns,b.e)}};wsc.Flow.prototype.kicked=function(b,a){if(b.r.toLowerCase().indexOf("autokicked")>-1){return}a.join(b.ns)};wsc.Flow.prototype.property=function(b,a){chan=a.channel(b.pkt.param);if(!chan){return}chan.property(b)};wsc.Flow.prototype.recv_joinpart=function(b,a){c=a.channel(b.ns);if(b.name=="recv_join"){c.recv_join(b)}else{c.recv_part(b)}};wsc.Flow.prototype.recv_join=wsc.Flow.prototype.recv_joinpart;wsc.Flow.prototype.recv_part=wsc.Flow.prototype.recv_joinpart;wsc.Flow.prototype.recv_msg=function(b,a){a.channel(b.ns).recv_msg(b)};wsc.Flow.prototype.recv_action=wsc.Flow.prototype.recv_msg;wsc.Flow.prototype.recv_npmsg=wsc.Flow.prototype.recv_msg;wsc.Flow.prototype.recv_privchg=function(b,a){a.channel(b.ns).recv_privchg(b)};wsc.Flow.prototype.recv_kicked=function(b,a){a.channel(b.ns).recv_kicked(b)};wsc.defaults.Extension=function(k){var C=function(){k.bind("cmd.set",a);
k.bind("cmd.connect",p);k.bind("cmd.join",H);k.bind("cmd.part",J);k.bind("cmd.title",g);k.bind("cmd.promote",s);k.bind("cmd.demote",m);k.bind("cmd.me",v);k.bind("cmd.kick",B);k.bind("cmd.raw",I);k.bind("cmd.say",q);k.bind("cmd.npmsg",K);k.bind("cmd.clear",F);k.bind("cmd.clearall",E);k.bind("cmd.whois",D);k.bind("cmd.admin",A);k.bind("pkt.property",j);k.bind("pkt.get",z);k.bind("cmd.theme",b);k.ui.on("settings.open",l);k.ui.on("settings.open.ran",G)};var l=function(N,M){page=N.settings.page("Main");var O={};O.theme=replaceAll(k.ui.settings.theme,"wsct_","");O.clock=k.ui.clock();O.tc=k.ui.nav.closer();O.username=k.settings.username;O.pk=k.settings.pk;var L=[];for(i in k.ui.settings.themes){name=replaceAll(k.ui.settings.themes[i],"wsct_","");L.push({value:name,title:name,selected:O.theme==name})}page.item("Text",{ref:"intro",title:"Main",text:"Use this window to view and change your settings.\n\nCheck                    the different pages to see what settings can be changed.",});page.item("Form",{ref:"login",title:"Login",text:"Here you can change the username and token used to                    log into the chat server.",fields:[["Textfield",{ref:"username",label:"Username","default":O.username}],["Textfield",{ref:"token",label:"Token","default":O.pk}]],event:{save:function(P){k.settings.username=P.data.username;
k.settings.pk=P.data.token}}});page.item("Form",{ref:"ui",title:"UI",hint:"<b>Timestamp</b><br/>Choose between a 24 hour clock and                    a 12 hour clock.\n\n<b>Theme</b><br/>Change the look of the                    client.\n\n<b>Close Buttons</b><br/>Turn tab close buttons on/off.",fields:[["Dropdown",{ref:"theme",label:"Theme",items:L}],["Dropdown",{ref:"clock",label:"Timestamp Format",items:[{value:"24",title:"24 hour",selected:O.clock},{value:"12",title:"12 hour",selected:!O.clock}]}],["Check",{ref:"tabclose",label:"Close Buttons",items:[{value:"yes",title:"On",selected:O.tc}]}],],event:{change:function(P){k.ui.clock(P.data.clock=="24");k.ui.theme(P.data.theme);k.ui.nav.closer(P.data.tabclose.indexOf("yes")>-1)},save:function(P){O.clock=P.data.clock=="24";O.theme=P.data.theme;O.tc=P.data.tabclose.indexOf("yes")>-1},close:function(P){k.ui.clock(O.clock);k.ui.theme(O.theme);k.ui.nav.closer(O.tc)}}});page.item("Text",{ref:"debug",wclass:"faint",title:"Debug Information",text:"Chat Agent: <code>"+k.settings.agent+"</code>\n\nUser                    Agent: <code>"+navigator.userAgent+"</code>"})
};var G=function(N,M){var L=N.settings.page("About",true);L.item("Text",{ref:"about-wsc",title:"Wsc",text:'Currently using <a href="http://github.com/photofroggy/wsc/">wsc</a>                    version '+wsc.VERSION+" "+wsc.STATE+'.\n\nWsc                    works using HTML5, javascript, and CSS3. WebSocket is used for the connection                    where possible. The source code for this client is pretty huge.\n\nWsc was created                    by ~<a href="http://photofroggy.deviantart.com/">photofroggy</a>'})};var b=function(M,L){L.ui.theme(M.args.split(" ").shift())};var a=function(N){var M=N.args.split(" ");var L=M.shift().toLowerCase();var M=M.join(" ");if(M.length==0){k.cchannel.serverMessage("Could not set "+L,"No data supplied");return}if(!(L in k.settings)){k.cchannel.serverMessage('Unknown setting "'+L+'"');return}k.settings[L]=M;k.cchannel.serverMessage("Changed "+L+" setting","value: "+M);k.control.setLabel()};var p=function(L){k.connect()};var H=function(L){var M=L.args.split(" ");
var M=M.toString()==""?[]:M;if(L.ns!=L.target){M.unshift(L.target)}if(M.toString()==""){return}for(index in M){k.join(M[index])}};var J=function(L){var M=L.args.split(" ");if(L.ns!=L.target){M.unshift(L.target)}if(M.toString()==""){k.part(L.ns);return}for(index in M){k.part(M[index])}};var g=function(L){k.set(L.target,"title",L.args)};var s=function(M){var L=M.args.split(" ");k.promote(M.target,L[0],L[1])};var m=function(M){var L=M.args.split(" ");k.demote(M.target,L[0],L[1])};var v=function(L){k.action(L.target,L.args)};var I=function(L){k.send(L.args.replace(/\\n/gm,"\n"))};var B=function(L){d=L.args.split(" ");u=d.shift();r=d.length>0?d.join(" "):null;k.kick(L.target,u,r)};var q=function(L){k.say(L.target,L.args)};var K=function(L){k.npmsg(L.target,L.args)};var F=function(M,L){L.cchannel.clear()};var E=function(M,L){for(c in L.channelo){L.channelo[c].clear()}};var D=function(M,L){L.whois(M.args.split(" ")[0])};var A=function(M,L){L.admin(M.target,M.args)};var j=function(M,L){if(M.p!="info"){return
}subs=M.pkt.sub;data=subs.shift().arg;data.username=M.sns.substr(1);data.connections=[];while(subs.length>0){conn=subs.shift().arg;conn.channels=[];while(subs.length>0){if(subs[0].cmd!="ns"){break}conn.channels.unshift(L.deform_ns(subs.shift().param))}data.connections.push(conn)}L.cchannel.show_whois(data)};var z=function(M,L){if(M.ns.indexOf("login:")!=0){return}L.cchannel.server_message("Whois failed for "+(M.sns.substr(1)),"not online")};C()};wsc.Client=function(a,g,j){this.mozilla=j;this.fresh=true;this.attempts=0;this.protocol=null;this.flow=null;this.ui=null;this.events=new EventEmitter();this.conn=null;this.channelo={};this.cchannel=null;this.cmds=[];this.settings={domain:"website.com",server:"ws://website.com/wsendpoint",symbol:"",username:"",userinfo:{},pk:"",monitor:["~Monitor",true],welcome:"Welcome to the wsc web client!",autojoin:"chat:channel",control:wsc.Control,protocol:wsc.Protocol,tablumps:wsc.Tablumps,flow:wsc.Flow,ui:Chatterbox.UI,extend:[wsc.defaults.Extension],client:"chatclient",clientver:"0.3",theme:wsc.defaults.theme,themes:wsc.defaults.themes,};
...
b=b.slice(1);bits=b.split(" ");cmdn=bits.shift();ens=this.client.cchannel.namespace;etarget=ens;if(bits[0]){hash=bits[0][0];if((hash=="#"||hash=="@")&&bits[0].length>1){etarget=this.client.format_ns(bits.shift())}}arg=bits.join(" ");this.client.trigger("cmd."+cmdn,{name:"cmd",cmd:cmdn,args:arg,target:etarget,ns:ens})};var Chatterbox={};Chatterbox.VERSION="0.5.38";Chatterbox.STATE="beta";Chatterbox.UI=function(a,b,j,g){this.events=g||new EventEmitter();this.mozilla=j;this.settings={themes:["wsct_default","wsct_dAmn"],theme:"wsct_default",monitor:["~Monitor",true],username:"",domain:"website.com",clock:true};a.extend(this.settings,b);a.append('<div class="wsc '+this.settings.theme+'"></div>');this.view=a.find(".wsc");this.mns=this.format_ns(this.settings.monitor[0]);this.lun=this.settings.username.toLowerCase();this.monitoro=null;this.swidth=(function(){if($.browser.msie){var m=$('<textarea cols="10" rows="2"></textarea>').css({position:"absolute",top:-1000,left:-1000}).appendTo("body"),l=$('<textarea cols="10" rows="2" style="overflow: hidden;"></textarea>').css({position:"absolute",top:-1000,left:-1000}).appendTo("body");
...
time=new Date(parseInt(el.prop("id")));el.text(formatTime(a,time))})};Chatterbox.Channel.prototype.server_message=function(b,a){data={ns:this.namespace,message:b,info:a};this.manager.trigger("server_message.before",data);this.log_item(Chatterbox.render("servermsg",{message:data.message,info:data.info}))};Chatterbox.Channel.prototype.clear=function(){this.logpanel.find("li.logmsg").remove();this.logpanel.find("li.loginfo").remove();this.wrap.height(0);this.resize()};Chatterbox.Channel.prototype.log_info=function(g,b){data={ns:this.namespace,ref:g,content:b};this.manager.trigger("log_info.before",data);delete data.ns;this.wrap.append(Chatterbox.render("loginfobox",data));this.scroll();var j=this;var a=this.wrap.find("li."+data.ref);a.find("a.close").click(function(k){j.wrap.find(this).parent().remove();j.resize();return false});return a};Chatterbox.Channel.prototype.show_whois=function(p){var j={avatar:'<a href="#"><img height="50" width="50" alt="avatar"/></a>',username:"<b>"+p.symbol+p.username+"</b>",info:[],conns:[],raw:p,};
for(var q in p.connections){var m=p.connections[q];var v=[];if(m.online){stamp=(new Date-(m.online*1000));v.push(["online",DateStamp(stamp/1000)+formatTime(" [{HH}:{mm}:{ss}]",new Date(stamp))])}if(m.idle){v.push(["idle",timeLengthString(m.idle)])}if(m.agent){v.push(["agent",m.agent])}if(m.debug){v.push(["debug",m.debug])}v.push(["chatrooms",m.channels.join(" ")]);j.conns.push(v)}this.manager.trigger("log_whois.before",j);var a="";for(var q in j.conns){var l=j.conns[q];var z='<section class="conn"><p><em>connection '+((parseInt(q)+1).toString())+":</em></p>";z+="<ul>";for(x in l){z+="<li><strong>"+l[x][0]+":</strong> "+l[x][1]+"</li>"}z+="</ul>";a+=z+"</section>"}var k="";for(var q in j.info){k+="<li>"+j.info[q]+"</li>"}var s=this.log_info("whois-"+p.username,Chatterbox.render("whoiswrap",{avatar:j.avatar,info:Chatterbox.render("whoisinfo",{username:j.username,info:k,connections:a})}));var g=s.find("div.avatar");var b=s.find("div.info");b.width(s.find(".whoiswrap").width()-100);g.height(s.height()-10);
