...
};wsc.Flow.prototype.recv_action=wsc.Flow.prototype.recv_msg;wsc.Flow.prototype.recv_npmsg=wsc.Flow.prototype.recv_msg;wsc.Flow.prototype.recv_privchg=function(b,a){a.channel(b.ns).recv_privchg(b)};wsc.Flow.prototype.recv_kicked=function(b,a){a.channel(b.ns).recv_kicked(b)};wsc.defaults.Extension=function(f){var a={};a.away={on:false,reason:"",last:{},since:0,store:f.storage.folder("away"),format:{setaway:"/me is away: {reason}",setback:"/me is back",away:"{from}: I am away, reason: {reason}"}};var n=function(){f.bind("cmd.connect",b.connection);f.bind("cmd.set",b.setter);f.bind("cmd.join",b.join);f.bind("cmd.chat",b.pjoin);f.bind("cmd.part",b.part);f.bind("cmd.say",b.say);f.bind("cmd.npmsg",b.npmsg);f.bind("cmd.me",b.action);f.bind("cmd.promote",b.chgpriv);f.bind("cmd.demote",b.chgpriv);f.bind("cmd.ban",b.ban);f.bind("cmd.unban",b.ban);f.bind("cmd.kick",b.killk);f.bind("cmd.whois",b.whois);f.bind("cmd.title",b.title);f.bind("cmd.topic",b.title);f.bind("cmd.admin",b.admin);f.bind("cmd.disconnect",b.connection);
f.bind("cmd.kill",b.killk);f.bind("cmd.raw",b.raw);f.bind("cmd.clear",b.clear);f.bind("cmd.clearall",b.clearall);f.bind("cmd.close",b.close);f.bind("pkt.property",m);f.bind("pkt.recv_admin_show",g);f.bind("pkt.recv_admin_showverbose",g);f.bind("pkt.get",l);f.bind("cmd.gettitle",b.gett);f.bind("cmd.gettopic",b.gett);f.bind("cmd.theme",b.theme)};var k=function(p,o){f.settings.ui.theme=p.theme;f.settings.ui.clock=p.clock;f.settings.ui.tabclose=p.tabclose};var j=function(q,p){var o=q.settings.page("Main");var r={};r.username=f.settings.username;r.pk=f.settings.pk;r.devel=f.settings.developer;o.item("Form",{ref:"login",title:"Login",text:"Here you can change the username and token used to                    log into the chat server.",fields:[["Textfield",{ref:"username",label:"Username","default":r.username}],["Textfield",{ref:"token",label:"Token","default":r.pk}]],event:{save:function(s){f.settings.username=s.data.username;f.settings.pk=s.data.token}}},true);o.item("Form",{ref:"developer",title:"Developer Mode",text:"Turn developer mode on or off.\n\nDeveloper mode will expose any hidden                channel tabs, amongst other things. Keep this turned off unless you're working                on implementing something.",fields:[["Checkbox",{ref:"enabled",items:[{value:"on",title:"On",selected:r.devel}]}]],event:{change:function(s){f.settings.developer=(s.data.enabled.indexOf("on")!=-1);
...
})};wsc.dAmn.Extension=function(a,f){a.settings.client="dAmnClient";a.settings.clientver="0.3";a.settings.domain="deviantart.com";a.settings.agent+=" wsc/dAmn/"+wsc.dAmn.VERSION;var g=a.storage.folder("dAmn");g.bds=g.folder("bds");g.emotes=g.folder("emotes");g.colours=g.folder("colours");var b={emotes:{on:false,emote:[]},colours:{on:false,send:false,msg:"000000",user:"000000"}};b.save=function(){g.emotes.set("on",b.emotes.on);g.colours.set("on",b.colours.on);g.colours.set("send",b.colours.send);g.colours.set("msg",b.colours.msg);g.colours.set("user",b.colours.user)};b.load=function(){b.emotes.on=(g.emotes.get("on","false")=="true");b.colours.on=(g.colours.get("on","false")=="true");b.colours.send=(g.colours.get("send","false")=="true");b.colours.msg=g.colours.get("msg","");b.colours.user=g.colours.get("user","")};b.load();b.save();a.protocol.extend_maps({dAmnServer:["version"]});a.protocol.mparser=new wsc.dAmn.TablumpParser;a.flow.dAmnServer=a.flow.chatserver;a.exclude.add("chat:devart");
a.exclude.add("chat:damnidlers");wsc.dAmn.BDS(a,g.bds,b);wsc.dAmn.BDS.Link(a,g.bds,b);wsc.dAmn.BDS.Peer(a,g.bds,b)};wsc.dAmn.avatar={};wsc.dAmn.avatar.ext=["gif","gif","jpg","png"];wsc.dAmn.avatar.link=function(a,b){var f=wsc.dAmn.avatar.src(a,b);return'<a target="_blank" title=":icon'+a+':" href="http://'+a+'.deviantart.com/"><img class="avatar"            alt=":icon'+a+':" src="'+f+'" height="50" /></a>'};wsc.dAmn.avatar.src=function(b,g){g=parseInt(g);var j=(g>>2)&15;g=g&3;var f=wsc.dAmn.avatar.ext[g]||"gif";var h="default";if(j){j="?"+j}else{j=""}if(g!=0){var a=new RegExp("\\$un(\\[([0-9]+)\\])","g");var h="$un[0]/$un[1]/{un}".replace(a,function(k,n,l){return b[l]=="-"?"_":b[l].toLowerCase()});h=replaceAll(h,"{un}",b.toLowerCase())}return"http://a.deviantart.net/avatars/"+h+"."+f+j};wsc.dAmn.BDS.Link=function(a,h,f){var g=function(){a.bind("BDS",b.cmd);a.bind("BDS.LINK.REQUEST",b.request);a.bind("BDS.LINK.REJECT",b.reject);a.bind("BDS.LINK.ACCEPT",b.accept);a.bind("BDS.LINK.CLOSE",b.close);
