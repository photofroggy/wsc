...
a.attempts=0};wsc.Flow.prototype.close=function(b,f){var a={name:"closed",pkt:new wsc.Packet("connection closed\n\n"),reason:"",evt:f,connected:b.connected,sio:b.conn instanceof wsc.SocketIO,cause:"",reconnect:true};b.trigger("closed",a);if(b.connected){b.connected=false;if(b.conn instanceof wsc.SocketIO){return}}else{}a.name="quit";if(b.attempts>2){a.reconnect=false;b.attempts=0;b.trigger("quit",a);return}if(f.cause){if(f.cause.hasOwnProperty("name")&&f.cause.name=="login"){b.trigger("quit",a);return}}setTimeout(function(){b.connect()},2000)};wsc.Flow.prototype.message=function(a,g){var b=new wsc.Packet(unescape(replaceAll(g.data,"+"," ")));if(b==null){return}var f=this.protocol.parse(b);if(f.ns==null){f.ns=a.mns}f.sns=a.deform_ns(f.ns);this.handle(a,f);a.trigger("pkt",f);a.trigger("pkt."+f.name,f)};wsc.Flow.prototype.handle=function(a,f){try{this[f.name](f,a)}catch(b){}};wsc.Flow.prototype.ping=function(b,a){a.pong()};wsc.Flow.prototype.chatserver=function(b,a){a.login()};wsc.Flow.prototype.login=function(f,a){if(f.pkt.arg.e=="ok"){var h=f.pkt.sub[0];
a.settings.username=f.pkt.param;a.settings.symbol=h.arg.symbol;a.settings.userinfo=h.arg;var b=function(){a.join(a.settings.autojoin);if(a.autojoin.on){for(var j in a.autojoin.channel){if(!a.autojoin.channel.hasOwnProperty(j)){continue}a.join(a.autojoin.channel[j])}}};if(a.fresh){b()}else{var g=false;for(key in a.channelo){if(a.channelo[key].namespace[0]!="~"){a.join(key);g=true}}if(!g){b()}}}else{a.close(f)}if(a.fresh){a.fresh=false}};wsc.Flow.prototype.join=function(f,a){if(f.pkt.arg["e"]=="ok"){var b=a.deform_ns(f.pkt.param);a.create_ns(b,a.hidden.contains(f.pkt.param));a.ui.channel(b).server_message("You have joined "+b)}else{a.ui.chatbook.current.server_message("Failed to join "+a.deform_ns(f.pkt.param),f.pkt.arg["e"])}};wsc.Flow.prototype.part=function(f,a){var b=a.deform_ns(f.ns);var g=a.channel(b);if(f.e=="ok"){info="";if(f.r.length>0){info="["+f.r+"]"}else{a.remove_ns(f.ns)}msg="You have left "+b;g.server_message(msg,info);if(a.channels()==0){switch(f.r){case"bad data":case"bad msg":case"msg too big":break;
default:if(f.r.indexOf("killed:")<0){return}break}this.message(a,{data:"disconnect\ne="+e.r+"\n"})}}else{a.monitor("Couldn't leave "+b,f.e);g.server_message("Couldn't leave "+b,f.e)}};wsc.Flow.prototype.kicked=function(b,a){if(b.r.toLowerCase().indexOf("autokicked")>-1){return}a.join(b.ns)};wsc.Flow.prototype.property=function(b,a){chan=a.channel(b.pkt.param);if(!chan){return}chan.property(b)};wsc.Flow.prototype.recv_joinpart=function(b,a){c=a.channel(b.ns);if(b.name=="recv_join"){c.recv_join(b)}else{c.recv_part(b)}};wsc.Flow.prototype.recv_join=wsc.Flow.prototype.recv_joinpart;wsc.Flow.prototype.recv_part=wsc.Flow.prototype.recv_joinpart;wsc.Flow.prototype.recv_msg=function(b,a){a.channel(b.ns).recv_msg(b)};wsc.Flow.prototype.recv_action=wsc.Flow.prototype.recv_msg;wsc.Flow.prototype.recv_npmsg=wsc.Flow.prototype.recv_msg;wsc.Flow.prototype.recv_privchg=function(b,a){a.channel(b.ns).recv_privchg(b)};wsc.Flow.prototype.recv_kicked=function(b,a){a.channel(b.ns).recv_kicked(b)};wsc.defaults.Extension=function(f){var a={};
a.away={on:false,reason:"",last:{},since:0,store:f.storage.folder("away"),format:{setaway:"/me is away: {reason}",setback:"/me is back",away:"{from}: I am away, reason: {reason}"}};var n=function(){f.bind("cmd.connect",b.connection);f.bind("cmd.set",b.setter);f.bind("cmd.join",b.join);f.bind("cmd.chat",b.pjoin);f.bind("cmd.part",b.part);f.bind("cmd.say",b.say);f.bind("cmd.npmsg",b.npmsg);f.bind("cmd.me",b.action);f.bind("cmd.promote",b.chgpriv);f.bind("cmd.demote",b.chgpriv);f.bind("cmd.ban",b.ban);f.bind("cmd.unban",b.ban);f.bind("cmd.kick",b.killk);f.bind("cmd.whois",b.whois);f.bind("cmd.title",b.title);f.bind("cmd.topic",b.title);f.bind("cmd.admin",b.admin);f.bind("cmd.disconnect",b.connection);f.bind("cmd.kill",b.killk);f.bind("cmd.raw",b.raw);f.bind("cmd.clear",b.clear);f.bind("cmd.clearall",b.clearall);f.bind("cmd.close",b.close);f.bind("pkt.property",m);f.bind("pkt.recv_admin_show",g);f.bind("pkt.recv_admin_showverbose",g);f.bind("pkt.get",l);f.bind("cmd.gettitle",b.gett);f.bind("cmd.gettopic",b.gett);
f.bind("cmd.theme",b.theme);f.ui.on("settings.open",j);f.ui.on("settings.open.ran",h);f.ui.on("settings.save.ui",k);f.ui.on("settings.save",function(){f.config_save()})};var k=function(p,o){f.settings.ui.theme=p.theme;f.settings.ui.clock=p.clock;f.settings.ui.tabclose=p.tabclose};var j=function(q,p){var o=q.settings.page("Main");var r={};r.username=f.settings.username;r.pk=f.settings.pk;r.devel=f.settings.developer;o.item("Form",{ref:"login",title:"Login",text:"Here you can change the username and token used to                    log into the chat server.",fields:[["Textfield",{ref:"username",label:"Username","default":r.username}],["Textfield",{ref:"token",label:"Token","default":r.pk}]],event:{save:function(s){f.settings.username=s.data.username;f.settings.pk=s.data.token}}},true);o.item("Form",{ref:"developer",title:"Developer Mode",text:"Turn developer mode on or off.\n\nDeveloper mode will expose any hidden                channel tabs, amongst other things. Keep this turned off unless you're working                on implementing something.",fields:[["Checkbox",{ref:"enabled",items:[{value:"on",title:"On",selected:r.devel}]}]],event:{change:function(s){f.settings.developer=(s.data.enabled.indexOf("on")!=-1);
...
f.action(r.target,u)}else{f.say(r.target,u)}}if(q){k()}};var l=function(){g.ignore=h.get("ignore","/me is ignoring {user} now");g.unignore=h.get("unignore","/me is not ignoring {user} anymore");g.count=parseInt(h.get("count",0));var o=null;for(var n in f.ui.umuted){if(!f.ui.umuted.hasOwnProperty(n)){continue}f.unmute_user(f.ui.umuted[n])}f.ui.umuted=[];if(g.count>0){o=null;for(var n=0;n<g.count;n++){f.mute_user(a.get(n,null))}}};var k=function(){h.set("ignore",g.ignore);h.set("unignore",g.unignore);for(var n=0;n<g.count;n++){a.remove(n)}if(f.ui.umuted.length==0){h.set("count",0)}else{var o=-1;for(var n in f.ui.umuted){if(!f.ui.umuted.hasOwnProperty(n)){continue}o++;a.set(o.toString(),f.ui.umuted[n])}o++;g.count=o;h.set("count",o)}};m()};wsc.Client=function(b,f,g){this.mozilla=g;this.storage=new wsc.Storage;this.storage.ui=this.storage.folder("ui");this.storage.aj=this.storage.folder("autojoin");this.storage.aj.channel=this.storage.aj.folder("channel");this.fresh=true;this.attempts=0;this.connected=false;
this.protocol=null;this.flow=null;this.ui=null;this.events=new EventEmitter();this.conn=null;this.channelo={};this.cchannel=null;this.cmds=[];this.settings={domain:"website.com",server:"ws://website.com/wsendpoint",symbol:"",username:"",userinfo:{},pk:"",monitor:["~Monitor",true],welcome:"Welcome to the wsc web client!",autojoin:"chat:channel",protocol:wsc.Protocol,mparser:wsc.MessageParser,flow:wsc.Flow,ui_object:Chatterbox.UI,extend:[wsc.defaults.Extension],client:"chatclient",clientver:"0.3",ui:{theme:wsc.defaults.theme,themes:wsc.defaults.themes,tabclose:true,clock:true,media:"/static/"},developer:false};this.autojoin={on:true,count:0,channel:[]};this.away={};var a=this;this.exclude=new StringSet();this.hidden=new StringSet();this.settings=Object.extend(this.settings,f);this.config_load();this.config_save();this.mw=new wsc.Middleware();this.settings.agent="wsc/"+wsc.VERSION+"-r"+wsc.REVISION;this.mns=this.format_ns(this.settings.monitor[0]);this.lun=this.settings.username.toLowerCase();
...
a.protocol.mparser=new wsc.dAmn.TablumpParser;a.flow.dAmnServer=a.flow.chatserver;a.exclude.add("chat:devart");a.exclude.add("chat:damnidlers");wsc.dAmn.BDS(a,f.bds,b);wsc.dAmn.BDS.Link(a,f.bds,b);wsc.dAmn.BDS.Peer(a,f.bds,b)};wsc.dAmn.avatar={};wsc.dAmn.avatar.ext=["gif","gif","jpg","png"];wsc.dAmn.avatar.link=function(a,b){var f=wsc.dAmn.avatar.src(a,b);return'<a target="_blank" title=":icon'+a+':" href="http://'+a+'.deviantart.com/"><img class="avatar"            alt=":icon'+a+':" src="'+f+'" height="50" /></a>'};wsc.dAmn.avatar.src=function(b,g){g=parseInt(g);var j=(g>>2)&15;g=g&3;var f=wsc.dAmn.avatar.ext[g]||"gif";var h="default";if(j){j="?"+j}else{j=""}if(g!=0){var a=new RegExp("\\$un(\\[([0-9]+)\\])","g");var h="$un[0]/$un[1]/{un}".replace(a,function(k,n,l){return b[l]=="-"?"_":b[l].toLowerCase()});h=replaceAll(h,"{un}",b.toLowerCase())}return"http://a.deviantart.net/avatars/"+h+"."+f+j};wsc.dAmn.BDS.Link=function(a,h,f){var g=function(){a.bind("BDS",b.cmd);a.bind("BDS.LINK.REQUEST",b.request);
a.bind("BDS.LINK.REJECT",b.reject);a.bind("BDS.LINK.ACCEPT",b.accept);a.bind("BDS.LINK.CLOSE",b.close);a.bind("pkt.join",b.join);a.bind("pkt.part",b.part);a.bind("pkt.recv_join",b.recv_join);a.bind("pkt.recv_part",b.recv_part)};f.bds.link={white:(new StringSet()),connections:{},state:{closed:0,opening:1,open:2},conn:function(k,j,l,n){k=k.toLowerCase();if(k in f.bds.link.connections){return f.bds.link.connections[k]}var o=a.format_ns("@"+k);var m={state:f.bds.link.state.closed,ns:o,un:k,close:function(){f.bds.link.close(m.un)},send:function(p){if(m.state!=f.bds.link.state.open){return false}a.npmsg(m.ns,p);return true},oncmd:(j||function(q,p){}),onopen:(l||function(q,p){}),onclose:(n||function(q,p){})};return m},request:function(k,o,j,l,n){var m=f.bds.link.open(k,j);if(m==null){return null}if(o===true){return m}a.npmsg(f.bds.mns,"BDS:LINK:REQUEST:"+k);return m},open:function(k,j,l,n){k=k.toLowerCase();if(k in f.bds.link.connections){return null}var m=f.bds.link.conn(k,j);m.state=f.bds.link.state.opening;
f.bds.link.connections[k]=m;f.bds.channel.add(m.ns);a.hidden.add(m.ns);a.exclude.add(m.ns);a.join(m.ns);return m},close:function(j,l){j=j.toLowerCase();if(!(j in f.bds.link.connections)){return false}var k=f.bds.link.conn(j);if(l!==true){k.send("BDS:LINK:CLOSE")}a.part(k.ns);k.state=f.bds.link.state.closed;return true}};var b={cmd:function(j){if(!f.bds.channel.contains(j.ns)){return}if(j.sns[0]!="@"){return}f.bds.link.conn(j.sns.substr(1)).oncmd(j,a)},request:function(k){if(k.payload.toLowerCase()!=a.settings.username.toLowerCase()){return}var j=a.channel(k.ns).info.members[k.user];if(parseInt(a.channel(k.ns).get_privclass_order(j.pc))<39){if(!f.bds.link.white.contains(k.user)){a.npmsg(k.ns,"BDS:LINK:REJECT:"+k.user);return}}f.bds.link.open(k.user);a.npmsg(k.ns,"BDS:LINK:ACCEPT:"+k.user)},reject:function(k){var j=k.user.toLowerCase();if(!(j in f.bds.link.connections)){return}f.bds.link.connections[j].close()},accept:function(j){if(j.payload.toLowerCase()!=a.settings.username.toLowerCase()){return
}f.bds.link.open(j.user)},close:function(j){f.bds.link.close(j.user,true)},join:function(k){if(k.pkt.arg.e!="ok"){return}if(!f.bds.channel.contains(k.ns)){return}var j=f.bds.link.conn(k.sns.substr(1));if(a.channel(k.ns).get_usernames().length==1){return}j.state=f.bds.link.state.open;var l={name:"BDS.LINK.OPEN",link:j,ns:j.ns};j.onopen(l,a);a.trigger("BDS.LINK.OPEN",l)},part:function(k){if(!f.bds.channel.contains(k.ns)){return}var j=f.bds.link.conn(k.sns.substr(1));j.state=f.bds.link.state.closed;var l={name:"BDS.LINK.CLOSED",link:j,ns:j.ns};j.onclose(l,a);a.trigger("BDS.LINK.CLOSED",l);delete f.bds.link.connections[j.un]},recv_join:function(k){if(!f.bds.channel.contains(k.ns)){return}var j=f.bds.link.conn(k.sns.substr(1));if(j.state!=f.bds.link.state.opening){return}j.state=f.bds.link.state.open;var l={name:"BDS.LINK.OPEN",link:j,ns:j.ns};j.onopen(l,a);a.trigger("BDS.LINK.OPEN",l)},recv_part:function(k){if(!f.bds.channel.contains(k.ns)){return}var j=f.bds.link.conn(k.sns.substr(1));j.close(true)
},};g()};wsc.dAmn.Stash=function(a,h,b){var g=function(){a.ui.on("log_item.after",f.log_item)};var f={log_item:function(k){var j=k.item.find('a[href^="http://sta.sh"]');j.each(function(m,l){var n=k.item.find(l);wsc.dAmn.Stash.fetch(k,n)})}};g()};wsc.dAmn.Stash.fetch=function(b,a){$.getJSON("http://backend.deviantart.com/oembed?url="+a.prop("href")+"&format=jsonp&callback=?",function(f){wsc.dAmn.Stash.render(b,a,f)})};wsc.dAmn.Stash.render=function(r,j,x){if("error" in x){return}if(x.type!="photo"){return}var f=j.prop("href");var l=x.thumbnail_width;var s=x.thumbnail_height;var v,b;var y=x.title+" by "+x.author_name;var k='<a target="_blank" href="'+f+'" title="'+y+'">';if(l/s>1){b=parseInt((s*100)/l);v=100}else{v=parseInt((l*100)/s);b=100}if(v>l||b>s){v=l;b=s}var q='width="'+v+'" height="'+b+'"';var g=!x.thumbnail_url.match(/.png$/i)?" shadow":"";var o=f.split("/").pop();var u='<span class="stashthumb" id="'+o+'">'+k+'<img class="smaller thumb'+g+'"'+q+' " alt="'+f+'" src="'+x.thumbnail_url_150+'" />        </a><a href="#toggle" class="button iconic plus" title="Larger"></a></span>';
