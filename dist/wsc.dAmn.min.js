...
if(f.ns==null){f.ns=a.mns}f.sns=a.deform_ns(f.ns);this.handle(a,f);a.trigger("pkt",f);a.trigger("pkt."+f.name,f)};wsc.Flow.prototype.handle=function(a,f){try{this[f.name](f,a)}catch(b){}};wsc.Flow.prototype.ping=function(b,a){a.pong()};wsc.Flow.prototype.chatserver=function(b,a){a.login()};wsc.Flow.prototype.login=function(f,a){if(f.pkt.arg.e=="ok"){var g=f.pkt.sub[0];a.settings.username=f.pkt.param;a.settings.symbol=g.arg.symbol;a.settings.userinfo=g.arg;var b=function(){a.join(a.settings.autojoin);if(a.autojoin.on){for(var h in a.autojoin.channel){if(!a.autojoin.channel.hasOwnProperty(h)){continue}a.join(a.autojoin.channel[h])}}}}else{a.close(f)}if(a.fresh){a.fresh=false}};wsc.Flow.prototype.join=function(f,a){if(f.pkt.arg["e"]=="ok"){var b=a.deform_ns(f.pkt.param);a.create_ns(b,a.hidden.contains(f.pkt.param))}else{a.trigger("log",{name:"log",ns:"server:current",sns:"~current",msg:"Failed to join "+a.deform_ns(f.pkt.param),info:f.pkt.arg["e"]})}};wsc.Flow.prototype.part=function(f,a){var b=a.deform_ns(f.ns);
var g=a.channel(b);if(f.e=="ok"){info="";if(f.r.length>0){info="["+f.r+"]"}else{a.remove_ns(f.ns)}msg="You have left "+b;g.server_message(msg,info);if(a.channels()==0){switch(f.r){case"bad data":case"bad msg":case"msg too big":break;default:if(f.r.indexOf("killed:")<0){return}break}this.message(a,{data:"disconnect\ne="+e.r+"\n"})}}else{a.trigger("log",{name:"log",ns:"server:current",sns:"~current",msg:"Couldn't leave "+b,info:f.e})}};wsc.Flow.prototype.kicked=function(b,a){if(b.r.toLowerCase().indexOf("autokicked")>-1){return}a.join(b.ns)};wsc.Flow.prototype.property=function(b,a){chan=a.channel(b.pkt.param);if(!chan){return}chan.property(b)};wsc.Flow.prototype.recv_joinpart=function(b,a){c=a.channel(b.ns);if(b.name=="recv_join"){c.recv_join(b)}else{c.recv_part(b)}};wsc.Flow.prototype.recv_join=wsc.Flow.prototype.recv_joinpart;wsc.Flow.prototype.recv_part=wsc.Flow.prototype.recv_joinpart;wsc.Flow.prototype.recv_msg=function(b,a){a.channel(b.ns).recv_msg(b)};wsc.Flow.prototype.recv_action=wsc.Flow.prototype.recv_msg;
wsc.Flow.prototype.recv_npmsg=wsc.Flow.prototype.recv_msg;wsc.Flow.prototype.recv_privchg=function(b,a){a.channel(b.ns).recv_privchg(b)};wsc.Flow.prototype.recv_kicked=function(b,a){a.channel(b.ns).recv_kicked(b)};wsc.defaults.Extension=function(b){var f={};var k=function(){b.bind("cmd.connect",g.connection);b.bind("cmd.set",g.setter);b.bind("cmd.join",g.join);b.bind("cmd.chat",g.pjoin);b.bind("cmd.part",g.part);b.bind("cmd.say",g.say);b.bind("cmd.npmsg",g.npmsg);b.bind("cmd.me",g.action);b.bind("cmd.promote",g.chgpriv);b.bind("cmd.demote",g.chgpriv);b.bind("cmd.ban",g.ban);b.bind("cmd.unban",g.ban);b.bind("cmd.kick",g.killk);b.bind("cmd.whois",g.whois);b.bind("cmd.title",g.title);b.bind("cmd.topic",g.title);b.bind("cmd.admin",g.admin);b.bind("cmd.disconnect",g.connection);b.bind("cmd.kill",g.killk);b.bind("cmd.raw",g.raw);b.bind("cmd.close",g.close);b.bind("pkt.property",h);b.bind("pkt.recv_admin_show",a);b.bind("pkt.recv_admin_showverbose",a);b.bind("pkt.get",j)};f.save=function(m,l){b.settings.ui.theme=m.theme;
b.settings.ui.clock=m.clock;b.settings.ui.tabclose=m.tabclose};var g={};g.setter=function(n){var m=n.args.split(" ");var l=m.shift().toLowerCase();var m=m.join(" ");if(m.length==0){b.cchannel.serverMessage("Could not set "+l,"No data supplied");return}if(!(l in b.settings)){b.cchannel.serverMessage('Unknown setting "'+l+'"');return}b.settings[l]=m;b.cchannel.serverMessage("Changed "+l+" setting","value: "+m)};g.connection=function(l){b[l.cmd]()};g.join=function(l){var m=l.args.split(" ");var m=m.toString()==""?[]:m;if(l.ns!=l.target){m.unshift(l.target)}if(m.toString()==""){return}for(index in m){b.join(m[index])}};g.pjoin=function(l){var m=l.args.split(" ");var m=m.toString()==""?[]:m;if(l.ns!=l.target){m.unshift(l.target)}if(m.toString()==""){return}for(index in m){b.join("@"+m[index])}};g.part=function(l){var m=l.args.split(" ");if(l.ns!=l.target){m.unshift(l.target)}if(m.toString()==""){b.part(l.ns);return}for(index in m){b.part(m[index])}};g.title=function(l){b.set(l.target,l.cmd,l.args)
};g.chgpriv=function(m){var l=m.args.split(" ");b[m.cmd.toLowerCase()](m.target,l[0],l[1])};g.ban=function(p,l){var n=p.args.split(" ");var m=n.shift();var o=p.cmd;if(o=="ban"&&n.length>0){l.kick(p.target,m,n.join(" "))}l[o](p.target,m)};g.action=function(l){b.action(l.target,l.args)};g.raw=function(l){b.send(l.args.replace(/\\n/gm,"\n"))};g.killk=function(o,l){var p=o.args.split(" ");var m=p.shift();var n=p.length>0?p.join(" "):null;if(o.cmd=="kick"){l.kick(o.target,m,n)}else{l.kill(m,n)}};g.say=function(l){if(b.channel(l.target).monitor){return}b.say(l.target,l.args)};g.npmsg=function(l){b.npmsg(l.target,l.args)};g.close=function(l){b.part(l.target);b.remove_ns(l.target)};g.whois=function(m,l){l.whois(m.args.split(" ")[0])};g.admin=function(m,l){l.admin(m.target,m.args)};g.disconnect=function(m,l){l.disconnect()};var h=function(p,l){if(p.p!="info"){return}var n=p.pkt.sub;var q=n.shift().arg;q.username=p.sns.substr(1);q.connections=[];var o=null;var r={};while(n.length>0){r=n.shift();switch(r.cmd){case"conn":if(o!=null){q.connections.push(o)
}o=r.arg;o.channels=[];break;case"agent":o.agent=[["agent",r.arg.agent]];if(r.arg.hasOwnProperty("url")){o.agent.push(["url",r.arg.url])}if(r.arg.hasOwnProperty("browser")){o.agent.push(["browser",r.arg.browser])}for(var m in r.arg){if(m=="agent"||m=="url"||m=="browser"){continue}if(r.arg.hasOwnProperty(m)){o.agent.push([m,r.arg[m]])}}break;case"ns":o.channels.unshift(l.deform_ns(r.param));break}}q.connections.push(o);l.trigger("pkt.whois",{name:"pkt.whois",pkt:p.pkt,info:q})};var j=function(m,l){if(m.ns.indexOf("login:")!=0){return}var n=m.sns.substr(1);l.ui.pager.notice({ref:"whois-"+n,heading:"Whois Failed",content:"Whois failed for "+n+".\nNo such user online."},false,5000)};var a=function(p,m){var s=m.channel(p.ns);var l=p.info.split("\n");var r="";var q=[];var n="";for(var o in l){if(!l.hasOwnProperty(o)){continue}r=l[o].split(" ");if(p.p=="privclass"){q.push([r.shift(),r.shift().split("=")[1],r.join(" ")])}else{if(p.p=="users"){n=r.shift().split(":",1)[0];q.push([n,s.get_privclass_order(n),r.join(" ")])
}}}m.ui.chatbook.current.log_pc(p.p=="privclass",q)};k();wsc.defaults.Extension.Ignore(b,f);wsc.defaults.Extension.Away(b,f);wsc.defaults.Extension.Autojoin(b,f);return f};wsc.defaults.Extension.Autojoin=function(b,g){g.autojoin={on:false,channel:[]};var k=true;var j=b.storage.folder("autojoin");var h=function(){g.autojoin.load();g.autojoin.save();b.bind("cmd.autojoin",f);b.bind("pkt.login",a)};g.autojoin.load=function(){g.autojoin.on=(j.get("on","true")=="true");g.autojoin.channel=JSON.parse(j.get("channel","[]"))};g.autojoin.save=function(){j.set("on",g.autojoin.on.toString());j.set("channel",JSON.stringify(g.autojoin.channel))};g.autojoin.join=function(){for(var l in g.autojoin.channel){if(!g.autojoin.channel.hasOwnProperty(l)){continue}b.join(g.autojoin.channel[l])}};g.autojoin.add=function(l){if(g.autojoin.channel.indexOf(l)!=-1){return false}g.autojoin.channel.push(l);g.autojoin.save();return true};g.autojoin.remove=function(m){var l=g.autojoin.channel.indexOf(m);if(l==-1){return false
}g.autojoin.channel.splice(l,1);g.autojoin.save();return true};var a=function(m,l){if(m.e!="ok"){return}if(k){l.join(l.settings.autojoin);if(g.autojoin.on){g.autojoin.join()}}else{var n=false;for(key in l.channelo){if(l.channelo[key].namespace[0]!="~"){l.join(key);n=true}}if(!n){g.autojoin.join()}}k=false};var f=function(m){var q=m.args.split(" ");if(!q){return}var u=q.shift().toLowerCase();var r=null;var v="";var n="";var s=false;var l=b.channel(m.ns);switch(u){case"add":r=g.autojoin.add;v="Added {item} to your autojoin.";n="Already have {item} on your autojoin.";break;case"rem":case"remove":r=g.autojoin.remove;v="Removed {item} from your autojoin.";n=w+" is not on your autojoin list.";break;case"on":b.trigger("log",{name:"log",ns:"server:current",sns:"~current",msg:"Autojoin on"});if(!g.autojoin.on){s=true;g.autojoin.on=true}break;case"off":b.trigger("log",{name:"log",ns:"server:current",sns:"~current",msg:"Autojoin off"});if(g.autojoin.on){s=true;g.autojoin.on=false}break;default:g.autojoin.join();
break}var w="";var p=false;if(r!=null){for(var o in q){if(!q.hasOwnProperty(o)){continue}w=b.deform_ns(q[o]).toLowerCase();p=r(w);b.trigger("log",{name:"log",ns:"server:current",sns:"~current",msg:replaceAll((p?v:n),"{item}",w)})}}if(s){g.autojoin.save()}};h()};wsc.defaults.Extension.Away=function(b,g){var k=b.storage.folder("away");g.away={on:false,reason:"",last:{},since:0,interval:60000,format:{setaway:"/me is away: {reason}",setback:"/me is back",away:"{user}: I've been away for {timesince}. Reason: {reason}"}};b.away=g.away;var j=function(){g.away.load();g.away.save();b.bind("cmd.setaway",f);b.bind("cmd.setback",h);b.bind("pkt.recv_msg",a);b.bind("pkt.recv_action",a)};var f=function(n,l){g.away.on=true;g.away.last={};g.away.since=new Date();g.away.reason=n.args;var o=l.say;var m=replaceAll(g.away.format.setaway,"{reason}",g.away.reason||"[silent away]");if(m.indexOf("/me ")==0){m=m.substr(4);o=l.action}l.each_channel(function(p){o.call(l,p,m)});l.trigger("ext.away.away",{name:"ext.away.away",reason:g.away.reason||"[silent away]"})
};var h=function(n,l){if(!g.away.on){return}g.away.on=false;var o=l.say;var m=g.away.format.setback;if(m.indexOf("/me ")==0){m=m.substr(4);o=l.action}l.each_channel(function(p){o.call(l,p,m)});l.trigger("ext.away.back",{name:"ext.away.back"})};var a=function(p){if(!g.away.on){return}if(g.away.reason.length==0){return}if(b.exclude.contains(p.ns)){return}var q=p.user.toLowerCase();var m=b.settings.username.toLowerCase();if(q==m){return}if(p.message.text().toLowerCase().indexOf(m)==-1){return}var n=new Date();var o=p.sns.toLowerCase();if(o in g.away.last){if((n-g.away.last[o])<=g.away.interval){return}}var l=timeLengthString((n-g.away.since)/1000);var r=replaceAll(g.away.format.away,"{user}",p.user);r=replaceAll(r,"{timesince}",l);b.say(p.ns,replaceAll(r,"{reason}",g.away.reason));g.away.last[o]=n};g.away.load=function(){g.away.format.setaway=k.get("setaway","/me is away: {reason}");g.away.format.setback=k.get("setback","/me is back");g.away.format.away=k.get("away","{user}: I've been away for {timesince}. Reason: {reason}");
g.away.interval=parseInt(k.get("interval",60000))};g.away.save=function(){k.set("interval",g.away.interval);k.set("setaway",g.away.format.setaway);k.set("setback",g.away.format.setback);k.set("away",g.away.format.away)};j()};wsc.defaults.Extension.Ignore=function(a,f){var b={};var k=a.storage.folder("ignore");f.ignore={fignore:"/me is ignoring {user} now",funignore:"/me is not ignoring {user} anymore",ignored:[]};var j=function(){f.ignore.load();f.ignore.save();a.bind("cmd.ignore",g);a.bind("cmd.unignore",h)};f.ignore.load=function(){f.ignore.fignore=k.get("ignore",f.ignore.fignore);f.ignore.funignore=k.get("unignore",f.ignore.funignore);f.ignore.ignored=JSON.parse(k.get("ignored",JSON.stringify(f.ignore.ignored)));a.trigger("ignore.loaded",{name:"ignore.loaded",ignored:f.ignore.ignored})};f.ignore.save=function(){k.set("ignore",f.ignore.fignore);k.set("unignore",f.ignore.funignore);k.set("ignored",JSON.stringify(f.ignore.ignored))};f.ignore.add=function(l,o){var l=l.split(" ");var m="";
o=o||false;for(var n=0;n<l.length;n++){if(!l.hasOwnProperty(n)){continue}m=l[n].toLowerCase();if(f.ignore.ignored.indexOf(m)!=-1){continue}f.ignore.ignored.push(m);a.trigger("ignore.add",{name:"ignore.add",user:l[n],suppress:o})}f.ignore.save()};f.ignore.remove=function(l,p){var l=l.split(" ");var m="";var o=-1;p=p||false;for(var n=0;n<l.length;n++){if(!l.hasOwnProperty(n)){continue}m=l[n].toLowerCase();o=f.ignore.ignored.indexOf(m);if(o==-1){continue}f.ignore.ignored.splice(o,1);a.trigger("ignore.remove",{name:"ignore.remove",user:l[n],suppress:p})}f.ignore.save()};var g=function(l){f.ignore.add(l.args)};var h=function(l){f.ignore.remove(l.args)};j()};wsc.Client=function(b,f,g){this.mozilla=g;this.storage=new wsc.Storage;this.storage.aj=this.storage.folder("autojoin");this.storage.aj.channel=this.storage.aj.folder("channel");this.fresh=true;this.attempts=0;this.connected=false;this.protocol=null;this.flow=null;this.ui=null;this.events=new EventEmitter();this.conn=null;this.channelo={};
this.cchannel=null;this.cmds=[];this.settings={domain:"website.com",server:"ws://website.com/wsendpoint",symbol:"",username:"",userinfo:{},pk:"",monitor:["~Monitor",true],welcome:"Welcome to the wsc web client!",autojoin:"chat:channel",protocol:wsc.Protocol,mparser:wsc.MessageParser,flow:wsc.Flow,extend:[wsc.defaults.Extension],client:"chatclient",clientver:"0.3",developer:false};var a=this;this.exclude=new StringSet();this.hidden=new StringSet();this.settings=Object.extend(this.settings,f);this.config_load();this.config_save();this.mw=new wsc.Middleware();this.settings.agent="Client ("+navigator.platform+"; HTML5; JavaScript) wsc/"+wsc.VERSION+"-r"+wsc.REVISION;this.mns=this.format_ns(this.settings.monitor[0]);this.lun=this.settings.username.toLowerCase();this.protocol=new this.settings.protocol(new this.settings.mparser());this.flow=new this.settings.flow(this.protocol);this.build();this.ext={};this.ext.defaults=wsc.defaults.Extension(this)};wsc.Client.prototype.config_load=function(){this.settings.developer=(this.storage.get("developer",this.settings.developer.toString())=="true")
};wsc.Client.prototype.config_save=function(){this.storage.set("developer",this.settings.developer)};wsc.Client.prototype.build=function(){this.create_ns(this.settings.monitor[0],true,true)};wsc.Client.prototype.loop=function(){};wsc.Client.prototype.add_extension=function(a){this.settings.extend.push(a);a(this)};wsc.Client.prototype.bind=function(b,a){this.events.addListener(b,a);if(b.indexOf("cmd.")!=0||b.length<=4){return}cmd=b.slice(4).toLowerCase();this.cmds.push(cmd)};wsc.Client.prototype.clear_listeners=function(a){this.events.removeListeners(a)};wsc.Client.prototype.trigger=function(a,b){return this.events.emit(a,b,this)};wsc.Client.prototype.middle=function(a,b){return this.mw.add(a,b)};wsc.Client.prototype.cascade=function(a,f,b){this.mw.run(a,f,b)};wsc.Client.prototype.connect=function(){if(this.connected){return}this.attempts++;try{var a=this;this.conn=wsc.Transport.Create(this.settings.server);this.conn.open(function(f,g){a.flow.open(a,f,g)});this.conn.disconnect(function(f){a.flow.close(a,f)
});this.conn.message(function(f){a.flow.message(a,f)});this.conn.connect();this.trigger("start",new wsc.Packet("client connecting\ne=ok\n\n"))}catch(b){console.log(b);this.trigger("start",new wsc.Packet("client connecting\ne=no websockets available\n\n"))}};wsc.Client.prototype.close=function(a){this.conn.close(a)};wsc.Client.prototype.channel=function(a,b){a=this.format_ns(a).toLowerCase();if(!this.channelo[a]){if(b){this.channelo[a]=b;return b}return null}return this.channelo[a]};wsc.Client.prototype.channels=function(b){var a=[];for(ns in this.channelo){if(!this.channelo.hasOwnProperty(ns)){continue}if(this.channelo[ns].hidden){continue}a.push(this.channelo[ns].namespace)}return b?a:a.length};wsc.Client.prototype.each_channel=function(g,a){var f=null;for(var b in this.channelo){if(!this.channelo.hasOwnProperty(b)){continue}f=this.channelo[b];if(!a){if(this.exclude.contains(f.raw)){continue}}if(g(f.raw,f)===false){break}}};wsc.Client.prototype.deform_ns=function(b){var a=b[0];if(a=="#"||a=="@"||a=="~"||a=="+"){return b
}if(b.indexOf("chat:")==0){return"#"+b.slice(5)}if(b.indexOf("server:")==0){return"~"+b.slice(7)}if(b.indexOf("feed:")==0){return"#"+b.slice(5)}if(b.indexOf("login:")==0){return"@"+b.slice(6)}if(b.indexOf("pchat:")==0){var f=b.split(":");f.shift();for(i in f){name=f[i];if(name.toLowerCase()!=this.lun){return"@"+name}}}return"#"+b};wsc.Client.prototype.format_ns=function(a){var f=a.slice(1);switch(a[0]){case"@":var b=[f,this.lun];b.sort(caseInsensitiveSort);b.unshift("pchat");a=b.join(":");break;case"~":a="server:"+f;break;case"+":a="feed:"+f;break;case"#":a="chat:"+f;break;default:if(a.indexOf("chat:")==0||a.indexOf("pchat:")==0||a.indexOf("server:")==0||a.indexOf("feed:")==0){break}a="chat:"+a;break}return a};wsc.Client.prototype.create_ns=function(b,f,a){var g=this.channel(b,new wsc.Channel(this,b,f,a));this.trigger("ns.create",{name:"ns.create",ns:b,chan:g,client:this});g.build()};wsc.Client.prototype.remove_ns=function(a){if(!a){return}this.cascade("ns.remove",function(b){var f=b.client.channel(b.ns);
if(!f){return}delete b.client.channelo[f.raw.toLowerCase()]},{ns:a,client:this})};wsc.Client.prototype.select_ns=function(a){this.cchannel=this.channel(a)||this.cchannel};wsc.Client.prototype.log=function(a,b){var f=this.channel(a);if(!f){return}f.log(b)};wsc.Client.prototype.monitor=function(a){console.log(a)};wsc.Client.prototype.mute_user=function(a){};wsc.Client.prototype.unmute_user=function(a){};wsc.Client.prototype.send=function(a){return this.conn.send(a)};wsc.Client.prototype.handshake=function(){this.send(wsc_packetstr(this.settings.client,this.settings.clientver,{agent:this.settings.agent,url:window.location.href,browser:navigator.userAgent}))};wsc.Client.prototype.login=function(){pkt="login "+this.settings.username+"\npk="+this.settings.pk+"\n";this.send(pkt)};wsc.Client.prototype.pong=function(){this.send(wsc_packetstr("pong"))};wsc.Client.prototype.join=function(a){this.send(wsc_packetstr("join",this.format_ns(a)))};wsc.Client.prototype.part=function(a){this.send(wsc_packetstr("part",this.format_ns(a)))
...
};wsc.Client.prototype.kick=function(b,a,f){var g=this;this.cascade("send.kick",function(h){g.send(wsc_packetstr("kick",g.format_ns(h.ns),{u:h.user},h.input||null))},{input:f||"",ns:b,user:a})};wsc.Client.prototype.kill=function(a,b){this.send(wsc_packetstr("kill","login:"+a,{},b||null))};wsc.Client.prototype.admin=function(a,b){this.send(wsc_packetstr("send",this.format_ns(a),{},wsc_packetstr("admin","",{},b)))};wsc.Client.prototype.property=function(a,b){this.send(wsc_packetstr("get",this.format_ns(a),{p:b}))};wsc.Client.prototype.set=function(a,f,b){var g=this;this.cascade("send.set",function(h){g.send(wsc_packetstr("set",g.format_ns(h.ns),{p:h.property},h.input))},{input:b,ns:a,property:f})};wsc.Client.prototype.whois=function(a){this.send(wsc_packetstr("get","login:"+a,{p:"info"}))};wsc.Client.prototype.disconnect=function(){this.send(wsc_packetstr("disconnect"))};wsc.dAmn={};wsc.dAmn.VERSION="0.10.36";wsc.dAmn.STATE="alpha";wsc.dAmn.wsc=function(a,f){a.settings.client="dAmnClient";
a.settings.clientver="0.3";a.settings.domain="deviantart.com";a.settings.agent+=" wsc/dAmn/"+wsc.dAmn.VERSION;var g=a.storage.folder("dAmn");g.bds=g.folder("bds");g.emotes=g.folder("emotes");g.colours=g.folder("colours");var b={emotes:{on:false,emote:[]},colours:{on:false,send:false,msg:"000000",user:"000000"}};b.save=function(){g.emotes.set("on",b.emotes.on);g.colours.set("on",b.colours.on);g.colours.set("send",b.colours.send);g.colours.set("msg",b.colours.msg);g.colours.set("user",b.colours.user)};b.load=function(){b.emotes.on=(g.emotes.get("on","false")=="true");b.colours.on=(g.colours.get("on","false")=="true");b.colours.send=(g.colours.get("send","false")=="true");b.colours.msg=g.colours.get("msg","");b.colours.user=g.colours.get("user","")};b.load();b.save();a.protocol.extend_maps({dAmnServer:["version"]});a.protocol.mparser=new wsc.dAmn.TablumpParser;a.flow.dAmnServer=a.flow.chatserver;a.exclude.add("chat:devart");a.exclude.add("chat:damnidlers");wsc.dAmn.BDS(a,g.bds,b);wsc.dAmn.BDS.Link(a,g.bds,b);
wsc.dAmn.BDS.Peer(a,g.bds,b);wsc.dAmn.wsc.Colours(a,g.colours,b);wsc.dAmn.wsc.Emotes(a,g.emotes,b);return b};wsc.dAmn.chatterbox=function(f){var a=f.client;var b={colours:{},emotes:{}};f.on("settings.save",a.ext.dAmn.save);f.on("settings.close",a.ext.dAmn.load);f.protocol.extend_messages({dAmnServer:{keys:["version"],template:'<span class="servermsg">** Connected to dAmnServer {version} *</span>',global:true}});f.middle("user.hover",function(h,g){h.avatar=wsc.dAmn.avatar.link(h.name,h.member.usericon);if(h.member.realname&&h.info.indexOf(h.member.realname)==-1){h.info.push(h.member.realname)}if(h.member.typename&&h.info.indexOf(h.member.typename)==-1){h.info.push(h.member.typename)}g(h)});f.on("log_whois.before",function(g,h){g.avatar=wsc.dAmn.avatar.link(g.raw.username,g.raw.usericon);g.username=g.raw.symbol+'<b><a href="http://'+g.raw.username+'.deviantart.com/">'+g.raw.username+"</a></b>";if(g.raw.realname){g.info.push(g.raw.realname)}if(g.raw.typename){g.info.push(g.raw.typename)}});
wsc.dAmn.chatterbox.Colours(a,f,b);wsc.dAmn.chatterbox.Emotes(a,f,b);wsc.dAmn.chatterbox.Stash(f,b);return b};wsc.dAmn.BDS=function(f,k,g){var h={};var a={};g.bds={version:"0.4",mns:"chat:datashare",gate:"chat:dsgateway",channel:(new StringSet()),provides:["BOTCHECK","CLINK"]};f.bds=g.bds;g.bds.channel.add(g.bds.mns);g.bds.channel.add(g.bds.gate);var l=function(){f.hidden.add(g.bds.mns);f.exclude.add(g.bds.mns);f.hidden.add(g.bds.gate);f.exclude.add(g.bds.gate);f.bind("pkt.login",m);f.bind("pkt.recv_msg",b);f.bind("pkt.join",j.join);f.bind("BDS.BOTCHECK.DIRECT",j.botcheck);f.bind("BDS.BOTCHECK.ALL",j.botcheck);f.bind("BDS.BOTCHECK.OK",j.checkresp);f.bind("BDS.BOTCHECK.DENIED",j.checkresp);f.bind("CDS.LINK.REQUEST",j.clreq);f.bind("CDS.LINK.REJECT",j.clrj);f.bind("CDS.LINK.ACK",j.clra);f.bind("pkt.recv_join",j.pcrj);f.bind("pkt.recv_part",j.pcrp);f.bind("pkt.property",j.pcp);f.bind("closed",j.closed);f.middle("chan.recv_msg",function(o,n){j.hfilter(o,n)})};var m=function(n){if(n.pkt.arg.e!="ok"){return
}f.join(g.bds.gate)};var b=function(q){if(q.user.toLowerCase()==f.settings.username.toLowerCase()){return}if(!g.bds.channel.contains(q.ns)){return}var p={ns:q.ns,sns:q.sns,message:q.message,user:q.user,name:"",payload:"",head:[null,null,null],params:[]};var s=q.message.split(":");var o=[null,null,null];var r=null;for(var n in o){o[n]=s.shift()||null;if(o[n]==null){return}}r=s.join(":");p.name=o.join(".");p.payload=r;p.head=o;p.param=r.split(",");f.trigger(o[0],p);f.trigger(o[0]+"."+o[1],p);f.trigger(p.name,p)};var j={filter:function(o,n){if(f.settings.developer){n(o);return}if(o.ns[0]!="@"){n(o);return}var p=o.message.match(/<span class="cmsg u-([^"]+)">(.*)<\/span>/);if(!p){n(o);return}if(p[2].match(/^([A-Z0-9-_]+):([A-Z0-9-_]+):([A-Z0-9-_]+)(:.*|)$/)){return}n(o)},hfilter:function(o,n){if(f.settings.developer){n(o);return}if(o.sns[0]!="@"){n(o);return}if(o.message.match(/^([A-Z0-9-_]+):([A-Z0-9-_]+):([A-Z0-9-_]+)(:.*|)$/)){return}n(o)},closed:function(n){f.remove_ns(g.bds.mns);f.remove_ns(g.bds.gate)
},join:function(n){if(n.ns.toLowerCase()!=g.bds.mns){return}if(n.pkt.arg.e!="ok"){return}},botcheck:function(o){if(o.head[2]!="ALL"&&o.payload!=f.settings.username){return}if(o.ns.toLowerCase()==g.bds.gate&&f.channel(g.bds.mns)!=null){return}var n=wsc.VERSION+"/0.0.0/"+wsc.dAmn.VERSION+"/"+g.bds.version;var p=CryptoJS.MD5(("wsc.dAmn"+n+f.settings.username+o.user).toLowerCase());f.npmsg(o.ns,"BDS:BOTCHECK:CLIENT:"+o.user+",wsc.dAmn,"+n+","+p)},checkresp:function(n){if(n.ns.toLowerCase()!=g.bds.gate){return}if(n.param[0].toLowerCase()!=f.settings.username.toLowerCase()){return}if(f.channel(n.ns).info.members[f.settings.username].pc=="Visitors"){f.part(n.ns)}if(n.head[2]=="OK"){if(f.channel(g.bds.mns)==null){f.join(g.bds.mns)}return}f.ui.pager.notice({ref:"botcheckdenied",heading:"BDS Error",content:"Denied entry to backend channel.\nReason provided: <code>"+n.param.slice(1).join(",")+"</code>"})},clreq:function(r){if(r.payload.toLowerCase()!=f.settings.username.toLowerCase()){return}if(f.ui.umuted.indexOf(r.user.toLowerCase())!=-1){f.npmsg(r.ns,"CDS:LINK:REJECT:"+r.user+",You have been blocked");
return}if(f.away.on){var p=new Date();var q=r.sns.toLowerCase();var n=timeLengthString((p-f.away.since)/1000);var s=replaceAll(f.away.format.away,"{user}",r.user);s=replaceAll(s,"{timesince}",n);f.npmsg(r.ns,"CDS:LINK:REJECT:"+r.user+",Away; "+f.away.reason);return}f.npmsg(r.ns,"CDS:LINK:ACK:"+r.user);var o=f.ui.pager.notice({ref:"clink-"+r.user,icon:'<img src="'+wsc.dAmn.avatar.src(r.user,f.channel(r.ns).info.members[r.user].usericon)+'" />',heading:"Chat "+r.user,content:r.user+" wants to talk in private.\nType <code>/chat "+r.user+"</code> to talk to them, or use the buttons below.",buttons:{accept:{ref:"accept",target:"accept",label:"Accept",title:"Join the private chat",click:function(){f.ui.pager.remove_notice(o);f.join("@"+r.user);return false}},reject:{ref:"reject",target:"reject",label:"Reject",title:"Reject the private chat",click:function(){f.npmsg(r.ns,"CDS:LINK:REJECT:"+r.user);f.ui.pager.remove_notice(o);return false}}}},true);a[r.user]=o;o.onclose=function(){f.npmsg(r.ns,"CDS:LINK:REJECT:"+r.user)
}},clrj:function(q){var n=q.user.toLowerCase();var r=q.payload.split(",");var o=r.shift();r=r.join(",");if(o.toLowerCase()!=f.settings.username.toLowerCase()){return}if(!(n in h)){return}clearTimeout(h[n]);f.channel("@"+n).server_message("Chat request rejected",r)},clra:function(q){var n=q.user.toLowerCase();var r=q.payload.split(",");var o=r.shift();if(o.toLowerCase()!=f.settings.username.toLowerCase()){return}clearTimeout(h[n])},pcp:function(p){if(p.ns.indexOf("pchat")!=0){return}if(f.bds.channel.contains(p.ns)){return}if(p.pkt.arg.p!="members"){return}if(f.channel(p.ns).get_usernames().length==2){f.bds.channel.add(p.ns);return}var n=p.sns.substr(1);var o=p.ns;f.ui.channel(o).server_message(n+" has not yet joined","Sending them a notice...");f.npmsg("chat:datashare","CDS:LINK:REQUEST:"+n);h[n.toLowerCase()]=setTimeout(function(){try{f.ui.channel(o).server_message("Notification failed",n+" does not seem to be using a client that supports pchat notices.")}catch(q){}},10000)},pcrj:function(o){if(o.sns[0]!="@"){return
}try{clearTimeout(h[o.user.toLowerCase()])}catch(n){}f.bds.channel.add(o.ns);if(!(o.user in a)){return}f.ui.pager.remove_notice(a[o.user])},pcrp:function(n){if(n.sns[0]!="@"){return}f.bds.channel.remove(n.ns)}};l()};wsc.dAmn.wsc.Colours=function(a,f,b){b.colours.send_colour=function(h,g){if(!b.colours.send){g(h);return}h.input+='<abbr title="colors:'+b.colours.user+":"+b.colours.msg+'"></abbr>';g(h)};a.middle("send.msg",b.colours.send_colour);a.middle("send.action",b.colours.send_colour)};wsc.dAmn.chatterbox.Colours=function(a,g,f){var b=a.ext.dAmn;f.colours.page=null;f.colours.configure_page=function(h,k){var j=h.settings.page("Colours");f.colours.page=j;var l={};l.on=b.colours.on;l.send=b.colours.send;l.msg=b.colours.msg;l.user=b.colours.user;j.item("Form",{ref:"switch",title:"Enable Chromacity Colours",text:"Here you can turn chromacity colours on and off.\n\nYou                    can choose to parse other people's colours, and send your                    own colours.",fields:[["Checkbox",{ref:"enabled",items:[{value:"parse",title:"Parse colours",selected:l.on},{value:"send",title:"Send colours",selected:l.send}]}]],event:{change:function(m){b.colours.on=(m.data.enabled.indexOf("parse")!=-1);
b.colours.send=(m.data.enabled.indexOf("send")!=-1)},save:function(m){l.on=b.colours.on;l.send=b.colours.send},close:function(m){b.colours.on=l.on;b.colours.send=l.send}}});j.item("Form",{ref:"colours",title:"Custom Colours",text:"Here you can set your own username colour and message colour.",fields:[["Colour",{ref:"usr",label:"Username Colour","default":"#"+l.user}],["Colour",{ref:"msg",label:"Message Colour","default":"#"+l.msg}]],event:{change:function(m){b.colours.user=m.data.usr.substr(1).toUpperCase();b.colours.msg=m.data.msg.substr(1).toUpperCase()},save:function(m){l.user=b.colours.user;l.msg=b.colours.msg},close:function(m){b.colours.user=l.user;b.colours.msg=l.msg;f.colours.page=null},}})};f.colours.parse_colour=function(k){if(!b.colours.on){return}var j=k.item.find("abbr");if(j.length==0){return}j=j.last();var h=j.prop("title").match(/colors:([A-F0-9]{6}):([A-F0-9]{6})/);if(h==null){return}j.remove();k.item.find(".cmsg, .caction").css("color","#"+h[2]);k.item.find(".cmsg.user, .caction.user").css("color","#"+h[1])
};g.on("settings.open",f.colours.configure_page);g.on("log_item.after",f.colours.parse_colour)};wsc.dAmn.Emotes={};wsc.dAmn.wsc.Emotes=function(a,f,b){b.emotes.emote={};b.emotes.fint=null;b.emotes.fetching=false;b.emotes.loaded=false;b.emotes.fetch=function(){if(b.emotes.loaded){}else{}b.emotes.fetching=true;jQuery.getJSON("http://www.thezikes.org/publicemotes.php?format=jsonp&jsoncallback=?&"+(new Date()).getDay(),function(g){b.emotes.fetching=false;b.emotes.emote=g;if(!b.emotes.loaded){if(b.emotes.on){a.trigger("dAmn.emotes.loaded",{name:"dAmn.emotes.loaded"})}}b.emotes.sort();b.emotes.loaded=true;b.emotes.fint=setTimeout(b.emotes.fetch,3600000)},function(){return false})};b.emotes.swap=function(m,g){if(!b.emotes.on){g(m);return}var h=m.input.match(/:([\S]+):/g);var l="";var k=-1;var j=function(n){return n!=h[k]};for(k in h){l=h[k];h=h.filter(j);if(!b.emotes.emote.hasOwnProperty(l)){continue}m.input=replaceAll(m.input,l,":thumb"+b.emotes.emote[l]["devid"]+":")}m.input=replaceAll(m.input,":B",":bucktooth:");
g(m)};b.emotes.prepare=function(g){g=replaceAll(g,"&","&amp;");g=replaceAll(g,"<","&lt;");g=replaceAll(g,">","&gt;");return replaceAll(g,'"',"&quot;")};b.emotes.repair=function(g){g=replaceAll(g,"&quot;",'"');g=replaceAll(g,"&lt;","<");g=replaceAll(g,"&gt;",">");return replaceAll(g,"&amp;","&")};b.emotes.sort=function(){var m=[[],[],[],[],[],[],[],[],[],[],[],[],[],[],[],[],[],[],[],[],[],[],[],[],[],[],[],];var l="ABCDEFGHIJKLMNOPQRSTUVWXYZ#";var n=null;var h=null;var g=false;var k=-1;var j=true};b.emotes.select=function(g){a.ui.control.set_text(a.ui.control.get_text()+b.emotes.repair(g))};a.middle("send.msg",b.emotes.swap);a.middle("send.action",b.emotes.swap);a.middle("send.kick",b.emotes.swap);a.middle("send.set",b.emotes.swap);if(!b.emotes.on){return}b.emotes.fetch()};wsc.dAmn.chatterbox.Emotes=function(a,g,f){var b=a.ext.dAmn;f.emotes.page=null;f.emotes.configure_page=function(j,l){var k=j.settings.page("Emotes");f.emotes.page=k;var m={};m.on=b.emotes.on;var h="";if(m.on){if(!b.emotes.loaded||b.emotes.fetching){h="<em>Fetching emotes...</em>"
}else{h="<em>Emotes loaded.</em>"}}k.item("Form",{ref:"switch",title:"Enable Emotes",text:"Here you can turn custom emotes on and off.",fields:[["Checkbox",{ref:"enabled",items:[{value:"yes",title:"On",selected:m.on}]}]],event:{change:function(n){b.emotes.on=(n.data.enabled.indexOf("yes")!=-1);b.emotes.picker.state(b.emotes.on);if(b.emotes.on){b.emotes.fetch();return}if(b.emotes.fint===null){return}clearTimeout(b.emotes.fint);b.emotes.fint=null},save:function(n){m.on=b.emotes.on;if(!b.emotes.on){}else{if(!b.emotes.fetching){}}},close:function(n){b.emotes.on=m.on;b.load();b.emotes.page=null;if(b.emotes.fint===null){return}clearTimeout(b.emotes.fint);b.emotes.fint=null}}})};g.on("settings.open",f.emotes.configure_page);a.bind("dAmn.emotes.loaded",function(j,h){g.pager.notice({ref:"emotes-loaded",heading:"Emote CLOUD",content:"Emoticons from zike's Emote CLOUD have been loaded."},false,5000,true)})};wsc.dAmn.Emotes.Tablumps=function(g){var m={id:g[0],thumb:g[4],server:parseInt(g[3]),flags:g[5].split(":"),dimensions:"",title:"",anchor:"",otitle:g[1]};
