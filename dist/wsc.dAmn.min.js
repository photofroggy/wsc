...
wsc.Flow.prototype.recv_privchg=function(b,a){a.channel(b.ns).recv_privchg(b)};wsc.Flow.prototype.recv_kicked=function(b,a){a.channel(b.ns).recv_kicked(b)};wsc.defaults.Extension.Ignore=function(g){var j={};var k=g.folder("ignore");var a=k.folder("ignored");var q=function(){p();m();g.bind("cmd.ignore",b);g.bind("cmd.unignore",l);g.bind("settings.open",j.page)};j.page=function(r,s){};var b=function(r){};var l=function(r){};var p=function(){j.ignore=k.get("ignore","/me is ignoring {user} now");j.unignore=k.get("unignore","/me is not ignoring {user} anymore");j.count=parseInt(k.get("count",0));j.ignored=[];if(j.count>0){var s=null;for(var r=0;r<j.count;r++){s=a.get(r,null);if(s===null){continue}j.ignored.push(s)}}};var m=function(){k.set("ignore",j.ignore);k.set("unignore",j.unignore);if(j.count==0){k.set("count",0)}else{var s=-1;for(var r in j.ignored.length){if(!j.ignored.hasOwnProperty(r)){continue}s++;a.set(s,j.ignored[r])}k.set("count",s)}};q()};wsc.defaults.Extension=function(p){var r={};
r.away={on:false,reason:"",last:{},since:0};var G=function(){p.bind("cmd.connect",g);p.bind("cmd.set",a);p.bind("cmd.join",L);p.bind("cmd.part",O);p.bind("cmd.say",u);p.bind("cmd.npmsg",P);p.bind("cmd.me",B);p.bind("cmd.promote",E);p.bind("cmd.demote",E);p.bind("cmd.ban",m);p.bind("cmd.unban",m);p.bind("cmd.kick",j);p.bind("cmd.whois",H);p.bind("cmd.title",k);p.bind("cmd.topic",k);p.bind("cmd.admin",F);p.bind("cmd.disconnect",g);p.bind("cmd.kill",j);p.bind("cmd.raw",M);p.bind("cmd.clear",J);p.bind("cmd.clearall",I);p.bind("pkt.property",l);p.bind("pkt.recv_admin_show",N);p.bind("pkt.recv_admin_showverbose",N);p.bind("pkt.get",C);p.bind("cmd.gettitle",A);p.bind("cmd.gettopic",A);p.bind("cmd.setaway",v);p.bind("cmd.setback",s);p.bind("pkt.recv_msg.highlighted",z);p.bind("cmd.theme",b);p.ui.on("settings.open",q);p.ui.on("settings.open.ran",K)};var q=function(S,R){page=S.settings.page("Main");var T={};T.theme=replaceAll(p.ui.settings.theme,"wsct_","");T.clock=p.ui.clock();T.tc=p.ui.nav.closer();
T.username=p.settings.username;T.pk=p.settings.pk;var Q=[];for(i in p.ui.settings.themes){name=replaceAll(p.ui.settings.themes[i],"wsct_","");Q.push({value:name,title:name,selected:T.theme==name})}page.item("Text",{ref:"intro",title:"Main",text:"Use this window to view and change your settings.\n\nCheck                    the different pages to see what settings can be changed.",});page.item("Form",{ref:"login",title:"Login",text:"Here you can change the username and token used to                    log into the chat server.",fields:[["Textfield",{ref:"username",label:"Username","default":T.username}],["Textfield",{ref:"token",label:"Token","default":T.pk}]],event:{save:function(U){p.settings.username=U.data.username;p.settings.pk=U.data.token}}});page.item("Form",{ref:"ui",title:"UI",hint:"<b>Timestamp</b><br/>Choose between a 24 hour clock and                    a 12 hour clock.\n\n<b>Theme</b><br/>Change the look of the                    client.\n\n<b>Close Buttons</b><br/>Turn tab close buttons on/off.",fields:[["Dropdown",{ref:"theme",label:"Theme",items:Q}],["Dropdown",{ref:"clock",label:"Timestamp Format",items:[{value:"24",title:"24 hour",selected:T.clock},{value:"12",title:"12 hour",selected:!T.clock}]}],["Check",{ref:"tabclose",label:"Close Buttons",items:[{value:"yes",title:"On",selected:T.tc}]}],],event:{change:function(U){p.ui.clock(U.data.clock=="24");
