...
}var h=f.peer(b);if(!h){h=f.new_peer(b)}a.trigger("peer.answer",{name:"peer.answer",ns:g.ns,pns:f.pns,call:f,peer:h,answer:j});h.set_remote_description(j,1)};wsc.dAmn.BDS.Peer.SignalHandler.prototype.candidate=function(h,a){if(h.sns[0]!="@"){return}var f=a.bds.peer.call(h.param[0]);if(!f){return}var j=f.peer(h.param[1]);if(!j){return}var k=h.param[2];if(k.toLowerCase()!=a.settings.username.toLowerCase()){return}var b=JSON.parse(h.param.slice(3).join(","));if(!b){j.ice_completed();return}var g=new wsc.dAmn.BDS.Peer.RTC.IceCandidate(b);j.candidate(g);a.trigger("peer.candidate",{name:"peer.candidate",ns:h.ns,pns:f.pns,call:f,peer:j,candidate:b,ice:g})};wsc.dAmn.BDS.Peer.SignalHandler.prototype.close=function(f,a){if(f.sns[0]!="@"){return}var b=a.bds.peer.call(f.param[0]);if(!b){return}var g=b.peer(f.param[1]);if(!g){return}if(g.user.toLowerCase()==a.settings.username.toLowerCase()){return}g.close();a.trigger("peer.close",{name:"peer.close",ns:f.ns,evt:f,call:b,peer:g})};wsc.dAmn.BDS.Peer.RTC={PeerConnection:null,SessionDescription:null,IceCandidate:null,};
