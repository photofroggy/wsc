var wsc={};wsc.VERSION="0.9.63";wsc.STATE="beta";wsc.defaults={};wsc.defaults.theme="wsct_default";wsc.defaults.themes=["wsct_default","wsct_dAmn"];wsc.base={};wsc.base.UI=function(a,b,j,g){this.LIB="base";this.VERSION="1";this.STATE="rc";this.channels=[]};wsc.base.UI.prototype.build=function(){};wsc.base.UI.prototype.on=function(b,a){};wsc.base.UI.prototype.loop=function(){};wsc.base.UI.prototype.create_channel=function(a,b){};wsc.base.UI.prototype.remove_channel=function(a){};wsc.base.UI.prototype.channel=function(a){};wsc.base.UI.Channel=function(a,b){this.namespace=a;this.hidden=b};wsc.defaults.UI=wsc.base.UI;function EventEmitter(){var l={},a=this;function j(q,s){var p=l[q]||false;if(p===false){l[q]=[s];return a}l[q].unshift(s);return a}function g(p){l[p]=[];return a}function b(){var p=Array.prototype.slice.call(arguments);var v=p.shift();var s=l[v]||false;if(s===false){return a}for(var q in s){if(s.hasOwnProperty(q)){bubble=s[q].apply({},p);if(bubble===false){break}}}return a}function m(p){return l[p]||[]
...
this.logpanel.find("li.loginfo").remove();this.wrap.height(0);this.resize()};Chatterbox.Channel.prototype.log_info=function(g,b){data={ns:this.namespace,ref:g,content:b};this.manager.trigger("log_info.before",data);delete data.ns;this.wrap.append(Chatterbox.render("loginfobox",data));this.scroll();var j=this;var a=this.wrap.find("li."+data.ref);a.find("a.close").click(function(l){j.wrap.find(this).parent().remove();j.resize();return false});return a};Chatterbox.Channel.prototype.show_whois=function(q){var j={avatar:'<a href="#"><img height="50" width="50" alt="avatar"/></a>',username:"<b>"+q.symbol+q.username+"</b>",info:[],conns:[],raw:q,};for(var s in q.connections){var p=q.connections[s];var z=[];if(p.online){stamp=(new Date-(p.online*1000));z.push(["online",DateStamp(stamp/1000)+formatTime(" [{HH}:{mm}:{ss}]",new Date(stamp))])}if(p.idle){z.push(["idle",timeLengthString(p.idle)])}if(p.agent){z.push(["agent",p.agent])}if(p.debug){z.push(["debug",p.debug])}z.push(["chatrooms",p.channels.join(" ")]);
j.conns.push(z)}this.manager.trigger("log_whois.before",j);var a="";for(var s in j.conns){var m=j.conns[s];var A='<section class="conn"><p><em>connection '+((parseInt(s)+1).toString())+":</em></p>";A+="<ul>";for(x in m){A+="<li><strong>"+m[x][0]+":</strong> "+m[x][1]+"</li>"}A+="</ul>";a+=A+"</section>"}var l="";for(var s in j.info){l+="<li>"+j.info[s]+"</li>"}var v=this.log_info("whois-"+q.username,Chatterbox.render("whoiswrap",{avatar:j.avatar,info:Chatterbox.render("whoisinfo",{username:j.username,info:l,connections:a})}));var g=v.find("div.avatar");var b=v.find("div.info");b.width(v.find(".whoiswrap").width()-100);g.height(v.height()-10);this.scroll()};Chatterbox.Channel.prototype.set_header=function(a,b){headd=this.window.find("header div."+a);headd.replaceWith(Chatterbox.render("header",{head:a,content:b||""}));headd=this.window.find("header div."+a);if(b){headd.css({display:"block"})}else{this.window.find("header div."+a).css({display:"none"})}this.resize()};Chatterbox.Channel.prototype.get_header=function(a){return this.window.find("header div."+a)
...
return chan};Chatterbox.Chatbook.prototype.channel_object=function(a,b){return new Chatterbox.Channel(this.manager,a,b)};Chatterbox.Chatbook.prototype.toggle_channel=function(a){chan=this.channel(a);prev=chan;if(!chan){return}if(this.current){if(this.current==chan){return}this.current.hide();prev=this.current}chan.show();this.manager.control.focus();this.current=chan;this.manager.resize();if(this.trail.indexOf(chan.namespace)==-1){this.trail.push(chan.namespace)}this.manager.trigger("channel.selected",{ns:chan.namespace,chan:chan,prev:prev})};Chatterbox.Chatbook.prototype.remove_channel=function(a){if(this.channels()==0){return}chan=this.channel(a);chan.remove();delete this.chan[chan.selector];rpos=this.trail.indexOf(chan.namespace);this.trail.splice(rpos,1);if(this.current!=chan){return}select=this.trail[this.trail.length-1];this.toggle_channel(select);this.channel(select).resize()};Chatterbox.Chatbook.prototype.channel_left=function(){var b=this.current.namespace;var a=this.trail.indexOf(b);
if(a<1){return}var j=null;while(true){try{j=this.channel(this.trail[--a])}catch(g){return}if(!j.hidden){break}}this.toggle_channel(j.namespace)};Chatterbox.Chatbook.prototype.channel_right=function(){var b=this.current.namespace;var a=this.trail.indexOf(b);if(a==-1||a>=(this.trail.length-1)){return}var j=null;while(true){try{j=this.channel(this.trail[++a])}catch(g){return}if(!j.hidden){break}}this.toggle_channel(j.namespace)};Chatterbox.Chatbook.prototype.server_message=function(b,a){for(ns in this.chan){this.chan[ns].server_message(b,a)}};Chatterbox.Chatbook.prototype.log_item=function(a){for(ns in this.chan){this.chan[ns].log_item(a)}};Chatterbox.Chatbook.prototype.retime=function(){for(ns in this.chan){this.chan[ns].retime()}};Chatterbox.Control=function(b){this.manager=b;this.manager.view.append(Chatterbox.template.control);this.view=this.manager.view.find("div.chatcontrol");this.form=this.view.find("form.msg");this.input=this.form.find("input.msg");this.mli=this.form.find("textarea.msg");
...
this.ci=this.mli;this.manager.resize();return this.ml}this.mli.css("display","none");this.input.css("display","inline-block");this.ci=this.input;this.manager.resize();return this.mli};Chatterbox.Control.prototype._onkeypress=function(a){};Chatterbox.Control.prototype._onsubmit=function(a){};Chatterbox.Control.prototype.chomp=function(){d=this.ci.val();i=d.lastIndexOf(" ");if(i==-1){this.ci.val("");return d}chunk=d.slice(i+1);this.ci.val(d.slice(0,i));if(chunk.length==0){return this.chomp()}return chunk};Chatterbox.Control.prototype.unchomp=function(a){d=this.ci.val();if(!d){this.ci.val(a)}else{this.ci.val(d+" "+a)}};Chatterbox.Control.prototype.get_text=function(a){if(a==undefined){return this.ci.val()}this.ci.val(a||"");return this.ci.val()};Chatterbox.Control.prototype.set_text=function(a){this.ci.val(a||"")};Chatterbox.Navigation=function(a){this.manager=a;this.nav=this.manager.view.find("nav.tabs");this.tabs=this.nav.find("#chattabs");this.buttons=this.nav.find("#tabnav");this.tableft=this.buttons.find(".arrow_left");
...
};Chatterbox.template.settings.item.twopane={};Chatterbox.template.settings.item.twopane.frame='{title}<div class="twopane">                                        <div class="text left">                                            <p>{text}</p>                                        </div>                                        <div class="right">                                            {template}                                        </div>                                    </div>';Chatterbox.template.settings.item.twopane.wrap=function(a,b){if(!b.hasOwnProperty("text")){return a}a=replaceAll(Chatterbox.template.settings.item.twopane.frame,"{template}",replaceAll(a,"{title}",""));return a};Chatterbox.template.settings.item.text={};Chatterbox.template.settings.item.text.pre=Chatterbox.template.settings.item.hint.prep;Chatterbox.template.settings.item.text.post=Chatterbox.template.clean(["title","text"]);Chatterbox.template.settings.item.text.render={title:Chatterbox.template.settings.krender.title,text:Chatterbox.template.settings.krender.text};
Chatterbox.template.settings.item.text.frame="{title}<p>                                        {text}                                    </p>";Chatterbox.template.settings.item.dropdown={};Chatterbox.template.settings.item.dropdown.pre=[Chatterbox.template.settings.item.twopane.wrap,Chatterbox.template.settings.item.hint.prep];Chatterbox.template.settings.item.dropdown.render={title:Chatterbox.template.settings.krender.title,text:Chatterbox.template.settings.krender.text,items:Chatterbox.template.settings.krender.dditems};Chatterbox.template.settings.item.dropdown.post=Chatterbox.template.clean(["title","items"]);Chatterbox.template.settings.item.dropdown.events=[["change","select"],["inspect","select"]];Chatterbox.template.settings.item.dropdown.frame="{title}<form>                                                <select>                                                    {items}                                                </select>                                            </form>";
