...
Chatterbox.template.settings.item.form.field.text.pre=Chatterbox.template.settings.item.hint.prep;Chatterbox.template.settings.item.form.field.text.post=Chatterbox.template.clean(["title","text"]);Chatterbox.template.settings.item.form.field.text.render={title:Chatterbox.template.settings.krender.title,text:Chatterbox.template.settings.krender.text};Chatterbox.template.settings.item.form.field.text.frame="{title}<p>                                        {text}                                    </p>";wsc.dAmn={};wsc.dAmn.VERSION="0.2.9";wsc.dAmn.STATE="alpha";wsc.dAmn.Emotes=function(a,g,b){b.emotes.page=null;b.emotes.configure_page=function(k,m){var l=k.settings.page("Emotes");b.emotes.page=l;var p={};p.on=b.emotes.on;p.es=[];l.item("Form",{ref:"switch",title:"Enable Emotes",text:"Here you can turn custom emotes on and off.",fields:[["Checkbox",{ref:"enabled",items:[{value:"yes",title:"On",selected:p.on}]}]],event:{change:function(q){b.emotes.on=(q.data.enabled.indexOf("yes")!=-1);if(b.emotes.on){b.emotes.fetch()
}},save:function(q){p.on=b.emotes.on;b.save()},close:function(q){b.emotes.on=p.on;b.load();b.emotes.page=null}}});var j=l.item("Items",{ref:"emotes",title:"Emotes",text:"This is the list of emoticons you can use.",items:p.es})};a.ui.on("settings.open.ran",b.emotes.configure_page);b.emotes.fetch=function(){console.log("requesting...");jQuery.getJSON("http://www.thezikes.org/publicemotes.php?format=jsonp&jsoncallback=?&"+(new Date()).getDay(),function(j){b.emotes.emote=j;console.log(b.emotes)})};if(!b.emotes.on){return}b.emotes.fetch()};wsc.dAmn.Extension=function(a){a.settings.client="dAmnClient";a.settings.clientver="0.3";a.settings.domain="deviantart.com";a.settings.agent+=" wsc/dAmn/"+wsc.dAmn.VERSION;var g=a.storage.folder("dAmn");g.emotes=g.folder("emotes");var b={emotes:{on:false,emote:[]}};b.save=function(){g.emotes.set("on",b.emotes.on)};b.load=function(){b.emotes.on=(g.emotes.get("on","false")=="true")};b.load();b.save();a.protocol.extend_maps({dAmnServer:["version"]});a.protocol.extend_messages({dAmnServer:['<span class="servermsg">** Connected to dAmnServer {version} *</span>',false,true]});
