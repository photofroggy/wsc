...
p.bind("cmd.connect",g);p.bind("cmd.join",G);p.bind("cmd.part",J);p.bind("cmd.say",r);p.bind("cmd.npmsg",K);p.bind("cmd.me",u);p.bind("cmd.promote",z);p.bind("cmd.demote",z);p.bind("cmd.ban",m);p.bind("cmd.unban",m);p.bind("cmd.kick",j);p.bind("cmd.whois",C);p.bind("cmd.title",k);p.bind("cmd.topic",k);p.bind("cmd.admin",A);p.bind("cmd.disconnect",g);p.bind("cmd.kill",j);p.bind("cmd.raw",H);p.bind("cmd.clear",E);p.bind("cmd.clearall",D);p.bind("pkt.property",l);p.bind("pkt.recv_admin_show",I);p.bind("pkt.recv_admin_showverbose",I);p.bind("pkt.get",v);p.bind("cmd.gettopic",s);p.bind("cmd.theme",b);p.ui.on("settings.open",q);p.ui.on("settings.open.ran",F)};var q=function(N,M){page=N.settings.page("Main");var O={};O.theme=replaceAll(p.ui.settings.theme,"wsct_","");O.clock=p.ui.clock();O.tc=p.ui.nav.closer();O.username=p.settings.username;O.pk=p.settings.pk;var L=[];for(i in p.ui.settings.themes){name=replaceAll(p.ui.settings.themes[i],"wsct_","");L.push({value:name,title:name,selected:O.theme==name})
