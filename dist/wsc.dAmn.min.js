...
}}catch(b){console.log(">> Failed to log message for "+g.sns+"::");console.log(">> "+msg)}};wsc.Flow=function(a){this.protocol=a||new wsc.Protocol()};wsc.Flow.prototype.open=function(a,g,b){a.trigger("connected",{name:"connected",pkt:new wsc.Packet("connected\n\n")});a.connected=true;a.handshake();a.attempts=0};wsc.Flow.prototype.close=function(a,b){a.trigger("closed",{name:"closed",pkt:new wsc.Packet("connection closed\n\n")});console.log(b);if(a.connected){a.ui.server_message("Connection closed");a.connected=false}else{a.ui.server_message("Connection failed")}if(a.attempts>2){a.ui.server_message("Can't connect. Try again later.");a.attempts=0;return}a.ui.server_message("Connecting in 2 seconds");setTimeout(function(){a.conn.connect()},2000)};wsc.Flow.prototype.message=function(a,j){var b=new wsc.Packet(decodeURIComponent(j.data));if(b==null){return}var g=this.protocol.parse(b);if(g.ns==null){g.ns=a.mns}g.sns=a.deform_ns(g.ns);this.protocol.log(a,g);this.handle(a,g);a.trigger("pkt",g);
a.trigger("pkt."+g.name,g)};wsc.Flow.prototype.handle=function(a,g){try{this[g.name](g,a)}catch(b){}};wsc.Flow.prototype.ping=function(b,a){a.pong()};wsc.Flow.prototype.chatserver=function(b,a){a.login()};wsc.Flow.prototype.login=function(b,a){if(b.pkt.arg["e"]=="ok"){info=new wsc.Packet("info\n"+b.data);a.settings.username=b.pkt.param;a.settings.symbol=info.arg.symbol;a.settings.userinfo=info.arg;if(a.fresh){a.join(a.settings.autojoin)}else{for(key in a.channelo){if(a.channelo[key].namespace[0]!="~"){a.join(key)}}}}else{}if(a.fresh){a.fresh=false}};wsc.Flow.prototype.join=function(b,a){if(b.pkt.arg["e"]=="ok"){ns=a.deform_ns(b.pkt.param);a.create_ns(ns);a.ui.channel(ns).server_message("You have joined "+ns)}else{a.ui.chatbook.current.server_message("Failed to join "+a.deform_ns(b.pkt.param),b.pkt.arg["e"])}};wsc.Flow.prototype.part=function(b,a){ns=a.deform_ns(b.ns);c=a.channel(ns);if(b.e=="ok"){info="";if(b.r.length>0){info="["+b.r+"]"}else{a.remove_ns(ns)}msg="You have left "+ns;c.server_message(msg,info);
if(a.channels()==0){switch(b.r){case"bad data":case"bad msg":case"msg too big":break;default:if(b.r.indexOf("killed:")<0){return}break}this.process_data({data:"disconnect\ne="+e.r+"\n"})}}else{a.monitor("Couldn't leave "+ns,b.e);c.server_message("Couldn't leave "+ns,b.e)}};wsc.Flow.prototype.kicked=function(b,a){if(b.r.toLowerCase().indexOf("autokicked")>-1){return}a.join(b.ns)};wsc.Flow.prototype.property=function(b,a){chan=a.channel(b.pkt.param);if(!chan){return}chan.property(b)};wsc.Flow.prototype.recv_joinpart=function(b,a){c=a.channel(b.ns);if(b.name=="recv_join"){c.recv_join(b)}else{c.recv_part(b)}};wsc.Flow.prototype.recv_join=wsc.Flow.prototype.recv_joinpart;wsc.Flow.prototype.recv_part=wsc.Flow.prototype.recv_joinpart;wsc.Flow.prototype.recv_msg=function(b,a){a.channel(b.ns).recv_msg(b)};wsc.Flow.prototype.recv_action=wsc.Flow.prototype.recv_msg;wsc.Flow.prototype.recv_npmsg=wsc.Flow.prototype.recv_msg;wsc.Flow.prototype.recv_privchg=function(b,a){a.channel(b.ns).recv_privchg(b)
};wsc.Flow.prototype.recv_kicked=function(b,a){a.channel(b.ns).recv_kicked(b)};wsc.defaults.Extension=function(l){var C=function(){l.bind("cmd.set",a);l.bind("cmd.connect",q);l.bind("cmd.join",H);l.bind("cmd.part",J);l.bind("cmd.title",g);l.bind("cmd.promote",v);l.bind("cmd.demote",p);l.bind("cmd.me",z);l.bind("cmd.kick",B);l.bind("cmd.raw",I);l.bind("cmd.say",s);l.bind("cmd.npmsg",K);l.bind("cmd.clear",F);l.bind("cmd.clearall",E);l.bind("cmd.whois",D);l.bind("pkt.property",j);l.bind("pkt.get",A);l.bind("cmd.theme",b);l.ui.on("settings.open",m);l.ui.on("settings.open.ran",G)};var m=function(N,M){page=N.settings.page("Main");var O={};O.theme=replaceAll(l.ui.settings.theme,"wsct_","");O.clock=l.ui.clock();O.tc=l.ui.nav.closer();O.username=l.settings.username;O.pk=l.settings.pk;var L=[];for(i in l.ui.settings.themes){name=replaceAll(l.ui.settings.themes[i],"wsct_","");L.push({value:name,title:name,selected:O.theme==name})}page.item("Text",{ref:"intro",title:"Main",text:"Use this window to view and change your settings.\n\nCheck                    the different pages to see what settings can be changed.",});
page.item("Form",{ref:"login",title:"Login",text:"Here you can change the username and token used to                    log into the chat server.",fields:[["Textfield",{ref:"username",label:"Username","default":O.username}],["Textfield",{ref:"token",label:"Token","default":O.pk}]],event:{save:function(P){l.settings.username=P.data.username;l.settings.pk=P.data.token}}});page.item("Form",{ref:"ui",title:"UI",hint:"<b>Timestamp</b><br/>Choose between a 24 hour clock and                    a 12 hour clock.\n\n<b>Theme</b><br/>Change the look of the                    client.\n\n<b>Close Buttons</b><br/>Turn tab close buttons on/off.",fields:[["Dropdown",{ref:"theme",label:"Theme",items:L}],["Dropdown",{ref:"clock",label:"Timestamp Format",items:[{value:"24",title:"24 hour",selected:O.clock},{value:"12",title:"12 hour",selected:!O.clock}]}],["Check",{ref:"tabclose",label:"Close Buttons",items:[{value:"yes",title:"On",selected:O.tc}]}],],event:{change:function(P){l.ui.clock(P.data.clock=="24");
l.ui.theme(P.data.theme);l.ui.nav.closer(P.data.tabclose.indexOf("yes")>-1)},save:function(P){O.clock=P.data.clock=="24";O.theme=P.data.theme;O.tc=P.data.tabclose.indexOf("yes")>-1},close:function(P){l.ui.clock(O.clock);l.ui.theme(O.theme);l.ui.nav.closer(O.tc)}}});page.item("Text",{ref:"debug",wclass:"faint",title:"Debug Information",text:"Chat Agent: <code>"+l.settings.agent+"</code>\n\nUser                    Agent: <code>"+navigator.userAgent+"</code>"})};var G=function(N,M){var L=N.settings.page("About",true);L.item("Text",{ref:"about-wsc",title:"Wsc",text:'Currently using <a href="http://github.com/photofroggy/wsc/">wsc</a>                    version '+wsc.VERSION+" "+wsc.STATE+'.\n\nWsc                    works using HTML5, javascript, and CSS3. WebSocket is used for the connection                    where possible. The source code for this client is pretty huge.\n\nWsc was created                    by ~<a href="http://photofroggy.deviantart.com/">photofroggy</a>'})};var b=function(M,L){L.ui.theme(M.args.split(" ").shift())
};var a=function(N){var M=N.args.split(" ");var L=M.shift().toLowerCase();var M=M.join(" ");if(M.length==0){l.cchannel.serverMessage("Could not set "+L,"No data supplied");return}if(!(L in l.settings)){l.cchannel.serverMessage('Unknown setting "'+L+'"');return}l.settings[L]=M;l.cchannel.serverMessage("Changed "+L+" setting","value: "+M);l.control.setLabel()};var q=function(L){l.connect()};var H=function(L){var M=L.args.split(" ");var M=M.toString()==""?[]:M;if(L.ns!=L.target){M.unshift(L.target)}if(M.toString()==""){return}for(index in M){l.join(M[index])}};var J=function(L){var M=L.args.split(" ");if(L.ns!=L.target){M.unshift(L.target)}if(M.toString()==""){l.part(L.ns);return}for(index in M){l.part(M[index])}};var g=function(L){l.set(L.target,"title",L.args)};var v=function(M){var L=M.args.split(" ");l.promote(M.target,L[0],L[1])};var p=function(M){var L=M.args.split(" ");l.demote(M.target,L[0],L[1])};var z=function(L){l.action(L.target,L.args)};var I=function(L){l.send(L.args.replace(/\\n/gm,"\n"))
};var B=function(L){d=L.args.split(" ");u=d.shift();r=d.length>0?d.join(" "):null;l.kick(L.target,u,r)};var s=function(L){l.say(L.target,L.args)};var K=function(L){l.npmsg(L.target,L.args)};var F=function(M,L){L.cchannel.clear()};var E=function(M,L){for(c in L.channelo){L.channelo[c].clear()}};var D=function(M,L){L.whois(M.args.split(" ")[0])};var j=function(M,L){if(M.p!="info"){return}subs=M.pkt.sub;data=subs.shift().arg;data.username=M.sns.substr(1);data.connections=[];while(subs.length>0){conn=subs.shift().arg;conn.channels=[];while(subs.length>0){if(subs[0].cmd!="ns"){break}conn.channels.unshift(L.deform_ns(subs.shift().param))}data.connections.push(conn)}L.cchannel.show_whois(data)};var A=function(M,L){if(M.ns.indexOf("login:")!=0){return}L.cchannel.server_message("Whois failed for "+(M.sns.substr(1)),"not online")};C()};wsc.Client=function(a,g,j){this.mozilla=j;this.fresh=true;this.attempts=0;this.protocol=null;this.flow=null;this.ui=null;this.events=new EventEmitter();this.conn=null;
this.channelo={};this.cchannel=null;this.cmds=[];this.settings={domain:"website.com",server:"ws://website.com/wsendpoint",symbol:"",username:"",userinfo:{},pk:"",monitor:["~Monitor",true],welcome:"Welcome to the wsc web client!",autojoin:"chat:channel",control:wsc.Control,protocol:wsc.Protocol,tablumps:wsc.Tablumps,flow:wsc.Flow,ui:Chatterbox.UI,extend:[wsc.defaults.Extension],client:"chatclient",clientver:"0.3",theme:wsc.defaults.theme,themes:wsc.defaults.themes,};a.extend(this.settings,g);this.ui=new this.settings.ui(a,{themes:this.settings.themes,theme:this.settings.theme,monitor:this.settings.monitor,username:this.settings.username,domain:this.settings.domain},j);this.settings.agent=this.ui.LIB+"/"+this.ui.VERSION+" ("+navigator.appVersion.match(/\(([^)]+)\)/)[1]+") wsc/"+wsc.VERSION;this.mns=this.format_ns(this.settings.monitor[0]);this.lun=this.settings.username.toLowerCase();this.protocol=new this.settings.protocol(new this.settings.tablumps());this.flow=new this.settings.flow(this.protocol);
this.build();for(var b in this.settings.extend){this.settings.extend[b](this)}this.monitor(this.settings.welcome)};wsc.Client.prototype.build=function(){this.ui.build();this.control=new this.settings.control(this);var a=this;this.ui.on("channel.selected",function(b,g){a.cchannel=a.channel(b.ns);a.control.cache_input(b)});this.ui.on("tab.close.clicked",function(b,g){if(b.chan.monitor){return}a.part(b.ns)})};wsc.Client.prototype.loop=function(){this.ui.loop()};wsc.Client.prototype.add_extension=function(a){this.settings.extend.push(a);a(this)};wsc.Client.prototype.bind=function(b,a){this.events.addListener(b,a);if(b.indexOf("cmd.")!=0||b.length<=4){return}cmd=b.slice(4).toLowerCase();this.cmds.push(cmd)};wsc.Client.prototype.clear_listeners=function(a){this.events.removeListeners(a)};wsc.Client.prototype.trigger=function(a,b){this.events.emit(a,b,this)};wsc.Client.prototype.connect=function(){if(this.connected){return}this.attempts++;try{var a=this;this.conn=wsc.Transport.Create(this.settings.server);
...
if(b.raw.realname){b.info.push(b.raw.realname)}if(b.raw.typename){b.info.push(b.raw.typename)}})};wsc.dAmn.avatar={};wsc.dAmn.avatar.ext=["gif","gif","jpg","png"];wsc.dAmn.avatar.link=function(b,j){j=parseInt(j);var m=(j>>2)&15;j=j&3;var g=wsc.dAmn.avatar.ext[j]||"gif";if(m){m="?"+m}else{m=""}if(j==0){l="default"}else{var a=new RegExp("\\$un(\\[([0-9]+)\\])","g");var l="$un[0]/$un[1]/{un}".replace(a,function(p,v,q){return b[q]=="-"?"_":b[q].toLowerCase()});l=replaceAll(l,"{un}",b.toLowerCase())}return'<a target="_blank" title=":icon'+b+':" href="http://'+b+'.deviantart.com/"><img class="avatar"            alt=":icon'+b+':" src="http://a.deviantart.net/avatars/'+l+"."+g+m+'" height="50" width="50" /></a>'};(function(a){a("*").hover(function(b){a(this).data("hover",true)},function(b){a(this).data("hover",false)});a.fn.wsc=function(g,b){client=a(window).data("wscclient");if(g=="init"||client===undefined){if(client==undefined){client=new wsc.Client(a(this),b,(a.browser.mozilla||false));a(window).resize(function(){client.ui.resize()
