...
},{input:b,ns:a,property:f})};wsc.Client.prototype.whois=function(a){this.send(wsc_packetstr("get","login:"+a,{p:"info"}))};wsc.Client.prototype.disconnect=function(){this.send(wsc_packetstr("disconnect"))};var Chatterbox={};Chatterbox.VERSION="0.20.104";Chatterbox.STATE="beta";Chatterbox._gum=function(){};(function(a){a.fn.chatterbox=function(h,b,f){var g=a(window).data("chatterbox");if(h=="init"||g===undefined){if(g==undefined){g=new Chatterbox.UI(a(this),b,f,(a.browser.mozilla||false));a(window).resize(g.resize)}a(window).data("chatterbox",g)}if(h!="init"&&h!=undefined){h="jq_"+h;if(h in g){g[h](a(this),f)}}return g}})(jQuery);Chatterbox.UI=function(b,a,f,h,g){this.client=a;this.events=g||new EventEmitter();this.mozilla=h;this.umuted=[];this.viewing=true;this.settings={themes:["wsct_dAmn","wsct_dark"],theme:"wsct_dark",monitor:["~Monitor",true],username:"",domain:"website.com",clock:true,tabclose:true,developer:false,media:"/static/"};var j=this;this.sound={bank:{m:null},add:function(k,l){if(j.sound.hasOwnProperty(k)){return false
}j.sound.bank[k]=l;l.load();j.sound[k]=function(){j.sound.play(j.sound.bank[k])};return true},play:function(k){k.pause();k.currentTime=0;k.play()},toggle:function(l){for(var k in j.sound.bank){if(!j.sound.bank.hasOwnProperty(k)){continue}j.sound.bank[k].muted=l}},mute:function(){j.sound.toggle(true)},unmute:function(){j.sound.toggle(false)},};this.settings=b.extend(this.settings,f);b.append('<div class="wsc '+this.settings.theme+'"></div>');this.protocol=new Chatterbox.Protocol();this.mw=new wsc.Middleware();this.view=b.find(".wsc");this.mns=this.format_ns(this.settings.monitor[0]);this.lun=this.settings.username.toLowerCase();this.monitoro=null;this.swidth=(function(){if($.browser.msie){var m=$('<textarea cols="10" rows="2"></textarea>').css({position:"absolute",top:-1000,left:-1000}).appendTo("body"),l=$('<textarea cols="10" rows="2" style="overflow: hidden;"></textarea>').css({position:"absolute",top:-1000,left:-1000}).appendTo("body");scrollbarWidth=m.width()-l.width();m.add(l).remove()
