...
this.mli.keydown(b||this._onkeypress)}this.form.submit(a||this._onsubmit)};Chatterbox.Control.prototype.multiline=function(a){if(a==undefined||this.ml==a){return this.ml}this.ml=a;if(this.ml){this.input.css("display","none");this.mli.css("display","inline-block");this.ci=this.mli;this.manager.resize();return this.ml}this.mli.css("display","none");this.input.css("display","inline-block");this.ci=this.input;this.manager.resize();return this.mli};Chatterbox.Control.prototype._onkeypress=function(a){};Chatterbox.Control.prototype._onsubmit=function(a){};Chatterbox.Control.prototype.chomp=function(){d=this.ci.val();i=d.lastIndexOf(" ");if(i==-1){this.ci.val("");return d}chunk=d.slice(i+1);this.ci.val(d.slice(0,i));if(chunk.length==0){return this.chomp()}return chunk};Chatterbox.Control.prototype.unchomp=function(a){d=this.ci.val();if(!d){this.ci.val(a)}else{this.ci.val(d+" "+a)}};Chatterbox.Control.prototype.get_text=function(a){if(a==undefined){return this.ci.val()}this.ci.val(a||"");return this.ci.val()
};Chatterbox.Control.prototype.set_text=function(a){this.ci.val(a||"")};Chatterbox.Navigation=function(a){this.manager=a;this.nav=this.manager.view.find("nav.tabs");this.tabs=this.nav.find("#chattabs");this.buttons=this.nav.find("#tabnav");this.tableft=this.buttons.find(".arrow_left");this.tabright=this.buttons.find(".arrow_right");this.settingsb=this.buttons.find("#settings-button");this.settings={};this.settings.open=false;this.showclose=true;var b=this;this.settingsb.click(function(j){if(b.settings.open){return false}var g={e:j,settings:new Chatterbox.Settings.Config()};b.manager.trigger("settings.open",g);b.manager.trigger("settings.open.ran",g);about=g.settings.page("About",true);about.item("text",{ref:"about-chatterbox",wclass:"centered faint",text:'Using <a href="http://github.com/photofroggy/wsc/">Chatterbox</a> version '+Chatterbox.VERSION+" "+Chatterbox.STATE+' by ~<a href="http://photofroggy.deviantart.com/">photofroggy</a>.'});b.settings.window=new Chatterbox.Settings(b.manager,g.settings);
b.settings.window.build();b.settings.open=true;return false})};Chatterbox.Navigation.prototype.height=function(){return this.nav.height()};Chatterbox.Navigation.prototype.add_tab=function(a,b){this.tabs.append(Chatterbox.render("tab",{selector:a,ns:b}));return this.tabs.find("#"+a+"-tab")};Chatterbox.Navigation.prototype.resize=function(){this.tabs.width(this.nav.width()-this.buttons.outerWidth()-20);if(this.settings.open){this.settings.window.resize()}};Chatterbox.Navigation.prototype.closer=function(a){if(a==undefined||a==this.showclose){return this.showclose}this.showclose=a;if(this.showclose){if(!this.tabs.hasClass("hc")){return}this.tabs.removeClass("hc");return}if(this.tabs.hasClass("hc")){return}this.tabs.addClass("hc")};Chatterbox.Settings=function(b,a){this.manager=b;this.config=a;this.window=null;this.saveb=null;this.closeb=null;this.scb=null;this.tabs=null;this.book=null;this.changed=false};Chatterbox.Settings.prototype.build=function(){wrap=Chatterbox.template.popup;swindow=Chatterbox.template.settings.main;
wrap=replaceAll(wrap,"{content}",swindow);wrap=replaceAll(wrap,"{ref}","settings");this.manager.view.append(wrap);this.window=this.manager.view.find(".floater.settings");this.saveb=this.window.find("a.button.save");this.closeb=this.window.find("a.close");this.scb=this.window.find("a.button.saveclose");this.tabs=this.window.find("nav.tabs ul.tabs");this.book=this.window.find("div.book");this.config.build(this);this.window.find("ul.tabs li").first().addClass("active");this.window.find("div.book div.page").first().addClass("active");var a=this;this.window.find("form").bind("change",function(){a.changed=true});this.saveb.click(function(b){a.save();return false});this.closeb.click(function(b){if(a.changed){if(!confirm("Are you sure? You will lose any unsaved changes.")){return false}}a.close();return false});this.scb.click(function(b){a.save();a.close();return false});this.resize()};Chatterbox.Settings.prototype.resize=function(){inner=this.window.find(".inner");head=inner.find("h2");wrap=inner.find(".bookwrap");
foot=inner.find("footer");wrap.height(inner.height()-foot.outerHeight()-head.outerHeight()-15);this.book.height(wrap.innerHeight()-this.tabs.outerHeight()-25);this.config.resize()};Chatterbox.Settings.prototype.switch_page=function(a){active=this.tabs.find("li.active").first();activeref=active.prop("id").split("-",1)[0];active=this.config.page(activeref.split("_").join(" "));active.hide();a.show()};Chatterbox.Settings.prototype.save=function(){this.config.save(this);this.changed=false};Chatterbox.Settings.prototype.close=function(){this.window.remove();this.manager.nav.settings.open=false;this.manager.nav.settings.window=null;this.config.close(this)};Chatterbox.Settings.Config=function(){this.pages=[]};Chatterbox.Settings.Config.prototype.find_page=function(a){n=a.toLowerCase();for(index in this.pages){page=this.pages[index];if(page.lname==n){return page}}return null};Chatterbox.Settings.Config.prototype.build=function(a){for(i in this.pages){this.pages[i].build(a)}};Chatterbox.Settings.Config.prototype.resize=function(){for(i in this.pages){this.pages[i].resize()
}};Chatterbox.Settings.Config.prototype.page=function(b,a){page=this.find_page(b);a=a||false;if(page==null){page=new Chatterbox.Settings.Page(b);if(a){this.pages.push(page)}else{this.pages.unshift(page)}}return page};Chatterbox.Settings.Config.prototype.save=function(a){for(i in this.pages){this.pages[i].save(a)}};Chatterbox.Settings.Config.prototype.close=function(a){for(i in this.pages){this.pages[i].close(a)}};Chatterbox.Settings.Page=function(a){this.name=a;this.lname=a.toLowerCase();this.ref=replaceAll(this.lname," ","_");this.items=[]};Chatterbox.Settings.Page.prototype.build=function(a){tab=replaceAll(Chatterbox.template.settings.tab,"{ref}",this.ref);tab=replaceAll(tab,"{name}",this.name);b=replaceAll(Chatterbox.template.settings.page,"{ref}",this.ref);b=replaceAll(b,"{page-name}",this.name);a.tabs.append(tab);a.book.append(b);this.view=a.book.find("div#"+this.ref+"-page");this.tab=a.tabs.find("li#"+this.ref+"-tab");var b=this;this.tab.find("a").click(function(g){if(b.tab.hasClass("active")){return false
}a.switch_page(b);return false});this.content()};Chatterbox.Settings.Page.prototype.content=function(){for(i in this.items){this.items[i].build(this.view)}};Chatterbox.Settings.Page.prototype.resize=function(){for(i in this.items){this.items[i].resize()}};Chatterbox.Settings.Page.prototype.show=function(){if(!this.tab.hasClass("active")){this.tab.addClass("active")}if(!this.view.hasClass("active")){this.view.addClass("active")}};Chatterbox.Settings.Page.prototype.hide=function(){if(this.tab.hasClass("active")){this.tab.removeClass("active")}if(this.view.hasClass("active")){this.view.removeClass("active")}};Chatterbox.Settings.Page.prototype.item=function(g,b,a){a=a||false;item=Chatterbox.Settings.Item.get(g,b);if(a){this.items.unshift(item)}else{this.items.push(item)}return item};Chatterbox.Settings.Page.prototype.save=function(a){for(i in this.items){this.items[i].save(a,this)}};Chatterbox.Settings.Page.prototype.close=function(a){for(i in this.items){this.items[i].close(a,this)}};Chatterbox.Settings.Item=function(b,a){this.options=a||{};
...
this.value=this.view.find("input[checked]:radio").val();var a=this;this.field.bind("change",function(j){a.value=a.view.find(this).val()})};Chatterbox.Settings.Item.Check=function(b,a){Chatterbox.Settings.Item.Radio.call(this,b,a);this.value=[]};Chatterbox.Settings.Item.Check.prototype=new Chatterbox.Settings.Item.Radio();Chatterbox.Settings.Item.Check.prototype.constructor=Chatterbox.Settings.Item.Check;Chatterbox.Settings.Item.Check.prototype.build=function(g){if(this.options.hasOwnProperty("items")){for(i in this.options.items){var b=this.options.items[i];b.name=this.options.ref||"ref"}}Chatterbox.Settings.Item.prototype.build.call(this,g);this.field=this.view.find("input:checkbox");var a=this;this.value=[];this.view.find("input[checked]:checkbox").each(function(){a.value.push(a.view.find(this).val())});this.field.bind("change",function(j){a.value=[];a.view.find("input[checked]:checkbox").each(function(){a.value.push(a.view.find(this).val())})})};Chatterbox.Settings.Item.Form.Field.prototype.get=function(){return this.value
