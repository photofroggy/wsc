...
p.ui.on("settings.open.ran",K)};var q=function(T,S){var R=T.settings.page("Main");var U={};U.theme=replaceAll(p.ui.settings.theme,"wsct_","");U.clock=p.ui.clock();U.tc=p.ui.nav.closer();U.username=p.settings.username;U.pk=p.settings.pk;var Q=[];for(i in p.ui.settings.themes){name=replaceAll(p.ui.settings.themes[i],"wsct_","");Q.push({value:name,title:name,selected:U.theme==name})}R.item("Text",{ref:"intro",title:"Main",text:"Use this window to view and change your settings.\n\nCheck                    the different pages to see what settings can be changed.",});R.item("Form",{ref:"login",title:"Login",text:"Here you can change the username and token used to                    log into the chat server.",fields:[["Textfield",{ref:"username",label:"Username","default":U.username}],["Textfield",{ref:"token",label:"Token","default":U.pk}]],event:{save:function(V){p.settings.username=V.data.username;p.settings.pk=V.data.token}}});R.item("Form",{ref:"ui",title:"UI",hint:"<b>Timestamp</b><br/>Choose between a 24 hour clock and                    a 12 hour clock.\n\n<b>Theme</b><br/>Change the look of the                    client.\n\n<b>Close Buttons</b><br/>Turn tab close buttons on/off.",fields:[["Dropdown",{ref:"theme",label:"Theme",items:Q}],["Dropdown",{ref:"clock",label:"Timestamp Format",items:[{value:"24",title:"24 hour",selected:U.clock},{value:"12",title:"12 hour",selected:!U.clock}]}],["Check",{ref:"tabclose",label:"Close Buttons",items:[{value:"yes",title:"On",selected:U.tc}]}],],event:{change:function(V){p.ui.clock(V.data.clock=="24");
p.ui.theme(V.data.theme);p.ui.nav.closer(V.data.tabclose.indexOf("yes")>-1)},save:function(V){U.clock=V.data.clock=="24";U.theme=V.data.theme;U.tc=V.data.tabclose.indexOf("yes")>-1;p.storage.set("theme","wsct_"+U.theme)},close:function(V){p.ui.clock(U.clock);p.ui.theme(U.theme);p.ui.nav.closer(U.tc)}}});R.item("Text",{ref:"debug",wclass:"faint",title:"Debug Information",text:"Chat Agent: <code>"+p.settings.agent+"</code>\n\nUser                    Agent: <code>"+navigator.userAgent+"</code>"})};var K=function(S,R){var Q=S.settings.page("About",true);Q.item("Text",{ref:"about-wsc",title:"Wsc",text:'Currently using <a href="http://github.com/photofroggy/wsc/">wsc</a>                    version '+wsc.VERSION+" "+wsc.STATE+'.\n\nWsc                    works using HTML5, javascript, and CSS3. WebSocket is used for the connection                    where possible. The source code for this client is pretty huge.\n\nWsc was created                    by ~<a href="http://photofroggy.deviantart.com/">photofroggy</a>'})
...
}Q[T](U.target,R)};var B=function(Q){p.action(Q.target,Q.args)};var M=function(Q){p.send(Q.args.replace(/\\n/gm,"\n"))};var j=function(T,Q){var U=T.args.split(" ");var R=U.shift();var S=U.length>0?U.join(" "):null;if(T.cmd=="kick"){Q.kick(T.target,R,S)}else{Q.kill(R,S)}};var u=function(Q){p.say(Q.target,Q.args)};var P=function(Q){p.npmsg(Q.target,Q.args)};var J=function(S,Q){if(S.args.length>0){var T=S.args.split(" ");for(var R in T){if(!T.hasOwnProperty(R)){continue}Q.channel(S.target).clear(T[R])}}else{Q.channel(S.target).clear()}};var I=function(R,Q){var T=null;if(R.args.length>0){var S=R.args.split(" ");T=function(V,W){for(var U in S){if(!S.hasOwnProperty(U)){continue}W.clear(S[U])}}}else{T=function(U,V){V.clear()}}Q.each_channel(T,true)};var H=function(R,Q){Q.whois(R.args.split(" ")[0])};var F=function(R,Q){Q.admin(R.target,R.args)};var D=function(R,Q){Q.disconnect()};var A=function(R,Q){var S=R.cmd.indexOf("title")>-1?"title":"topic";Q.control.ui.set_text("/"+S+" "+Q.channel(R.target).info[S].content)
};var v=function(S,Q){r.away.on=true;r.away.last={};r.away.since=new Date();r.away.reason=S.args;var R="is away"+(r.away.reason.length>0?": "+r.away.reason:"");Q.each_channel(function(T){Q.action(T,R)})};var s=function(R,Q){r.away.on=false;Q.each_channel(function(S){Q.action(S,"is back")})};var z=function(T,Q){if(!r.away.on){return}if(r.away.reason.length==0){return}if(T.user==Q.settings.username){return}if(Q.exclude.indexOf(T.sns.toLowerCase())!=-1){return}var R=new Date();var S=T.sns.toLowerCase();if(S in r.away.last){if((R-r.away.last[S])<=60000){return}}Q.say(T.ns,T.user+": I am currently away; reason: "+r.away.reason);r.away.last[S]=R};var l=function(T,Q){if(T.p!="info"){return}var R=T.pkt.sub;var U=R.shift().arg;U.username=T.sns.substr(1);U.connections=[];var S={};while(R.length>0){S=R.shift().arg;S.channels=[];while(R.length>0){if(R[0].cmd!="ns"){break}S.channels.unshift(Q.deform_ns(R.shift().param))}U.connections.push(S)}Q.cchannel.log_whois(U)};var C=function(R,Q){if(R.ns.indexOf("login:")!=0){return
}Q.cchannel.server_message("Whois failed for "+(R.sns.substr(1)),"not online")};var N=function(U,R){var X=R.channel(U.ns);var Q=U.info.split("\n");var W="";var V=[];var S="";for(var T in Q){if(!Q.hasOwnProperty(T)){continue}W=Q[T].split(" ");if(U.p=="privclass"){V.push([W.shift(),W.shift().split("=")[1],W.join(" ")])}else{if(U.p=="users"){S=W.shift().split(":",1)[0];V.push([S,X.get_privclass_order(S),W.join(" ")])}}}X.log_pc(U.p=="privclass",V)};G();wsc.defaults.Extension.Ignore(p)};wsc.defaults.Extension.Ignore=function(g){var j={};var k=g.storage.folder("ignore");var a=k.folder("ignored");var q=function(){p();m();g.bind("cmd.ignore",b);g.bind("cmd.unignore",l);g.ui.on("settings.open",j.page)};j.page=function(u,z){var v=u.settings.page("Ignores");var B={};var s="<ul>";B.im=j.ignore;B.uim=j.unignore;if(g.ui.umuted.length==0){s+="<li><i>No one ignored yet</i></li></ul>"}else{for(var r in g.ui.umuted){if(!g.ui.umuted.hasOwnProperty(r)){continue}s+="<li>"+g.ui.umuted[r]+"</li>"}s+="</ul>"}v.item("Text",{ref:"intro",title:"Ignores",text:'Use <code>ignore</code> to ignore people.\n\n                    You can "ignore" other users of the chat server using the\n                    <code>/ignore</code> command. Ignoring a user hides their                    messages from you in the channel log.',});
v.item("Form",{ref:"msgs",title:"Messages",text:"Here you can set the messages displayed when you ignore or                    unignore a user.\n\nThe text <code>{user}</code> is replaced                    with the name of the user your are ignoring or unignoring.",fields:[["Textfield",{ref:"ignore",label:"Ignore","default":B.im}],["Textfield",{ref:"unignore",label:"Unignore","default":B.uim}]],event:{save:function(C){j.ignore=C.data.ignore;j.unignore=C.data.unignore}}});var A=v.item("Form",{ref:"ignored",wclass:"boxed-ff-indv",title:"Users",text:"This is the list of users that you have silenced.\n\nUse the                    commands <code>/ignore</code> and <code>/unignore</code>                    to edit the list.",fields:[["Text",{ref:"users",text:s}]]})};var b=function(v){var A=v.args.split(" ");var r="";var z="";var u=false;for(var s in A){if(!A.hasOwnProperty(s)){continue}r=A[s];tmod=g.mute_user(r);if(!tmod){continue}u=tmod;z=replaceAll(j.ignore,"{user}",r);if(z.indexOf("/me ")==0){z=z.substr(4);
...
}return a};wsc.Client.prototype.create_ns=function(a,b){chan=this.channel(a,new wsc.Channel(this,a,b));chan.build()};wsc.Client.prototype.remove_ns=function(a){this.ui.remove_channel(a)};wsc.Client.prototype.log=function(a,b){var g=this.channel(a);if(!g){return}g.log(b)};wsc.Client.prototype.monitor=function(a){this.ui.monitor(a)};wsc.Client.prototype.mute_user=function(a){return this.ui.mute_user(a)};wsc.Client.prototype.unmute_user=function(a){return this.ui.unmute_user(a)};wsc.Client.prototype.send=function(a){return this.conn.send(a)};wsc.Client.prototype.handshake=function(){this.send(wsc_packetstr(this.settings.client,this.settings.clientver,{agent:this.settings.agent}))};wsc.Client.prototype.login=function(){pkt="login "+this.settings.username+"\npk="+this.settings.pk+"\n";this.send(pkt)};wsc.Client.prototype.pong=function(){this.send(wsc_packetstr("pong"))};wsc.Client.prototype.join=function(a){this.send(wsc_packetstr("join",this.format_ns(a)))};wsc.Client.prototype.part=function(a){this.send(wsc_packetstr("part",this.format_ns(a)))
...
b=b.slice(1);bits=b.split(" ");cmdn=bits.shift().toLowerCase();ens=this.client.cchannel.namespace;etarget=ens;if(bits[0]){hash=bits[0][0];if((hash=="#"||hash=="@")&&bits[0].length>1){etarget=this.client.format_ns(bits.shift())}}arg=bits.join(" ");this.client.trigger("cmd."+cmdn,{name:"cmd",cmd:cmdn,args:arg,target:etarget,ns:ens})};var Chatterbox={};Chatterbox.VERSION="0.6.42";Chatterbox.STATE="beta";Chatterbox.UI=function(a,b,j,g){this.events=g||new EventEmitter();this.mozilla=j;this.umuted=[];this.settings={themes:["wsct_default","wsct_dAmn"],theme:"wsct_default",monitor:["~Monitor",true],username:"",domain:"website.com",clock:true};a.extend(this.settings,b);a.append('<div class="wsc '+this.settings.theme+'"></div>');this.view=a.find(".wsc");this.mns=this.format_ns(this.settings.monitor[0]);this.lun=this.settings.username.toLowerCase();this.monitoro=null;this.swidth=(function(){if($.browser.msie){var m=$('<textarea cols="10" rows="2"></textarea>').css({position:"absolute",top:-1000,left:-1000}).appendTo("body"),l=$('<textarea cols="10" rows="2" style="overflow: hidden;"></textarea>').css({position:"absolute",top:-1000,left:-1000}).appendTo("body");
