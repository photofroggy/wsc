...
if(f.match(/.png$/i)){shadow=false}return dal+'><img class="thumb'+(shadow?" shadow":"")+'" width="'+tw+'" height="'+th+'" alt=":thumb'+id+':" src="'+path+'" /></a>'}],EOF:[0,"",null,"\x1b[m"]}};wsc.dAmn.TablumpParser.prototype.parse=function(b,a){return new wsc.dAmn.TablumpString(b,this)};wsc.dAmn.TablumpParser.prototype.tokenise=function(m){if(!m){return[]}var j="\t";var g=[];var l=-1;var b="";var a="";var p=null;for(var k=0;k<m.length;k++){if(m[k]!="&"){continue}g.push(["raw",m.substring(0,k)]);a=m.substring(k);m=a;k=1;l=a.indexOf("\t");if(l==-1){continue}b=a.substring(0,l+1);a=a.substring(l+1);m=a;p=this.crop(flag,b,a);if(p===null){continue}g.push(p[0]);m=p[1]}return g};wsc.dAmn.TablumpParser.prototype.crop=function(b,a){var j=this.lumps[b];if(j===undefined){return null}if(j[0]==0){return[[b,[]],a]}else{var g=this.tokens(a,j[0],sep);return[[b,g[0]],g[1]]}};wsc.dAmn.TablumpParser.prototype.render=function(a,j){if(!j){return""}if(!j.hasOwnProperty("tokens")){return""}a=a+1;var k="";var g=[];
for(var b in j.tokens){if(!j.tokens.hasOwnProperty(b)){continue}g=j.tokens[b];if(g[0]=="raw"){k+=g[1];continue}k+=this.renderOne(a,g[0],g[1])}return k+this.renderOne(a,"EOF","")[0]};wsc.dAmn.TablumpParser.prototype.renderOne=function(b,a,k){var g=this.lumps[a];if(g===undefined){return"&"+a+"\t"+k.join("\t")}var j=g[b]||g[1];if(typeof(j)=="string"){return String.format(j,k)}else{return j.call(this,k)}};wsc.dAmn.TablumpParser.prototype.tokens=function(j,b,g,a){g=g||"\t";a=a||"&";var k=[];var l=-1;for(i=b;i>0;i--){l=j.indexOf(g);if(l==-1){break}k.push(j.substring(0,l));j=j.substring(l+1);if(k[k.length-1]==a){k.pop();break}}return[k,j]};(function(a){a("*").hover(function(b){a(this).data("hover",true)},function(b){a(this).data("hover",false)});a.fn.wsc=function(g,b){client=a(window).data("wscclient");if(g=="init"||client===undefined){if(client==undefined){client=new wsc.Client(a(this),b,(a.browser.mozilla||false));a(window).resize(function(){client.ui.resize()});a(window).focus(function(){client.ui.control.focus()
