...
wsc.Flow.prototype.recv_privchg=function(b,a){a.channel(b.ns).recv_privchg(b)};wsc.Flow.prototype.recv_kicked=function(b,a){a.channel(b.ns).recv_kicked(b)};wsc.defaults.Extension=function(p){var r={};r.away={on:false,reason:"",last:{},since:0};var F=function(){p.bind("cmd.connect",g);p.bind("cmd.set",a);p.bind("cmd.join",L);p.bind("cmd.part",O);p.bind("cmd.say",t);p.bind("cmd.npmsg",P);p.bind("cmd.me",A);p.bind("cmd.promote",D);p.bind("cmd.demote",D);p.bind("cmd.ban",m);p.bind("cmd.unban",m);p.bind("cmd.kick",j);p.bind("cmd.whois",H);p.bind("cmd.title",k);p.bind("cmd.topic",k);p.bind("cmd.admin",E);p.bind("cmd.disconnect",g);p.bind("cmd.kill",j);p.bind("cmd.raw",M);p.bind("cmd.clear",J);p.bind("cmd.clearall",I);p.bind("pkt.property",l);p.bind("pkt.recv_admin_show",N);p.bind("pkt.recv_admin_showverbose",N);p.bind("pkt.get",B);p.bind("cmd.gettitle",z);p.bind("cmd.gettopic",z);p.bind("cmd.setaway",u);p.bind("cmd.setback",s);p.bind("pkt.recv_msg.highlighted",v);p.bind("cmd.theme",b);p.ui.on("settings.open",q);
p.ui.on("settings.open.ran",K);p.ui.on("settings.save",G)};var G=function(R,Q){p.settings.ui.theme=R.theme;p.settings.ui.clock=R.clock;p.settings.ui.tabclose=R.tabclose;p.config_save()};var q=function(S,R){var Q=S.settings.page("Main",true);var T={};T.username=p.settings.username;T.pk=p.settings.pk;Q.item("Form",{ref:"login",title:"Login",text:"Here you can change the username and token used to                    log into the chat server.",fields:[["Textfield",{ref:"username",label:"Username","default":T.username}],["Textfield",{ref:"token",label:"Token","default":T.pk}]],event:{save:function(U){p.settings.username=U.data.username;p.settings.pk=U.data.token}}},true);Q.item("Text",{ref:"intro",title:"Main",text:"Use this window to view and change your settings.\n\nCheck                    the different pages to see what settings can be changed.",},true);Q.item("Text",{ref:"debug",wclass:"faint",title:"Debug Information",text:"Chat Agent: <code>"+p.settings.agent+"</code>\n\nUser                    Agent: <code>"+navigator.userAgent+"</code>"})
};var K=function(S,R){var Q=S.settings.page("About",true);Q.item("Text",{ref:"about-wsc",title:"Wsc",text:'Currently using <a href="http://github.com/photofroggy/wsc/">wsc</a>                    version '+wsc.VERSION+" "+wsc.STATE+'.\n\nWsc                    works using HTML5, javascript, and CSS3. WebSocket is used for the connection                    where possible. The source code for this client is pretty huge.\n\nWsc was created                    by ~<a href="http://photofroggy.deviantart.com/">photofroggy</a>'})};var b=function(R,Q){Q.ui.theme(R.args.split(" ").shift())};var a=function(S){var R=S.args.split(" ");var Q=R.shift().toLowerCase();var R=R.join(" ");if(R.length==0){p.cchannel.serverMessage("Could not set "+Q,"No data supplied");return}if(!(Q in p.settings)){p.cchannel.serverMessage('Unknown setting "'+Q+'"');return}p.settings[Q]=R;p.cchannel.serverMessage("Changed "+Q+" setting","value: "+R);p.control.setLabel()};var g=function(Q){p[Q.cmd]()};var L=function(Q){var R=Q.args.split(" ");
var R=R.toString()==""?[]:R;if(Q.ns!=Q.target){R.unshift(Q.target)}if(R.toString()==""){return}for(index in R){p.join(R[index])}};var O=function(Q){var R=Q.args.split(" ");if(Q.ns!=Q.target){R.unshift(Q.target)}if(R.toString()==""){p.part(Q.ns);return}for(index in R){p.part(R[index])}};var k=function(Q){p.set(Q.target,Q.cmd,Q.args)};var D=function(R){var Q=R.args.split(" ");p[R.cmd.toLowerCase()](R.target,Q[0],Q[1])};var m=function(U,Q){var S=U.args.split(" ");var R=S.shift();var T=U.cmd;if(T=="ban"&&S.length>0){Q.kick(U.target,R,S.join(" "))}Q[T](U.target,R)};var A=function(Q){p.action(Q.target,Q.args)};var M=function(Q){p.send(Q.args.replace(/\\n/gm,"\n"))};var j=function(T,Q){var U=T.args.split(" ");var R=U.shift();var S=U.length>0?U.join(" "):null;if(T.cmd=="kick"){Q.kick(T.target,R,S)}else{Q.kill(R,S)}};var t=function(Q){p.say(Q.target,Q.args)};var P=function(Q){p.npmsg(Q.target,Q.args)};var J=function(S,Q){if(S.args.length>0){var T=S.args.split(" ");for(var R in T){if(!T.hasOwnProperty(R)){continue
}Q.channel(S.target).clear(T[R])}}else{Q.channel(S.target).clear()}};var I=function(R,Q){var T=null;if(R.args.length>0){var S=R.args.split(" ");T=function(V,W){for(var U in S){if(!S.hasOwnProperty(U)){continue}W.clear(S[U])}}}else{T=function(U,V){V.clear()}}Q.each_channel(T,true)};var H=function(R,Q){Q.whois(R.args.split(" ")[0])};var E=function(R,Q){Q.admin(R.target,R.args)};var C=function(R,Q){Q.disconnect()};var z=function(R,Q){var S=R.cmd.indexOf("title")>-1?"title":"topic";Q.control.ui.set_text("/"+S+" "+Q.channel(R.target).info[S].content)};var u=function(S,Q){r.away.on=true;r.away.last={};r.away.since=new Date();r.away.reason=S.args;var R="is away"+(r.away.reason.length>0?": "+r.away.reason:"");Q.each_channel(function(T){Q.action(T,R)})};var s=function(R,Q){r.away.on=false;Q.each_channel(function(S){Q.action(S,"is back")})};var v=function(T,Q){if(!r.away.on){return}if(r.away.reason.length==0){return}if(T.user==Q.settings.username){return}if(Q.exclude.indexOf(T.sns.toLowerCase())!=-1){return
}var R=new Date();var S=T.sns.toLowerCase();if(S in r.away.last){if((R-r.away.last[S])<=60000){return}}Q.say(T.ns,T.user+": I am currently away; reason: "+r.away.reason);r.away.last[S]=R};var l=function(T,Q){if(T.p!="info"){return}var R=T.pkt.sub;var U=R.shift().arg;U.username=T.sns.substr(1);U.connections=[];var S={};while(R.length>0){S=R.shift().arg;S.channels=[];while(R.length>0){if(R[0].cmd!="ns"){break}S.channels.unshift(Q.deform_ns(R.shift().param))}U.connections.push(S)}Q.cchannel.log_whois(U)};var B=function(R,Q){if(R.ns.indexOf("login:")!=0){return}Q.cchannel.server_message("Whois failed for "+(R.sns.substr(1)),"not online")};var N=function(U,R){var X=R.channel(U.ns);var Q=U.info.split("\n");var W="";var V=[];var S="";for(var T in Q){if(!Q.hasOwnProperty(T)){continue}W=Q[T].split(" ");if(U.p=="privclass"){V.push([W.shift(),W.shift().split("=")[1],W.join(" ")])}else{if(U.p=="users"){S=W.shift().split(":",1)[0];V.push([S,X.get_privclass_order(S),W.join(" ")])}}}X.log_pc(U.p=="privclass",V)
};F();wsc.defaults.Extension.Ignore(p)};wsc.defaults.Extension.Ignore=function(g){var j={};var k=g.storage.folder("ignore");var a=k.folder("ignored");var q=function(){p();m();g.bind("cmd.ignore",b);g.bind("cmd.unignore",l);g.ui.on("settings.open",j.page)};j.page=function(t,v){var u=t.settings.page("Ignores",true);var A={};var s="<ul>";A.im=j.ignore;A.uim=j.unignore;if(g.ui.umuted.length==0){s+="<li><i>No one ignored yet</i></li></ul>"}else{for(var r in g.ui.umuted){if(!g.ui.umuted.hasOwnProperty(r)){continue}s+="<li>"+g.ui.umuted[r]+"</li>"}s+="</ul>"}u.item("Text",{ref:"intro",title:"Ignores",text:'Use <code>ignore</code> to ignore people.\n\n                    You can "ignore" other users of the chat server using the\n                    <code>/ignore</code> command. Ignoring a user hides their                    messages from you in the channel log.',});u.item("Form",{ref:"msgs",title:"Messages",text:"Here you can set the messages displayed when you ignore or                    unignore a user.\n\nThe text <code>{user}</code> is replaced                    with the name of the user your are ignoring or unignoring.",fields:[["Textfield",{ref:"ignore",label:"Ignore","default":A.im}],["Textfield",{ref:"unignore",label:"Unignore","default":A.uim}]],event:{save:function(B){j.ignore=B.data.ignore;
j.unignore=B.data.unignore}}});var z=u.item("Form",{ref:"ignored",wclass:"boxed-ff-indv",title:"Users",text:"This is the list of users that you have silenced.\n\nUse the                    commands <code>/ignore</code> and <code>/unignore</code>                    to edit the list.",fields:[["Text",{ref:"users",text:s}]]})};var b=function(u){var z=u.args.split(" ");var r="";var v="";var t=false;for(var s in z){if(!z.hasOwnProperty(s)){continue}r=z[s];tmod=g.mute_user(r);if(!tmod){continue}t=tmod;v=replaceAll(j.ignore,"{user}",r);if(v.indexOf("/me ")==0){v=v.substr(4);g.action(u.target,v)}else{g.say(u.target,v)}}if(t){m()}};var l=function(v){var B=v.args.split(" ");var s="";var A="";var r=-1;var u=false;var z=false;for(var t in B){if(!B.hasOwnProperty(t)){continue}s=B[t];z=g.unmute_user(s);if(!z){continue}u=z;A=replaceAll(j.unignore,"{user}",s);if(A.indexOf("/me ")==0){A=A.substr(4);g.action(v.target,A)}else{g.say(v.target,A)}}if(u){m()}};var p=function(){j.ignore=k.get("ignore","/me is ignoring {user} now");
