...
for(var r=0;r<j.count;r++){s=a.get(r,null);if(s===null){continue}j.ignored.push(s)}}};var m=function(){k.set("ignore",j.ignore);k.set("unignore",j.unignore);if(j.ignored.length==0){k.set("count",0)}else{var s=-1;for(var r in j.ignored){if(!j.ignored.hasOwnProperty(r)){continue}s++;a.set(s.toString(),j.ignored[r])}if(s<0){s=0}k.set("count",s)}};q()};wsc.defaults.Extension=function(p){var r={};r.away={on:false,reason:"",last:{},since:0};var F=function(){p.bind("cmd.connect",g);p.bind("cmd.set",a);p.bind("cmd.join",K);p.bind("cmd.part",N);p.bind("cmd.say",t);p.bind("cmd.npmsg",O);p.bind("cmd.me",A);p.bind("cmd.promote",D);p.bind("cmd.demote",D);p.bind("cmd.ban",m);p.bind("cmd.unban",m);p.bind("cmd.kick",j);p.bind("cmd.whois",G);p.bind("cmd.title",k);p.bind("cmd.topic",k);p.bind("cmd.admin",E);p.bind("cmd.disconnect",g);p.bind("cmd.kill",j);p.bind("cmd.raw",L);p.bind("cmd.clear",I);p.bind("cmd.clearall",H);p.bind("pkt.property",l);p.bind("pkt.recv_admin_show",M);p.bind("pkt.recv_admin_showverbose",M);
