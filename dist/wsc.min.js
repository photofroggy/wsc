...
wsc.Flow.prototype.recv_privchg=function(b,a){a.channel(b.ns).recv_privchg(b)};wsc.Flow.prototype.recv_kicked=function(b,a){a.channel(b.ns).recv_kicked(b)};wsc.defaults.Extension=function(p){var r={};r.away={on:false,reason:"",last:{},since:0,store:p.storage.folder("away"),format:{setaway:"/me is away: {reason}",setback:"/me is back",away:"{from}: I am away, reason: {reason}"}};var G=function(){p.bind("cmd.connect",g);p.bind("cmd.set",a);p.bind("cmd.join",M);p.bind("cmd.part",P);p.bind("cmd.say",t);p.bind("cmd.npmsg",Q);p.bind("cmd.me",A);p.bind("cmd.promote",D);p.bind("cmd.demote",D);p.bind("cmd.ban",m);p.bind("cmd.unban",m);p.bind("cmd.kick",j);p.bind("cmd.whois",I);p.bind("cmd.title",k);p.bind("cmd.topic",k);p.bind("cmd.admin",E);p.bind("cmd.disconnect",g);p.bind("cmd.kill",j);p.bind("cmd.raw",N);p.bind("cmd.clear",K);p.bind("cmd.clearall",J);p.bind("cmd.close",F);p.bind("pkt.property",l);p.bind("pkt.recv_admin_show",O);p.bind("pkt.recv_admin_showverbose",O);p.bind("pkt.get",B);p.bind("cmd.gettitle",z);
p.bind("cmd.gettopic",z);p.bind("cmd.setaway",u);p.bind("cmd.setback",s);p.bind("pkt.recv_msg.highlighted",v);p.bind("cmd.theme",b);p.ui.on("settings.open",q);p.ui.on("settings.open.ran",L);p.ui.on("settings.save",H)};var H=function(S,R){p.settings.ui.theme=S.theme;p.settings.ui.clock=S.clock;p.settings.ui.tabclose=S.tabclose;p.config_save()};var q=function(T,S){var R=T.settings.page("Main",true);var U={};U.username=p.settings.username;U.pk=p.settings.pk;R.item("Form",{ref:"login",title:"Login",text:"Here you can change the username and token used to                    log into the chat server.",fields:[["Textfield",{ref:"username",label:"Username","default":U.username}],["Textfield",{ref:"token",label:"Token","default":U.pk}]],event:{save:function(V){p.settings.username=V.data.username;p.settings.pk=V.data.token}}},true);R.item("Text",{ref:"intro",title:"Main",text:"Use this window to view and change your settings.\n\nCheck                    the different pages to see what settings can be changed.",},true);
R.item("Text",{ref:"debug",wclass:"faint",title:"Debug Information",text:"Chat Agent: <code>"+p.settings.agent+"</code>\n\nUser                    Agent: <code>"+navigator.userAgent+"</code>"})};var L=function(T,S){var R=T.settings.page("About",true);R.item("Text",{ref:"about-wsc",title:"Wsc",text:'Currently using <a href="http://github.com/photofroggy/wsc/">wsc</a>                    version '+wsc.VERSION+" "+wsc.STATE+'.\n\nWsc                    works using HTML5, javascript, and CSS3. WebSocket is used for the connection                    where possible. The source code for this client is pretty huge.\n\nWsc was created                    by ~<a href="http://photofroggy.deviantart.com/">photofroggy</a>'})};var b=function(S,R){R.ui.theme(S.args.split(" ").shift())};var a=function(T){var S=T.args.split(" ");var R=S.shift().toLowerCase();var S=S.join(" ");if(S.length==0){p.cchannel.serverMessage("Could not set "+R,"No data supplied");return}if(!(R in p.settings)){p.cchannel.serverMessage('Unknown setting "'+R+'"');
return}p.settings[R]=S;p.cchannel.serverMessage("Changed "+R+" setting","value: "+S);p.control.setLabel()};var g=function(R){p[R.cmd]()};var M=function(R){var S=R.args.split(" ");var S=S.toString()==""?[]:S;if(R.ns!=R.target){S.unshift(R.target)}if(S.toString()==""){return}for(index in S){p.join(S[index])}};var P=function(R){var S=R.args.split(" ");if(R.ns!=R.target){S.unshift(R.target)}if(S.toString()==""){p.part(R.ns);return}for(index in S){p.part(S[index])}};var k=function(R){p.set(R.target,R.cmd,R.args)};var D=function(S){var R=S.args.split(" ");p[S.cmd.toLowerCase()](S.target,R[0],R[1])};var m=function(V,R){var T=V.args.split(" ");var S=T.shift();var U=V.cmd;if(U=="ban"&&T.length>0){R.kick(V.target,S,T.join(" "))}R[U](V.target,S)};var A=function(R){p.action(R.target,R.args)};var N=function(R){p.send(R.args.replace(/\\n/gm,"\n"))};var j=function(U,R){var V=U.args.split(" ");var S=V.shift();var T=V.length>0?V.join(" "):null;if(U.cmd=="kick"){R.kick(U.target,S,T)}else{R.kill(S,T)}};var t=function(R){p.say(R.target,R.args)
};var Q=function(R){p.npmsg(R.target,R.args)};var K=function(T,R){if(T.args.length>0){var U=T.args.split(" ");for(var S in U){if(!U.hasOwnProperty(S)){continue}R.channel(T.target).clear(U[S])}}else{R.channel(T.target).clear()}};var J=function(S,R){var U=null;if(S.args.length>0){var T=S.args.split(" ");U=function(W,X){for(var V in T){if(!T.hasOwnProperty(V)){continue}X.clear(T[V])}}}else{U=function(V,W){W.clear()}}R.each_channel(U,true)};var F=function(R){p.part(R.target);p.remove_ns(R.target)};var I=function(S,R){R.whois(S.args.split(" ")[0])};var E=function(S,R){R.admin(S.target,S.args)};var C=function(S,R){R.disconnect()};var z=function(S,R){var T=S.cmd.indexOf("title")>-1?"title":"topic";R.control.ui.set_text("/"+T+" "+R.channel(S.target).info[T].content)};var u=function(T,R){r.away.on=true;r.away.last={};r.away.since=new Date();r.away.reason=T.args;var S="is away"+(r.away.reason.length>0?": "+r.away.reason:"");R.each_channel(function(U){R.action(U,S)})};var s=function(S,R){r.away.on=false;
R.each_channel(function(T){R.action(T,"is back")})};var v=function(U,R){if(!r.away.on){return}if(r.away.reason.length==0){return}if(U.user==R.settings.username){return}if(R.exclude.indexOf(U.sns.toLowerCase())!=-1){return}var S=new Date();var T=U.sns.toLowerCase();if(T in r.away.last){if((S-r.away.last[T])<=60000){return}}R.say(U.ns,U.user+": I am currently away; reason: "+r.away.reason);r.away.last[T]=S};var l=function(U,R){if(U.p!="info"){return}var S=U.pkt.sub;var V=S.shift().arg;V.username=U.sns.substr(1);V.connections=[];var T={};while(S.length>0){T=S.shift().arg;T.channels=[];while(S.length>0){if(S[0].cmd!="ns"){break}T.channels.unshift(R.deform_ns(S.shift().param))}V.connections.push(T)}R.cchannel.log_whois(V)};var B=function(S,R){if(S.ns.indexOf("login:")!=0){return}R.cchannel.server_message("Whois failed for "+(S.sns.substr(1)),"not online")};var O=function(V,S){var Y=S.channel(V.ns);var R=V.info.split("\n");var X="";var W=[];var T="";for(var U in R){if(!R.hasOwnProperty(U)){continue
}X=R[U].split(" ");if(V.p=="privclass"){W.push([X.shift(),X.shift().split("=")[1],X.join(" ")])}else{if(V.p=="users"){T=X.shift().split(":",1)[0];W.push([T,Y.get_privclass_order(T),X.join(" ")])}}}Y.log_pc(V.p=="privclass",W)};G();wsc.defaults.Extension.Away(p);wsc.defaults.Extension.Ignore(p)};wsc.defaults.Extension.Away=function(a){var l=a.storage.folder("away");var b={on:false,reason:"",last:{},since:0,store:a.storage.folder("away"),format:{setaway:"/me is away: {reason}",setback:"/me is back",away:"{from}: I am away, reason: {reason}"}};var k=function(){j();g()};var j=function(){b.format.setaway=l.get("setaway","/me is away: {reason}");b.format.setback=l.get("setback","/me is back");b.format.away=l.get("away","{from}: I am away, reason: {reason}")};var g=function(){l.set("setaway",b.format.setaway);l.set("setback",b.format.setback);l.set("away",b.format.away)};k()};wsc.defaults.Extension.Ignore=function(g){var j={};var k=g.storage.folder("ignore");var a=k.folder("ignored");var q=function(){p();
