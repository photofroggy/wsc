...
}chans++}return chans};Chatterbox.Chatbook.prototype.create_channel=function(b,d,a){var f=this.channel(b,this.channel_object(b,d,a));f.build();if(this.trail.indexOf(f.namespace)==-1){this.trail.push(f.namespace)}if(!f.visible){this.toggle_channel(b)}this.manager.resize();return f};Chatterbox.Chatbook.prototype.create_feed=function(a,f,b){var d=this.channel(a,this.feed_object(a,f,b));d.build();if(this.trail.indexOf(d.namespace)==-1){this.trail.push(d.namespace)}if(!d.visible){this.toggle_channel(a)}this.manager.resize();return d};Chatterbox.Chatbook.prototype.channel_object=function(a){return new Chatterbox.Channel(this.manager,a)};Chatterbox.Chatbook.prototype.feed_object=function(a,d,b){return new Chatterbox.Feed(this.manager,a,d,b)};Chatterbox.Chatbook.prototype.toggle_channel=function(a){var d=this.channel(a);var b=d;if(!d){return}if(d.hidden){if(this.current&&this.current==d){return}if(!this.manager.settings.developer){d.hide();return}}if(this.current){if(this.current==d){return}this.current.hide();
