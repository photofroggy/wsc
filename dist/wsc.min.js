...
}chans++}return chans};Chatterbox.Chatbook.prototype.create_channel=function(b,d,a){var f=this.channel(b,this.channel_object(b,d,a));f.build();if(this.trail.indexOf(f.namespace)==-1){this.trail.push(f.namespace)}if(!f.visible){this.toggle_channel(b)}this.manager.resize();return f};Chatterbox.Chatbook.prototype.create_feed=function(b,a,d){var f=this.channel(b,this.feed_object(b,a,d));f.build();if(this.trail.indexOf(f.namespace)==-1){this.trail.push(f.namespace)}if(!f.visible){this.toggle_channel(b)}this.manager.resize();return f};Chatterbox.Chatbook.prototype.channel_object=function(a){return new Chatterbox.Channel(this.manager,a)};Chatterbox.Chatbook.prototype.feed_object=function(b,a,d){return new Chatterbox.Feed(this.manager,b,a,d)};Chatterbox.Chatbook.prototype.toggle_channel=function(a){var d=this.channel(a);var b=d;if(!d){return}if(d.hidden){if(this.current&&this.current==d){return}if(!this.manager.settings.developer){d.hide();return}}if(this.current){if(this.current==d){return}this.current.hide();
...
Chatterbox.template.feed='<div class="window feed" id="{selector}-window">                    <div class="log" id="{selector}-log">                        <header class="info">                            <div class="info">                                <h3>{name}<span>{type}</span></h3>                                <p>{info}</p>                            </div>                            <ul>                                <li><a href="#post" class="button iconic check" title="Post to this feed"></a></li>                                <li><a href="#refresh" class="button iconic cycle" title="Refresh the feed"></a></li>                                <li><a href="#close" class="button iconic x" title="Close this feed"></a></li>                            </ul>                        </header>                        <ul class="logwrap"></ul>                    </div>                    <div class="users" id="{selector}-users">                </div>            </div>';Chatterbox.template.feedmsg='<li id="{ref}">                                <article>                                    <a href="#close" class="iconic x" title="Remove feed message"></a>                                    <section class="label">{icon}</section>                                    <section class="content">                                        <h3>{title}</h3>                                        {content}                                    </section>                                </article>                            </li>';
