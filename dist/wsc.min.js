...
};wsc.Flow.prototype.recv_action=wsc.Flow.prototype.recv_msg;wsc.Flow.prototype.recv_npmsg=wsc.Flow.prototype.recv_msg;wsc.Flow.prototype.recv_privchg=function(b,a){a.channel(b.ns).recv_privchg(b)};wsc.Flow.prototype.recv_kicked=function(b,a){a.channel(b.ns).recv_kicked(b)};wsc.defaults.Extension=function(p){var r={};r.away={on:false,reason:"",last:{},since:0,store:p.storage.folder("away"),format:{setaway:"/me is away: {reason}",setback:"/me is back",away:"{from}: I am away, reason: {reason}"}};var D=function(){p.bind("cmd.connect",g);p.bind("cmd.set",a);p.bind("cmd.join",J);p.bind("cmd.part",M);p.bind("cmd.say",s);p.bind("cmd.npmsg",N);p.bind("cmd.me",u);p.bind("cmd.promote",A);p.bind("cmd.demote",A);p.bind("cmd.ban",m);p.bind("cmd.unban",m);p.bind("cmd.kick",j);p.bind("cmd.whois",F);p.bind("cmd.title",k);p.bind("cmd.topic",k);p.bind("cmd.admin",B);p.bind("cmd.disconnect",g);p.bind("cmd.kill",j);p.bind("cmd.raw",K);p.bind("cmd.clear",H);p.bind("cmd.clearall",G);p.bind("cmd.close",C);p.bind("pkt.property",l);
p.bind("pkt.recv_admin_show",L);p.bind("pkt.recv_admin_showverbose",L);p.bind("pkt.get",v);p.bind("cmd.gettitle",t);p.bind("cmd.gettopic",t);p.bind("cmd.theme",b);p.ui.on("settings.open",q);p.ui.on("settings.open.ran",I);p.ui.on("settings.save",E)};var E=function(P,O){p.settings.ui.theme=P.theme;p.settings.ui.clock=P.clock;p.settings.ui.tabclose=P.tabclose;p.config_save()};var q=function(Q,P){var O=Q.settings.page("Main",true);var R={};R.username=p.settings.username;R.pk=p.settings.pk;O.item("Form",{ref:"login",title:"Login",text:"Here you can change the username and token used to                    log into the chat server.",fields:[["Textfield",{ref:"username",label:"Username","default":R.username}],["Textfield",{ref:"token",label:"Token","default":R.pk}]],event:{save:function(S){p.settings.username=S.data.username;p.settings.pk=S.data.token}}},true);O.item("Text",{ref:"intro",title:"Main",text:"Use this window to view and change your settings.\n\nCheck                    the different pages to see what settings can be changed.",},true);
O.item("Text",{ref:"debug",wclass:"faint",title:"Debug Information",text:"Chat Agent: <code>"+p.settings.agent+"</code>\n\nUser                    Agent: <code>"+navigator.userAgent+"</code>"})};var I=function(Q,P){var O=Q.settings.page("About",true);O.item("Text",{ref:"about-wsc",title:"Wsc",text:'Currently using <a href="http://github.com/photofroggy/wsc/">wsc</a>                    version '+wsc.VERSION+" "+wsc.STATE+'.\n\nWsc                    works using HTML5, javascript, and CSS3. WebSocket is used for the connection                    where possible. The source code for this client is pretty huge.\n\nWsc was created                    by ~<a href="http://photofroggy.deviantart.com/">photofroggy</a>'})};var b=function(P,O){O.ui.theme(P.args.split(" ").shift())};var a=function(Q){var P=Q.args.split(" ");var O=P.shift().toLowerCase();var P=P.join(" ");if(P.length==0){p.cchannel.serverMessage("Could not set "+O,"No data supplied");return}if(!(O in p.settings)){p.cchannel.serverMessage('Unknown setting "'+O+'"');
return}p.settings[O]=P;p.cchannel.serverMessage("Changed "+O+" setting","value: "+P);p.control.setLabel()};var g=function(O){p[O.cmd]()};var J=function(O){var P=O.args.split(" ");var P=P.toString()==""?[]:P;if(O.ns!=O.target){P.unshift(O.target)}if(P.toString()==""){return}for(index in P){p.join(P[index])}};var M=function(O){var P=O.args.split(" ");if(O.ns!=O.target){P.unshift(O.target)}if(P.toString()==""){p.part(O.ns);return}for(index in P){p.part(P[index])}};var k=function(O){p.set(O.target,O.cmd,O.args)};var A=function(P){var O=P.args.split(" ");p[P.cmd.toLowerCase()](P.target,O[0],O[1])};var m=function(S,O){var Q=S.args.split(" ");var P=Q.shift();var R=S.cmd;if(R=="ban"&&Q.length>0){O.kick(S.target,P,Q.join(" "))}O[R](S.target,P)};var u=function(O){p.action(O.target,O.args)};var K=function(O){p.send(O.args.replace(/\\n/gm,"\n"))};var j=function(R,O){var S=R.args.split(" ");var P=S.shift();var Q=S.length>0?S.join(" "):null;if(R.cmd=="kick"){O.kick(R.target,P,Q)}else{O.kill(P,Q)}};var s=function(O){p.say(O.target,O.args)
};var N=function(O){p.npmsg(O.target,O.args)};var H=function(Q,O){if(Q.args.length>0){var R=Q.args.split(" ");for(var P in R){if(!R.hasOwnProperty(P)){continue}O.channel(Q.target).clear(R[P])}}else{O.channel(Q.target).clear()}};var G=function(P,O){var R=null;if(P.args.length>0){var Q=P.args.split(" ");R=function(T,U){for(var S in Q){if(!Q.hasOwnProperty(S)){continue}U.clear(Q[S])}}}else{R=function(S,T){T.clear()}}O.each_channel(R,true)};var C=function(O){p.part(O.target);p.remove_ns(O.target)};var F=function(P,O){O.whois(P.args.split(" ")[0])};var B=function(P,O){O.admin(P.target,P.args)};var z=function(P,O){O.disconnect()};var t=function(P,O){var Q=P.cmd.indexOf("title")>-1?"title":"topic";O.control.ui.set_text("/"+Q+" "+O.channel(P.target).info[Q].content)};var l=function(R,O){if(R.p!="info"){return}var P=R.pkt.sub;var S=P.shift().arg;S.username=R.sns.substr(1);S.connections=[];var Q={};while(P.length>0){Q=P.shift().arg;Q.channels=[];while(P.length>0){if(P[0].cmd!="ns"){break}Q.channels.unshift(O.deform_ns(P.shift().param))
}S.connections.push(Q)}O.cchannel.log_whois(S)};var v=function(P,O){if(P.ns.indexOf("login:")!=0){return}O.cchannel.server_message("Whois failed for "+(P.sns.substr(1)),"not online")};var L=function(S,P){var V=P.channel(S.ns);var O=S.info.split("\n");var U="";var T=[];var Q="";for(var R in O){if(!O.hasOwnProperty(R)){continue}U=O[R].split(" ");if(S.p=="privclass"){T.push([U.shift(),U.shift().split("=")[1],U.join(" ")])}else{if(S.p=="users"){Q=U.shift().split(":",1)[0];T.push([Q,V.get_privclass_order(Q),U.join(" ")])}}}V.log_pc(S.p=="privclass",T)};D();wsc.defaults.Extension.Autojoin(p);wsc.defaults.Extension.Away(p);wsc.defaults.Extension.Ignore(p)};wsc.defaults.Extension.Autojoin=function(a){var g=a.autojoin;var j=function(){a.bind("cmd.autojoin",b);a.ui.on("settings.open",g.page)};g.page=function(p,r){var q=p.settings.page("Autojoin",true);var m="<ul>";var t={};t.ajon=a.autojoin.on;if(a.autojoin.channel.length==0){m+="<li><i>No autojoin channels set</i></li></ul>"}else{for(var l in a.autojoin.channel){if(!a.autojoin.channel.hasOwnProperty(l)){continue
}m+="<li>"+a.autojoin.channel[l]+"</li>"}m+="</ul>"}q.item("Checkbox",{ref:"eaj",title:"Autojoin",text:"Turn on autojoin to automatically join selected channels                    when you connect to the chat server.",items:[{value:"yes",title:"On",selected:t.ajon}],event:{change:function(u){if(u.target.value=="yes"){a.autojoin.on=u.target.checked}},save:function(u){t.ajon=a.autojoin.on;a.config_save()},close:function(u){a.autojoin.on=t.ajon}}});var k=q.item("Items",{ref:"channelss",title:"Channels",text:"Add any channels you want to join automatically when you                    connect to the chat server.",items:a.autojoin.channel,event:{}});var s=q.item("Form",{ref:"channels",wclass:"boxed-ff-indv",title:"Channels",text:"Add any channels you want to join automatically when you                    connect to the chat server.",fields:[["Text",{ref:"channels",text:m}]],event:{change:function(u){},save:function(u){},close:function(u){a.config_save()}}})};var b=function(s){var l=s.args.split(" ");
if(!l){return}var q=l.shift().toLowerCase();var k=function(u){};var p=false;var t=a.channel(s.ns);switch(q){case"add":k=function(u){if(a.autojoin.channel.indexOf(u)==-1){p=true;a.autojoin.channel.push(u);t.server_message("Added "+u+" to your autojoin.")}else{t.server_message("Already have "+u+" on your autojoin.")}};break;case"rem":case"remove":k=function(v){var u=a.autojoin.channel.indexOf(v);if(u!=-1){p=true;a.autojoin.channel.splice(u,1);t.server_message("Removed "+v+" from your autojoin.")}else{t.server_message(v+" is not on your autojoin list.")}};break;case"on":t.server_message("Autojoin on.");if(!a.autojoin.on){p=true;a.autojoin.on=true}break;case"off":t.server_message("Autojoin off.");if(a.autojoin.on){p=true;a.autojoin.on=false}break}var r="";for(var m in l){if(!l.hasOwnProperty(m)){continue}r=a.deform_ns(l[m]).toLowerCase();k(r)}if(p){a.config_save()}};j()};wsc.defaults.Extension.Away=function(b){var j=b.storage.folder("away");var g={on:false,reason:"",last:{},since:0,interval:60000,format:{setaway:"/me is away: {reason}",setback:"/me is back",away:"{user}: I've been away for {timesince}. Reason: {reason}"}};
var q=function(){m();k();b.bind("cmd.setaway",a);b.bind("cmd.setback",p);b.bind("pkt.recv_msg.highlighted",l);b.ui.on("settings.open",g.page)};g.page=function(r,t){var s=r.settings.page("Away",true);var u={};u.away=g.format.away;u.sa=g.format.setaway;u.sb=g.format.setback;u.intr=g.interval;s.item("Text",{ref:"intro",title:"Away Messages",text:"Use away messages when you are away from the chats.\n\n                    You can set yourself away using the                     <code>/setaway [reason]</code> command. When you get back,                    use <code>/setback</code>. While you are away, the client                    will automatically respond when people try to talk to you,                    telling them you're away.",});s.item("Form",{ref:"msgs",title:"Messages",text:"Here you can set the messages displayed when you set                    yourself away or back. You can also change the away message                    format.",hint:"<b>{user}</b><br/>This is replaced with the username of the person trying to talk to you\n\n<b>{reason}</b>                    <br/>This is replaced by your reason for being away.\n\n<b>{timesince}</b>                    <br/>Use this to show how long you have been away for.",fields:[["Textfield",{ref:"away",label:"Away","default":u.away}],["Textfield",{ref:"setaway",label:"Setaway","default":u.sa}],["Textfield",{ref:"setback",label:"Setback","default":u.sb}]],event:{save:function(v){g.format.away=v.data.away;
g.format.setaway=v.data.setaway;g.format.setback=v.data.setback;k()}}});s.item("Form",{ref:"interval",title:"Message Interval",text:"Here you can set the amount of time to wait before                     displaying another away message. The interval is in seconds.",fields:[["Textfield",{ref:"interval",label:"Interval","default":(u.intr/1000).toString()}]],event:{save:function(v){g.interval=parseInt(v.data.interval)*1000;k()}}})};var a=function(t,r){g.on=true;g.last={};g.since=new Date();g.reason=t.args;var u=r.say;var s=replaceAll(g.format.setaway,"{reason}",g.reason||"[silent away]");if(s.indexOf("/me ")==0){s=s.substr(4);u=r.action}r.each_channel(function(v){u.call(r,v,s)})};var p=function(t,r){g.on=false;var u=r.say;var s=g.format.setback;if(s.indexOf("/me ")==0){s=s.substr(4);u=r.action}r.each_channel(function(v){u.call(r,v,s)})};var l=function(z,r){if(!g.on){return}if(g.reason.length==0){return}if(z.user==r.settings.username){return}if(r.exclude.indexOf(z.sns.toLowerCase())!=-1){return
}var u=new Date();var v=z.sns.toLowerCase();if(v in g.last){if((u-g.last[v])<=g.interval){return}}var s=timeLengthString((u-g.since)/1000);var A=replaceAll(g.format.away,"{user}",z.user);A=replaceAll(A,"{timesince}",s);r.say(z.ns,replaceAll(A,"{reason}",g.reason));g.last[v]=u};var m=function(){g.format.setaway=j.get("setaway","/me is away: {reason}");g.format.setback=j.get("setback","/me is back");g.format.away=j.get("away","{user}: I've been away for {timesince}. Reason: {reason}");g.interval=parseInt(j.get("interval",60000))};var k=function(){j.set("interval",g.interval);j.set("setaway",g.format.setaway);j.set("setback",g.format.setback);j.set("away",g.format.away)};q()};wsc.defaults.Extension.Ignore=function(g){var j={};var k=g.storage.folder("ignore");var a=k.folder("ignored");var q=function(){p();m();g.bind("cmd.ignore",b);g.bind("cmd.unignore",l);g.ui.on("settings.open",j.page)};j.page=function(t,v){var u=t.settings.page("Ignores",true);var A={};var s="<ul>";A.im=j.ignore;A.uim=j.unignore;
if(g.ui.umuted.length==0){s+="<li><i>No one ignored yet</i></li></ul>"}else{for(var r in g.ui.umuted){if(!g.ui.umuted.hasOwnProperty(r)){continue}s+="<li>"+g.ui.umuted[r]+"</li>"}s+="</ul>"}u.item("Text",{ref:"intro",title:"Ignores",text:'Use <code>ignore</code> to ignore people.\n\n                    You can "ignore" other users of the chat server using the\n                    <code>/ignore</code> command. Ignoring a user hides their                    messages from you in the channel log.',});u.item("Form",{ref:"msgs",title:"Messages",text:"Here you can set the messages displayed when you ignore or                    unignore a user.\n\nThe text <code>{user}</code> is replaced                    with the name of the user your are ignoring or unignoring.",fields:[["Textfield",{ref:"ignore",label:"Ignore","default":A.im}],["Textfield",{ref:"unignore",label:"Unignore","default":A.uim}]],event:{save:function(B){j.ignore=B.data.ignore;j.unignore=B.data.unignore;m()}}});var z=u.item("Form",{ref:"ignored",wclass:"boxed-ff-indv",title:"Users",text:"This is the list of users that you have silenced.\n\nUse the                    commands <code>/ignore</code> and <code>/unignore</code>                    to edit the list.",fields:[["Text",{ref:"users",text:s}]]})
};var b=function(u){var z=u.args.split(" ");var r="";var v="";var t=false;for(var s in z){if(!z.hasOwnProperty(s)){continue}r=z[s];tmod=g.mute_user(r);if(!tmod){continue}t=tmod;v=replaceAll(j.ignore,"{user}",r);if(v.indexOf("/me ")==0){v=v.substr(4);g.action(u.target,v)}else{g.say(u.target,v)}}if(t){m()}};var l=function(v){var B=v.args.split(" ");var s="";var A="";var r=-1;var u=false;var z=false;for(var t in B){if(!B.hasOwnProperty(t)){continue}s=B[t];z=g.unmute_user(s);if(!z){continue}u=z;A=replaceAll(j.unignore,"{user}",s);if(A.indexOf("/me ")==0){A=A.substr(4);g.action(v.target,A)}else{g.say(v.target,A)}}if(u){m()}};var p=function(){j.ignore=k.get("ignore","/me is ignoring {user} now");j.unignore=k.get("unignore","/me is not ignoring {user} anymore");j.count=parseInt(k.get("count",0));g.ui.umuted=[];if(j.count>0){var s=null;for(var r=0;r<j.count;r++){g.mute_user.push(a.get(r,null))}}};var m=function(){k.set("ignore",j.ignore);k.set("unignore",j.unignore);for(var r=0;r<j.count;r++){a.remove(r)
}if(g.ui.umuted.length==0){k.set("count",0)}else{var s=-1;for(var r in g.ui.umuted){if(!g.ui.umuted.hasOwnProperty(r)){continue}s++;a.set(s.toString(),g.ui.umuted[r])}s++;j.count=s;k.set("count",s)}};q()};wsc.Client=function(a,g,j){this.mozilla=j;this.storage=new wsc.Storage;this.storage.ui=this.storage.folder("ui");this.storage.aj=this.storage.folder("autojoin");this.storage.aj.channel=this.storage.aj.folder("channel");this.fresh=true;this.attempts=0;this.connected=false;this.protocol=null;this.flow=null;this.ui=null;this.events=new EventEmitter();this.conn=null;this.channelo={};this.cchannel=null;this.exclude=[];this.cmds=[];this.settings={domain:"website.com",server:"ws://website.com/wsendpoint",symbol:"",username:"",userinfo:{},pk:"",monitor:["~Monitor",true],welcome:"Welcome to the wsc web client!",autojoin:"chat:channel",control:wsc.Control,protocol:wsc.Protocol,mparser:wsc.MessageParser,flow:wsc.Flow,ui_object:Chatterbox.UI,extend:[wsc.defaults.Extension],client:"chatclient",clientver:"0.3",ui:{theme:wsc.defaults.theme,themes:wsc.defaults.themes,tabclose:true,clock:true}};
...
var b=k?this.view.find(k[1]):null;var a=this._get_cb("save");if(typeof a=="function"){a({input:b,item:this,page:j,window:g})}else{for(i in a){var l=b.hasOwnProperty("slice")?b.slice(i,1):b;a[i]({input:l,item:this,page:j,window:g})}}for(i in this.items){this.items[i].save(g,j)}};Chatterbox.Settings.Item.prototype.close=function(a,b){pair=this._get_ep("inspect");inps=pair?this.view.find(pair[1]):null;cb=this._get_cb("close");if(typeof cb=="function"){cb({input:inps,item:this,page:b,window:a});return}for(i in cb){sinps=inps.hasOwnProperty("slice")?inps.slice(i,1):inps;cb[i]({input:sinps,item:this,page:b,window:a})}for(i in this.items){this.items[i].close(a,b)}};Chatterbox.Settings.Item.get=function(g,b,j,a){types=g.split(".");item=j||Chatterbox.Settings.Item;for(i in types){cls=types[i];if(!item.hasOwnProperty(cls)){item=a||Chatterbox.Settings.Item;break}item=item[cls]}return new item(g,b)};Chatterbox.Settings.Item.Form=function(b,a){Chatterbox.Settings.Item.call(this,b,a);this.form=null;this.fields=[];
this.lsection=null;this.fsection=null;this.fieldo={}};Chatterbox.Settings.Item.Form.prototype=new Chatterbox.Settings.Item();Chatterbox.Settings.Item.Form.prototype.constructor=Chatterbox.Settings.Item.Form;Chatterbox.Settings.Item.Form.field=function(b,a){return Chatterbox.Settings.Item.get(b,a,Chatterbox.Settings.Item.Form,Chatterbox.Settings.Item.Form.Field)};Chatterbox.Settings.Item.Form.prototype.build=function(b){Chatterbox.Settings.Item.prototype.build.call(this,b);if(this.view==null){return}this.lsection=this.view.find("section.labels");this.fsection=this.view.find("section.fields");if(!this.options.hasOwnProperty("fields")){return}for(i in this.options.fields){f=this.options.fields[i];field=Chatterbox.Settings.Item.Form.field(f[0],f[1]);this.fields.push(field);field.build(this);if(f[1].hasOwnProperty("ref")){this.fieldo[f[1].ref]=field}}this.form=this.view.find("form");var a=this;this.form.bind("change",function(g){a.change()})};Chatterbox.Settings.Item.Form.prototype.resize=function(){for(i in this.fields){this.fields[i].resize()
...
this.value=[];this.view.find("input[checked]:checkbox").each(function(){a.value.push(a.view.find(this).val())});this.field.bind("change",function(j){a.value=[];a.view.find("input[checked]:checkbox").each(function(){a.value.push(a.view.find(this).val())})})};Chatterbox.Settings.Item.Items=function(b,a){Chatterbox.Settings.Item.call(this,b,a);this.selected=[]};Chatterbox.Settings.Item.Items.prototype=new Chatterbox.Settings.Item();Chatterbox.Settings.Item.Items.prototype.constructor=Chatterbox.Settings.Item.Items;Chatterbox.Settings.Item.Items.prototype.build=function(b){Chatterbox.Settings.Item.prototype.build.call(this,b);var a=this};Chatterbox.template={};Chatterbox.render=function(j,l){var g=Chatterbox.template;var b=j.split(".");var k={};var a=null;for(ind in b){part=b[ind];if(!g.hasOwnProperty(part)){return""}g=g[part]}if(g.hasOwnProperty("frame")){a=g;k=g.render||{};g=g.frame;if(a.hasOwnProperty("pre")){if(typeof a.pre=="function"){g=a.pre(g,l)}else{for(i in a.pre){g=a.pre[i](g,l)}}}}for(key in l){g=replaceAll(g,"{"+key+"}",(k[key]||Chatterbox.template.render_stub)(l[key]||""))
...
Chatterbox.template.settings.item.dropdown.frame="{title}<form>                                                <select>                                                    {items}                                                </select>                                            </form>";Chatterbox.template.settings.item.radio={};Chatterbox.template.settings.item.radio.pre=[Chatterbox.template.settings.item.twopane.wrap,Chatterbox.template.settings.item.hint.prep];Chatterbox.template.settings.item.radio.render={title:Chatterbox.template.settings.krender.title,text:Chatterbox.template.settings.krender.text,items:Chatterbox.template.settings.krender.radioitems};Chatterbox.template.settings.item.radio.post=Chatterbox.template.clean(["ref","title","items"]);Chatterbox.template.settings.item.radio.events=[["change","input:radio"],["inspect","input:radio"]];Chatterbox.template.settings.item.radio.frame='{title}<div class="{ref} radiobox"><form>{items}</form></div>';Chatterbox.template.settings.item.checkbox={};
Chatterbox.template.settings.item.checkbox.pre=[Chatterbox.template.settings.item.twopane.wrap,Chatterbox.template.settings.item.hint.prep];Chatterbox.template.settings.item.checkbox.render={title:Chatterbox.template.settings.krender.title,text:Chatterbox.template.settings.krender.text,items:Chatterbox.template.settings.krender.checkitems};Chatterbox.template.settings.item.checkbox.post=Chatterbox.template.clean(["ref","title","items"]);Chatterbox.template.settings.item.checkbox.events=[["change","input:checkbox"],["inspect","input:checkbox"]];Chatterbox.template.settings.item.checkbox.frame='{title}<div class="{ref} checkbox"><form>{items}</form></div>';Chatterbox.template.settings.item.textfield={};Chatterbox.template.settings.item.textfield.pre=[Chatterbox.template.settings.item.twopane.wrap,Chatterbox.template.settings.item.hint.prep];Chatterbox.template.settings.item.textfield.render={title:Chatterbox.template.settings.krender.title,text:Chatterbox.template.settings.krender.text};Chatterbox.template.settings.item.textfield.post=Chatterbox.template.clean(["ref","title","default"]);
Chatterbox.template.settings.item.textfield.events=[["blur","input"],["inspect","input"]];Chatterbox.template.settings.item.textfield.frame='{title}<div class="{ref} textfield"><form><input type="text" value="{default}" /></form></div>';Chatterbox.template.settings.item.textarea={};Chatterbox.template.settings.item.textarea.pre=[Chatterbox.template.settings.item.twopane.wrap,Chatterbox.template.settings.item.hint.prep];Chatterbox.template.settings.item.textarea.render={title:Chatterbox.template.settings.krender.title,text:Chatterbox.template.settings.krender.text};Chatterbox.template.settings.item.textarea.post=Chatterbox.template.clean(["ref","title","default"]);Chatterbox.template.settings.item.textarea.events=[["blur","textarea"],["inspect","textarea"]];Chatterbox.template.settings.item.textarea.frame='{title}<div class="{ref} textarea"><form><textarea rows="4" cols="20" value="{default}"></textarea></form></div>';Chatterbox.template.settings.item.items={};Chatterbox.template.settings.item.items.pre=[Chatterbox.template.settings.item.twopane.wrap,Chatterbox.template.settings.item.hint.prep];
Chatterbox.template.settings.item.items.render={title:Chatterbox.template.settings.krender.title,text:Chatterbox.template.settings.krender.text,items:Chatterbox.template.settings.krender.manageditems};Chatterbox.template.settings.item.items.post=Chatterbox.template.clean(["ref","title","items"]);Chatterbox.template.settings.item.items.events=[];Chatterbox.template.settings.item.items.frame='{title}<div class="{ref} items">    <section class="mitems">{items}</section></div>    <section class="buttons"><p><a href="#up" title="Move item up" class="button iconic arrow_up"></a>    <a href="#down" title="Move item down" class="button iconic arrow_down"></a>    <a href="#remove" title="Remove item from list" class="button close big square iconic x"></a>    </p></section>';Chatterbox.template.settings.item.form={};Chatterbox.template.settings.item.form.pre=[Chatterbox.template.settings.item.twopane.wrap,Chatterbox.template.settings.item.hint.prep];Chatterbox.template.settings.item.form.render={title:Chatterbox.template.settings.krender.title,text:Chatterbox.template.settings.krender.text,items:Chatterbox.template.settings.krender.dditems};
