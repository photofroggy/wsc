...
};wsc.Flow.prototype.recv_action=wsc.Flow.prototype.recv_msg;wsc.Flow.prototype.recv_npmsg=wsc.Flow.prototype.recv_msg;wsc.Flow.prototype.recv_privchg=function(b,a){a.channel(b.ns).recv_privchg(b)};wsc.Flow.prototype.recv_kicked=function(b,a){a.channel(b.ns).recv_kicked(b)};wsc.defaults.Extension=function(m){var o={};o.away={on:false,reason:"",last:{},since:0,store:m.storage.folder("away"),format:{setaway:"/me is away: {reason}",setback:"/me is back",away:"{from}: I am away, reason: {reason}"}};var y=function(){m.bind("cmd.connect",f);m.bind("cmd.set",a);m.bind("cmd.join",E);m.bind("cmd.part",H);m.bind("cmd.say",p);m.bind("cmd.npmsg",I);m.bind("cmd.me",r);m.bind("cmd.promote",u);m.bind("cmd.demote",u);m.bind("cmd.ban",l);m.bind("cmd.unban",l);m.bind("cmd.kick",g);m.bind("cmd.whois",A);m.bind("cmd.title",j);m.bind("cmd.topic",j);m.bind("cmd.admin",v);m.bind("cmd.disconnect",f);m.bind("cmd.kill",g);m.bind("cmd.raw",F);m.bind("cmd.clear",C);m.bind("cmd.clearall",B);m.bind("cmd.close",x);m.bind("pkt.property",k);
m.bind("pkt.recv_admin_show",G);m.bind("pkt.recv_admin_showverbose",G);m.bind("pkt.get",s);m.bind("cmd.gettitle",q);m.bind("cmd.gettopic",q);m.bind("cmd.theme",b);m.ui.on("settings.open",n);m.ui.on("settings.open.ran",D);m.ui.on("settings.save.ui",z);m.ui.on("settings.save",function(){m.config_save()})};var z=function(K,J){m.settings.ui.theme=K.theme;m.settings.ui.clock=K.clock;m.settings.ui.tabclose=K.tabclose};var n=function(L,K){var J=L.settings.page("Main");var M={};M.username=m.settings.username;M.pk=m.settings.pk;J.item("Form",{ref:"login",title:"Login",text:"Here you can change the username and token used to                    log into the chat server.",fields:[["Textfield",{ref:"username",label:"Username","default":M.username}],["Textfield",{ref:"token",label:"Token","default":M.pk}]],event:{save:function(N){m.settings.username=N.data.username;m.settings.pk=N.data.token}}},true);J.item("Text",{ref:"intro",title:"Main",text:"Use this window to view and change your settings.\n\nCheck                    the different pages to see what settings can be changed.",},true);
J.item("Text",{ref:"debug",wclass:"faint",title:"Debug Information",text:"Chat Agent: <code>"+m.settings.agent+"</code>\n\nUser                    Agent: <code>"+navigator.userAgent+"</code>"})};var D=function(L,K){var J=L.settings.page("About",true);J.item("Text",{ref:"about-wsc",title:"Wsc",text:'Currently using <a href="http://github.com/photofroggy/wsc/">wsc</a>                    version '+wsc.VERSION+" "+wsc.STATE+'.\n\nWsc                    works using HTML5, javascript, and CSS3. WebSocket is used for the connection                    where possible. The source code for this client is pretty huge.\n\nWsc was created                    by ~<a href="http://photofroggy.deviantart.com/">photofroggy</a>'})};var b=function(K,J){J.ui.theme(K.args.split(" ").shift())};var a=function(L){var K=L.args.split(" ");var J=K.shift().toLowerCase();var K=K.join(" ");if(K.length==0){m.cchannel.serverMessage("Could not set "+J,"No data supplied");return}if(!(J in m.settings)){m.cchannel.serverMessage('Unknown setting "'+J+'"');
return}m.settings[J]=K;m.cchannel.serverMessage("Changed "+J+" setting","value: "+K);m.control.setLabel()};var f=function(J){m[J.cmd]()};var E=function(J){var K=J.args.split(" ");var K=K.toString()==""?[]:K;if(J.ns!=J.target){K.unshift(J.target)}if(K.toString()==""){return}for(index in K){m.join(K[index])}};var H=function(J){var K=J.args.split(" ");if(J.ns!=J.target){K.unshift(J.target)}if(K.toString()==""){m.part(J.ns);return}for(index in K){m.part(K[index])}};var j=function(J){m.set(J.target,J.cmd,J.args)};var u=function(K){var J=K.args.split(" ");m[K.cmd.toLowerCase()](K.target,J[0],J[1])};var l=function(N,J){var L=N.args.split(" ");var K=L.shift();var M=N.cmd;if(M=="ban"&&L.length>0){J.kick(N.target,K,L.join(" "))}J[M](N.target,K)};var r=function(J){m.action(J.target,J.args)};var F=function(J){m.send(J.args.replace(/\\n/gm,"\n"))};var g=function(M,J){var N=M.args.split(" ");var K=N.shift();var L=N.length>0?N.join(" "):null;if(M.cmd=="kick"){J.kick(M.target,K,L)}else{J.kill(K,L)}};var p=function(J){m.say(J.target,J.args)
};var I=function(J){m.npmsg(J.target,J.args)};var C=function(L,J){if(L.args.length>0){var M=L.args.split(" ");for(var K in M){if(!M.hasOwnProperty(K)){continue}J.channel(L.target).clear(M[K])}}else{J.channel(L.target).clear()}};var B=function(K,J){var M=null;if(K.args.length>0){var L=K.args.split(" ");M=function(O,P){for(var N in L){if(!L.hasOwnProperty(N)){continue}P.clear(L[N])}}}else{M=function(N,O){O.clear()}}J.each_channel(M,true)};var x=function(J){m.part(J.target);m.remove_ns(J.target)};var A=function(K,J){J.whois(K.args.split(" ")[0])};var v=function(K,J){J.admin(K.target,K.args)};var t=function(K,J){J.disconnect()};var q=function(K,J){var L=K.cmd.indexOf("title")>-1?"title":"topic";J.control.ui.set_text("/"+L+" "+J.channel(K.target).info[L].content)};var k=function(M,J){if(M.p!="info"){return}var K=M.pkt.sub;var N=K.shift().arg;N.username=M.sns.substr(1);N.connections=[];var L={};while(K.length>0){L=K.shift().arg;L.channels=[];while(K.length>0){if(K[0].cmd!="ns"){break}L.channels.unshift(J.deform_ns(K.shift().param))
}N.connections.push(L)}J.cchannel.log_whois(N)};var s=function(K,J){if(K.ns.indexOf("login:")!=0){return}J.cchannel.server_message("Whois failed for "+(K.sns.substr(1)),"not online")};var G=function(N,K){var Q=K.channel(N.ns);var J=N.info.split("\n");var P="";var O=[];var L="";for(var M in J){if(!J.hasOwnProperty(M)){continue}P=J[M].split(" ");if(N.p=="privclass"){O.push([P.shift(),P.shift().split("=")[1],P.join(" ")])}else{if(N.p=="users"){L=P.shift().split(":",1)[0];O.push([L,Q.get_privclass_order(L),P.join(" ")])}}}Q.log_pc(N.p=="privclass",O)};y();wsc.defaults.Extension.Ignore(m);wsc.defaults.Extension.Away(m);wsc.defaults.Extension.Autojoin(m)};wsc.defaults.Extension.Autojoin=function(a){var f=a.autojoin;a.ui.control.add_button({label:"Autojoin",title:"Join your autojoin channels",href:"#autojoin-do",handler:function(){for(var j in a.autojoin.channel){if(!a.autojoin.channel.hasOwnProperty(j)){continue}a.join(a.autojoin.channel[j])}}});var g=function(){a.bind("cmd.autojoin",b);a.ui.on("settings.open",f.page)
