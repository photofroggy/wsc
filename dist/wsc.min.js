...
b.name=this.options.ref||"ref"}}Chatterbox.Settings.Item.prototype.build.call(this,f);this.field=this.view.find("input:checkbox");var a=this;this.value=[];this.view.find("input[checked]:checkbox").each(function(){a.value.push(a.view.find(this).val())});this.field.bind("change",function(g){a.value=[];a.view.find("input[checked]:checkbox").each(function(){a.value.push(a.view.find(this).val())})})};Chatterbox.Settings.Item.Items=function(b,a,f){Chatterbox.Settings.Item.call(this,b,a,f);this.selected=false};Chatterbox.Settings.Item.Items.prototype=new Chatterbox.Settings.Item();Chatterbox.Settings.Item.Items.prototype.constructor=Chatterbox.Settings.Item.Items;Chatterbox.Settings.Item.Items.prototype.build=function(b){Chatterbox.Settings.Item.prototype.build.call(this,b);var a=this;this.list=this.view.find("ul");this.buttons=this.view.find("section.buttons p");var f=this.list.find("li");f.click(function(j){var g=a.list.find(this);a.list.find("li.selected").removeClass("selected");a.selected=g.find(".item").html();
g.addClass("selected")});f.each(function(g,k){var j=a.list.find(k);j.find("a.close").click(function(l){a.list.find("li.selected").removeClass("selected");a.selected=j.find(".item").html();j.addClass("selected");a.remove_item();return false})});this.buttons.find("a.button.up").click(function(g){a.shift_item(true);return false});this.buttons.find("a.button.down").click(function(g){a.shift_item();return false});this.buttons.find("a.button.add").click(function(j){console.log(j);var g=new Chatterbox.Popup.Prompt(a.manager,{title:"Add item",label:"Item:",position:[j.clientX-100,j.clientY-50]});g.build();return false});this.buttons.find("a.button.close").click(function(g){a.remove_item();return false})};Chatterbox.Settings.Item.Items.prototype.shift_item=function(b){if(this.selected===false){return}var f=this.options.items.indexOf(this.selected);var a=f+1;if(b){a=f-1}if(f==-1||f>=this.options.items.length){return}if(a<0||a>=this.options.items.length){return}this._fevent((b?"up":"down"),{swap:{"this":{index:f,item:this.options.items[f]},that:{index:a,item:this.options.items[a]}}});
this.refresh();this._onchange({});return};Chatterbox.Settings.Item.Items.prototype.remove_item=function(){if(this.selected===false){return false}var a=this.options.items.indexOf(this.selected);if(a==-1||a>=this.options.items.length){return false}this._fevent("remove",{index:a,item:this.selected});this.refresh();this._onchange({})};Chatterbox.Settings.Item.Items.prototype.refresh=function(){this.view.find("section.mitems").html(Chatterbox.template.settings.krender.manageditems(this.options.items));this.list=this.view.find("ul");this.list.find("li[title="+(this.selected||"").toLowerCase()+"]").addClass("selected");var a=this;var b=this.list.find("li");b.click(function(g){var f=a.list.find(this);a.list.find("li.selected").removeClass("selected");a.selected=f.find(".item").html();f.addClass("selected")});b.each(function(f,j){var g=a.list.find(j);g.find("a.close").click(function(k){a.list.find("li.selected").removeClass("selected");a.selected=g.find(".item").html();g.addClass("selected");a.remove_item();
return false})})};Chatterbox.Settings.Item.Items.prototype._fevent=function(b,g){var k=this._get_ep("inspect");var f=k?this.view.find(k[1]):null;var a=this._get_cb(b);if(typeof a=="function"){a({input:f,item:this,args:g})}else{for(var j in a){var l=f.hasOwnProperty("slice")?f.slice(j,1):f;a[j]({input:l,item:this,args:g})}}};Chatterbox.Settings.Item.Items.prototype.save=function(){this._fevent("save",{items:this.options.items||[]})};Chatterbox.template={};Chatterbox.render=function(g,k){var f=Chatterbox.template;var b=g.split(".");var j={};var a=null;for(ind in b){part=b[ind];if(!f.hasOwnProperty(part)){return""}f=f[part]}if(f.hasOwnProperty("frame")){a=f;j=f.render||{};f=f.frame;if(a.hasOwnProperty("pre")){if(typeof a.pre=="function"){f=a.pre(f,k)}else{for(i in a.pre){f=a.pre[i](f,k)}}}}for(key in k){f=replaceAll(f,"{"+key+"}",(j[key]||Chatterbox.template.render_stub)(k[key]||""))}if(a!=null){if(a.hasOwnProperty("post")){if(typeof a.post=="function"){f=a.post(f,k)}else{for(i in a.post){f=a.post[i](f,k)
}}}}return f};Chatterbox.template.render_stub=function(a){return a};Chatterbox.template.clean=function(a){return function(b,f){for(i in a){b=replaceAll(b,"{"+a[i]+"}","")}return b}};Chatterbox.template.ui='<nav class="tabs"><ul id="chattabs" class="tabs"></ul>        <ul id="tabnav">            <li><a href="#left" class="button iconic arrow_left"></a></li>            <li><a href="#right" class="button iconic arrow_right"></a></li>            <li><a href="#settings" title="Change client settings" class="button iconic cog" id="settings-button"></a></li>        </ul>        </nav>        <div class="chatbook"></div>';Chatterbox.template.control='<div class="chatcontrol">            <p><a href="#multiline" title="Toggle multiline input" class="button iconic list"></a></p>            <form class="msg">                <input type="text" class="msg" />                <textarea class="msg"></textarea>                <input type="submit" value="Send" class="sendmsg" />            </form>        </div>';
