...
j.unignore=v.data.unignore}}})};var b=function(u){var z=u.args.split(" ");var r="";var v="";var t=false;for(var s in z){if(!z.hasOwnProperty(s)){continue}r=z[s];if(r.length==0){continue}t=true;v=replaceAll(j.ignore,"{user}",r.toLowerCase());if(v.indexOf("/me ")==0){v=v.substr(4);g.action(u.target,v)}else{g.say(u.target,v)}r=r.toLowerCase();j.ignored.push(r)}if(t){m()}};var l=function(v){var A=v.args.split(" ");var s="";var z="";var r=-1;var u=false;console.log(j.ignored);for(var t in A){if(!A.hasOwnProperty(t)){continue}s=A[t];if(s.length==0){continue}r=j.ignored.indexOf(s);console.log(r);if(r<0){continue}u=true;z=replaceAll(j.unignore,"{user}",s.toLowerCase());if(z.indexOf("/me ")==0){z=z.substr(4);g.action(v.target,z)}else{g.say(v.target,z)}s=s.toLowerCase();j.ignored.splice(r,1)}if(u){m()}};var p=function(){j.ignore=k.get("ignore","/me is ignoring {user} now");j.unignore=k.get("unignore","/me is not ignoring {user} anymore");j.count=parseInt(k.get("count",0));j.ignored=[];if(j.count>0){var s=null;
