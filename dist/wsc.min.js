...
return}m.settings[K]=L;m.cchannel.serverMessage("Changed "+K+" setting","value: "+L);m.control.setLabel()};var f=function(K){m[K.cmd]()};var F=function(K){var L=K.args.split(" ");var L=L.toString()==""?[]:L;if(K.ns!=K.target){L.unshift(K.target)}if(L.toString()==""){return}for(index in L){m.join(L[index])}};var I=function(K){var L=K.args.split(" ");if(K.ns!=K.target){L.unshift(K.target)}if(L.toString()==""){m.part(K.ns);return}for(index in L){m.part(L[index])}};var j=function(K){m.set(K.target,K.cmd,K.args)};var u=function(L){var K=L.args.split(" ");m[L.cmd.toLowerCase()](L.target,K[0],K[1])};var l=function(O,K){var M=O.args.split(" ");var L=M.shift();var N=O.cmd;if(N=="ban"&&M.length>0){K.kick(O.target,L,M.join(" "))}K[N](O.target,L)};var r=function(K){m.action(K.target,K.args)};var G=function(K){m.send(K.args.replace(/\\n/gm,"\n"))};var g=function(N,K){var O=N.args.split(" ");var L=O.shift();var M=O.length>0?O.join(" "):null;if(N.cmd=="kick"){K.kick(N.target,L,M)}else{K.kill(L,M)}};var p=function(K){m.say(K.target,K.args)
};var J=function(K){m.npmsg(K.target,K.args)};var D=function(M,K){if(M.args.length>0){var N=M.args.split(" ");for(var L in N){if(!N.hasOwnProperty(L)){continue}K.channel(M.target).clear(N[L])}}else{K.channel(M.target).clear()}};var C=function(L,K){var N=null;if(L.args.length>0){var M=L.args.split(" ");N=function(P,Q){for(var O in M){if(!M.hasOwnProperty(O)){continue}Q.clear(M[O])}}}else{N=function(O,P){P.clear()}}K.each_channel(N,true)};var y=function(K){m.part(K.target);m.remove_ns(K.target)};var B=function(L,K){K.whois(L.args.split(" ")[0])};var v=function(L,K){K.admin(L.target,L.args)};var t=function(L,K){K.disconnect()};var q=function(L,K){var M=L.cmd.indexOf("title")>-1?"title":"topic";K.control.ui.set_text("/"+M+" "+K.channel(L.target).info[M].content)};var k=function(N,K){if(N.p!="info"){return}var L=N.pkt.sub;var O=L.shift().arg;O.username=N.sns.substr(1);O.connections=[];var M={};while(L.length>0){M=L.shift().arg;M.channels=[];while(L.length>0){if(L[0].cmd!="ns"){break}M.channels.unshift(K.deform_ns(L.shift().param))
