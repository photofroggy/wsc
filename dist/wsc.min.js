...
return false};Chatterbox.Control.prototype.keypress=function(d){var b=d.which||d.keyCode;var f=this.tab.hit;var a=false;switch(b){case 13:if(!this.multiline()){this.submit(d)}else{if(d.shiftKey){this.submit(d)}else{a=true}}break;case 38:if(!this.multiline()){this.scroll_history(true);break}a=true;break;case 40:if(!this.multiline()){this.scroll_history(false);break}a=true;break;case 9:if(d.shiftKey){this.manager.channel_right()}else{this.tab_item(d);f=false}break;case 219:if(d.ctrlKey){this.manager.channel_left()}else{a=true}break;case 221:if(d.ctrlKey){this.manager.channel_right()}else{a=true}break;case 84:if(d.ctrlKey){this.manager.nav.toggle_sidebar()}else{a=true}break;default:a=true;break}if(f){this.end_tab(d)}return a};Chatterbox.Control.prototype.handle=function(b,g){if(g==""){return}if(!this.manager.chatbook.current){return}var k=false;if(g[0]!="/"){k=true}g=(b.shiftKey?"/npmsg ":(g[0]=="/"?"":"/say "))+g;g=g.slice(1);var l=g.split(" ");var d=l.shift().toLowerCase();var j=this.manager.chatbook.current.namespace;
