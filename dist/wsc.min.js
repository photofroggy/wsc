...
};wsc.Flow.prototype.recv_action=wsc.Flow.prototype.recv_msg;wsc.Flow.prototype.recv_npmsg=wsc.Flow.prototype.recv_msg;wsc.Flow.prototype.recv_privchg=function(b,a){a.channel(b.ns).recv_privchg(b)};wsc.Flow.prototype.recv_kicked=function(b,a){a.channel(b.ns).recv_kicked(b)};wsc.defaults.Extension=function(b){var d={};d.away={on:false,reason:"",last:{},since:0,store:b.storage.folder("away"),format:{setaway:"/me is away: {reason}",setback:"/me is back",away:"{from}: I am away, reason: {reason}"}};var j=function(){b.bind("cmd.connect",f.connection);b.bind("cmd.set",f.setter);b.bind("cmd.join",f.join);b.bind("cmd.chat",f.pjoin);b.bind("cmd.part",f.part);b.bind("cmd.say",f.say);b.bind("cmd.npmsg",f.npmsg);b.bind("cmd.me",f.action);b.bind("cmd.promote",f.chgpriv);b.bind("cmd.demote",f.chgpriv);b.bind("cmd.ban",f.ban);b.bind("cmd.unban",f.ban);b.bind("cmd.kick",f.killk);b.bind("cmd.whois",f.whois);b.bind("cmd.title",f.title);b.bind("cmd.topic",f.title);b.bind("cmd.admin",f.admin);b.bind("cmd.disconnect",f.connection);
b.bind("cmd.kill",f.killk);b.bind("cmd.raw",f.raw);b.bind("cmd.close",f.close);b.bind("pkt.property",g);b.bind("pkt.recv_admin_show",a);b.bind("pkt.recv_admin_showverbose",a);b.bind("pkt.get",h);b.bind("cmd.gettitle",f.gett);b.bind("cmd.gettopic",f.gett);b.bind("cmd.theme",f.theme)};d.save=function(l,k){b.settings.ui.theme=l.theme;b.settings.ui.clock=l.clock;b.settings.ui.tabclose=l.tabclose};var f={};f.theme=function(l,k){k.ui.theme(l.args.split(" ").shift())};f.setter=function(m){var l=m.args.split(" ");var k=l.shift().toLowerCase();var l=l.join(" ");if(l.length==0){b.cchannel.serverMessage("Could not set "+k,"No data supplied");return}if(!(k in b.settings)){b.cchannel.serverMessage('Unknown setting "'+k+'"');return}b.settings[k]=l;b.cchannel.serverMessage("Changed "+k+" setting","value: "+l)};f.connection=function(k){b[k.cmd]()};f.join=function(k){var l=k.args.split(" ");var l=l.toString()==""?[]:l;if(k.ns!=k.target){l.unshift(k.target)}if(l.toString()==""){return}for(index in l){b.join(l[index])
}};f.pjoin=function(k){var l=k.args.split(" ");var l=l.toString()==""?[]:l;if(k.ns!=k.target){l.unshift(k.target)}if(l.toString()==""){return}for(index in l){b.join("@"+l[index])}};f.part=function(k){var l=k.args.split(" ");if(k.ns!=k.target){l.unshift(k.target)}if(l.toString()==""){b.part(k.ns);return}for(index in l){b.part(l[index])}};f.title=function(k){b.set(k.target,k.cmd,k.args)};f.chgpriv=function(l){var k=l.args.split(" ");b[l.cmd.toLowerCase()](l.target,k[0],k[1])};f.ban=function(o,k){var m=o.args.split(" ");var l=m.shift();var n=o.cmd;if(n=="ban"&&m.length>0){k.kick(o.target,l,m.join(" "))}k[n](o.target,l)};f.action=function(k){b.action(k.target,k.args)};f.raw=function(k){b.send(k.args.replace(/\\n/gm,"\n"))};f.killk=function(n,k){var o=n.args.split(" ");var l=o.shift();var m=o.length>0?o.join(" "):null;if(n.cmd=="kick"){k.kick(n.target,l,m)}else{k.kill(l,m)}};f.say=function(k){if(b.channel(k.target).monitor){return}b.say(k.target,k.args)};f.npmsg=function(k){b.npmsg(k.target,k.args)
};f.clear=function(m,k){if(m.args.length>0){var n=m.args.split(" ");for(var l in n){if(!n.hasOwnProperty(l)){continue}k.ui.channel(m.target).clear_user(n[l])}}else{k.ui.channel(m.target).clear()}};f.clearall=function(l,k){var n=null;if(l.args.length>0){var m=l.args.split(" ");n=function(p,q){for(var o in m){if(!m.hasOwnProperty(o)){continue}q.clear_user(m[o])}}}else{n=function(o,p){p.clear()}}k.ui.chatbook.each(n,true)};f.close=function(k){b.part(k.target);b.remove_ns(k.target)};f.whois=function(l,k){k.whois(l.args.split(" ")[0])};f.admin=function(l,k){k.admin(l.target,l.args)};f.disconnect=function(l,k){k.disconnect()};f.gett=function(l,k){var m=l.cmd.indexOf("title")>-1?"title":"topic";k.ui.control.set_text("/"+m+" "+k.channel(l.target).info[m].content)};var g=function(p,l){if(p.p!="info"){return}var n=p.pkt.sub;var q=n.shift().arg;q.username=p.sns.substr(1);q.connections=[];var o=null;var r={};while(n.length>0){r=n.shift();switch(r.cmd){case"conn":if(o!=null){q.connections.push(o)}o=r.arg;
