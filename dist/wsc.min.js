...
r.away={on:false,reason:"",last:{},since:0};var F=function(){p.bind("cmd.connect",g);p.bind("cmd.set",a);p.bind("cmd.join",K);p.bind("cmd.part",N);p.bind("cmd.say",t);p.bind("cmd.npmsg",O);p.bind("cmd.me",A);p.bind("cmd.promote",D);p.bind("cmd.demote",D);p.bind("cmd.ban",m);p.bind("cmd.unban",m);p.bind("cmd.kick",j);p.bind("cmd.whois",G);p.bind("cmd.title",k);p.bind("cmd.topic",k);p.bind("cmd.admin",E);p.bind("cmd.disconnect",g);p.bind("cmd.kill",j);p.bind("cmd.raw",L);p.bind("cmd.clear",I);p.bind("cmd.clearall",H);p.bind("pkt.property",l);p.bind("pkt.recv_admin_show",M);p.bind("pkt.recv_admin_showverbose",M);p.bind("pkt.get",B);p.bind("cmd.gettitle",z);p.bind("cmd.gettopic",z);p.bind("cmd.setaway",u);p.bind("cmd.setback",s);p.bind("pkt.recv_msg.highlighted",v);p.bind("cmd.theme",b);p.ui.on("settings.open",q);p.ui.on("settings.open.ran",J)};var q=function(R,Q){page=R.settings.page("Main");var S={};S.theme=replaceAll(p.ui.settings.theme,"wsct_","");S.clock=p.ui.clock();S.tc=p.ui.nav.closer();
S.username=p.settings.username;S.pk=p.settings.pk;var P=[];for(i in p.ui.settings.themes){name=replaceAll(p.ui.settings.themes[i],"wsct_","");P.push({value:name,title:name,selected:S.theme==name})}page.item("Text",{ref:"intro",title:"Main",text:"Use this window to view and change your settings.\n\nCheck                    the different pages to see what settings can be changed.",});page.item("Form",{ref:"login",title:"Login",text:"Here you can change the username and token used to                    log into the chat server.",fields:[["Textfield",{ref:"username",label:"Username","default":S.username}],["Textfield",{ref:"token",label:"Token","default":S.pk}]],event:{save:function(T){p.settings.username=T.data.username;p.settings.pk=T.data.token}}});page.item("Form",{ref:"ui",title:"UI",hint:"<b>Timestamp</b><br/>Choose between a 24 hour clock and                    a 12 hour clock.\n\n<b>Theme</b><br/>Change the look of the                    client.\n\n<b>Close Buttons</b><br/>Turn tab close buttons on/off.",fields:[["Dropdown",{ref:"theme",label:"Theme",items:P}],["Dropdown",{ref:"clock",label:"Timestamp Format",items:[{value:"24",title:"24 hour",selected:S.clock},{value:"12",title:"12 hour",selected:!S.clock}]}],["Check",{ref:"tabclose",label:"Close Buttons",items:[{value:"yes",title:"On",selected:S.tc}]}],],event:{change:function(T){p.ui.clock(T.data.clock=="24");
