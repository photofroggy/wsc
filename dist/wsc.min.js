...
f.build();if(this.trail.indexOf(f.namespace)==-1){this.trail.push(f.namespace)}if(!f.visible){this.toggle_channel(b)}this.manager.resize();return f};Chatterbox.Chatbook.prototype.create_feed=function(d,b,a,g){var f=this.channel(d,this.feed_object(d,b,a,g));f.build();if(this.trail.indexOf(f.namespace)==-1){this.trail.push(f.namespace)}if(!f.visible){this.toggle_channel(d)}this.manager.resize();return f};Chatterbox.Chatbook.prototype.channel_object=function(a){return new Chatterbox.Channel(this.manager,a)};Chatterbox.Chatbook.prototype.feed_object=function(d,b,a,f){return new Chatterbox.Feed(this.manager,d,b,a,f)};Chatterbox.Chatbook.prototype.toggle_channel=function(a){var d=this.channel(a);var b=d;if(!d){return}if(d.hidden){if(this.current&&this.current==d){return}if(!this.manager.settings.developer){d.hide();return}}if(this.current){if(this.current==d){return}this.current.hide();b=this.current}else{if(this.manager.monitoro!==null){this.manager.monitoro.hide()}}d.show();this.manager.control.focus();
this.current=d;this.manager.resize();this.manager.control.cache_input(b,d);this.manager.trigger("channel.selected",{ns:d.raw,chan:d,prev:b});this.manager.client.select_ns(d.raw)};Chatterbox.Chatbook.prototype.remove_channel=function(a){var b=this.channel(a);if(this.channels()==0&&!b.hidden){return}b.remove();delete this.chan[b.raw.toLowerCase()];if(this.current==b){this.channel_left()}var d=this.trail.indexOf(b.namespace);this.trail.splice(d,1)};Chatterbox.Chatbook.prototype.channel_left=function(){var b=this.current.namespace;var a=this.trail.indexOf(b);if(a<0){return}var f=null;while(true){try{f=this.channel(this.trail[--a])}catch(d){a=this.trail.length-1;f=this.channel(this.trail[a])}if(!f.hidden){break}if(this.manager.settings.developer){break}}this.toggle_channel(f.namespace)};Chatterbox.Chatbook.prototype.channel_right=function(){var b=this.current.namespace;var a=this.trail.indexOf(b);if(a==-1){return}var f=null;while(true){try{f=this.channel(this.trail[++a])}catch(d){a=0;f=this.channel(this.trail[0])
