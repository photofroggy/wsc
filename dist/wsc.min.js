...
a.attempts=0};wsc.Flow.prototype.close=function(b,d){var a={name:"closed",pkt:new wsc.Packet("connection closed\n\n"),reason:"",evt:d};b.trigger("closed",a);if(b.connected){b.ui.server_message("Connection closed");b.connected=false;if(b.conn instanceof wsc.SocketIO){b.ui.server_message("At the moment there is a problem with reconnecting under socket.io.");b.ui.server_message("Refresh the page to connect.");return}}else{b.ui.server_message("Connection failed")}a.name="quit";if(b.attempts>2){b.ui.server_message("Can't connect. Try again later.");b.attempts=0;b.trigger("quit",a);return}if(d.cause){if(d.cause.hasOwnProperty("name")&&d.cause.name=="login"){b.trigger("quit",a);return}}b.ui.server_message("Connecting in 2 seconds");setTimeout(function(){b.connect()},2000)};wsc.Flow.prototype.message=function(a,f){var b=new wsc.Packet(unescape(replaceAll(f.data,"+"," ")));if(b==null){return}var d=this.protocol.parse(b);if(d.ns==null){d.ns=a.mns}d.sns=a.deform_ns(d.ns);this.handle(a,d);a.trigger("pkt",d);
a.trigger("pkt."+d.name,d)};wsc.Flow.prototype.handle=function(a,d){try{this[d.name](d,a)}catch(b){}};wsc.Flow.prototype.ping=function(b,a){a.pong()};wsc.Flow.prototype.chatserver=function(b,a){a.login()};wsc.Flow.prototype.login=function(d,a){if(d.pkt.arg.e=="ok"){var g=d.pkt.sub[0];a.settings.username=d.pkt.param;a.settings.symbol=g.arg.symbol;a.settings.userinfo=g.arg;var b=function(){a.join(a.settings.autojoin);if(a.autojoin.on){for(var h in a.autojoin.channel){if(!a.autojoin.channel.hasOwnProperty(h)){continue}a.join(a.autojoin.channel[h])}}};if(a.fresh){b()}else{var f=false;for(key in a.channelo){if(a.channelo[key].namespace[0]!="~"){a.join(key);f=true}}if(!f){b()}}}else{a.close(d)}if(a.fresh){a.fresh=false}};wsc.Flow.prototype.join=function(d,a){if(d.pkt.arg["e"]=="ok"){var b=a.deform_ns(d.pkt.param);a.create_ns(b,a.hidden.contains(d.pkt.param));a.ui.channel(b).server_message("You have joined "+b)}else{a.ui.chatbook.current.server_message("Failed to join "+a.deform_ns(d.pkt.param),d.pkt.arg["e"])
...
}};b.say=function(n){if(d.channel(n.target).monitor){return}d.say(n.target,n.args)};b.npmsg=function(n){d.npmsg(n.target,n.args)};b.clear=function(p,n){if(p.args.length>0){var q=p.args.split(" ");for(var o in q){if(!q.hasOwnProperty(o)){continue}n.ui.channel(p.target).clear_user(q[o])}}else{n.ui.channel(p.target).clear()}};b.clearall=function(o,n){var q=null;if(o.args.length>0){var p=o.args.split(" ");q=function(s,t){for(var r in p){if(!p.hasOwnProperty(r)){continue}t.clear_user(p[r])}}}else{q=function(r,s){s.clear()}}n.ui.chatbook.each(q,true)};b.close=function(n){d.part(n.target);d.remove_ns(n.target)};b.whois=function(o,n){n.whois(o.args.split(" ")[0])};b.admin=function(o,n){n.admin(o.target,o.args)};b.disconnect=function(o,n){n.disconnect()};b.gett=function(o,n){var p=o.cmd.indexOf("title")>-1?"title":"topic";n.ui.control.set_text("/"+p+" "+n.channel(o.target).info[p].content)};var l=function(q,n){if(q.p!="info"){return}var o=q.pkt.sub;var r=o.shift().arg;r.username=q.sns.substr(1);
r.connections=[];var p={};while(o.length>0){p=o.shift().arg;p.channels=[];while(o.length>0){if(o[0].cmd!="ns"){break}p.channels.unshift(n.deform_ns(o.shift().param))}r.connections.push(p)}n.ui.chatbook.current.log_whois(r)};var k=function(o,n){if(o.ns.indexOf("login:")!=0){return}var p=o.sns.substr(1);n.ui.pager.notice({ref:"whois-"+p,heading:"Whois Failed",content:"Whois failed for "+p+".\nNo such user online."},false,5000)};var f=function(r,o){var u=o.channel(r.ns);var n=r.info.split("\n");var t="";var s=[];var p="";for(var q in n){if(!n.hasOwnProperty(q)){continue}t=n[q].split(" ");if(r.p=="privclass"){s.push([t.shift(),t.shift().split("=")[1],t.join(" ")])}else{if(r.p=="users"){p=t.shift().split(":",1)[0];s.push([p,u.get_privclass_order(p),t.join(" ")])}}}o.ui.chatbook.current.log_pc(r.p=="privclass",s)};m();wsc.defaults.Extension.Ignore(d);wsc.defaults.Extension.Away(d);wsc.defaults.Extension.Autojoin(d)};wsc.defaults.Extension.Autojoin=function(a){var d=a.autojoin;a.ui.nav.add_button({icon:"chat",label:"",title:"Join your autojoin channels",href:"#autojoin-do",handler:function(){for(var g in a.autojoin.channel){if(!a.autojoin.channel.hasOwnProperty(g)){continue
