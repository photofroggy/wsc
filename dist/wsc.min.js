...
if(d.ns==null){d.ns=a.mns}d.sns=a.deform_ns(d.ns);this.handle(a,d);a.trigger("pkt",d);a.trigger("pkt."+d.name,d)};wsc.Flow.prototype.handle=function(a,d){try{this[d.name](d,a)}catch(b){}};wsc.Flow.prototype.ping=function(b,a){a.pong()};wsc.Flow.prototype.chatserver=function(b,a){a.login()};wsc.Flow.prototype.login=function(d,a){if(d.pkt.arg.e=="ok"){var f=d.pkt.sub[0];a.settings.username=d.pkt.param;a.settings.symbol=f.arg.symbol;a.settings.userinfo=f.arg;var b=function(){a.join(a.settings.autojoin);if(a.autojoin.on){for(var g in a.autojoin.channel){if(!a.autojoin.channel.hasOwnProperty(g)){continue}a.join(a.autojoin.channel[g])}}}}else{a.close(d)}if(a.fresh){a.fresh=false}};wsc.Flow.prototype.join=function(d,a){if(d.pkt.arg["e"]=="ok"){var b=a.deform_ns(d.pkt.param);a.create_ns(b,a.hidden.contains(d.pkt.param))}else{a.trigger("log",{name:"log",ns:"server:current",sns:"~current",msg:"Failed to join "+a.deform_ns(d.pkt.param),info:d.pkt.arg["e"]})}};wsc.Flow.prototype.part=function(d,a){var b=a.deform_ns(d.ns);
var f=a.channel(b);if(d.e=="ok"){info="";if(d.r.length>0){info="["+d.r+"]"}else{a.remove_ns(d.ns)}msg="You have left "+b;f.server_message(msg,info);if(a.channels()==0){switch(d.r){case"bad data":case"bad msg":case"msg too big":break;default:if(d.r.indexOf("killed:")<0){return}break}this.message(a,{data:"disconnect\ne="+e.r+"\n"})}}else{a.trigger("log",{name:"log",ns:"server:current",sns:"~current",msg:"Couldn't leave "+b,info:d.e})}};wsc.Flow.prototype.kicked=function(b,a){if(b.r.toLowerCase().indexOf("autokicked")>-1){return}a.join(b.ns)};wsc.Flow.prototype.property=function(b,a){chan=a.channel(b.pkt.param);if(!chan){return}chan.property(b)};wsc.Flow.prototype.recv_joinpart=function(b,a){c=a.channel(b.ns);if(b.name=="recv_join"){c.recv_join(b)}else{c.recv_part(b)}};wsc.Flow.prototype.recv_join=wsc.Flow.prototype.recv_joinpart;wsc.Flow.prototype.recv_part=wsc.Flow.prototype.recv_joinpart;wsc.Flow.prototype.recv_msg=function(b,a){a.channel(b.ns).recv_msg(b)};wsc.Flow.prototype.recv_action=wsc.Flow.prototype.recv_msg;
wsc.Flow.prototype.recv_npmsg=wsc.Flow.prototype.recv_msg;wsc.Flow.prototype.recv_privchg=function(b,a){a.channel(b.ns).recv_privchg(b)};wsc.Flow.prototype.recv_kicked=function(b,a){a.channel(b.ns).recv_kicked(b)};wsc.defaults.Extension=function(b){var d={};var j=function(){b.bind("cmd.connect",f.connection);b.bind("cmd.set",f.setter);b.bind("cmd.join",f.join);b.bind("cmd.chat",f.pjoin);b.bind("cmd.part",f.part);b.bind("cmd.say",f.say);b.bind("cmd.npmsg",f.npmsg);b.bind("cmd.me",f.action);b.bind("cmd.promote",f.chgpriv);b.bind("cmd.demote",f.chgpriv);b.bind("cmd.ban",f.ban);b.bind("cmd.unban",f.ban);b.bind("cmd.kick",f.killk);b.bind("cmd.whois",f.whois);b.bind("cmd.title",f.title);b.bind("cmd.topic",f.title);b.bind("cmd.admin",f.admin);b.bind("cmd.disconnect",f.connection);b.bind("cmd.kill",f.killk);b.bind("cmd.raw",f.raw);b.bind("cmd.close",f.close);b.bind("pkt.property",g);b.bind("pkt.recv_admin_show",a);b.bind("pkt.recv_admin_showverbose",a);b.bind("pkt.get",h)};d.save=function(l,k){b.settings.ui.theme=l.theme;
b.settings.ui.clock=l.clock;b.settings.ui.tabclose=l.tabclose};var f={};f.setter=function(m){var l=m.args.split(" ");var k=l.shift().toLowerCase();var l=l.join(" ");if(l.length==0){b.cchannel.serverMessage("Could not set "+k,"No data supplied");return}if(!(k in b.settings)){b.cchannel.serverMessage('Unknown setting "'+k+'"');return}b.settings[k]=l;b.cchannel.serverMessage("Changed "+k+" setting","value: "+l)};f.connection=function(k){b[k.cmd]()};f.join=function(k){var l=k.args.split(" ");var l=l.toString()==""?[]:l;if(k.ns!=k.target){l.unshift(k.target)}if(l.toString()==""){return}for(index in l){b.join(l[index])}};f.pjoin=function(k){var l=k.args.split(" ");var l=l.toString()==""?[]:l;if(k.ns!=k.target){l.unshift(k.target)}if(l.toString()==""){return}for(index in l){b.join("@"+l[index])}};f.part=function(k){var l=k.args.split(" ");if(k.ns!=k.target){l.unshift(k.target)}if(l.toString()==""){b.part(k.ns);return}for(index in l){b.part(l[index])}};f.title=function(k){b.set(k.target,k.cmd,k.args)
};f.chgpriv=function(l){var k=l.args.split(" ");b[l.cmd.toLowerCase()](l.target,k[0],k[1])};f.ban=function(o,k){var m=o.args.split(" ");var l=m.shift();var n=o.cmd;if(n=="ban"&&m.length>0){k.kick(o.target,l,m.join(" "))}k[n](o.target,l)};f.action=function(k){b.action(k.target,k.args)};f.raw=function(k){b.send(k.args.replace(/\\n/gm,"\n"))};f.killk=function(n,k){var o=n.args.split(" ");var l=o.shift();var m=o.length>0?o.join(" "):null;if(n.cmd=="kick"){k.kick(n.target,l,m)}else{k.kill(l,m)}};f.say=function(k){if(b.channel(k.target).monitor){return}b.say(k.target,k.args)};f.npmsg=function(k){b.npmsg(k.target,k.args)};f.close=function(k){b.part(k.target);b.remove_ns(k.target)};f.whois=function(l,k){k.whois(l.args.split(" ")[0])};f.admin=function(l,k){k.admin(l.target,l.args)};f.disconnect=function(l,k){k.disconnect()};var g=function(p,l){if(p.p!="info"){return}var n=p.pkt.sub;var q=n.shift().arg;q.username=p.sns.substr(1);q.connections=[];var o=null;var r={};while(n.length>0){r=n.shift();switch(r.cmd){case"conn":if(o!=null){q.connections.push(o)
}o=r.arg;o.channels=[];break;case"agent":o.agent=[["agent",r.arg.agent]];if(r.arg.hasOwnProperty("url")){o.agent.push(["url",r.arg.url])}if(r.arg.hasOwnProperty("browser")){o.agent.push(["browser",r.arg.browser])}for(var m in r.arg){if(m=="agent"||m=="url"||m=="browser"){continue}if(r.arg.hasOwnProperty(m)){o.agent.push([m,r.arg[m]])}}break;case"ns":o.channels.unshift(l.deform_ns(r.param));break}}q.connections.push(o);l.trigger("pkt.whois",{name:"pkt.whois",pkt:p.pkt,info:q})};var h=function(l,k){if(l.ns.indexOf("login:")!=0){return}var m=l.sns.substr(1);k.ui.pager.notice({ref:"whois-"+m,heading:"Whois Failed",content:"Whois failed for "+m+".\nNo such user online."},false,5000)};var a=function(o,l){var r=l.channel(o.ns);var k=o.info.split("\n");var q="";var p=[];var m="";for(var n in k){if(!k.hasOwnProperty(n)){continue}q=k[n].split(" ");if(o.p=="privclass"){p.push([q.shift(),q.shift().split("=")[1],q.join(" ")])}else{if(o.p=="users"){m=q.shift().split(":",1)[0];p.push([m,r.get_privclass_order(m),q.join(" ")])
}}}l.ui.chatbook.current.log_pc(o.p=="privclass",p)};j();wsc.defaults.Extension.Ignore(b,d);wsc.defaults.Extension.Away(b,d);wsc.defaults.Extension.Autojoin(b,d);return d};wsc.defaults.Extension.Autojoin=function(b,f){f.autojoin={on:false,channel:[]};var j=true;var h=b.storage.folder("autojoin");var g=function(){f.autojoin.load();f.autojoin.save();b.bind("cmd.autojoin",d);b.bind("pkt.login",a)};f.autojoin.load=function(){f.autojoin.on=(h.get("on","true")=="true");f.autojoin.channel=JSON.parse(h.get("channel","[]"))};f.autojoin.save=function(){h.set("on",f.autojoin.on.toString());h.set("channel",JSON.stringify(f.autojoin.channel))};f.autojoin.join=function(){for(var k in f.autojoin.channel){if(!f.autojoin.channel.hasOwnProperty(k)){continue}b.join(f.autojoin.channel[k])}};f.autojoin.add=function(k){if(f.autojoin.channel.indexOf(k)!=-1){return false}f.autojoin.channel.push(k);f.autojoin.save();return true};f.autojoin.remove=function(l){var k=f.autojoin.channel.indexOf(l);if(k==-1){return false
}f.autojoin.channel.splice(k,1);f.autojoin.save();return true};var a=function(l,k){if(l.e!="ok"){return}if(j){k.join(k.settings.autojoin);if(f.autojoin.on){f.autojoin.join()}}else{var m=false;for(key in k.channelo){if(k.channelo[key].namespace[0]!="~"){k.join(key);m=true}}if(!m){f.autojoin.join()}}j=false};var d=function(l){var p=l.args.split(" ");if(!p){return}var s=p.shift().toLowerCase();var q=null;var t="";var m="";var r=false;var k=b.channel(l.ns);switch(s){case"add":q=f.autojoin.add;t="Added {item} to your autojoin.";m="Already have {item} on your autojoin.";break;case"rem":case"remove":q=f.autojoin.remove;t="Removed {item} from your autojoin.";m=u+" is not on your autojoin list.";break;case"on":b.trigger("log",{name:"log",ns:"server:current",sns:"~current",msg:"Autojoin on"});if(!f.autojoin.on){r=true;f.autojoin.on=true}break;case"off":b.trigger("log",{name:"log",ns:"server:current",sns:"~current",msg:"Autojoin off"});if(f.autojoin.on){r=true;f.autojoin.on=false}break;default:f.autojoin.join();
break}var u="";var o=false;if(q!=null){for(var n in p){if(!p.hasOwnProperty(n)){continue}u=b.deform_ns(p[n]).toLowerCase();o=q(u);b.trigger("log",{name:"log",ns:"server:current",sns:"~current",msg:replaceAll((o?t:m),"{item}",u)})}}if(r){f.autojoin.save()}};g()};wsc.defaults.Extension.Away=function(b,f){var j=b.storage.folder("away");f.away={on:false,reason:"",last:{},since:0,interval:60000,format:{setaway:"/me is away: {reason}",setback:"/me is back",away:"{user}: I've been away for {timesince}. Reason: {reason}"}};b.away=f.away;var h=function(){f.away.load();f.away.save();b.bind("cmd.setaway",d);b.bind("cmd.setback",g);b.bind("pkt.recv_msg",a);b.bind("pkt.recv_action",a)};var d=function(m,k){f.away.on=true;f.away.last={};f.away.since=new Date();f.away.reason=m.args;var n=k.say;var l=replaceAll(f.away.format.setaway,"{reason}",f.away.reason||"[silent away]");if(l.indexOf("/me ")==0){l=l.substr(4);n=k.action}k.each_channel(function(o){n.call(k,o,l)});k.trigger("ext.away.away",{name:"ext.away.away",reason:f.away.reason||"[silent away]"})
};var g=function(m,k){if(!f.away.on){return}f.away.on=false;var n=k.say;var l=f.away.format.setback;if(l.indexOf("/me ")==0){l=l.substr(4);n=k.action}k.each_channel(function(o){n.call(k,o,l)});k.trigger("ext.away.back",{name:"ext.away.back"})};var a=function(o){if(!f.away.on){return}if(f.away.reason.length==0){return}if(b.exclude.contains(o.ns)){return}var p=o.user.toLowerCase();var l=b.settings.username.toLowerCase();if(p==l){return}if(o.message.text().toLowerCase().indexOf(l)==-1){return}var m=new Date();var n=o.sns.toLowerCase();if(n in f.away.last){if((m-f.away.last[n])<=f.away.interval){return}}var k=timeLengthString((m-f.away.since)/1000);var q=replaceAll(f.away.format.away,"{user}",o.user);q=replaceAll(q,"{timesince}",k);b.say(o.ns,replaceAll(q,"{reason}",f.away.reason));f.away.last[n]=m};f.away.load=function(){f.away.format.setaway=j.get("setaway","/me is away: {reason}");f.away.format.setback=j.get("setback","/me is back");f.away.format.away=j.get("away","{user}: I've been away for {timesince}. Reason: {reason}");
f.away.interval=parseInt(j.get("interval",60000))};f.away.save=function(){j.set("interval",f.away.interval);j.set("setaway",f.away.format.setaway);j.set("setback",f.away.format.setback);j.set("away",f.away.format.away)};h()};wsc.defaults.Extension.Ignore=function(a,d){var b={};var j=a.storage.folder("ignore");d.ignore={fignore:"/me is ignoring {user} now",funignore:"/me is not ignoring {user} anymore",ignored:[]};var h=function(){d.ignore.load();d.ignore.save();a.bind("cmd.ignore",f);a.bind("cmd.unignore",g)};d.ignore.load=function(){d.ignore.fignore=j.get("ignore",d.ignore.fignore);d.ignore.funignore=j.get("unignore",d.ignore.funignore);d.ignore.ignored=JSON.parse(j.get("ignored",JSON.stringify(d.ignore.ignored)));a.trigger("ignore.loaded",{name:"ignore.loaded",ignored:d.ignore.ignored})};d.ignore.save=function(){j.set("ignore",d.ignore.fignore);j.set("unignore",d.ignore.funignore);j.set("ignored",JSON.stringify(d.ignore.ignored))};d.ignore.add=function(k,n){var k=k.split(" ");var l="";
n=n||false;for(var m=0;m<k.length;m++){if(!k.hasOwnProperty(m)){continue}l=k[m].toLowerCase();if(d.ignore.ignored.indexOf(l)!=-1){continue}d.ignore.ignored.push(l);a.trigger("ignore.add",{name:"ignore.add",user:k[m],suppress:n})}d.ignore.save()};d.ignore.remove=function(k,o){var k=k.split(" ");var l="";var n=-1;o=o||false;for(var m=0;m<k.length;m++){if(!k.hasOwnProperty(m)){continue}l=k[m].toLowerCase();n=d.ignore.ignored.indexOf(l);if(n==-1){continue}d.ignore.ignored.splice(n,1);a.trigger("ignore.remove",{name:"ignore.remove",user:k[m],suppress:o})}d.ignore.save()};var f=function(k){d.ignore.add(k.args)};var g=function(k){d.ignore.remove(k.args)};h()};wsc.Client=function(b,d,f){this.mozilla=f;this.storage=new wsc.Storage;this.storage.aj=this.storage.folder("autojoin");this.storage.aj.channel=this.storage.aj.folder("channel");this.fresh=true;this.attempts=0;this.connected=false;this.protocol=null;this.flow=null;this.ui=null;this.events=new EventEmitter();this.conn=null;this.channelo={};
this.cchannel=null;this.cmds=[];this.settings={domain:"website.com",server:"ws://website.com/wsendpoint",symbol:"",username:"",userinfo:{},pk:"",monitor:["~Monitor",true],welcome:"Welcome to the wsc web client!",autojoin:"chat:channel",protocol:wsc.Protocol,mparser:wsc.MessageParser,flow:wsc.Flow,extend:[wsc.defaults.Extension],client:"chatclient",clientver:"0.3",developer:false};var a=this;this.exclude=new StringSet();this.hidden=new StringSet();this.settings=Object.extend(this.settings,d);this.config_load();this.config_save();this.mw=new wsc.Middleware();this.settings.agent="Client ("+navigator.platform+"; HTML5; JavaScript) wsc/"+wsc.VERSION+"-r"+wsc.REVISION;this.mns=this.format_ns(this.settings.monitor[0]);this.lun=this.settings.username.toLowerCase();this.protocol=new this.settings.protocol(new this.settings.mparser());this.flow=new this.settings.flow(this.protocol);this.build();this.ext={};this.ext.defaults=wsc.defaults.Extension(this)};wsc.Client.prototype.config_load=function(){this.settings.developer=(this.storage.get("developer",this.settings.developer.toString())=="true")
