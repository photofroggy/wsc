...
if(s===null){continue}j.ignored.push(s)}}};var m=function(){k.set("ignore",j.ignore);k.set("unignore",j.unignore);for(var r=0;r<j.count;r++){a.remove(r)}if(j.ignored.length==0){k.set("count",0)}else{var s=-1;for(var r in j.ignored){if(!j.ignored.hasOwnProperty(r)){continue}s++;a.set(s.toString(),j.ignored[r])}s++;j.count=s;k.set("count",s)}};q()};wsc.defaults.Extension=function(p){var r={};r.away={on:false,reason:"",last:{},since:0};var F=function(){p.bind("cmd.connect",g);p.bind("cmd.set",a);p.bind("cmd.join",K);p.bind("cmd.part",N);p.bind("cmd.say",t);p.bind("cmd.npmsg",O);p.bind("cmd.me",A);p.bind("cmd.promote",D);p.bind("cmd.demote",D);p.bind("cmd.ban",m);p.bind("cmd.unban",m);p.bind("cmd.kick",j);p.bind("cmd.whois",G);p.bind("cmd.title",k);p.bind("cmd.topic",k);p.bind("cmd.admin",E);p.bind("cmd.disconnect",g);p.bind("cmd.kill",j);p.bind("cmd.raw",L);p.bind("cmd.clear",I);p.bind("cmd.clearall",H);p.bind("pkt.property",l);p.bind("pkt.recv_admin_show",M);p.bind("pkt.recv_admin_showverbose",M);
p.bind("pkt.get",B);p.bind("cmd.gettitle",z);p.bind("cmd.gettopic",z);p.bind("cmd.setaway",u);p.bind("cmd.setback",s);p.bind("pkt.recv_msg.highlighted",v);p.bind("cmd.theme",b);p.ui.on("settings.open",q);p.ui.on("settings.open.ran",J)};var q=function(R,Q){page=R.settings.page("Main");var S={};S.theme=replaceAll(p.ui.settings.theme,"wsct_","");S.clock=p.ui.clock();S.tc=p.ui.nav.closer();S.username=p.settings.username;S.pk=p.settings.pk;var P=[];for(i in p.ui.settings.themes){name=replaceAll(p.ui.settings.themes[i],"wsct_","");P.push({value:name,title:name,selected:S.theme==name})}page.item("Text",{ref:"intro",title:"Main",text:"Use this window to view and change your settings.\n\nCheck                    the different pages to see what settings can be changed.",});page.item("Form",{ref:"login",title:"Login",text:"Here you can change the username and token used to                    log into the chat server.",fields:[["Textfield",{ref:"username",label:"Username","default":S.username}],["Textfield",{ref:"token",label:"Token","default":S.pk}]],event:{save:function(T){p.settings.username=T.data.username;
p.settings.pk=T.data.token}}});page.item("Form",{ref:"ui",title:"UI",hint:"<b>Timestamp</b><br/>Choose between a 24 hour clock and                    a 12 hour clock.\n\n<b>Theme</b><br/>Change the look of the                    client.\n\n<b>Close Buttons</b><br/>Turn tab close buttons on/off.",fields:[["Dropdown",{ref:"theme",label:"Theme",items:P}],["Dropdown",{ref:"clock",label:"Timestamp Format",items:[{value:"24",title:"24 hour",selected:S.clock},{value:"12",title:"12 hour",selected:!S.clock}]}],["Check",{ref:"tabclose",label:"Close Buttons",items:[{value:"yes",title:"On",selected:S.tc}]}],],event:{change:function(T){p.ui.clock(T.data.clock=="24");p.ui.theme(T.data.theme);p.ui.nav.closer(T.data.tabclose.indexOf("yes")>-1)},save:function(T){S.clock=T.data.clock=="24";S.theme=T.data.theme;S.tc=T.data.tabclose.indexOf("yes")>-1;p.storage.set("theme","wsct_"+S.theme)},close:function(T){p.ui.clock(S.clock);p.ui.theme(S.theme);p.ui.nav.closer(S.tc)}}});page.item("Text",{ref:"debug",wclass:"faint",title:"Debug Information",text:"Chat Agent: <code>"+p.settings.agent+"</code>\n\nUser                    Agent: <code>"+navigator.userAgent+"</code>"})
};var J=function(R,Q){var P=R.settings.page("About",true);P.item("Text",{ref:"about-wsc",title:"Wsc",text:'Currently using <a href="http://github.com/photofroggy/wsc/">wsc</a>                    version '+wsc.VERSION+" "+wsc.STATE+'.\n\nWsc                    works using HTML5, javascript, and CSS3. WebSocket is used for the connection                    where possible. The source code for this client is pretty huge.\n\nWsc was created                    by ~<a href="http://photofroggy.deviantart.com/">photofroggy</a>'})};var b=function(Q,P){P.ui.theme(Q.args.split(" ").shift())};var a=function(R){var Q=R.args.split(" ");var P=Q.shift().toLowerCase();var Q=Q.join(" ");if(Q.length==0){p.cchannel.serverMessage("Could not set "+P,"No data supplied");return}if(!(P in p.settings)){p.cchannel.serverMessage('Unknown setting "'+P+'"');return}p.settings[P]=Q;p.cchannel.serverMessage("Changed "+P+" setting","value: "+Q);p.control.setLabel()};var g=function(P){p[P.cmd]()};var K=function(P){var Q=P.args.split(" ");
var Q=Q.toString()==""?[]:Q;if(P.ns!=P.target){Q.unshift(P.target)}if(Q.toString()==""){return}for(index in Q){p.join(Q[index])}};var N=function(P){var Q=P.args.split(" ");if(P.ns!=P.target){Q.unshift(P.target)}if(Q.toString()==""){p.part(P.ns);return}for(index in Q){p.part(Q[index])}};var k=function(P){p.set(P.target,P.cmd,P.args)};var D=function(Q){var P=Q.args.split(" ");p[Q.cmd.toLowerCase()](Q.target,P[0],P[1])};var m=function(T,P){var R=T.args.split(" ");var Q=R.shift();var S=T.cmd;if(S=="ban"&&R.length>0){P.kick(T.target,Q,R.join(" "))}P[S](T.target,Q)};var A=function(P){p.action(P.target,P.args)};var L=function(P){p.send(P.args.replace(/\\n/gm,"\n"))};var j=function(S,P){var T=S.args.split(" ");var Q=T.shift();var R=T.length>0?T.join(" "):null;if(S.cmd=="kick"){P.kick(S.target,Q,R)}else{P.kill(Q,R)}};var t=function(P){p.say(P.target,P.args)};var O=function(P){p.npmsg(P.target,P.args)};var I=function(Q,P){P.cchannel.clear()};var H=function(Q,P){for(c in P.channelo){P.channelo[c].clear()
}};var G=function(Q,P){P.whois(Q.args.split(" ")[0])};var E=function(Q,P){P.admin(Q.target,Q.args)};var C=function(Q,P){P.disconnect()};var z=function(Q,P){var R=Q.cmd.indexOf("title")>-1?"title":"topic";P.control.ui.set_text("/"+R+" "+P.channel(Q.target).info[R].content)};var u=function(R,P){r.away.on=true;r.away.last={};r.away.since=new Date();r.away.reason=R.args;var Q="is away"+(r.away.reason.length>0?": "+r.away.reason:"");P.each_channel(function(S){P.action(S,Q)})};var s=function(Q,P){r.away.on=false;P.each_channel(function(R){P.action(R,"is back")})};var v=function(S,P){if(!r.away.on){return}if(r.away.reason.length==0){return}if(S.user==P.settings.username){return}if(P.exclude.indexOf(S.sns.toLowerCase())!=-1){return}var Q=new Date();var R=S.sns.toLowerCase();if(R in r.away.last){if((Q-r.away.last[R])<=60000){return}}P.say(S.ns,S.user+": I am currently away; reason: "+r.away.reason);r.away.last[R]=Q};var l=function(S,P){if(S.p!="info"){return}var Q=S.pkt.sub;var T=Q.shift().arg;T.username=S.sns.substr(1);
T.connections=[];var R={};while(Q.length>0){R=Q.shift().arg;R.channels=[];while(Q.length>0){if(Q[0].cmd!="ns"){break}R.channels.unshift(P.deform_ns(Q.shift().param))}T.connections.push(R)}P.cchannel.log_whois(T)};var B=function(Q,P){if(Q.ns.indexOf("login:")!=0){return}P.cchannel.server_message("Whois failed for "+(Q.sns.substr(1)),"not online")};var M=function(T,Q){var W=Q.channel(T.ns);var P=T.info.split("\n");var V="";var U=[];var R="";for(var S in P){if(!P.hasOwnProperty(S)){continue}V=P[S].split(" ");if(T.p=="privclass"){U.push([V.shift(),V.shift().split("=")[1],V.join(" ")])}else{if(T.p=="users"){R=V.shift().split(":",1)[0];U.push([R,W.get_privclass_order(R),V.join(" ")])}}}W.log_pc(T.p=="privclass",U)};F()};wsc.Client=function(a,g,j){this.mozilla=j;this.storage=new wsc.Storage;this.fresh=true;this.attempts=0;this.connected=false;this.protocol=null;this.flow=null;this.ui=null;this.events=new EventEmitter();this.conn=null;this.channelo={};this.cchannel=null;this.exclude=[];this.cmds=[];
