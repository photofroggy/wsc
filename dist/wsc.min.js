...
j.options.items=a.autojoin.channel},add:function(s){var t=s.args.swap;a.autojoin.channel[t["this"].index]=t.that.item;a.autojoin.channel[t.that.index]=t["this"].item;j.options.items=a.autojoin.channel},remove:function(s){var t=s.args.swap;a.autojoin.channel[t["this"].index]=t.that.item;a.autojoin.channel[t.that.index]=t["this"].item;j.options.items=a.autojoin.channel},save:function(s){r.chan=a.autojoin.channel;a.config_save()},close:function(s){a.autojoin.channel=r.chan;a.config_save()}}})};var b=function(r){var k=r.args.split(" ");if(!k){return}var p=k.shift().toLowerCase();var j=function(t){};var m=false;var s=a.channel(r.ns);switch(p){case"add":j=function(t){if(a.autojoin.channel.indexOf(t)==-1){m=true;a.autojoin.channel.push(t);s.server_message("Added "+t+" to your autojoin.")}else{s.server_message("Already have "+t+" on your autojoin.")}};break;case"rem":case"remove":j=function(u){var t=a.autojoin.channel.indexOf(u);if(t!=-1){m=true;a.autojoin.channel.splice(t,1);s.server_message("Removed "+u+" from your autojoin.")
...
f.interval=parseInt(g.get("interval",60000))};var j=function(){g.set("interval",f.interval);g.set("setaway",f.format.setaway);g.set("setback",f.format.setback);g.set("away",f.format.away)};p()};wsc.defaults.Extension.Ignore=function(f){var g={};var j=f.storage.folder("ignore");var a=j.folder("ignored");var p=function(){m();l();f.bind("cmd.ignore",b);f.bind("cmd.unignore",k);f.ui.on("settings.open",g.page)};g.page=function(s,u){var t=s.settings.page("Ignores",true);var v={};var r="<ul>";v.im=g.ignore;v.uim=g.unignore;if(f.ui.umuted.length==0){r+="<li><i>No one ignored yet</i></li></ul>"}else{for(var q in f.ui.umuted){if(!f.ui.umuted.hasOwnProperty(q)){continue}r+="<li>"+f.ui.umuted[q]+"</li>"}r+="</ul>"}t.item("Text",{ref:"intro",title:"Ignores",text:'Use <code>ignore</code> to ignore people.\n\n                    You can "ignore" other users of the chat server using the\n                    <code>/ignore</code> command. Ignoring a user hides their                    messages from you in the channel log.',});
t.item("Form",{ref:"msgs",title:"Messages",text:"Here you can set the messages displayed when you ignore or                    unignore a user.\n\nThe text <code>{user}</code> is replaced                    with the name of the user your are ignoring or unignoring.",fields:[["Textfield",{ref:"ignore",label:"Ignore","default":v.im}],["Textfield",{ref:"unignore",label:"Unignore","default":v.uim}]],event:{save:function(z){g.ignore=z.data.ignore;g.unignore=z.data.unignore;l()}}});t.item("Items",{ref:"ignoreds",title:"Users",text:"This is the list of users that you have silenced.\n\nUse the                    commands <code>/ignore</code> and <code>/unignore</code>                    to edit the list.",items:f.ui.umuted})};var b=function(t){var v=t.args.split(" ");var q="";var u="";var s=false;for(var r in v){if(!v.hasOwnProperty(r)){continue}q=v[r];tmod=f.mute_user(q);if(!tmod){continue}s=tmod;u=replaceAll(g.ignore,"{user}",q);if(u.indexOf("/me ")==0){u=u.substr(4);f.action(t.target,u)}else{f.say(t.target,u)
}}if(s){l()}};var k=function(u){var A=u.args.split(" ");var r="";var z="";var q=-1;var t=false;var v=false;for(var s in A){if(!A.hasOwnProperty(s)){continue}r=A[s];v=f.unmute_user(r);if(!v){continue}t=v;z=replaceAll(g.unignore,"{user}",r);if(z.indexOf("/me ")==0){z=z.substr(4);f.action(u.target,z)}else{f.say(u.target,z)}}if(t){l()}};var m=function(){g.ignore=j.get("ignore","/me is ignoring {user} now");g.unignore=j.get("unignore","/me is not ignoring {user} anymore");g.count=parseInt(j.get("count",0));f.ui.umuted=[];if(g.count>0){var r=null;for(var q=0;q<g.count;q++){f.mute_user.push(a.get(q,null))}}};var l=function(){j.set("ignore",g.ignore);j.set("unignore",g.unignore);for(var q=0;q<g.count;q++){a.remove(q)}if(f.ui.umuted.length==0){j.set("count",0)}else{var r=-1;for(var q in f.ui.umuted){if(!f.ui.umuted.hasOwnProperty(q)){continue}r++;a.set(r.toString(),f.ui.umuted[q])}r++;g.count=r;j.set("count",r)}};p()};wsc.Client=function(a,f,g){this.mozilla=g;this.storage=new wsc.Storage;this.storage.ui=this.storage.folder("ui");
this.storage.aj=this.storage.folder("autojoin");this.storage.aj.channel=this.storage.aj.folder("channel");this.fresh=true;this.attempts=0;this.connected=false;this.protocol=null;this.flow=null;this.ui=null;this.events=new EventEmitter();this.conn=null;this.channelo={};this.cchannel=null;this.exclude=[];this.cmds=[];this.settings={domain:"website.com",server:"ws://website.com/wsendpoint",symbol:"",username:"",userinfo:{},pk:"",monitor:["~Monitor",true],welcome:"Welcome to the wsc web client!",autojoin:"chat:channel",control:wsc.Control,protocol:wsc.Protocol,mparser:wsc.MessageParser,flow:wsc.Flow,ui_object:Chatterbox.UI,extend:[wsc.defaults.Extension],client:"chatclient",clientver:"0.3",ui:{theme:wsc.defaults.theme,themes:wsc.defaults.themes,tabclose:true,clock:true}};this.autojoin={on:true,count:0,channel:[]};this.settings=Object.extend(this.settings,f);this.config_load();this.config_save();this.ui=new this.settings.ui_object(a,{themes:this.settings.ui.themes,theme:this.settings.ui.theme,monitor:this.settings.monitor,username:this.settings.username,domain:this.settings.domain,clock:this.settings.ui.clock,tabclose:this.settings.ui.tabclose},g);
...
}else{if(f.shiftKey){this.submit(f)}else{a=true}}break;case 38:if(!this.ui.multiline()){this.scroll_history(true);break}a=true;break;case 40:if(!this.ui.multiline()){this.scroll_history(false);break}a=true;break;case 9:if(f.shiftKey){this.client.ui.channel_right()}else{this.tab_item(f);g=false}break;case 219:if(f.ctrlKey){this.client.ui.channel_left()}else{a=true}break;case 221:if(f.ctrlKey){this.client.ui.channel_right()}else{a=true}break;default:a=true;break}if(g){this.end_tab(f)}return a};wsc.Control.prototype.handle=function(a,b){if(b==""){return}if(b[0]!="/"){if(!this.client.cchannel){return}}b=(a.shiftKey?"/npmsg ":(b[0]=="/"?"":"/say "))+b;b=b.slice(1);bits=b.split(" ");cmdn=bits.shift().toLowerCase();ens=this.client.cchannel.namespace;etarget=ens;if(bits[0]){hash=bits[0][0];if((hash=="#"||hash=="@")&&bits[0].length>1){etarget=this.client.format_ns(bits.shift())}}arg=bits.join(" ");this.client.trigger("cmd."+cmdn,{name:"cmd",cmd:cmdn,args:arg,target:etarget,ns:ens})};var Chatterbox={};
Chatterbox.VERSION="0.6.50";Chatterbox.STATE="beta";Chatterbox.UI=function(a,b,g,f){this.events=f||new EventEmitter();this.mozilla=g;this.umuted=[];this.settings={themes:["wsct_default","wsct_dAmn"],theme:"wsct_default",monitor:["~Monitor",true],username:"",domain:"website.com",clock:true,tabclose:true};a.extend(this.settings,b);a.append('<div class="wsc '+this.settings.theme+'"></div>');this.view=a.find(".wsc");this.mns=this.format_ns(this.settings.monitor[0]);this.lun=this.settings.username.toLowerCase();this.monitoro=null;this.swidth=(function(){if($.browser.msie){var l=$('<textarea cols="10" rows="2"></textarea>').css({position:"absolute",top:-1000,left:-1000}).appendTo("body"),k=$('<textarea cols="10" rows="2" style="overflow: hidden;"></textarea>').css({position:"absolute",top:-1000,left:-1000}).appendTo("body");scrollbarWidth=l.width()-k.width();l.add(k).remove()}else{var j=$("<div />").css({width:100,height:100,overflow:"auto",position:"absolute",top:-1000,left:-1000}).prependTo("body").append("<div />").find("div").css({width:"100%",height:200});
...
a.list.find("li.selected").removeClass("selected");a.selected=b.html();b.addClass("selected")})};Chatterbox.Settings.Item.Items.prototype._fevent=function(b,g){var k=this._get_ep("inspect");var f=k?this.view.find(k[1]):null;var a=this._get_cb(b);if(typeof a=="function"){a({input:f,item:this,args:g})}else{for(var j in a){var l=f.hasOwnProperty("slice")?f.slice(j,1):f;a[j]({input:l,item:this,args:g})}}};Chatterbox.Settings.Item.Items.prototype.save=function(){this._fevent("save",{items:this.options.items||[]})};Chatterbox.template={};Chatterbox.render=function(g,k){var f=Chatterbox.template;var b=g.split(".");var j={};var a=null;for(ind in b){part=b[ind];if(!f.hasOwnProperty(part)){return""}f=f[part]}if(f.hasOwnProperty("frame")){a=f;j=f.render||{};f=f.frame;if(a.hasOwnProperty("pre")){if(typeof a.pre=="function"){f=a.pre(f,k)}else{for(i in a.pre){f=a.pre[i](f,k)}}}}for(key in k){f=replaceAll(f,"{"+key+"}",(j[key]||Chatterbox.template.render_stub)(k[key]||""))}if(a!=null){if(a.hasOwnProperty("post")){if(typeof a.post=="function"){f=a.post(f,k)
}else{for(i in a.post){f=a.post[i](f,k)}}}}return f};Chatterbox.template.render_stub=function(a){return a};Chatterbox.template.clean=function(a){return function(b,f){for(i in a){b=replaceAll(b,"{"+a[i]+"}","")}return b}};Chatterbox.template.ui='<nav class="tabs"><ul id="chattabs" class="tabs"></ul>        <ul id="tabnav">            <li><a href="#left" class="button iconic arrow_left"></a></li>            <li><a href="#right" class="button iconic arrow_right"></a></li>            <li><a href="#settings" title="Change client settings" class="button iconic cog" id="settings-button"></a></li>        </ul>        </nav>        <div class="chatbook"></div>';Chatterbox.template.control='<div class="chatcontrol">            <p><a href="#multiline" title="Toggle multiline input" class="button iconic list"></a></p>            <form class="msg">                <input type="text" class="msg" />                <textarea class="msg"></textarea>                <input type="submit" value="Send" class="sendmsg" />            </form>        </div>';
Chatterbox.template.tab='<li id="{selector}-tab"><a href="#{selector}" class="tab">{ns}<a href="#{selector}" class="close iconic x"></a></a></li>';Chatterbox.template.channel='<div class="chatwindow" id="{selector}-window">                    <header>                        <div class="title"></div>                    </header>                    <div class="chatlog" id="{selector}-log">                        <header>                            <div class="topic"></div>                        </header>                        <ul class="logwrap"></ul>                    </div>                    <div class="chatusers" id="{selector}-users">                </div>            </div>';Chatterbox.template.header='<div class="{head}">{content}</div>';Chatterbox.template.logmsg='<span class="message">{message}</span>';Chatterbox.template.logitem='<li class="logmsg u-{user}"><span class="ts" id="{ms}">{ts}</span> {message}</li>';Chatterbox.template.servermsg='<span class="servermsg">** {message} * <em>{info}</em></span>';
...
}render="";labels=[];fields=[];for(i in a){item=a[i];if("title" in item){labels.push(Chatterbox.render("settings.item.form.label",{ref:item.value,label:item.title}))}ritem='<div class="'+item.value+' field check"><input class="'+item.value+'" type="checkbox" name="'+item.name+'" value="'+item.value+'"';if(item.selected){ritem+=' checked="yes"'}fields.push(ritem+" /></div>")}if(labels.length>0){render+='<section class="labels">'+labels.join("")+"</section>"}return render+'<section class="fields">'+fields.join("")+"</section>"};Chatterbox.template.settings.krender.manageditems=function(b){if(b.length==0){return"<i>No items in this list</i>"}var g="<ul>";var k=[];var a=[];var j;for(var f in b){if(!b.hasOwnProperty(f)){continue}j=b[f];g+='<li title="'+j.toLowerCase()+'">'+j+"</li>"}return g+"</ul>"};Chatterbox.template.settings.item={};Chatterbox.template.settings.item.get=function(a){var f=a.split(".");var b=Chatterbox.template.settings.item;for(i in f){tc=f[i];if(b.hasOwnProperty(tc)){b=b[tc];
