...
for(var q in n){if(!n.hasOwnProperty(q)){continue}t=n[q].split(" ");if(r.p=="privclass"){s.push([t.shift(),t.shift().split("=")[1],t.join(" ")])}else{if(r.p=="users"){p=t.shift().split(":",1)[0];s.push([p,u.get_privclass_order(p),t.join(" ")])}}}o.ui.chatbook.current.log_pc(r.p=="privclass",s)};m();wsc.defaults.Extension.Autojoin(d)};wsc.defaults.Extension.Autojoin=function(a){var d=a.autojoin;var f=function(){a.bind("cmd.autojoin",b)};var b=function(n){var h=n.args.split(" ");if(!h){return}var l=h.shift().toLowerCase();var g=function(p){};var k=false;var o=a.channel(n.ns);switch(l){case"add":g=function(p){if(a.autojoin.channel.indexOf(p)==-1){k=true;a.autojoin.channel.push(p)}else{}};break;case"rem":case"remove":g=function(q){var p=a.autojoin.channel.indexOf(q);if(p!=-1){k=true;a.autojoin.channel.splice(p,1)}else{}};break;case"on":o.server_message("Autojoin on.");if(!a.autojoin.on){k=true;a.autojoin.on=true}break;case"off":o.server_message("Autojoin off.");if(a.autojoin.on){k=true;a.autojoin.on=false
}break;default:console.log("> start autojoin");console.log(a.autojoin);for(var j in a.autojoin.channel){if(!a.autojoin.channel.hasOwnProperty(j)){continue}a.join(a.autojoin.channel[j])}return;break}var m="";for(var j in h){if(!h.hasOwnProperty(j)){continue}m=a.deform_ns(h[j]).toLowerCase();g(m)}if(k){a.config_save()}};f()};wsc.defaults.Extension.Away=function(b){var f=b.storage.folder("away");var d={on:false,reason:"",last:{},since:0,interval:60000,format:{setaway:"/me is away: {reason}",setback:"/me is back",away:"{user}: I've been away for {timesince}. Reason: {reason}"}};b.away=d;var l=function(){j();g();b.bind("cmd.setaway",a);b.bind("cmd.setback",k);b.ui.on("tabbed",h);b.ui.on("settings.open",d.page)};d.page=function(n,p){var m=function(s){s=replaceAll(s,"<","&lt;");s=replaceAll(s,">","&gt;");s=replaceAll(s,'"',"&quot;");return s};var q=function(s){s=replaceAll(s,"&lt;","<");s=replaceAll(s,"&gt;",">");s=replaceAll(s,"&quot;",'"');return s};var o=n.settings.page("Away");var r={};r.away=d.format.away;
r.sa=d.format.setaway;r.sb=d.format.setback;r.intr=d.interval;o.item("Text",{ref:"intro",title:"Away Messages",text:"Use away messages when you are away from the chats.\n\n                    You can set yourself away using the                     <code>/setaway [reason]</code> command. When you get back,                    use <code>/setback</code>. While you are away, the client                    will automatically respond when people try to talk to you,                    telling them you're away.",});o.item("Form",{ref:"msgs",title:"Messages",text:"Here you can set the messages displayed when you set                    yourself away or back. You can also change the away message                    format.",hint:"<b>{user}</b><br/>This is replaced with the username of the person trying to talk to you\n\n<b>{reason}</b>                    <br/>This is replaced by your reason for being away.\n\n<b>{timesince}</b>                    <br/>Use this to show how long you have been away for.",fields:[["Textfield",{ref:"away",label:"Away","default":m(r.away)}],["Textfield",{ref:"setaway",label:"Setaway","default":m(r.sa)}],["Textfield",{ref:"setback",label:"Setback","default":m(r.sb)}]],event:{save:function(s){d.format.away=q(s.data.away);
