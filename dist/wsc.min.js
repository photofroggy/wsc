...
}}}l.ui.chatbook.current.log_pc(o.p=="privclass",p)};j();wsc.defaults.Extension.Ignore(b);wsc.defaults.Extension.Away(b,d);wsc.defaults.Extension.Autojoin(b,d);return d};wsc.defaults.Extension.Autojoin=function(b,f){f.autojoin={on:false,channel:[]};var j=true;var h=b.storage.folder("autojoin");var g=function(){f.autojoin.load();f.autojoin.save();b.bind("cmd.autojoin",d);b.bind("pkt.login",a)};f.autojoin.load=function(){f.autojoin.on=(h.get("on","true")=="true");f.autojoin.channel=JSON.parse(h.get("channel","[]"))};f.autojoin.save=function(){h.set("on",f.autojoin.on.toString());h.set("channel",JSON.stringify(f.autojoin.channel))};f.autojoin.join=function(){for(var k in f.autojoin.channel){if(!f.autojoin.channel.hasOwnProperty(k)){continue}b.join(f.autojoin.channel[k])}};f.autojoin.add=function(k){if(f.autojoin.channel.indexOf(k)!=-1){return false}f.autojoin.channel.push(k);f.autojoin.save();return true};f.autojoin.remove=function(l){var k=f.autojoin.channel.indexOf(l);if(k==-1){return false
