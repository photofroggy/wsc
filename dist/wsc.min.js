...
wsc.Flow.prototype.recv_privchg=function(b,a){a.channel(b.ns).recv_privchg(b)};wsc.Flow.prototype.recv_kicked=function(b,a){a.channel(b.ns).recv_kicked(b)};wsc.defaults.Extension=function(p){var r={};r.away={on:false,reason:"",last:{},since:0};var G=function(){p.bind("cmd.connect",g);p.bind("cmd.set",a);p.bind("cmd.join",M);p.bind("cmd.part",P);p.bind("cmd.say",t);p.bind("cmd.npmsg",Q);p.bind("cmd.me",A);p.bind("cmd.promote",D);p.bind("cmd.demote",D);p.bind("cmd.ban",m);p.bind("cmd.unban",m);p.bind("cmd.kick",j);p.bind("cmd.whois",I);p.bind("cmd.title",k);p.bind("cmd.topic",k);p.bind("cmd.admin",E);p.bind("cmd.disconnect",g);p.bind("cmd.kill",j);p.bind("cmd.raw",N);p.bind("cmd.clear",K);p.bind("cmd.clearall",J);p.bind("cmd.close",F);p.bind("pkt.property",l);p.bind("pkt.recv_admin_show",O);p.bind("pkt.recv_admin_showverbose",O);p.bind("pkt.get",B);p.bind("cmd.gettitle",z);p.bind("cmd.gettopic",z);p.bind("cmd.setaway",u);p.bind("cmd.setback",s);p.bind("pkt.recv_msg.highlighted",v);p.bind("cmd.theme",b);
p.ui.on("settings.open",q);p.ui.on("settings.open.ran",L);p.ui.on("settings.save",H)};var H=function(S,R){p.settings.ui.theme=S.theme;p.settings.ui.clock=S.clock;p.settings.ui.tabclose=S.tabclose;p.config_save()};var q=function(T,S){var R=T.settings.page("Main",true);var U={};U.username=p.settings.username;U.pk=p.settings.pk;R.item("Form",{ref:"login",title:"Login",text:"Here you can change the username and token used to                    log into the chat server.",fields:[["Textfield",{ref:"username",label:"Username","default":U.username}],["Textfield",{ref:"token",label:"Token","default":U.pk}]],event:{save:function(V){p.settings.username=V.data.username;p.settings.pk=V.data.token}}},true);R.item("Text",{ref:"intro",title:"Main",text:"Use this window to view and change your settings.\n\nCheck                    the different pages to see what settings can be changed.",},true);R.item("Text",{ref:"debug",wclass:"faint",title:"Debug Information",text:"Chat Agent: <code>"+p.settings.agent+"</code>\n\nUser                    Agent: <code>"+navigator.userAgent+"</code>"})
