...
}P[S](T.target,Q)};var A=function(P){p.action(P.target,P.args)};var L=function(P){p.send(P.args.replace(/\\n/gm,"\n"))};var j=function(S,P){var T=S.args.split(" ");var Q=T.shift();var R=T.length>0?T.join(" "):null;if(S.cmd=="kick"){P.kick(S.target,Q,R)}else{P.kill(Q,R)}};var t=function(P){p.say(P.target,P.args)};var O=function(P){p.npmsg(P.target,P.args)};var I=function(R,P){if(R.args.length>0){var S=R.args.split(" ");for(var Q in S){if(!S.hasOwnProperty(Q)){continue}P.channel(R.target).clear(S[Q])}}else{P.channel(R.target).clear()}};var H=function(Q,P){var S=null;if(Q.args.length>0){var R=Q.args.split(" ");S=function(U,V){for(var T in R){if(!R.hasOwnProperty(T)){continue}V.clear(R[T])}}}else{S=function(T,U){U.clear()}}P.each_channel(S,true)};var G=function(Q,P){P.whois(Q.args.split(" ")[0])};var E=function(Q,P){P.admin(Q.target,Q.args)};var C=function(Q,P){P.disconnect()};var z=function(Q,P){var R=Q.cmd.indexOf("title")>-1?"title":"topic";P.control.ui.set_text("/"+R+" "+P.channel(Q.target).info[R].content)
};var u=function(R,P){r.away.on=true;r.away.last={};r.away.since=new Date();r.away.reason=R.args;var Q="is away"+(r.away.reason.length>0?": "+r.away.reason:"");P.each_channel(function(S){P.action(S,Q)})};var s=function(Q,P){r.away.on=false;P.each_channel(function(R){P.action(R,"is back")})};var v=function(S,P){if(!r.away.on){return}if(r.away.reason.length==0){return}if(S.user==P.settings.username){return}if(P.exclude.indexOf(S.sns.toLowerCase())!=-1){return}var Q=new Date();var R=S.sns.toLowerCase();if(R in r.away.last){if((Q-r.away.last[R])<=60000){return}}P.say(S.ns,S.user+": I am currently away; reason: "+r.away.reason);r.away.last[R]=Q};var l=function(S,P){if(S.p!="info"){return}var Q=S.pkt.sub;var T=Q.shift().arg;T.username=S.sns.substr(1);T.connections=[];var R={};while(Q.length>0){R=Q.shift().arg;R.channels=[];while(Q.length>0){if(Q[0].cmd!="ns"){break}R.channels.unshift(P.deform_ns(Q.shift().param))}T.connections.push(R)}P.cchannel.log_whois(T)};var B=function(Q,P){if(Q.ns.indexOf("login:")!=0){return
}P.cchannel.server_message("Whois failed for "+(Q.sns.substr(1)),"not online")};var M=function(T,Q){var W=Q.channel(T.ns);var P=T.info.split("\n");var V="";var U=[];var R="";for(var S in P){if(!P.hasOwnProperty(S)){continue}V=P[S].split(" ");if(T.p=="privclass"){U.push([V.shift(),V.shift().split("=")[1],V.join(" ")])}else{if(T.p=="users"){R=V.shift().split(":",1)[0];U.push([R,W.get_privclass_order(R),V.join(" ")])}}}W.log_pc(T.p=="privclass",U)};F();wsc.defaults.Extension.Ignore(p)};wsc.defaults.Extension.Ignore=function(g){var j={};var k=g.storage.folder("ignore");var a=k.folder("ignored");var q=function(){p();m();g.bind("cmd.ignore",b);g.bind("cmd.unignore",l);g.ui.on("settings.open",j.page)};j.page=function(t,v){var u=t.settings.page("Ignores");var A={};var s="<ul>";A.im=j.ignore;A.uim=j.unignore;if(g.ui.umuted.length==0){s+="<li><i>No one ignored yet</i></li></ul>"}else{for(var r in g.ui.umuted){if(!g.ui.umuted.hasOwnProperty(r)){continue}s+="<li>"+g.ui.umuted[r]+"</li>"}s+="</ul>"}u.item("Text",{ref:"intro",title:"Ignores",text:'Use <code>ignore</code> to ignore people.\n\n                    You can "ignore" other users of the chat server using the\n                    <code>/ignore</code> command. Ignoring a user hides their                    messages from you in the channel log.',});
u.item("Form",{ref:"msgs",title:"Messages",text:"Here you can set the messages displayed when you ignore or                    unignore a user.\n\nThe text <code>{user}</code> is replaced                    with the name of the user your are ignoring or unignoring.",fields:[["Textfield",{ref:"ignore",label:"Ignore","default":A.im}],["Textfield",{ref:"unignore",label:"Unignore","default":A.uim}]],event:{save:function(B){j.ignore=B.data.ignore;j.unignore=B.data.unignore}}});var z=u.item("Form",{ref:"ignored",wclass:"boxed-ff-indv",title:"Users",text:"This is the list of users that you have silenced.\n\nUse the                    commands <code>/ignore</code> and <code>/unignore</code>                    to edit the list.",fields:[["Text",{ref:"users",text:s}]]})};var b=function(u){var z=u.args.split(" ");var r="";var v="";var t=false;for(var s in z){if(!z.hasOwnProperty(s)){continue}r=z[s];tmod=g.mute_user(r);if(!tmod){continue}t=tmod;v=replaceAll(j.ignore,"{user}",r);if(v.indexOf("/me ")==0){v=v.substr(4);
g.action(u.target,v)}else{g.say(u.target,v)}}if(t){m()}};var l=function(v){var B=v.args.split(" ");var s="";var A="";var r=-1;var u=false;var z=false;for(var t in B){if(!B.hasOwnProperty(t)){continue}s=B[t];z=g.unmute_user(s);if(!z){continue}u=z;A=replaceAll(j.unignore,"{user}",s);if(A.indexOf("/me ")==0){A=A.substr(4);g.action(v.target,A)}else{g.say(v.target,A)}}if(u){m()}};var p=function(){j.ignore=k.get("ignore","/me is ignoring {user} now");j.unignore=k.get("unignore","/me is not ignoring {user} anymore");j.count=parseInt(k.get("count",0));g.ui.umuted=[];if(j.count>0){var s=null;for(var r=0;r<j.count;r++){g.mute_user.push(a.get(r,null))}}};var m=function(){k.set("ignore",j.ignore);k.set("unignore",j.unignore);for(var r=0;r<j.count;r++){a.remove(r)}if(g.ui.umuted.length==0){k.set("count",0)}else{var s=-1;for(var r in g.ui.umuted){if(!g.ui.umuted.hasOwnProperty(r)){continue}s++;a.set(s.toString(),g.ui.umuted[r])}s++;j.count=s;k.set("count",s)}};q()};wsc.Client=function(a,g,j){this.mozilla=j;
