...
wsc.Flow.prototype.recv_msg=function(b,a){a.channel(b.ns).recv_msg(b)};wsc.Flow.prototype.recv_action=wsc.Flow.prototype.recv_msg;wsc.Flow.prototype.recv_npmsg=wsc.Flow.prototype.recv_msg;wsc.Flow.prototype.recv_privchg=function(b,a){a.channel(b.ns).recv_privchg(b)};wsc.Flow.prototype.recv_kicked=function(b,a){a.channel(b.ns).recv_kicked(b)};wsc.defaults.Extension=function(d){var a={};a.away={on:false,reason:"",last:{},since:0,store:d.storage.folder("away"),format:{setaway:"/me is away: {reason}",setback:"/me is back",away:"{from}: I am away, reason: {reason}"}};var m=function(){d.bind("cmd.connect",b.connection);d.bind("cmd.set",b.setter);d.bind("cmd.join",b.join);d.bind("cmd.chat",b.pjoin);d.bind("cmd.part",b.part);d.bind("cmd.say",b.say);d.bind("cmd.npmsg",b.npmsg);d.bind("cmd.me",b.action);d.bind("cmd.promote",b.chgpriv);d.bind("cmd.demote",b.chgpriv);d.bind("cmd.ban",b.ban);d.bind("cmd.unban",b.ban);d.bind("cmd.kick",b.killk);d.bind("cmd.whois",b.whois);d.bind("cmd.title",b.title);
d.bind("cmd.topic",b.title);d.bind("cmd.admin",b.admin);d.bind("cmd.disconnect",b.connection);d.bind("cmd.kill",b.killk);d.bind("cmd.raw",b.raw);d.bind("cmd.clear",b.clear);d.bind("cmd.clearall",b.clearall);d.bind("cmd.close",b.close);d.bind("pkt.property",l);d.bind("pkt.recv_admin_show",f);d.bind("pkt.recv_admin_showverbose",f);d.bind("pkt.get",k);d.bind("cmd.gettitle",b.gett);d.bind("cmd.gettopic",b.gett);d.bind("cmd.theme",b.theme);d.ui.on("settings.open",h);d.ui.on("settings.open.ran",g);d.ui.on("settings.save.ui",j);d.ui.on("settings.save",function(){d.config_save()})};var j=function(o,n){d.settings.ui.theme=o.theme;d.settings.ui.clock=o.clock;d.settings.ui.tabclose=o.tabclose};var h=function(p,o){var n=p.settings.page("Main");var q={};q.username=d.settings.username;q.pk=d.settings.pk;q.devel=d.settings.developer;n.item("Form",{ref:"login",title:"Login",text:"Here you can change the username and token used to                    log into the chat server.",fields:[["Textfield",{ref:"username",label:"Username","default":q.username}],["Textfield",{ref:"token",label:"Token","default":q.pk}]],event:{save:function(r){d.settings.username=r.data.username;
d.settings.pk=r.data.token}}},true);n.item("Form",{ref:"developer",title:"Developer Mode",text:"Turn developer mode on or off.\n\nDeveloper mode will expose any hidden                channel tabs, amongst other things. Keep this turned off unless you're working                on implementing something.",fields:[["Checkbox",{ref:"enabled",items:[{value:"on",title:"On",selected:q.devel}]}]],event:{change:function(r){d.settings.developer=(r.data.enabled.indexOf("on")!=-1);d.ui.developer(d.settings.developer)},save:function(r){q.devel=d.settings.developer},close:function(r){d.settings.developer=q.devel;d.ui.developer(d.settings.developer)}}});n.item("Text",{ref:"intro",title:"Main",text:"Use this window to view and change your settings.\n\nCheck                    the different pages to see what settings can be changed.",},true);n.item("Text",{ref:"debug",wclass:"faint",title:"Debug Information",text:"Chat Agent: <code>"+d.settings.agent+"</code>\n\nUser                    Agent: <code>"+navigator.userAgent+"</code>"})
};var g=function(p,o){var n=p.settings.page("About",true);n.item("Text",{ref:"about-wsc",title:"Wsc",text:'Currently using <a href="http://github.com/photofroggy/wsc/">wsc</a>                    version '+wsc.VERSION+" "+wsc.STATE+'.\n\nWsc                    works using HTML5, javascript, and CSS3. WebSocket is used for the connection                    where possible. The source code for this client is pretty huge.\n\nWsc was created                    by ~<a href="http://photofroggy.deviantart.com/">photofroggy</a>'})};var b={};b.theme=function(o,n){n.ui.theme(o.args.split(" ").shift())};b.setter=function(p){var o=p.args.split(" ");var n=o.shift().toLowerCase();var o=o.join(" ");if(o.length==0){d.cchannel.serverMessage("Could not set "+n,"No data supplied");return}if(!(n in d.settings)){d.cchannel.serverMessage('Unknown setting "'+n+'"');return}d.settings[n]=o;d.cchannel.serverMessage("Changed "+n+" setting","value: "+o)};b.connection=function(n){d[n.cmd]()};b.join=function(n){var o=n.args.split(" ");
var o=o.toString()==""?[]:o;if(n.ns!=n.target){o.unshift(n.target)}if(o.toString()==""){return}for(index in o){d.join(o[index])}};b.pjoin=function(n){var o=n.args.split(" ");var o=o.toString()==""?[]:o;if(n.ns!=n.target){o.unshift(n.target)}if(o.toString()==""){return}for(index in o){d.join("@"+o[index])}};b.part=function(n){var o=n.args.split(" ");if(n.ns!=n.target){o.unshift(n.target)}if(o.toString()==""){d.part(n.ns);return}for(index in o){d.part(o[index])}};b.title=function(n){d.set(n.target,n.cmd,n.args)};b.chgpriv=function(o){var n=o.args.split(" ");d[o.cmd.toLowerCase()](o.target,n[0],n[1])};b.ban=function(r,n){var p=r.args.split(" ");var o=p.shift();var q=r.cmd;if(q=="ban"&&p.length>0){n.kick(r.target,o,p.join(" "))}n[q](r.target,o)};b.action=function(n){d.action(n.target,n.args)};b.raw=function(n){d.send(n.args.replace(/\\n/gm,"\n"))};b.killk=function(q,n){var s=q.args.split(" ");var o=s.shift();var p=s.length>0?s.join(" "):null;if(q.cmd=="kick"){n.kick(q.target,o,p)}else{n.kill(o,p)
}};b.say=function(n){if(d.channel(n.target).monitor){return}d.say(n.target,n.args)};b.npmsg=function(n){d.npmsg(n.target,n.args)};b.clear=function(p,n){if(p.args.length>0){var q=p.args.split(" ");for(var o in q){if(!q.hasOwnProperty(o)){continue}n.channel(p.target).clear(q[o])}}else{n.channel(p.target).clear()}};b.clearall=function(o,n){var q=null;if(o.args.length>0){var p=o.args.split(" ");q=function(s,t){for(var r in p){if(!p.hasOwnProperty(r)){continue}t.clear(p[r])}}}else{q=function(r,s){s.clear()}}n.each_channel(q,true)};b.close=function(n){d.part(n.target);d.remove_ns(n.target)};b.whois=function(o,n){n.whois(o.args.split(" ")[0])};b.admin=function(o,n){n.admin(o.target,o.args)};b.disconnect=function(o,n){n.disconnect()};b.gett=function(o,n){var p=o.cmd.indexOf("title")>-1?"title":"topic";n.ui.control.set_text("/"+p+" "+n.channel(o.target).info[p].content)};var l=function(q,n){if(q.p!="info"){return}var o=q.pkt.sub;var r=o.shift().arg;r.username=q.sns.substr(1);r.connections=[];var p={};
while(o.length>0){p=o.shift().arg;p.channels=[];while(o.length>0){if(o[0].cmd!="ns"){break}p.channels.unshift(n.deform_ns(o.shift().param))}r.connections.push(p)}n.cchannel.log_whois(r)};var k=function(o,n){if(o.ns.indexOf("login:")!=0){return}var p=o.sns.substr(1);n.ui.pager.notice({ref:"whois-"+p,heading:"Whois Failed",content:"Whois failed for "+p+".\nNo such user online."},false,5000)};var f=function(r,o){var u=o.channel(r.ns);var n=r.info.split("\n");var t="";var s=[];var p="";for(var q in n){if(!n.hasOwnProperty(q)){continue}t=n[q].split(" ");if(r.p=="privclass"){s.push([t.shift(),t.shift().split("=")[1],t.join(" ")])}else{if(r.p=="users"){p=t.shift().split(":",1)[0];s.push([p,u.get_privclass_order(p),t.join(" ")])}}}u.log_pc(r.p=="privclass",s)};m();wsc.defaults.Extension.Ignore(d);wsc.defaults.Extension.Away(d);wsc.defaults.Extension.Autojoin(d)};wsc.defaults.Extension.Autojoin=function(a){var d=a.autojoin;a.ui.nav.add_button({icon:"chat",label:"",title:"Join your autojoin channels",href:"#autojoin-do",handler:function(){for(var g in a.autojoin.channel){if(!a.autojoin.channel.hasOwnProperty(g)){continue
}a.join(a.autojoin.channel[g])}}});var f=function(){a.bind("cmd.autojoin",b);a.ui.on("settings.open",d.page)};d.page=function(k,m){var l=k.settings.page("Autojoin");var j="<ul>";var n={};n.ajon=a.autojoin.on;n.chan=a.autojoin.channel;if(a.autojoin.channel.length==0){j+="<li><i>No autojoin channels set</i></li></ul>"}else{for(var h in a.autojoin.channel){if(!a.autojoin.channel.hasOwnProperty(h)){continue}j+="<li>"+a.autojoin.channel[h]+"</li>"}j+="</ul>"}l.item("Checkbox",{ref:"eaj",title:"Autojoin",text:"Turn on autojoin to automatically join selected channels                    when you connect to the chat server.",items:[{value:"yes",title:"On",selected:n.ajon}],event:{change:function(o){if(o.target.value=="yes"){a.autojoin.on=o.target.checked}},save:function(o){n.ajon=a.autojoin.on;a.config_save()},close:function(o){a.autojoin.on=n.ajon}}});var g=l.item("Items",{ref:"channelss",title:"Channels",text:"Add any channels you want to join automatically when you                    connect to the chat server.",items:a.autojoin.channel,prompt:{title:"Add Channel",label:"Channel:",},event:{up:function(o){var p=o.args.swap;
a.autojoin.channel[p["this"].index]=p.that.item;a.autojoin.channel[p.that.index]=p["this"].item;g.options.items=a.autojoin.channel},down:function(o){var p=o.args.swap;a.autojoin.channel[p["this"].index]=p.that.item;a.autojoin.channel[p.that.index]=p["this"].item;g.options.items=a.autojoin.channel},add:function(q){var p=a.deform_ns(q.args.item).toLowerCase();var o=a.autojoin.channel.indexOf(p);if(o!=-1){return}a.autojoin.channel.push(p);g.options.items=a.autojoin.channel},remove:function(o){a.autojoin.channel.splice(o.args.index,1);g.options.items=a.autojoin.channel},save:function(o){n.chan=a.autojoin.channel;a.config_save()},close:function(o){a.config_load()}}})};var b=function(n){var h=n.args.split(" ");if(!h){return}var l=h.shift().toLowerCase();var g=function(p){};var k=false;var o=a.channel(n.ns);switch(l){case"add":g=function(p){if(a.autojoin.channel.indexOf(p)==-1){k=true;a.autojoin.channel.push(p);o.server_message("Added "+p+" to your autojoin.")}else{o.server_message("Already have "+p+" on your autojoin.")
}};break;case"rem":case"remove":g=function(q){var p=a.autojoin.channel.indexOf(q);if(p!=-1){k=true;a.autojoin.channel.splice(p,1);o.server_message("Removed "+q+" from your autojoin.")}else{o.server_message(q+" is not on your autojoin list.")}};break;case"on":o.server_message("Autojoin on.");if(!a.autojoin.on){k=true;a.autojoin.on=true}break;case"off":o.server_message("Autojoin off.");if(a.autojoin.on){k=true;a.autojoin.on=false}break}var m="";for(var j in h){if(!h.hasOwnProperty(j)){continue}m=a.deform_ns(h[j]).toLowerCase();g(m)}if(k){a.config_save()}};f()};wsc.defaults.Extension.Away=function(b){var f=b.storage.folder("away");var d={on:false,reason:"",last:{},since:0,interval:60000,format:{setaway:"/me is away: {reason}",setback:"/me is back",away:"{user}: I've been away for {timesince}. Reason: {reason}"}};b.away=d;var l=function(){j();g();b.bind("cmd.setaway",a);b.bind("cmd.setback",k);b.bind("pkt.recv_msg.highlighted",h);b.ui.on("settings.open",d.page)};d.page=function(n,p){var m=function(s){s=replaceAll(s,"<","&lt;");
s=replaceAll(s,">","&gt;");s=replaceAll(s,'"',"&quot;");return s};var q=function(s){s=replaceAll(s,"&lt;","<");s=replaceAll(s,"&gt;",">");s=replaceAll(s,"&quot;",'"');return s};var o=n.settings.page("Away");var r={};r.away=d.format.away;r.sa=d.format.setaway;r.sb=d.format.setback;r.intr=d.interval;o.item("Text",{ref:"intro",title:"Away Messages",text:"Use away messages when you are away from the chats.\n\n                    You can set yourself away using the                     <code>/setaway [reason]</code> command. When you get back,                    use <code>/setback</code>. While you are away, the client                    will automatically respond when people try to talk to you,                    telling them you're away.",});o.item("Form",{ref:"msgs",title:"Messages",text:"Here you can set the messages displayed when you set                    yourself away or back. You can also change the away message                    format.",hint:"<b>{user}</b><br/>This is replaced with the username of the person trying to talk to you\n\n<b>{reason}</b>                    <br/>This is replaced by your reason for being away.\n\n<b>{timesince}</b>                    <br/>Use this to show how long you have been away for.",fields:[["Textfield",{ref:"away",label:"Away","default":m(r.away)}],["Textfield",{ref:"setaway",label:"Setaway","default":m(r.sa)}],["Textfield",{ref:"setback",label:"Setback","default":m(r.sb)}]],event:{save:function(s){d.format.away=q(s.data.away);
d.format.setaway=q(s.data.setaway);d.format.setback=q(s.data.setback);g()}}});o.item("Form",{ref:"interval",title:"Message Interval",text:"Here you can set the amount of time to wait before                     displaying another away message. The interval is in seconds.",fields:[["Textfield",{ref:"interval",label:"Interval","default":(r.intr/1000).toString()}]],event:{save:function(s){d.interval=parseInt(s.data.interval)*1000;g()}}})};var a=function(o,m){d.on=true;d.last={};d.since=new Date();d.reason=o.args;var p=m.say;var n=replaceAll(d.format.setaway,"{reason}",d.reason||"[silent away]");if(n.indexOf("/me ")==0){n=n.substr(4);p=m.action}m.each_channel(function(q){p.call(m,q,n)});m.ui.control.add_state({ref:"away",label:"Away, reason: <i>"+(d.reason||"[silent away]")+"</i>"})};var k=function(o,m){d.on=false;var p=m.say;var n=d.format.setback;if(n.indexOf("/me ")==0){n=n.substr(4);p=m.action}m.each_channel(function(q){p.call(m,q,n)});m.ui.control.rem_state("away")};var h=function(q,m){if(!d.on){return
}if(d.reason.length==0){return}if(q.user==m.settings.username){return}if(m.exclude.contains(q.ns)){return}var o=new Date();var p=q.sns.toLowerCase();if(p in d.last){if((o-d.last[p])<=d.interval){return}}var n=timeLengthString((o-d.since)/1000);var r=replaceAll(d.format.away,"{user}",q.user);r=replaceAll(r,"{timesince}",n);m.say(q.ns,replaceAll(r,"{reason}",d.reason));d.last[p]=o};var j=function(){d.format.setaway=f.get("setaway","/me is away: {reason}");d.format.setback=f.get("setback","/me is back");d.format.away=f.get("away","{user}: I've been away for {timesince}. Reason: {reason}");d.interval=parseInt(f.get("interval",60000))};var g=function(){f.set("interval",d.interval);f.set("setaway",d.format.setaway);f.set("setback",d.format.setback);f.set("away",d.format.away)};l()};wsc.defaults.Extension.Ignore=function(d){var f={};var g=d.storage.folder("ignore");var a=g.folder("ignored");var l=function(){k();j();d.bind("cmd.ignore",b);d.bind("cmd.unignore",h);d.ui.on("settings.open",f.page)
};f.page=function(n,p){var o=n.settings.page("Ignores");var q={};q.im=f.ignore;q.uim=f.unignore;q.usr=d.ui.umuted;o.item("Text",{ref:"intro",title:"Ignores",text:'Use <code>ignore</code> to ignore people.\n\n                    You can "ignore" other users of the chat server using the\n                    <code>/ignore</code> command. Ignoring a user hides their                    messages from you in the channel log.',});o.item("Form",{ref:"msgs",title:"Messages",text:"Here you can set the messages displayed when you ignore or                    unignore a user.\n\nThe text <code>{user}</code> is replaced                    with the name of the user your are ignoring or unignoring.",fields:[["Textfield",{ref:"ignore",label:"Ignore","default":q.im}],["Textfield",{ref:"unignore",label:"Unignore","default":q.uim}]],event:{save:function(r){f.ignore=r.data.ignore;f.unignore=r.data.unignore;j()}}});var m=o.item("Items",{ref:"ignoreds",title:"Users",text:"This is the list of users that you have silenced.\n\nUse the                    commands <code>/ignore</code> and <code>/unignore</code>                    to edit the list.",items:q.usr,prompt:{title:"Add User",label:"User:",},event:{up:function(r){var s=r.args.swap;
d.ui.umuted[s["this"].index]=s.that.item;d.ui.umuted[s.that.index]=s["this"].item;m.options.items=d.ui.umuted},down:function(r){var s=r.args.swap;d.ui.umuted[s["this"].index]=s.that.item;d.ui.umuted[s.that.index]=s["this"].item;m.options.items=d.ui.umuted},add:function(r){d.mute_user(r.args.item);m.options.items=d.ui.umuted},remove:function(r){d.unmute_user(r.args.item);m.options.items=d.ui.umuted},save:function(r){q.usr=d.ui.umuted;j()},close:function(r){k()}}})};var b=function(p){var r=p.args.split(" ");var m="";var q="";var o=false;for(var n in r){if(!r.hasOwnProperty(n)){continue}m=r[n];tmod=d.mute_user(m);if(!tmod){continue}o=tmod;q=replaceAll(f.ignore,"{user}",m);if(q.indexOf("/me ")==0){q=q.substr(4);d.action(p.target,q)}else{d.say(p.target,q)}}if(o){j()}};var h=function(q){var t=q.args.split(" ");var n="";var s="";var m=-1;var p=false;var r=false;for(var o in t){if(!t.hasOwnProperty(o)){continue}n=t[o];r=d.unmute_user(n);if(!r){continue}p=r;s=replaceAll(f.unignore,"{user}",n);if(s.indexOf("/me ")==0){s=s.substr(4);
d.action(q.target,s)}else{d.say(q.target,s)}}if(p){j()}};var k=function(){f.ignore=g.get("ignore","/me is ignoring {user} now");f.unignore=g.get("unignore","/me is not ignoring {user} anymore");f.count=parseInt(g.get("count",0));var n=null;for(var m in d.ui.umuted){if(!d.ui.umuted.hasOwnProperty(m)){continue}d.unmute_user(d.ui.umuted[m])}d.ui.umuted=[];if(f.count>0){n=null;for(var m=0;m<f.count;m++){d.mute_user(a.get(m,null))}}};var j=function(){g.set("ignore",f.ignore);g.set("unignore",f.unignore);for(var m=0;m<f.count;m++){a.remove(m)}if(d.ui.umuted.length==0){g.set("count",0)}else{var n=-1;for(var m in d.ui.umuted){if(!d.ui.umuted.hasOwnProperty(m)){continue}n++;a.set(n.toString(),d.ui.umuted[m])}n++;f.count=n;g.set("count",n)}};l()};wsc.Client=function(b,f,g){this.mozilla=g;this.storage=new wsc.Storage;this.storage.ui=this.storage.folder("ui");this.storage.aj=this.storage.folder("autojoin");this.storage.aj.channel=this.storage.aj.folder("channel");this.fresh=true;this.attempts=0;this.connected=false;
...
return this.el.brow.s.find("li#"+a.ref)}b.html(a.label);return b};Chatterbox.Control.prototype.rem_state=function(a){return this.el.brow.s.find("li#"+a).remove()};Chatterbox.Control.prototype.chomp=function(){var f=this.el.i.c.val();var b=f.lastIndexOf(" ");if(b==-1){this.el.i.c.val("");return f}var a=f.slice(b+1);this.el.i.c.val(f.slice(0,b));if(a.length==0){return this.chomp()}return a};Chatterbox.Control.prototype.unchomp=function(a){var b=this.el.i.c.val();if(!b){this.el.i.c.val(a)}else{this.el.i.c.val(b+" "+a)}};Chatterbox.Control.prototype.get_text=function(a){if(a==undefined){return this.el.i.c.val()}this.el.i.c.val(a||"");return this.el.i.c.val()};Chatterbox.Control.prototype.set_text=function(a){this.el.i.c.val(a||"")};Chatterbox.Control.prototype.cache_input=function(b,d){var a=this.get_history(b.namespace);if(a.index>-1){return}a.tmp=this.get_text();this.set_text(this.get_history(d.namespace).tmp)};Chatterbox.Control.prototype.get_history=function(a){if(!a){if(!this.manager.chatbook.current){a="~monitor"
}}a=a||this.manager.chatbook.current.namespace;if(!this.history[a]){this.history[a]={index:-1,list:[],tmp:""}}return this.history[a]};Chatterbox.Control.prototype.append_history=function(b){if(!b){return}var a=this.get_history();a.list.unshift(b);a.index=-1;if(a.list.length>100){a.list.pop()}};Chatterbox.Control.prototype.scroll_history=function(a){var d=this.get_history();var b=this.get_text();if(d.index==-1){if(b){d.tmp=b}else{d.list[d.index]=b}}if(a){if(d.list.length>0&&d.index<(d.list.length-1)){d.index++}}else{if(d.index>-1){d.index--}}this.set_text(d.list[d.index]||d.tmp)};Chatterbox.Control.prototype.tab_item=function(b){if(!this.tab.hit){this.start_tab(b)}this.chomp();this.tab.index++;if(this.tab.index>=this.tab.matched.length){this.tab.index=-1}if(this.tab.index==-1){this.unchomp(this.tab.prefix[this.tab.type]+this.tab.cache);return}var a=this.get_text()==""?(this.tab.type==0?": ":" "):"";this.unchomp(this.tab.prefix[this.tab.type]+this.tab.matched[this.tab.index]+a)};Chatterbox.Control.prototype.start_tab=function(d){this.tab.hit=true;
this.tab.index=-1;this.tab.matched=[];this.tab.type=0;var h=this.chomp();this.unchomp(h);if(h[0]=="/"||h[0]=="#"||h[0]=="@"){this.tab.type=h[0]=="/"?1:2;if(h[0]=="/"){h=h.slice(1)}}else{this.tab.type=0}this.tab.cache=h;h=h.toLowerCase();this.tab.matched=[];if(this.tab.type==0){var j=this.manager.client.channel(this.manager.chatbook.current.namespace);for(var a in j.info.members){if(a.toLowerCase().indexOf(h)==0){this.tab.matched.push(a)}}}else{if(this.tab.type==1){var g="";for(var b in this.manager.client.cmds){g=this.manager.client.cmds[b];if(g.indexOf(h)==0){this.tab.matched.push(g)}}}else{if(this.tab.type==2){var f=this;this.manager.client.each_channel(function(k,l){if(l.namespace.toLowerCase().indexOf(h)==0){f.tab.matched.push(l.namespace)}})}}}};Chatterbox.Control.prototype.end_tab=function(a){this.tab.hit=false;this.tab.matched=[];this.tab.cache="";this.tab.index=-1};Chatterbox.Control.prototype.submit=function(a){var b=this.get_text();this.append_history(b);this.set_text("");this.handle(a,b);
return false};Chatterbox.Control.prototype.keypress=function(d){var b=d.which||d.keyCode;var f=this.tab.hit;var a=false;switch(b){case 13:if(!this.multiline()){this.submit(d)}else{if(d.shiftKey){this.submit(d)}else{a=true}}break;case 38:if(!this.multiline()){this.scroll_history(true);break}a=true;break;case 40:if(!this.multiline()){this.scroll_history(false);break}a=true;break;case 9:if(d.shiftKey){this.manager.channel_right()}else{this.tab_item(d);f=false}break;case 219:if(d.ctrlKey){this.manager.channel_left()}else{a=true}break;case 221:if(d.ctrlKey){this.manager.channel_right()}else{a=true}break;case 84:console.log(d.altKey,d);if(d.ctrlKey){this.manager.nav.toggle_sidebar()}else{a=true}break;default:a=true;break}if(f){this.end_tab(d)}return a};Chatterbox.Control.prototype.handle=function(b,g){if(g==""){return}if(!this.manager.chatbook.current){return}var k=false;if(g[0]!="/"){k=true}g=(b.shiftKey?"/npmsg ":(g[0]=="/"?"":"/say "))+g;g=g.slice(1);var l=g.split(" ");var d=l.shift().toLowerCase();
var j=this.manager.chatbook.current.namespace;var h=j;if(!k&&l[0]){var f=l[0][0];if((f=="#"||f=="@")&&l[0].length>1){h=this.manager.format_ns(l.shift())}}var m=l.join(" ");var a=this.manager.client.trigger("cmd."+d,{name:"cmd",cmd:d,args:m,target:h,ns:j});if(a==0){this.manager.pager.notice({ref:"cmd-fail",heading:"Command failed",content:'"'+d+'" is not a command.'},false,5000)}};Chatterbox.Navigation=function(b){this.manager=b;this.showclose=this.manager.settings.tabclose;this.settings={};this.settings.open=false;this.el={n:this.manager.view.find("nav.tabs"),tw:this.manager.view.find("nav.tabs div.tabwrap"),t:this.manager.view.find("nav.tabs #chattabs"),b:this.manager.view.find("nav.tabs #tabnav"),l:this.manager.view.find("nav.tabs #tabnav .arrow_left"),r:this.manager.view.find("nav.tabs #tabnav .arrow_right"),s:this.manager.view.find("nav.tabs #tabnav #settings-button"),side:this.manager.view.find("nav.channels"),sideb:this.manager.view.find("nav.tabs #chattabs .chans"),};this.side=false;
var a=this.el.side.outerWidth(true)*-1;this.el.side.css({left:a,"margin-right":a});if(!this.showclose){if(!this.el.t.hasClass("hc")){this.el.t.addClass("hc")}}var d=this;this.el.s.click(function(h){if(d.settings.open){return false}var f={e:h,settings:new Chatterbox.Settings.Config(d.manager)};d.configure_page(f);d.manager.trigger("settings.open",f);d.manager.trigger("settings.open.ran",f);var g=f.settings.page("About");g.item("text",{ref:"about-chatterbox",wclass:"centered faint",text:'Using <a href="http://github.com/photofroggy/wsc/">Chatterbox</a> version '+Chatterbox.VERSION+" "+Chatterbox.STATE+' by ~<a href="http://photofroggy.deviantart.com/">photofroggy</a>.'});d.settings.window=new Chatterbox.Settings(d.manager,f.settings);d.settings.window.build();d.settings.open=true;return false});this.el.l.click(function(){d.manager.channel_left();return false});this.el.r.click(function(){d.manager.channel_right();return false});this.el.sideb.find("a.tab").click(function(){d.toggle_sidebar();
return false})};Chatterbox.Navigation.prototype.configure_page=function(b){var f=this.manager;var d=b.settings.page("Main");var g={};g.theme=replaceAll(f.settings.theme,"wsct_","");g.clock=f.clock();g.tc=f.nav.closer();var a=[];for(i in f.settings.themes){name=replaceAll(f.settings.themes[i],"wsct_","");a.push({value:name,title:name,selected:g.theme==name})}d.item("Form",{ref:"ui",title:"UI",hint:"<b>Timestamp</b><br/>Choose between a 24 hour clock and                a 12 hour clock.\n\n<b>Theme</b><br/>Change the look of the                client.\n\n<b>Close Buttons</b><br/>Turn tab close buttons on/off.",fields:[["Dropdown",{ref:"theme",label:"Theme",items:a}],["Dropdown",{ref:"clock",label:"Timestamp Format",items:[{value:"24",title:"24 hour",selected:g.clock},{value:"12",title:"12 hour",selected:!g.clock}]}],["Checkbox",{ref:"tabclose",label:"Close Buttons",items:[{value:"yes",title:"On",selected:g.tc}]}],],event:{change:function(h){f.clock(h.data.clock=="24");f.theme(h.data.theme);
f.nav.closer(h.data.tabclose.indexOf("yes")>-1)},save:function(h){g.clock=f.clock();g.theme=replaceAll(f.theme(),"wsct_","");g.tc=f.nav.closer();f.trigger("settings.save.ui",{clock:g.clock,tabclose:g.tc,theme:"wsct_"+g.theme})},close:function(h){f.clock(g.clock);f.theme(g.theme);f.nav.closer(g.tc)}}})};Chatterbox.Navigation.prototype.height=function(){var a=this.el.n.outerHeight();return a};Chatterbox.Navigation.prototype.add_button=function(a){a=Object.extend({label:"New",icon:false,href:"#button",title:"Button.",handler:function(){}},(a||{}));if(a.icon!==false){a.icon=" iconic "+a.icon}else{a.icon=" text"}this.el.b.prepend(Chatterbox.render("nav_button",a));var b=this.el.b.find("a[href="+a.href+"].button");b.click(function(d){a.handler();return false});this.resize();return b};Chatterbox.Navigation.prototype.add_tab=function(a,d){var f=Chatterbox.render("tab",{selector:a,ns:d});this.el.t.append(f);var g=this.el.t.find("#"+a+"-tab");g.find("a.tab").click(function(){return false});var b=this.el.side.find("ul");
b.append(f);b=b.find("#"+a+"-tab");return[g,b]};Chatterbox.Navigation.prototype.resize=function(){var a=this.el.n.width()-this.el.b.outerWidth()-20;var b=((this.manager.view.parent().height()-this.height())-this.manager.control.height())-8;this.el.tw.width(a);this.el.t.width(a);if(this.settings.open){this.settings.window.resize()}this.el.side.height(b)};Chatterbox.Navigation.prototype.listwidth=function(){return this.side?this.manager.view.find("nav.channels").outerWidth(true)+8:0};Chatterbox.Navigation.prototype.toggle_sidebar=function(b){if(b===undefined){b=!this.side}this.side=b;if(b){this.el.sideb.removeClass("noise chatting tabbed fill");this.el.side.css({left:0,"margin-right":5});this.manager.resize();this.manager.control.focus();return}var a=this.el.side.outerWidth(true)*-1;this.el.side.css({left:a,"margin-right":a});this.manager.resize();this.manager.control.focus()};Chatterbox.Navigation.prototype.noise=function(d){if(this.side){return}var a=this.el.sideb;d=d||0;console.log(d);
if(d==0||d==1){a.addClass("noise");if(d==1){a.addClass("chatting")}}if(d==2){a.addClass("tabbed");a.removeClass("chatting")}};Chatterbox.Navigation.prototype.closer=function(a){if(a==undefined||a==this.showclose){return this.showclose}this.showclose=a;if(this.showclose){if(!this.el.t.hasClass("hc")){return}this.el.t.removeClass("hc");return}if(this.el.t.hasClass("hc")){return}this.el.t.addClass("hc")};Chatterbox.Pager=function(a){this.manager=a;this.lifespan=20000;this.halflife=5000;this.el={m:null,click:null};this.sound={click:function(){},};this.notices=[];this.build()};Chatterbox.Pager.prototype.build=function(){this.el.m=this.manager.view.find("div.pager")};Chatterbox.Pager.prototype.notice=function(f,l,k,d){var h={frame:null,close:null,foot:null,b:{},options:Object.extend({ref:"notice",icon:"",heading:"Some notice",content:"Notice content goes here."},(f||{})),onclose:function(){},ondestroy:function(){}};h.options.ref+="-"+(new Date()).valueOf();h.options.content=h.options.content.split("\n").join("</p><p>");
...
var f=Chatterbox.Settings.Item.get(d,b,this.manager);if(a){this.items.unshift(f)}else{this.items.push(f)}if(b.hasOwnProperty("ref")){this.itemo[b.ref]=f}return f};Chatterbox.Settings.Page.prototype.get=function(a){if(this.itemo.hasOwnProperty(a)){return this.itemo[a]}return null};Chatterbox.Settings.Page.prototype.each_item=function(f){var d=null;var a=null;for(var b in this.items){if(!this.items.hasOwnProperty(b)){continue}d=this.items[b];a=f(b,d);if(a===false){break}}};Chatterbox.Settings.Page.prototype.save=function(b){for(var a in this.items){this.items[a].save(b,this)}};Chatterbox.Settings.Page.prototype.close=function(b){for(var a in this.items){this.items[a].close(b,this)}};Chatterbox.Settings.Item=function(b,a,d){this.manager=d||null;this.options=a||{};this.type=b||"base";this.selector=this.type.toLowerCase();this.items=[];this.itemo={};this.view=null;this.val=null;this._onchange=this._event_stub};Chatterbox.Settings.Item.prototype.build=function(j){if(!this.options.hasOwnProperty("ref")){return
}var h=this.content();if(h.length==0){return}var a="";if(this.options.hasOwnProperty("wclass")){a=" "+this.options.wclass}var g=Chatterbox.render("settings.item.wrap",{type:this.type.toLowerCase().split(".").join("-"),ref:this.options.ref,"class":a});g=replaceAll(g,"{content}",h);j.append(g);this.view=j.find(".item."+this.options.ref);this.hooks(this.view);if(!this.options.hasOwnProperty("subitems")){return}var k;var f;var d;var b;for(i in this.options.subitems){k=this.options.subitems[i];f=k[0];d=k[1];sitem=Chatterbox.Settings.Item.get(f,d);b=["stacked"];if(sitem.options.wclass){b.push(sitem.options.wclass)}sitem.options.wclass=b.join(" ");sitem.build(this.view);this.items.push(sitem);if(d.hasOwnProperty("ref")){this.itemo[d.ref]=sitem}}};Chatterbox.Settings.Item.prototype.content=function(){return Chatterbox.render("settings.item."+this.selector,this.options)};Chatterbox.Settings.Item.prototype.resize=function(){for(var a in this.items){this.items[a].resize()}};Chatterbox.Settings.Item.prototype.hooks=function(d){if(!this.options.hasOwnProperty("event")){return
}var b=this.options.event;var g=Chatterbox.template.settings.item.get(this.selector);if(g==false){return}if(!g.hasOwnProperty("events")){return}var f=[];for(var a in g.events){f=g.events[a];if(!b.hasOwnProperty(f[0])){continue}d.find(f[1]).bind(f[0],b[f[0]])}};Chatterbox.Settings.Item.prototype._event_stub=function(){};Chatterbox.Settings.Item.prototype._get_cb=function(a){if(!this.options.hasOwnProperty("event")){return this._event_stub}return this.options.event[a]||this._event_stub};Chatterbox.Settings.Item.prototype._get_ep=function(b){var f=Chatterbox.template.settings.item.get(this.selector);if(f==null){return false}if(!f.hasOwnProperty("events")){return false}var d=[];for(var a in f.events){d=f.events[a];if(d[0]==b){return d}}};Chatterbox.Settings.Item.prototype.save=function(f,g){var h=this._get_ep("inspect");var b=h?this.view.find(h[1]):null;var a=this._get_cb("save");if(typeof a=="function"){a({input:b,item:this,page:g,window:f})}else{for(var d in a){var j=b.hasOwnProperty("slice")?b.slice(d,1):b;
a[d]({input:j,item:this,page:g,window:f})}}for(var d in this.items){this.items[d].save(f,g)}};Chatterbox.Settings.Item.prototype.close=function(f,g){var h=this._get_ep("inspect");var b=h?this.view.find(h[1]):null;var a=this._get_cb("close");if(typeof a=="function"){a({input:b,item:this,page:g,window:f});return}for(var d in a){var j=b.hasOwnProperty("slice")?b.slice(d,1):b;a[d]({input:j,item:this,page:g,window:f})}for(var d in this.items){this.items[d].close(f,g)}};Chatterbox.Settings.Item.get=function(g,l,h,b,a){var f=g.split(".");var j=b||Chatterbox.Settings.Item;var k;for(var d in f){k=f[d];if(!j.hasOwnProperty(k)){j=a||Chatterbox.Settings.Item;break}j=j[k]}return new j(g,l,h)};Chatterbox.Settings.Item.Form=function(b,a){Chatterbox.Settings.Item.call(this,b,a);this.form=null;this.fields=[];this.lsection=null;this.fsection=null;this.fieldo={}};Chatterbox.Settings.Item.Form.prototype=new Chatterbox.Settings.Item();Chatterbox.Settings.Item.Form.prototype.constructor=Chatterbox.Settings.Item.Form;
Chatterbox.Settings.Item.Form.field=function(b,a){return Chatterbox.Settings.Item.get(b,a,this.manager,Chatterbox.Settings.Item.Form,Chatterbox.Settings.Item.Form.Field)};Chatterbox.Settings.Item.Form.prototype.build=function(g){Chatterbox.Settings.Item.prototype.build.call(this,g);if(this.view==null){return}this.lsection=this.view.find("section.labels");this.fsection=this.view.find("section.fields");if(!this.options.hasOwnProperty("fields")){return}var d;var h;for(var a in this.options.fields){d=this.options.fields[a];h=Chatterbox.Settings.Item.Form.field(d[0],d[1]);this.fields.push(h);h.build(this);if(d[1].hasOwnProperty("ref")){this.fieldo[d[1].ref]=h}}this.form=this.view.find("form");var b=this;this.form.bind("change",function(f){b.change()})};Chatterbox.Settings.Item.Form.prototype.resize=function(){for(var a in this.fields){this.fields[a].resize()}};Chatterbox.Settings.Item.Form.prototype.change=function(){var d={};var f;for(var b in this.fields){f=this.fields[b];d[f.ref]=f.get()}var a=this._get_cb("change");
if(typeof a=="function"){a({data:d,form:this})}else{for(var b in a){a[b]({data:d,form:this})}}};Chatterbox.Settings.Item.Form.prototype.save=function(f,h){var g={};var b;for(var d in this.fields){field=this.fields[d];g[field.ref]=field.get()}var a=this._get_cb("save");if(typeof a=="function"){a({data:g,form:this,page:h,window:f})}else{for(var d in a){a[d]({data:g,form:this,page:h,window:f})}}};Chatterbox.Settings.Item.Form.prototype.close=function(d,g){var f={};var h;for(var b in this.fields){h=this.fields[b];f[h.ref]=h.get()}var a=this._get_cb("close");if(typeof a=="function"){a({data:f,form:this,page:g,window:d})}else{for(var b in a){a[b]({data:f,form:this,page:g,window:d})}}};Chatterbox.Settings.Item.Form.Field=function(b,a){Chatterbox.Settings.Item.call(this,b,a);this.ref=this.options.ref||"ref";this.label=null;this.field=null;this.value=""};Chatterbox.Settings.Item.Form.Field.prototype=new Chatterbox.Settings.Item();Chatterbox.Settings.Item.Form.Field.prototype.constructor=Chatterbox.Settings.Item.Form.Field;
...
a.list.find("li.selected").removeClass("selected");a.selected=f.find(".item").html();f.addClass("selected")});d.each(function(f,h){var g=a.list.find(h);g.find("a.close").click(function(j){a.list.find("li.selected").removeClass("selected");a.selected=g.find(".item").html();g.addClass("selected");a.remove_item();return false})});this.buttons.find("a.button.up").click(function(f){a.shift_item(true);return false});this.buttons.find("a.button.down").click(function(f){a.shift_item();return false});this.buttons.find("a.button.add").click(function(f){var g=new Chatterbox.Popup.Prompt(a.manager,{position:[f.clientX-100,f.clientY-50],title:a.options.prompt.title,label:a.options.prompt.label,"submit-button":a.options.prompt["submit-button"],event:{submit:function(h){var k=h.data;if(!k){h.options.event.cancel(h);return}k=k.toLowerCase();var j=a.options.items.indexOf(k);if(j!=-1){h.options.event.cancel(h);return}a._fevent("add",{item:k});a.refresh();a._onchange({})},cancel:function(h){a._fevent("cancel",{});
...
this.list=this.view.find("ul");this.list.find("li[title="+(this.selected||"").toLowerCase()+"]").addClass("selected");var a=this;var b=this.list.find("li");b.click(function(f){var d=a.list.find(this);a.list.find("li.selected").removeClass("selected");a.selected=d.find(".item").html();d.addClass("selected")});b.each(function(d,g){var f=a.list.find(g);f.find("a.close").click(function(h){a.list.find("li.selected").removeClass("selected");a.selected=f.find(".item").html();f.addClass("selected");a.remove_item();return false})})};Chatterbox.Settings.Item.Items.prototype._fevent=function(b,f){var h=this._get_ep("inspect");var d=h?this.view.find(h[1]):null;var a=this._get_cb(b);if(typeof a=="function"){a({input:d,item:this,args:f})}else{for(var g in a){var j=d.hasOwnProperty("slice")?d.slice(g,1):d;a[g]({input:j,item:this,args:f})}}};Chatterbox.Settings.Item.Items.prototype.save=function(){this._fevent("save",{items:this.options.items||[]})};Chatterbox.template={};Chatterbox.render=function(k,m,a,b){var g=b||Chatterbox.template;
var h={};var j=null;var d=null;if(a!==undefined&&a===true){g=k}else{var l=k.split(".");for(var f in l){d=l[f];if(!g.hasOwnProperty(d)){return""}g=g[d]}}if(g.hasOwnProperty("frame")){j=g;h=g.render||{};g=g.frame;if(j.hasOwnProperty("pre")){if(typeof j.pre=="function"){g=j.pre(g,m)}else{for(i in j.pre){g=j.pre[i](g,m)}}}}for(key in m){g=replaceAll(g,"{"+key+"}",(h[key]||Chatterbox.template.render_stub)(m[key]||""))}if(j!=null){if(j.hasOwnProperty("post")){if(typeof j.post=="function"){g=j.post(g,m)}else{for(i in j.post){g=j.post[i](g,m)}}}}return g};Chatterbox.template.render_stub=function(a){return a};Chatterbox.template.clean=function(a){return function(b,d){for(i in a){b=replaceAll(b,"{"+a[i]+"}","")}return b}};Chatterbox.template.ui='<div class="soundbank">            <audio class="click">                <source src="{media}click.ogg" type="audio/ogg">                <source src="{media}click.mp3" type="audio/mpeg">            </audio>        </div>        <div class="pager">        </div>        <nav class="tabs"><div class="tabwrap"><ul id="chattabs" class="tabs">        <li class="chans"><a href="#chans" class="tab">Channels</a></li>        </ul></div>        <ul id="tabnav">            <li><a href="#left" class="button iconic arrow_left"></a></li>            <li><a href="#right" class="button iconic arrow_right"></a></li>            <li><a href="#settings" title="Change client settings" class="button iconic cog" id="settings-button"></a></li>        </ul>        </nav>        <div class="chatbook">        <nav class="channels"><h3>Channels</h3><ul>        </ul></nav>        </div>';
...
Chatterbox.template.settings.item.form.field.colour={};Chatterbox.template.settings.item.form.field.colour.post=Chatterbox.template.clean(["ref","default"]);Chatterbox.template.settings.item.form.field.colour.frame='<input class="{ref}" type="color" value="{default}" />';(function(a){a("*").hover(function(b){a(this).data("hover",true)},function(b){a(this).data("hover",false)});a.fn.wsc=function(f,d){var b=a(window).data("wscclient");if(f=="init"||b===undefined){if(b==undefined){b=new wsc.Client(a(this),d,(a.browser.mozilla||false));a(window).resize(function(){b.ui.resize()});a(window).focus(function(){b.ui.control.focus()});setInterval(function(){b.loop()},120000)}a(window).data("wscclient",b)}if(f!="init"&&f!=undefined){f="jq_"+f;if(f in b){b[f](a(this),d)}}return b};a.fn.wscui=function(d,b){ui=a(window).data("wscui");if(d=="init"||ui===undefined){if(ui==undefined){ui=new Chatterbox.UI(a(this),b,(a.browser.mozilla||false));a(window).resize(ui.resize)}a(window).data("wscui",ui)}if(d!="init"&&d!=undefined){d="jq_"+d;
if(d in ui){ui[d](a(this),b)}}return ui}})(jQuery);