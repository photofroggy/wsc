...
};Chatterbox.UI.prototype.cascade=function(a,d,b){this.mw.run(a,d,b)};Chatterbox.UI.prototype.remove_listeners=function(){this.events.removeListeners()};Chatterbox.UI.prototype.deform_ns=function(a){if(a[0] in ["#","@","~","+"]){return a}if(a.indexOf("chat:")==0){return"#"+a.slice(5)}if(a.indexOf("server:")==0){return"~"+a.slice(7)}if(a.indexOf("feed:")==0){return"#"+a.slice(5)}if(a.indexOf("login:")==0){return"@"+a.slice(6)}if(a.indexOf("pchat:")==0){var b=a.split(":");b.shift();for(i in b){name=b[i];if(name.toLowerCase()!=this.lun){return"@"+name}}}return"#"+a};Chatterbox.UI.prototype.format_ns=function(a){var d=a.slice(1);switch(a[0]){case"@":var b=[d,this.lun];b.sort(caseInsensitiveSort);b.unshift("pchat");a=b.join(":");break;case"~":a="server:"+d;break;case"+":a="feed:"+d;break;case"#":a="chat:"+d;break;default:if(a.indexOf("chat:")==0||a.indexOf("pchat:")==0||a.indexOf("server:")==0||a.indexOf("feed:")==0){break}a="chat:"+d;break}return a};Chatterbox.UI.prototype.set_events=function(a){this.events=a||this.events
};Chatterbox.UI.prototype.clock=function(a){if(a===undefined||a==this.settings.clock){return this.settings.clock}this.settings.clock=a;this.chatbook.retime();return this.settings.clock};Chatterbox.UI.prototype.build=function(h,a,b){this.view.append(Chatterbox.render("ui",this.settings));this.control=new (h||Chatterbox.Control)(this);this.nav=new (a||Chatterbox.Navigation)(this);this.chatbook=new (b||Chatterbox.Chatbook)(this);this.pager=new Chatterbox.Pager(this);this.monitoro=this.chatbook.create_channel(this.mns,this.settings.monitor[1],true);this.monitoro.show();this.control.focus();this.sound.bank.m=this.view.find("div.soundbank");this.sound.add("click",this.sound.bank.m.find("audio.click")[0]);var f=false;var d=this.nav.add_button({label:"",icon:"volume",href:"#mute",title:"Mute the client",handler:function(){if(!f){sound.mute();d.removeClass("volume");d.addClass("volume_mute");d.prop("title","Unmute the client");f=true;return false}sound.unmute();d.removeClass("volume_mute");d.addClass("volume");
d.prop("title","Mute the client");f=false;return false}});var g=this;$(window).focus(function(){g.viewing=true});$(window).blur(function(){g.viewing=false});this.client.bind("pkt",function(k,j){g.packet(k,j)});this.client.middle("ns.remove",function(k,j){g.remove_channel(k.ns);j(k)});this.client.bind("ns.create",function(k,j){g.create_channel(k.chan.raw,k.chan.hidden)});this.client.bind("ns.user.list",function(j){g.channel(j.ns).set_user_list(j.users)});this.client.middle("ns.user.privchg",function(k,j){g.channel(k.ns).privchg(k,j)});this.client.bind("ns.user.remove",function(k,j){g.channel(k.ns).remove_one_user(k.user)});this.client.bind("ns.user.registered",function(j){g.channel(j.ns).register_user(j.user)})};Chatterbox.UI.prototype.resize=function(){this.control.resize();this.view.height(this.view.parent().height());this.nav.resize();this.chatbook.resize(((this.view.parent().height()-this.nav.height())-this.control.height())-5)};Chatterbox.UI.prototype.loop=function(){this.chatbook.loop()
};Chatterbox.UI.prototype.packet=function(b,a){var d=this;var f=a.protocol.log(b);if(f){if(this.settings.developer){console.log(">>>",b.sns,"|",f.text())}if(b.hasOwnProperty("s")&&b.s=="0"){this.chatbook.handle(b,a);return}b.html=f.html();this.cascade("log_message",function(h,g){d.chatbook.log_message(h.message,h.event)},{message:f,event:b})}this.chatbook.handle(b,a)};Chatterbox.UI.prototype.create_channel=function(b,a){this.chatbook.create_channel(b,a)};Chatterbox.UI.prototype.remove_channel=function(a){this.chatbook.remove_channel(a)};Chatterbox.UI.prototype.toggle_channel=function(a){return this.chatbook.toggle_channel(a)};Chatterbox.UI.prototype.channel=function(a,b){return this.chatbook.channel(a,b)};Chatterbox.UI.prototype.channels=function(){return this.chatbook.channels()};Chatterbox.UI.prototype.channel_left=function(){this.chatbook.channel_left()};Chatterbox.UI.prototype.channel_right=function(){this.chatbook.channel_right()};Chatterbox.UI.prototype.monitor=function(b,a){this.monitoro.server_message(b,a)
...
Chatterbox.template.feed='<div class="chatwindow feed" id="{selector}-window">                    <header class="info">                        <h2>{name}<span class="type">{type}</span></h2>                        <p>{info}</p>                    </header>                    <div class="log" id="{selector}-log">                        <header class="control">                            <a href="#post" class="button iconic pen" title="Post a message on"></a>                            <a href="#refresh" class="button iconic spin" title="Get updates"></a>                            <a href="#close" class="button iconic x" title="Close feed reader"></a>                        </header>                        <ul class="logwrap"></ul>                    </div>                    <div class="users" id="{selector}-users"></div>                </div>';Chatterbox.template.channel='<div class="chatwindow" id="{selector}-window">                    <header class="title">                        <div class="title"></div>                        <textarea></textarea>                        <a href="#edit" class="button iconic pen" title="Edit the title"></a>                        <a href="#save" class="button iconic check" title="Save changes"></a>                        <a href="#cancel" class="button iconic x" title="Cancel"></a>                    </header>                    <div class="chatlog" id="{selector}-log">                        <header class="topic">                            <div class="topic"></div>                            <textarea></textarea>                            <a href="#edit" class="button iconic pen" title="Edit the topic"></a>                            <a href="#save" class="button iconic check" title="Save changes"></a>                            <a href="#cancel" class="button iconic x" title="Cancel"></a>                        </header>                        <ul class="logwrap"></ul>                    </div>                    <div class="chatusers" id="{selector}-users">                </div>            </div>';
