...
}S.connections.push(Q)}O.cchannel.log_whois(S)};var v=function(P,O){if(P.ns.indexOf("login:")!=0){return}O.cchannel.server_message("Whois failed for "+(P.sns.substr(1)),"not online")};var L=function(S,P){var V=P.channel(S.ns);var O=S.info.split("\n");var U="";var T=[];var Q="";for(var R in O){if(!O.hasOwnProperty(R)){continue}U=O[R].split(" ");if(S.p=="privclass"){T.push([U.shift(),U.shift().split("=")[1],U.join(" ")])}else{if(S.p=="users"){Q=U.shift().split(":",1)[0];T.push([Q,V.get_privclass_order(Q),U.join(" ")])}}}V.log_pc(S.p=="privclass",T)};D();wsc.defaults.Extension.Autojoin(p);wsc.defaults.Extension.Away(p);wsc.defaults.Extension.Ignore(p)};wsc.defaults.Extension.Autojoin=function(a){var j=a.autojoin;var m=function(){a.bind("cmd.autojoin",g);a.ui.on("settings.open",j.page)};j.page=function(r,t){var s=r.settings.page("Autojoin",true);var q="<ul>";var v={};v.ajon=a.autojoin.on;if(a.autojoin.count==0){q+="<li><i>No autojoin channels set</i></li></ul>"}else{for(var p in a.autojoin.channel){if(!a.autojoin.channel.hasOwnProperty(p)){continue
}q+="<li>"+a.autojoin.channel[p]+"</li>"}q+="</ul>"}s.item("Text",{ref:"intro",title:"Autojoin",text:"Add any channels you want to join automatically when you                    connect to the chat server."});var u=s.item("Form",{ref:"autojoin",wclass:"boxed-ff-indv",title:"Channels",text:"This is the list of channels on your autojoin.\n\nUse the                    command <code>/autojoin</code> to edit the list.",fields:[["Checkbox",{ref:"enabled",label:"Autojoin",items:[{value:"yes",title:"On",selected:v.ajon}]}],["Text",{ref:"channels",text:q}]],event:{change:function(z){},save:function(z){}}})};var g=function(p){};var b=function(r,p){j.on=true;j.last={};j.since=new Date();j.reason=r.args;var s=p.say;var q=replaceAll(j.format.setaway,"{reason}",j.reason||"[silent away]");if(q.indexOf("/me ")==0){q=q.substr(4);s=p.action}p.each_channel(function(t){s.call(p,t,q)})};var k=function(r,p){j.on=false;var s=p.say;var q=j.format.setback;if(q.indexOf("/me ")==0){q=q.substr(4);s=p.action}p.each_channel(function(t){s.call(p,t,q)
})};var l=function(u,p){if(!j.on){return}if(j.reason.length==0){return}if(u.user==p.settings.username){return}if(p.exclude.indexOf(u.sns.toLowerCase())!=-1){return}var r=new Date();var s=u.sns.toLowerCase();if(s in j.last){if((r-j.last[s])<=j.interval){return}}var q=timeLengthString((r-j.since)/1000);var v=replaceAll(j.format.away,"{user}",u.user);v=replaceAll(v,"{timesince}",q);p.say(u.ns,replaceAll(v,"{reason}",j.reason));j.last[s]=r};m()};wsc.defaults.Extension.Away=function(b){var j=b.storage.folder("away");var g={on:false,reason:"",last:{},since:0,interval:60000,format:{setaway:"/me is away: {reason}",setback:"/me is back",away:"{user}: I've been away for {timesince}. Reason: {reason}"}};var q=function(){m();k();b.bind("cmd.setaway",a);b.bind("cmd.setback",p);b.bind("pkt.recv_msg.highlighted",l);b.ui.on("settings.open",g.page)};g.page=function(r,t){var s=r.settings.page("Away",true);var u={};u.away=g.format.away;u.sa=g.format.setaway;u.sb=g.format.setback;u.intr=g.interval;s.item("Text",{ref:"intro",title:"Away Messages",text:"Use away messages when you are away from the chats.\n\n                    You can set yourself away using the                     <code>/setaway [reason]</code> command. When you get back,                    use <code>/setback</code>. While you are away, the client                    will automatically respond when people try to talk to you,                    telling them you're away.",});
