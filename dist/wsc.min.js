...
wsc.Flow.prototype.recv_msg=function(b,a){a.channel(b.ns).recv_msg(b)};wsc.Flow.prototype.recv_action=wsc.Flow.prototype.recv_msg;wsc.Flow.prototype.recv_npmsg=wsc.Flow.prototype.recv_msg;wsc.Flow.prototype.recv_privchg=function(b,a){a.channel(b.ns).recv_privchg(b)};wsc.Flow.prototype.recv_kicked=function(b,a){a.channel(b.ns).recv_kicked(b)};wsc.defaults.Extension=function(k){var m={};m.away={on:false,reason:"",last:{},since:0,store:k.storage.folder("away"),format:{setaway:"/me is away: {reason}",setback:"/me is back",away:"{from}: I am away, reason: {reason}"}};var v=function(){k.bind("cmd.connect",f);k.bind("cmd.set",a);k.bind("cmd.join",C);k.bind("cmd.chat",E);k.bind("cmd.part",G);k.bind("cmd.say",n);k.bind("cmd.npmsg",H);k.bind("cmd.me",p);k.bind("cmd.promote",s);k.bind("cmd.demote",s);k.bind("cmd.ban",j);k.bind("cmd.unban",j);k.bind("cmd.kick",b);k.bind("cmd.whois",y);k.bind("cmd.title",g);k.bind("cmd.topic",g);k.bind("cmd.admin",t);k.bind("cmd.disconnect",f);k.bind("cmd.kill",b);k.bind("cmd.raw",D);
k.bind("cmd.clear",A);k.bind("cmd.clearall",z);k.bind("cmd.close",u);k.bind("pkt.property",h);k.bind("pkt.recv_admin_show",F);k.bind("pkt.recv_admin_showverbose",F);k.bind("pkt.get",q);k.bind("cmd.gettitle",o);k.bind("cmd.gettopic",o);k.bind("cmd.theme",d);k.ui.on("settings.open",l);k.ui.on("settings.open.ran",B);k.ui.on("settings.save.ui",x);k.ui.on("settings.save",function(){k.config_save()})};var x=function(J,I){k.settings.ui.theme=J.theme;k.settings.ui.clock=J.clock;k.settings.ui.tabclose=J.tabclose};var l=function(K,J){var I=K.settings.page("Main");var L={};L.username=k.settings.username;L.pk=k.settings.pk;L.devel=k.settings.developer;I.item("Form",{ref:"login",title:"Login",text:"Here you can change the username and token used to                    log into the chat server.",fields:[["Textfield",{ref:"username",label:"Username","default":L.username}],["Textfield",{ref:"token",label:"Token","default":L.pk}]],event:{save:function(M){k.settings.username=M.data.username;k.settings.pk=M.data.token
}}},true);I.item("Form",{ref:"developer",title:"Developer Mode",text:"Turn developer mode on or off.\n\nDeveloper mode will expose any hidden                channel tabs, amongst other things. Keep this turned off unless you're working                on implementing something.",fields:[["Checkbox",{ref:"enabled",items:[{value:"on",title:"On",selected:L.devel}]}]],event:{change:function(M){k.settings.developer=(M.data.enabled.indexOf("on")!=-1);k.ui.developer(k.settings.developer)},save:function(M){L.devel=k.settings.developer},close:function(M){k.settings.developer=L.devel;k.ui.developer(k.settings.developer)}}});I.item("Text",{ref:"intro",title:"Main",text:"Use this window to view and change your settings.\n\nCheck                    the different pages to see what settings can be changed.",},true);I.item("Text",{ref:"debug",wclass:"faint",title:"Debug Information",text:"Chat Agent: <code>"+k.settings.agent+"</code>\n\nUser                    Agent: <code>"+navigator.userAgent+"</code>"})
};var B=function(K,J){var I=K.settings.page("About",true);I.item("Text",{ref:"about-wsc",title:"Wsc",text:'Currently using <a href="http://github.com/photofroggy/wsc/">wsc</a>                    version '+wsc.VERSION+" "+wsc.STATE+'.\n\nWsc                    works using HTML5, javascript, and CSS3. WebSocket is used for the connection                    where possible. The source code for this client is pretty huge.\n\nWsc was created                    by ~<a href="http://photofroggy.deviantart.com/">photofroggy</a>'})};var d=function(J,I){I.ui.theme(J.args.split(" ").shift())};var a=function(K){var J=K.args.split(" ");var I=J.shift().toLowerCase();var J=J.join(" ");if(J.length==0){k.cchannel.serverMessage("Could not set "+I,"No data supplied");return}if(!(I in k.settings)){k.cchannel.serverMessage('Unknown setting "'+I+'"');return}k.settings[I]=J;k.cchannel.serverMessage("Changed "+I+" setting","value: "+J);k.control.setLabel()};var f=function(I){k[I.cmd]()};var C=function(I){var J=I.args.split(" ");
var J=J.toString()==""?[]:J;if(I.ns!=I.target){J.unshift(I.target)}if(J.toString()==""){return}for(index in J){k.join(J[index])}};var E=function(I){var J=I.args.split(" ");var J=J.toString()==""?[]:J;if(I.ns!=I.target){J.unshift(I.target)}if(J.toString()==""){return}for(index in J){k.join("@"+J[index])}};var G=function(I){var J=I.args.split(" ");if(I.ns!=I.target){J.unshift(I.target)}if(J.toString()==""){k.part(I.ns);return}for(index in J){k.part(J[index])}};var g=function(I){k.set(I.target,I.cmd,I.args)};var s=function(J){var I=J.args.split(" ");k[J.cmd.toLowerCase()](J.target,I[0],I[1])};var j=function(M,I){var K=M.args.split(" ");var J=K.shift();var L=M.cmd;if(L=="ban"&&K.length>0){I.kick(M.target,J,K.join(" "))}I[L](M.target,J)};var p=function(I){k.action(I.target,I.args)};var D=function(I){k.send(I.args.replace(/\\n/gm,"\n"))};var b=function(L,I){var M=L.args.split(" ");var J=M.shift();var K=M.length>0?M.join(" "):null;if(L.cmd=="kick"){I.kick(L.target,J,K)}else{I.kill(J,K)}};var n=function(I){if(k.channel(I.target).monitor){return
}k.say(I.target,I.args)};var H=function(I){k.npmsg(I.target,I.args)};var A=function(K,I){if(K.args.length>0){var L=K.args.split(" ");for(var J in L){if(!L.hasOwnProperty(J)){continue}I.channel(K.target).clear(L[J])}}else{I.channel(K.target).clear()}};var z=function(J,I){var L=null;if(J.args.length>0){var K=J.args.split(" ");L=function(N,O){for(var M in K){if(!K.hasOwnProperty(M)){continue}O.clear(K[M])}}}else{L=function(M,N){N.clear()}}I.each_channel(L,true)};var u=function(I){k.part(I.target);k.remove_ns(I.target)};var y=function(J,I){I.whois(J.args.split(" ")[0])};var t=function(J,I){I.admin(J.target,J.args)};var r=function(J,I){I.disconnect()};var o=function(J,I){var K=J.cmd.indexOf("title")>-1?"title":"topic";I.control.ui.set_text("/"+K+" "+I.channel(J.target).info[K].content)};var h=function(L,I){if(L.p!="info"){return}var J=L.pkt.sub;var M=J.shift().arg;M.username=L.sns.substr(1);M.connections=[];var K={};while(J.length>0){K=J.shift().arg;K.channels=[];while(J.length>0){if(J[0].cmd!="ns"){break
}K.channels.unshift(I.deform_ns(J.shift().param))}M.connections.push(K)}I.cchannel.log_whois(M)};var q=function(J,I){if(J.ns.indexOf("login:")!=0){return}var K=J.sns.substr(1);I.ui.pager.notice({ref:"whois-"+K,heading:"Whois Failed",content:"Whois failed for "+K+".\nNo such user online."},false,5000)};var F=function(M,J){var P=J.channel(M.ns);var I=M.info.split("\n");var O="";var N=[];var K="";for(var L in I){if(!I.hasOwnProperty(L)){continue}O=I[L].split(" ");if(M.p=="privclass"){N.push([O.shift(),O.shift().split("=")[1],O.join(" ")])}else{if(M.p=="users"){K=O.shift().split(":",1)[0];N.push([K,P.get_privclass_order(K),O.join(" ")])}}}P.log_pc(M.p=="privclass",N)};v();wsc.defaults.Extension.Ignore(k);wsc.defaults.Extension.Away(k);wsc.defaults.Extension.Autojoin(k)};wsc.defaults.Extension.Autojoin=function(a){var d=a.autojoin;a.ui.nav.add_button({icon:"chat",label:"",title:"Join your autojoin channels",href:"#autojoin-do",handler:function(){for(var g in a.autojoin.channel){if(!a.autojoin.channel.hasOwnProperty(g)){continue
